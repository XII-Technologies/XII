  ...
  namespace tidy
  {
    namespace xii
    {
      ...
      std::string AddPrefixForType(clang::StringRef typeName, std::string newName, bool* prefixAdded = nullptr)
      {
        ...
        else if (typeName == "xiiSharedPtr" || typeName == "xiiUniquePtr" ||
                 typeName == "xiiPointerWithFlags" || typeName == "QPointer" ||
        ...
        else if (typeName == "xiiStringView" || typeName == "xiiHybridString" ||
                 typeName == "xiiHashedString" || typeName == "xiiTempHashedString" ||
                 typeName == "basic_string" || typeName == "xiiStringBuilder" ||
                 typeName == "QString")
        {
          if (prefixAdded)
            *prefixAdded = true;
        ...
        else if (typeName == "xiiAtomicBool")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "b");
        }
        else if (typeName == "xiiAtomicInteger")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "i");
        }
        else if (typeName.startswith("xiiVec") || typeName.startswith("xiiSimdVec"))
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "v");
        }
        else if (typeName.startswith("xiiMat") || typeName.startswith("xiiSimdMat"))
        {
          if (prefixAdded)
            *prefixAdded = true;
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                auto typedefName = typedefType->getDecl()->getName();
                if (typedefName == "xiiUInt8" || typedefName == "xiiUInt16" ||
                    typedefName == "xiiUInt32" || typedefName == "xiiUInt64" ||
                    typedefName == "size_t" || typedefName == "DWORD")
                {
                  return newName.insert(0, "ui");
                }
                else if (typedefName == "xiiInt8" || typedefName == "xiiInt16" ||
                         typedefName == "xiiInt32" || typedefName == "xiiInt64" ||
...
// m_bIsUsed[3]; xiiDynmicArray<bool> m_bVar? what about xiiUInt8 m_uiData[128];
// -> On fixed size arrays keep the already existing prefix
// * static xiiWorldModuleTypeId TYPE_ID. This is not a constant. Why all upper
