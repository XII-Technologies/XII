...
{
  BlackboardDataBinding::BlackboardDataBinding(const xiiSharedPtr<xiiBlackboard>& pBlackboard) :
  ...
  xiiResult BlackboardDataBinding::Initialize(Rml::Context& ref_context)
  {
    if (m_pBlackboard == nullptr)
      return XII_FAILURE;

    const char* szModelName = m_pBlackboard->GetName();
    if (xiiStringUtils::IsNullOrEmpty(szModelName))
    {
      xiiLog::Error("Can't bind a blackboard without a valid name");
      return XII_FAILURE;
    }
  ...
  void BlackboardDataBinding::Update()
  {
    ...
    {
      xiiLog::Warning("Data Binding doesn't work with values that are registered or unregistered after setup");
      m_uiBlackboardChangeCounter = m_pBlackboard->GetBlackboardChangeCounter();
    }
  ...
  {
    xiiVariant::Type::Enum targetType = xiiVariant::Type::Invalid;
    if (auto pEntry = m_Blackboard.GetEntry(m_sName))
    {
      targetType = pEntry->m_Value.GetType();
    }

    if (m_Blackboard.SetEntryValue(m_sName, xiiRmlUiConversionUtils::ToVariant(value, targetType)).Failed())
    {
      xiiLog::Error("RmlUI: Can't set blackboard entry '{}', because it doesn't exist.", m_sName);
    }
  }

  void BlackboardDataBinding::EntryWrapper::GetValue(Rml::Variant& out_value) const
  {
    out_value = xiiRmlUiConversionUtils::ToVariant(m_Blackboard.GetEntryValue(m_sName));
  }
