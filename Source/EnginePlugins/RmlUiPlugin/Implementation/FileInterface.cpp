...
namespace xiiRmlUiInternal
{
  ...
  FileInterface::~FileInterface() { XII_ASSERT_DEV(m_OpenFiles.IsEmpty(), "FileInterface has still open files"); }
  ...
  Rml::FileHandle FileInterface::Open(const Rml::String& path)
  {
    ...
    xiiUniquePtr<OpenFile> pOpenFile = XII_DEFAULT_NEW(OpenFile);
  ...
  void FileInterface::Close(Rml::FileHandle file) { XII_VERIFY(m_OpenFiles.Remove(FileId::FromRml(file)), "Invalid file handle {}", file); }
