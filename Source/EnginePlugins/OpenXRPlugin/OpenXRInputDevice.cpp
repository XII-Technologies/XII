...
// clang-format off
XII_BEGIN_DYNAMIC_REFLECTED_TYPE(xiiOpenXRInputDevice, 1, xiiRTTINoAllocator);
...
#define XR_Squexiie_Click "squexiie_click"
...
void xiiOpenXRInputDevice::GetDeviceList(xiiHybridArray<xiiXRDeviceID, 64>& out_Devices) const
{
  XII_ASSERT_DEV(m_pOpenXR->IsInitialized(), "Need to call 'Initialize' first.");
  out_Devices.PushBack(0);
  for (xiiXRDeviceID i = 0; i < 3; i++)
  {
...
xiiXRDeviceID xiiOpenXRInputDevice::GetDeviceIDByType(xiiXRDeviceType::Enum type) const
{
  ...
  switch (type)
  {
    case xiiXRDeviceType::HMD:
      deviceID = 0;
      break;
    case xiiXRDeviceType::LeftController:
      deviceID = m_iLeftControllerDeviceID;
      break;
    case xiiXRDeviceType::RightController:
...
const xiiXRDeviceState& xiiOpenXRInputDevice::GetDeviceState(xiiXRDeviceID iDeviceID) const
{
  XII_ASSERT_DEV(m_pOpenXR->IsInitialized(), "Need to call 'Initialize' first.");
  XII_ASSERT_DEV(iDeviceID < 3 && iDeviceID >= 0, "Invalid device ID.");
  XII_ASSERT_DEV(m_DeviceState[iDeviceID].m_bDeviceIsConnected, "Invalid device ID.");
  return m_DeviceState[iDeviceID];
}

xiiString xiiOpenXRInputDevice::GetDeviceName(xiiXRDeviceID iDeviceID) const
{
  XII_ASSERT_DEV(m_pOpenXR->IsInitialized(), "Need to call 'Initialize' first.");
  XII_ASSERT_DEV(iDeviceID < 3 && iDeviceID >= 0, "Invalid device ID.");
  XII_ASSERT_DEV(m_DeviceState[iDeviceID].m_bDeviceIsConnected, "Invalid device ID.");
...
xiiBitflags<xiiXRDeviceFeatures> xiiOpenXRInputDevice::GetDeviceFeatures(xiiXRDeviceID iDeviceID) const
{
  XII_ASSERT_DEV(m_pOpenXR->IsInitialized(), "Need to call 'Initialize' first.");
  XII_ASSERT_DEV(iDeviceID < 3 && iDeviceID >= 0, "Invalid device ID.");
  XII_ASSERT_DEV(m_DeviceState[iDeviceID].m_bDeviceIsConnected, "Invalid device ID.");
...
xiiOpenXRInputDevice::xiiOpenXRInputDevice(xiiOpenXR* pOpenXR) :
  xiiXRInputDevice(), m_pOpenXR(pOpenXR)
{
  m_instance = m_pOpenXR->m_instance;
}

XrResult xiiOpenXRInputDevice::CreateActions(XrSession session, XrSpace sceneSpace)
{
  m_session = session;

  // HMD is always connected or we wouldn't have been able to create a session.
  m_DeviceState[0]                      = xiiXRDeviceState();
  m_DeviceState[0].m_bDeviceIsConnected = true;
  m_sActiveProfile[0]                   = "HMD";
  m_SupportedFeatures[0]                = xiiXRDeviceFeatures::AimPose | xiiXRDeviceFeatures::GripPose;

  // Controllers
  for (xiiUInt32 uiControllerId : {m_iLeftControllerDeviceID, m_iRightControllerDeviceID})
  {
    m_DeviceState[uiControllerId] = xiiXRDeviceState();
    m_sActiveProfile[uiControllerId].Clear();
  }

  XrActionSetCreateInfo actionSetInfo{XR_TYPE_ACTION_SET_CREATE_INFO};
  xiiStringUtils::Copy(actionSetInfo.actionSetName, XR_MAX_ACTION_SET_NAME_SIZE, "gameplay");
  xiiStringUtils::Copy(actionSetInfo.localizedActionSetName, XR_MAX_LOCALIZED_ACTION_SET_NAME_SIZE, "Gameplay");
  ...
  XrAction SquexiieClick = XR_NULL_HANDLE;
  ...
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::Trigger, XR_Trigger, XR_ACTION_TYPE_FLOAT_INPUT, Trigger), DestroyActions);

  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::Select, XR_Select_Click, XR_ACTION_TYPE_BOOLEAN_INPUT, SelectClick), DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::Menu, XR_Menu_Click, XR_ACTION_TYPE_BOOLEAN_INPUT, MenuClick), DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::Squexiie, XR_Squexiie_Click, XR_ACTION_TYPE_BOOLEAN_INPUT, SquexiieClick), DestroyActions);

  XR_SUCCEED_OR_CLEANUP_LOG(
    CreateAction(xiiXRDeviceFeatures::PrimaryAnalogStick, XR_Primary_Analog_Stick_Axis, XR_ACTION_TYPE_VECTOR2F_INPUT, PrimaryAnalogStickAxis),
    DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(
    CreateAction(xiiXRDeviceFeatures::PrimaryAnalogStickClick, XR_Primary_Analog_Stick_Click, XR_ACTION_TYPE_BOOLEAN_INPUT, PrimaryAnalogStickClick),
    DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(
    CreateAction(xiiXRDeviceFeatures::PrimaryAnalogStickTouch, XR_Primary_Analog_Stick_Touch, XR_ACTION_TYPE_BOOLEAN_INPUT, PrimaryAnalogStickTouch),
    DestroyActions);

  XR_SUCCEED_OR_CLEANUP_LOG(
    CreateAction(xiiXRDeviceFeatures::SecondaryAnalogStick, XR_Secondary_Analog_Stick_Axis, XR_ACTION_TYPE_VECTOR2F_INPUT, SecondaryAnalogStickAxis),
    DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::SecondaryAnalogStickClick, XR_Secondary_Analog_Stick_Click, XR_ACTION_TYPE_BOOLEAN_INPUT,
                                         SecondaryAnalogStickClick),
                            DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::SecondaryAnalogStickTouch, XR_Secondary_Analog_Stick_Touch, XR_ACTION_TYPE_BOOLEAN_INPUT,
                                         SecondaryAnalogStickTouch),
                            DestroyActions);

  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::GripPose, XR_Grip_Pose, XR_ACTION_TYPE_POSE_INPUT, GripPose), DestroyActions);
  XR_SUCCEED_OR_CLEANUP_LOG(CreateAction(xiiXRDeviceFeatures::AimPose, XR_Aim_Pose, XR_ACTION_TYPE_POSE_INPUT, AimPose), DestroyActions);
  ...
  Bind motionController[] = {
    ...
    {MenuClick, "/user/hand/left/input/menu"},
    {SquexiieClick, "/user/hand/left/input/squexiie"},
    ...
    {MenuClick, "/user/hand/right/input/menu"},
    {SquexiieClick, "/user/hand/right/input/squexiie"},
  ...
  spaceCreateInfo.poseInActionSpace = m_pOpenXR->ConvertTransform(xiiTransform::IdentityTransform());
...
void xiiOpenXRInputDevice::DestroyActions()
{
...
XrPath xiiOpenXRInputDevice::CreatePath(const char* szPath)
{
  ...
  {
    xiiLog::Error("OpenXR path conversion failure: {0}", szPath);
  }
  return path;
}

XrResult xiiOpenXRInputDevice::CreateAction(xiiXRDeviceFeatures::Enum feature, const char* actionName, XrActionType actionType, XrAction& out_action)
{
  ...
  xiiStringUtils::Copy(actionCreateInfo.actionName, XR_MAX_ACTION_NAME_SIZE, actionName);
  xiiStringUtils::Copy(actionCreateInfo.localizedActionName, XR_MAX_LOCALIZED_ACTION_NAME_SIZE, actionName);

  XR_SUCCEED_OR_CLEANUP_LOG(xrCreateAction(m_ActionSet, &actionCreateInfo, &out_action), voidFunction);

  xiiStringBuilder sLeft(m_subActionPrefix[0], actionName);
  xiiStringBuilder sRight(m_subActionPrefix[1], actionName);
...
XrResult xiiOpenXRInputDevice::SuggestInteractionProfileBindings(const char* szInteractionProfile, const char* szNiceName, xiiArrayPtr<Bind> bindings)
{
  XrInstance instance = m_pOpenXR->m_instance;

  XrPath InteractionProfile = CreatePath(szInteractionProfile);

  xiiDynamicArray<XrActionSuggestedBinding> xrBindings;
...
XrResult xiiOpenXRInputDevice::AttachSessionActionSets(XrSession session)
{
  m_session = session;
  XrSessionActionSetsAttachInfo  attachInfo{XR_TYPE_SESSION_ACTION_SETS_ATTACH_INFO};
  xiiHybridArray<XrActionSet, 1> actionSets;
...
XrResult xiiOpenXRInputDevice::UpdateCurrentInteractionProfile()
{
  ...
  auto GetActiveControllerProfile = [this](xiiUInt32 uiSide) -> XrPath {
    ...
    if (res == XR_SUCCESS)
    {
      ...
      if (state.interactionProfile != XR_NULL_PATH && !m_InteractionProfileToNiceName.Contains(state.interactionProfile))
      {
        ...
        XII_REPORT_FAILURE("Unknown interaction profile was selected by the OpenXR runtime: '{}'", buffer);
      }
#endif
  ...
  for (xiiUInt32 uiSide : {0, 1})
  {
    const xiiUInt32 uiControllerId   = uiSide == 0 ? m_iLeftControllerDeviceID : m_iRightControllerDeviceID;
...
void xiiOpenXRInputDevice::InitializeDevice() {}

void xiiOpenXRInputDevice::RegisterInputSlots()
{
  for (const Action& action : m_booleanActions)
  {
    for (xiiUInt32 uiSide : {0, 1})
    {
      RegisterInputSlot(action.m_sKey[uiSide], action.m_sKey[uiSide], xiiInputSlotFlags::IsButton);
    }
  }
  for (const Action& action : m_floatActions)
  {
    for (xiiUInt32 uiSide : {0, 1})
    {
      RegisterInputSlot(action.m_sKey[uiSide], action.m_sKey[uiSide], xiiInputSlotFlags::IsAnalogTrigger);
    }
  }
  for (const Vec2Action& action : m_vec2Actions)
  {
    for (xiiUInt32 uiSide : {0, 1})
    {
      RegisterInputSlot(action.m_sKey_negx[uiSide], action.m_sKey_negx[uiSide], xiiInputSlotFlags::IsAnalogStick);
      RegisterInputSlot(action.m_sKey_posx[uiSide], action.m_sKey_posx[uiSide], xiiInputSlotFlags::IsAnalogStick);
      RegisterInputSlot(action.m_sKey_negy[uiSide], action.m_sKey_negy[uiSide], xiiInputSlotFlags::IsAnalogStick);
      RegisterInputSlot(action.m_sKey_posy[uiSide], action.m_sKey_posy[uiSide], xiiInputSlotFlags::IsAnalogStick);
    }
  }
}

XrResult xiiOpenXRInputDevice::UpdateActions()
{
  if (m_session == XR_NULL_HANDLE)
    return XR_SUCCESS;

  XII_PROFILE_SCOPE("UpdateActions");
  const XrFrameState& frameState = m_pOpenXR->m_frameState;

  xiiHybridArray<XrActiveActionSet, 1> activeActionSets;
  ...
  for (xiiUInt32 uiSide : {0, 1})
  {
    ...
    auto UpdatePose = [](xiiVec3& vPosition, xiiQuat& qRotation, bool& m_bIsValid, const XrSpaceLocation& viewInScene) {
      if ((viewInScene.locationFlags & (XR_SPACE_LOCATION_POSITION_VALID_BIT | XR_SPACE_LOCATION_ORIENTATION_VALID_BIT)) ==
          (XR_SPACE_LOCATION_POSITION_VALID_BIT | XR_SPACE_LOCATION_ORIENTATION_VALID_BIT))
      {
        vPosition  = xiiOpenXR::ConvertPosition(viewInScene.pose.position);
        qRotation  = xiiOpenXR::ConvertOrientation(viewInScene.pose.orientation);
        m_bIsValid = true;
      }
      else
      {
    ...
    xiiXRDeviceState& state       = m_DeviceState[uiControllerId];
...
void xiiOpenXRInputDevice::UpdateControllerState()
{
  for (xiiUInt32 uiSide : {0, 1})
  {
    const xiiUInt32 uiControllerId   = uiSide == 0 ? m_iLeftControllerDeviceID : m_iRightControllerDeviceID;
    const bool      bDeviceConnected = m_SupportedFeatures[uiControllerId].IsSet(xiiXRDeviceFeatures::AimPose);
    ...
    if (!m_DeviceState[uiControllerId].m_bDeviceIsConnected && bDeviceConnected)
    {
      ...
      xiiXRDeviceEventData data;
      data.m_Type     = xiiXRDeviceEventData::Type::DeviceAdded;
    ...
    else if (m_DeviceState[uiControllerId].m_bDeviceIsConnected && !bDeviceConnected)
    {
      // Disconnected
      m_DeviceState[uiControllerId]       = xiiXRDeviceState();
      m_SupportedFeatures[uiControllerId] = xiiXRDeviceFeatures::None;

      xiiXRDeviceEventData data;
      data.m_Type     = xiiXRDeviceEventData::Type::DeviceRemoved;
      data.uiDeviceID = uiControllerId;
      m_InputEvents.Broadcast(data);
    }
  }
...
xiiOpenXRInputDevice::Vec2Action::Vec2Action(xiiXRDeviceFeatures::Enum feature, XrAction action, xiiStringView sLeft, xiiStringView sRight)
{
  m_Feature = feature;
  m_Action  = action;

  xiiStringView sides[2] = {sLeft, sRight};
  for (xiiUInt32 uiSide : {0, 1})
  {
    xiiStringBuilder temp = sides[uiSide];
    temp.Append("_negx");
