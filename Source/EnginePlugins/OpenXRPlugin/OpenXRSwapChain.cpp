...
void xiiGALOpenXRSwapChain::AcquireNextRenderTarget(xiiGALDevice* pDevice)
{
  XII_PROFILE_SCOPE("AcquireNextRenderTarget");
  XII_ASSERT_DEBUG(m_bImageAcquired == false, "PresentRenderTarget was not called.");
  m_bImageAcquired = true;
  auto pOpenXR     = static_cast<xiiOpenXR*>(m_pXrInterface);
  ...
  {
    XII_PROFILE_SCOPE("AquireAndWait");
...
void xiiGALOpenXRSwapChain::PresentRenderTarget(xiiGALDevice* pDevice)
{
  ...
  // Thus, we delay PresentRenderTarget here and xiiOpenXR will call PresentRenderTarget instead after the companion window was updated.
...
void xiiGALOpenXRSwapChain::PresentRenderTarget() const
{
  auto pOpenXR = static_cast<xiiOpenXR*>(m_pXrInterface);
  XII_PROFILE_SCOPE("xrReleaseSwapchainImage");
...
xiiResult xiiGALOpenXRSwapChain::InitPlatform(xiiGALDevice* pDevice)
{
...
xiiResult xiiGALOpenXRSwapChain::DeInitPlatform(xiiGALDevice* pDevice)
{
  DeinitSwapChain();
  return XII_SUCCESS;
}

xiiGALOpenXRSwapChain::xiiGALOpenXRSwapChain(xiiOpenXR* pXrInterface, xiiGALMSAASampleCount::Enum msaaCount) :
  xiiGALXRSwapChain(pXrInterface)
{
...
XrResult xiiGALOpenXRSwapChain::SelectSwapchainFormat(int64_t& colorFormat, int64_t& depthFormat)
{
  auto pOpenXR = static_cast<xiiOpenXR*>(m_pXrInterface);
...
XrResult xiiGALOpenXRSwapChain::CreateSwapchainImages(Swapchain& swapchain, SwapchainType type)
{
  ...
  for (xiiUInt32 i = 0; i < swapchain.imageCount; i++)
  {
    ...
    textureDesc.SetAsRenderTarget(backBufferDesc.Width, backBufferDesc.Height, xiiOpenXR::ConvertTextureFormat(swapchain.format), xiiGALMSAASampleCount::Enum(backBufferDesc.SampleDesc.Count));
    textureDesc.m_uiArraySize            = backBufferDesc.ArraySize;
    textureDesc.m_pExisitingNativeObject = pTex;
    // Need to add a ref as the XII texture will always remove one on destruction.
    pTex->AddRef();
    if (type == SwapchainType::Color)
    {
...
XrResult xiiGALOpenXRSwapChain::InitSwapChain(xiiGALMSAASampleCount::Enum msaaCount)
{
  auto pOpenXR = static_cast<xiiOpenXR*>(m_pXrInterface);
  ...
  if (viewCount != 2)
  {
    xiiLog::Error("No stereo view configuration present, can't create swap chain");
    DeinitSwapChain();
    return XR_ERROR_INITIALIZATION_FAILED;
  }
  xiiHybridArray<XrViewConfigurationView, 2> views;
  ...
  else
  {
    ...
    tcd.SetAsRenderTarget(m_CurrentSize.width, m_CurrentSize.height, xiiGALResourceFormat::DFloat, msaaCount);
    tcd.m_uiArraySize = 2;
...
void xiiGALOpenXRSwapChain::DeinitSwapChain()
{
  auto pOpenXR = static_cast<xiiOpenXR*>(m_pXrInterface);

  xiiGALDevice* pDevice = xiiGALDevice::GetDefaultDevice();

  for (xiiGALTextureHandle rt : m_hColorRTs)
  {
  ...
  if (pOpenXR->GetDepthComposition())
  {
    for (xiiGALTextureHandle rt : m_hDepthRTs)
    {
      pDevice->DestroyTexture(rt);
    }
  }
