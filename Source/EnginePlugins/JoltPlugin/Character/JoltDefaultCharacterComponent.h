...
struct xiiMsgApplyRootMotion;

using xiiJoltDefaultCharacterComponentManager = xiiComponentManager<class xiiJoltDefaultCharacterComponent, xiiBlockStorageType::FreeList>;

class XII_JOLTPLUGIN_DLL xiiJoltDefaultCharacterComponent : public xiiJoltCharacterControllerComponent
{
  XII_DECLARE_COMPONENT_TYPE(xiiJoltDefaultCharacterComponent, xiiJoltCharacterControllerComponent, xiiJoltDefaultCharacterComponentManager);
...
public:
  xiiJoltDefaultCharacterComponent();
  ~xiiJoltDefaultCharacterComponent();

  enum class GroundState : xiiUInt8
  {
  ...
  xiiAngle m_RotateSpeed           = xiiAngle::Degree(90.0f); ///< [ property ] How many degrees per second the character turns
  ...
  xiiHashedString          m_sWalkSurfaceInteraction;         ///< [ property ] The surface interaction to spawn regularly when walking
  xiiSurfaceResourceHandle m_hFallbackWalkSurface;            ///< [ property ] The surface type to use for interactions, when no other surface type is available
  ...
  void TeleportCharacter(const xiiVec3& vGlobalFootPosition);
  ...
  struct Config
  {
    ...
    xiiVec3         m_vVelocity            = xiiVec3::ZeroVector();
    float           m_fPushDownForce       = 0;
    xiiHashedString m_sGroundInteraction;
...
protected:
  ...
  virtual void OnApplyRootMotion(xiiMsgApplyRootMotion& msg);
  ...
  xiiUInt8     m_uiInputJumpBit : 1;
  xiiUInt8     m_uiInputCrouchBit : 1;
  xiiUInt8     m_uiInputRunBit : 1;
  xiiUInt8     m_uiIsCrouchingBit : 1;
  xiiAngle     m_InputRotateZ;
  xiiVec2      m_vInputDirection          = xiiVec2::ZeroVector();
  float        m_fVelocityUp              = 0.0f;
  float        m_fNextCylinderHeight      = 0;
  float        m_fAccumulatedWalkDistance = 0.0f;
  xiiVec2      m_vVelocityLateral         = xiiVec2::ZeroVector();
  xiiTransform m_PreviousTransform;
  ...
  xiiGameObjectHandle m_hHeadObject;

  xiiVec3 m_vAbsoluteRootMotion = xiiVec3::ZeroVector();
