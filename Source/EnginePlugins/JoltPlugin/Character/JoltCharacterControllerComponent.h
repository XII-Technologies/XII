...
struct xiiMsgMoveCharacterController;
...
XII_DECLARE_FLAGS(xiiUInt32, xiiJoltCharacterDebugFlags, PrintState, VisShape, VisContacts, VisCasts, VisGroundContact, VisFootCheck);
XII_DECLARE_REFLECTABLE_TYPE(XII_JOLTPLUGIN_DLL, xiiJoltCharacterDebugFlags);

class XII_JOLTPLUGIN_DLL xiiJoltCharacterControllerComponent : public xiiComponent
{
  XII_DECLARE_ABSTRACT_COMPONENT_TYPE(xiiJoltCharacterControllerComponent, xiiComponent);
...
protected:
  ...
  //////////////////////////////////////////////////////////////////////////
  // xiiJoltCharacterControllerComponent
...
public:
  ...
  struct ContactPoint
  {
    ...
    xiiVec3         m_vPosition;
    xiiVec3         m_vSurfaceNormal;
    xiiVec3         m_vContactNormal;
    JPH::BodyID     m_BodyID;
  ...
  // GroundState DetermineGroundState(ContactPoint& out_Contact, const xiiVec3& vFootPosition, float fAllowedStepUp, float fAllowedStepDown, float fCylinderRadius) const;
  ...
  // bool TestShapeOverlap(const xiiVec3& vGlobalFootPos, float fNewShapeHeight) const;
  ...
  void SetObjectToIgnore(xiiUInt32 uiObjectFilterID);
  void ClearObjectToIgnore();

public:                                                                   // [ properties ]
  xiiUInt8                                m_uiCollisionLayer         = 0; // [ property ]
  xiiUInt8                                m_uiPresenceCollisionLayer = 0; // [ property ]
  xiiBitflags<xiiJoltCharacterDebugFlags> m_DebugFlags;                   // [ property ]

  /// \brief The maximum slope that the character can walk up.
  void     SetMaxClimbingSlope(xiiAngle slope);                       // [ property ]
...
private:                                              // [ properties ]
  xiiAngle m_MaxClimbingSlope = xiiAngle::Degree(45); // [ property ]
...
protected:
  ...
  XII_ALWAYS_INLINE float GetUpdateTimeDelta() const { return m_fUpdateTimeDelta; }

  /// \brief Returns the inverse of update time delta.
  XII_ALWAYS_INLINE float GetInverseUpdateTimeDelta() const { return m_fInverseUpdateTimeDelta; }
  ...
  void CollectContacts(xiiDynamicArray<ContactPoint>& out_Contacts, const JPH::Shape* pShape, const xiiVec3& vQueryPosition, const xiiQuat& qQueryRotation, float fCollisionTolerance) const;
  ...
  xiiVec3 GetContactVelocityAndPushAway(const ContactPoint& contact, float fPushForce);
  ...
  void SpawnContactInteraction(const ContactPoint& contact, const xiiHashedString& sSurfaceInteraction, xiiSurfaceResourceHandle hFallbackSurface, const xiiVec3& vInteractionNormal = xiiVec3(0, 0, 1));
  ...
  struct ShapeContacts
  {
    ...
    {
      Default     = 0,
      FlatGround  = XII_BIT(0),
      SteepGround = XII_BIT(1),
      Ceiling     = XII_BIT(2),
    };
  ...
  // static xiiBitflags<ShapeContacts> ClassifyContacts(const xiiDynamicArray<ContactPoint>& contacts, xiiAngle maxSlopeAngle, const xiiVec3& vCenterPos, xiiUInt32* out_pBestGroundContact);

  using ContactFilter = xiiDelegate<bool(const ContactPoint&)>;

  /// \brief Returns the index or xiiInvalidIndex of the contact point whose normal is closest to vNormal.
  ///
  /// \param filter allows to ignore contact points by other criteria, such as position.
  // static xiiUInt32 FindFlattestContact(const xiiDynamicArray<ContactPoint>& contacts, const xiiVec3& vNormal, ContactFilter filter);

  void VisualizeContact(const ContactPoint& contact, const xiiColor& color) const;
  void VisualizeContacts(const xiiDynamicArray<ContactPoint>& contacts, const xiiColor& color) const;
...
private:
  ...
  void MovePresenceBody(xiiTime deltaTime);

  xiiUInt32 m_uiPresenceBodyID = xiiInvalidIndex;

  xiiJoltBodyFilter m_BodyFilter;
