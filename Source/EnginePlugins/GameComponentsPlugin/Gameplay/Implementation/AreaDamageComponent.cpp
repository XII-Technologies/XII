    ...
    if (!hit.m_hActorObject.IsInvalidated())
    {
      ...
      if (GetWorld()->TryGetObject(hit.m_hActorObject, pObject))
      {
        ...
        else
        {
          ...
          vDirToTarget = xiiVec3::MakeRandomDirection(GetWorld()->GetRandomNumberGenerator());
        }
        ...
        if (m_fDamage != 0.0f)
        {
          ...
          pObject->PostEventMessage(msg, this, xiiTime::MakeFromMilliseconds(200));
        }
      }
    }
  }
...
void xiiAreaDamageComponent::SerializeComponent(xiiWorldWriter& inout_stream) const
{
...
void xiiAreaDamageComponent::DeserializeComponent(xiiWorldReader& inout_stream)
{
