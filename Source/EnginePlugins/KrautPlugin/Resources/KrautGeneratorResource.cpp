...
// clang-format off
XII_BEGIN_DYNAMIC_REFLECTED_TYPE(xiiKrautGeneratorResource, 1, xiiRTTIDefaultAllocator<xiiKrautGeneratorResource>)
XII_END_DYNAMIC_REFLECTED_TYPE;

XII_RESOURCE_IMPLEMENT_COMMON_CODE(xiiKrautGeneratorResource);
// clang-format on

xiiKrautGeneratorResource::xiiKrautGeneratorResource() :
  xiiResource(DoUpdate::OnAnyThread, 1)
{
}

static XII_ALWAYS_INLINE xiiVec3 ToXII(const aeVec3& v)
{
  return xiiVec3(v.x, v.y, v.z);
}

static XII_ALWAYS_INLINE xiiVec3 ToXIISwizzle(const aeVec3& v)
{
  return xiiVec3(v.x, v.z, v.y);
}

struct AoData
{
  xiiDynamicArray<xiiDynamicArray<xiiBoundingSphere>>* m_pOcclusionSpheres;
  float                                                m_fAO;
  xiiUInt32                                            m_uiBranch;
  xiiVec3                                              m_vPosition;
  xiiUInt32*                                           m_pOccChecks;
};

struct AoPositionResult
{
  xiiVec3I32 m_iPos; // snapped to a regular grid at some fixed resolution
...
static void GenerateAmbientOcclusionSpheres(xiiDynamicOctree& octree, const xiiBoundingBox& bbox, xiiDynamicArray<xiiDynamicArray<xiiBoundingSphere>>& occlusionSpheres, const Kraut::TreeStructure& treeStructure)
{
  ...
  for (xiiUInt32 b = 0; b < treeStructure.m_BranchStructures.size(); ++b)
  {
    ...
    for (xiiUInt32 n = 4; n < branch.m_Nodes.size(); ++n)
    {
      ...
      if (fRequiredDistance <= 0)
      {
        ...
        const xiiVec3 pos = reinterpret_cast<const xiiVec3&>(branch.m_Nodes[n].m_vPosition);
...
xiiKrautTreeResourceHandle xiiKrautGeneratorResource::GenerateTreeWithGoodSeed(xiiUInt16 uiGoodSeedIndex) const
{
...
class xiiKrautResourceLoader : public xiiResourceTypeLoader
{
public:
  struct LoadedData
  {
  ...
  virtual xiiResourceLoadData OpenDataStream(const xiiResource* pResource) override
  {
    ...
    xiiResourceLock<xiiKrautGeneratorResource> pGenerator(m_hGeneratorResource, xiiResourceAcquireMode::BlockTillLoaded);
  ...
  xiiUInt32                       m_uiRandomSeed = 0;
  xiiKrautGeneratorResourceHandle m_hGeneratorResource;
};

xiiKrautTreeResourceHandle xiiKrautGeneratorResource::GenerateTree(xiiUInt32 uiRandomSeed) const
{
  XII_PROFILE_SCOPE("Kraut: GenerateTree");

  xiiStringBuilder sResourceID   = GetResourceID();
  xiiStringBuilder sResourceDesc = GetResourceDescription();
  ...
  xiiKrautTreeResourceHandle hTree = xiiResourceManager::GetExistingResource<xiiKrautTreeResource>(sResourceID);
  if (hTree.IsValid())
  {
    return hTree;
  }

  xiiUniquePtr<xiiKrautResourceLoader> pLoader = XII_DEFAULT_NEW(xiiKrautResourceLoader);
  pLoader->m_hGeneratorResource                = xiiKrautGeneratorResourceHandle(const_cast<xiiKrautGeneratorResource*>(this));
  pLoader->m_uiRandomSeed                      = uiRandomSeed;

  auto hRes = xiiResourceManager::GetExistingResourceOrCreateAsync<xiiKrautTreeResource>(sResourceID, std::move(pLoader), m_hFallbackResource);

  if (!m_hFallbackResource.IsValid())
  {
    m_hFallbackResource = hRes;
    xiiResourceManager::PreloadResource(m_hFallbackResource);
  }
...
void xiiKrautGeneratorResource::GenerateTreeDescriptor(xiiKrautTreeResourceDescriptor& dstDesc, xiiUInt32 uiRandomSeed) const
{
  ...
  xiiBoundingBox bbox2;
  bbox2.SetElements(ToXIISwizzle(bbox.m_vMin), ToXIISwizzle(bbox.m_vMax));

  // data for ambient occlusion computation
  xiiDynamicArray<xiiDynamicArray<xiiBoundingSphere>> occlusionSpheres;
  xiiDynamicOctree                                    octree;
  xiiUInt32                                           uiOccVertices = 0;
  xiiUInt32                                           uiOccChecks   = 0;
  xiiStaticRingBuffer<AoPositionResult, 16>           aoResults;
  ...
  for (xiiUInt32 lodIdx = 0; lodIdx < dstDesc.m_Lods.GetCapacity(); ++lodIdx)
  {
    ...
    xiiUInt32 uiMaxVertices         = 0;
    xiiUInt32 uiMaxTriangles        = 0;
    xiiUInt32 uiTriangleInSubmeshes = 0;

    // reserve enough memory
    {
      for (xiiUInt32 branchIdx = 0; branchIdx < mesh.m_BranchMeshes.size(); ++branchIdx)
      {
        for (xiiUInt32 geometryType = 0; geometryType < Kraut::BranchGeometryType::ENUM_COUNT; ++geometryType)
        {
      ...
      xiiLog::Debug("Vertices = {}, Triangles = {}", uiMaxVertices, uiMaxTriangles);
    }

    for (xiiUInt32 geometryType = 0; geometryType < Kraut::BranchGeometryType::ENUM_COUNT; ++geometryType)
    {
      for (xiiUInt32 branchType = 0; branchType < Kraut::BranchType::ENUM_COUNT; ++branchType)
      {
        const xiiUInt32 uiFirstTriangleIdx = dstMesh.m_Triangles.GetCount();
        ...
        for (xiiUInt32 branchIdx = 0; branchIdx < mesh.m_BranchMeshes.size(); ++branchIdx)
        {
          ...
          const xiiUInt32 uiVertexOffset = dstMesh.m_Vertices.GetCount();

          for (xiiUInt32 vidx = 0; vidx < srcMesh.m_Vertices.size(); ++vidx)
          {
            const auto& srcVtx = srcMesh.m_Vertices[vidx];

            auto& dstVtx               = dstMesh.m_Vertices.ExpandAndGetRef();
            dstVtx.m_vPosition         = ToXIISwizzle(srcVtx.m_vPosition) * fVertexScale;
            dstVtx.m_uiColorVariation  = srcVtx.m_uiColorVariation;
            dstVtx.m_vNormal           = ToXIISwizzle(srcVtx.m_vNormal);
            dstVtx.m_vTexCoord         = ToXII(srcVtx.m_vTexCoord);
            dstVtx.m_vTangent          = ToXIISwizzle(srcVtx.m_vTangent);
            dstVtx.m_fAmbientOcclusion = CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition));

            if (geometryType == Kraut::BranchGeometryType::Leaf)
            {
              const float fSize = dstVtx.m_vTexCoord.z * 0.7f;

              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) + xiiVec3(fSize, 0, 0));
              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) - xiiVec3(fSize, 0, 0));
              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) + xiiVec3(0, fSize, 0));
              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) - xiiVec3(0, fSize, 0));
              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) + xiiVec3(0, 0, fSize));
              dstVtx.m_fAmbientOcclusion += CheckOcclusion(branchIdx, reinterpret_cast<const xiiVec3&>(srcVtx.m_vPosition) - xiiVec3(0, 0, fSize));
            ...
            if (srcVtx.m_uiBranchNodeIdx >= treeStructure.m_BranchStructures[branchIdx].m_Nodes.size())
            {
              ...
              xiiUInt32 nodeIdx = srcVtx.m_uiBranchNodeIdx;
              nodeIdx -= treeStructure.m_BranchStructures[branchIdx].m_Nodes.size();

              const xiiVec3 lastPos = ToXIISwizzle(treeStructure.m_BranchStructures[branchIdx].m_Nodes.back().m_vPosition);
              const xiiVec3 tipPos  = ToXIISwizzle(treeLod.m_BranchLODs[branchIdx].m_TipNodes[nodeIdx].m_vPosition);
            ...
            {
              // trunk
              dstVtx.m_fBendAndFlutterStrength = xiiMath::Square(fBranchDist);
            ...
            else
            {
              ...
              dstVtx.m_fBendAndFlutterStrength = xiiMath::Square(branchExtra.m_fBendinessToAnchor + fBranchDist);
              dstVtx.m_uiBranchLevel           = 1; // or 2 etc, but currently not used

              xiiInt32 iMainBranchIdx = branchIdx;

              xiiInt32  iTrunkIdx      = branchExtra.m_iParentBranch;
              xiiUInt32 uiTrunkNodeIdx = branchExtra.m_uiParentBranchNodeID;
              ...
              dstVtx.m_fAnchorBendStrength = xiiMath::Square(trunkBranch.m_Nodes[uiTrunkNodeIdx].m_fBendinessAlongBranch);
          ...
          for (xiiUInt32 tidx = 0; tidx < srcMesh.m_Triangles.size(); ++tidx)
          {
        ...
        subMesh.m_uiFirstTriangle = static_cast<xiiUInt16>(uiFirstTriangleIdx);
        subMesh.m_uiNumTriangles  = static_cast<xiiUInt16>(dstMesh.m_Triangles.GetCount() - uiFirstTriangleIdx);

        uiTriangleInSubmeshes += subMesh.m_uiNumTriangles;

        for (const auto& srcMat : m_pDescriptor->m_Materials)
        {
          if ((xiiUInt32)srcMat.m_BranchType == branchType && (xiiUInt32)srcMat.m_MaterialType == geometryType)
          {
    ...
    XII_ASSERT_DEV(uiTriangleInSubmeshes == uiMaxTriangles, "Number of triangles is incorrect.");

    // compute the leaf center
    if (lodIdx == 0)
    {
      xiiBoundingBox leafBox;
      leafBox.SetInvalid();
      ...
      for (xiiUInt32 branchIdx = 0; branchIdx < mesh.m_BranchMeshes.size(); ++branchIdx)
      {
        ...
        for (xiiUInt32 vtxIdx = 0; vtxIdx < srcMesh.m_Vertices.size(); ++vtxIdx)
        {
          leafBox.ExpandToInclude(ToXIISwizzle(srcMesh.m_Vertices[vtxIdx].m_vPosition));
        }
      }
...
xiiResourceLoadDesc xiiKrautGeneratorResource::UnloadData(Unload WhatToUnload)
{
  m_pDescriptor.Clear();
...
xiiResourceLoadDesc xiiKrautGeneratorResource::UpdateContent(xiiStreamReader* Stream)
{
  xiiResourceLoadDesc res;
  res.m_uiQualityLevelsDiscardable = 0;
  res.m_uiQualityLevelsLoadable    = 0;
  ...
  // skip the absolute file path data that the standard file reader writes into the stream
  {
    ...
    if (xiiPathUtils::HasExtension(sAbsFilePath, ".tree"))
    {
      return res;
    }
  }
  ...
  m_pDescriptor = XII_DEFAULT_NEW(xiiKrautGeneratorResourceDescriptor);
...
void xiiKrautGeneratorResource::UpdateMemoryUsage(MemoryUsage& out_NewMemoryUsage)
{
  ...
  {
    out_NewMemoryUsage.m_uiMemoryCPU += sizeof(xiiKrautGeneratorResourceDescriptor) + m_pDescriptor->m_Materials.GetHeapMemoryUsage();
  }
}
...
void xiiKrautGeneratorResource::InitializeExtraData(TreeStructureExtraData& extraData, const Kraut::TreeStructure& treeStructure, xiiUInt32 uiRandomSeed) const
{
  ...
  for (xiiUInt32 branchIdx = 0; branchIdx < treeStructure.m_BranchStructures.size(); ++branchIdx)
  {
    ...
    dstData.m_uiParentBranchNodeID = static_cast<xiiUInt16>(srcBranch.m_uiParentBranchNodeID);
...
void xiiKrautGeneratorResource::ComputeDistancesAlongBranches(TreeStructureExtraData& extraData, const Kraut::TreeStructure& treeStructure) const
{
  for (xiiUInt32 branchIdx = 0; branchIdx < treeStructure.m_BranchStructures.size(); ++branchIdx)
  {
    const auto& srcBranch = treeStructure.m_BranchStructures[branchIdx];
    ...
    for (xiiUInt32 nodeIdx = 1; nodeIdx < srcBranch.m_Nodes.size(); ++nodeIdx)
    {
...
void xiiKrautGeneratorResource::ComputeDistancesToAnchors(TreeStructureExtraData& extraData, const Kraut::TreeStructure& treeStructure) const
{
  for (xiiUInt32 branchIdx = 0; branchIdx < treeStructure.m_BranchStructures.size(); ++branchIdx)
  {
...
void xiiKrautGeneratorResource::ComputeBendinessAlongBranches(TreeStructureExtraData& extraData, const Kraut::TreeStructure& treeStructure, float fWoodBendiness, float fTwigBendiness) const
{
  for (xiiUInt32 branchIdx = 0; branchIdx < treeStructure.m_BranchStructures.size(); ++branchIdx)
  {
    const auto& srcBranch = treeStructure.m_BranchStructures[branchIdx];
...
void xiiKrautGeneratorResource::ComputeBendinessToAnchors(TreeStructureExtraData& extraData, const Kraut::TreeStructure& treeStructure) const
{
  for (xiiUInt32 branchIdx = 0; branchIdx < treeStructure.m_BranchStructures.size(); ++branchIdx)
  {
    auto& thisBranch = extraData.m_Branches[branchIdx];
...
void xiiKrautGeneratorResource::GenerateExtraData(TreeStructureExtraData& extraData, const Kraut::TreeStructureDesc& treeStructureDesc, const Kraut::TreeStructure& treeStructure, xiiUInt32 uiRandomSeed, float fWoodBendiness, float fTwigBendiness) const
{
