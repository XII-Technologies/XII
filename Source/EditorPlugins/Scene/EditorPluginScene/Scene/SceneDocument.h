...
public:
  xiiSceneDocument(const char* szDocumentPath, DocumentType DocumentType);
  ~xiiSceneDocument();
  ...
  bool IsObjectEditorPrefab(const xiiUuid& object, xiiUuid* out_PrefabAssetGuid = nullptr) const;

  /// \brief Determines whether the given object is an engine prefab
  bool IsObjectEnginePrefab(const xiiUuid& object, xiiUuid* out_PrefabAssetGuid = nullptr) const;
  ...
  void         HandleVisualScriptActivityMsg(const xiiVisualScriptActivityMsgToEditor* pMsg);
