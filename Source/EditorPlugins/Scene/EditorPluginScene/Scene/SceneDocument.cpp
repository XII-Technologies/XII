...
xiiSceneDocument::xiiSceneDocument(xiiStringView sDocumentPath, DocumentType documentType) :
  xiiGameObjectDocument(sDocumentPath, XII_DEFAULT_NEW(xiiSceneObjectManager))
{
...
void xiiSceneDocument::InitializeAfterLoading(bool bFirstTimeCreation)
{
  ...
  m_ObjectMirror.SetFilterFunction([pManager = GetObjectManager()](const xiiDocumentObject* pObject, xiiStringView sProperty) -> bool { return pManager->IsUnderRootProperty("Settings", pObject, sProperty); });
  // (Remote IPC mirror only sends scene)
  m_pMirror->SetFilterFunction([pManager = GetObjectManager()](const xiiDocumentObject* pObject, xiiStringView sProperty) -> bool { return pManager->IsUnderRootProperty("Children", pObject, sProperty); });
...
xiiStatus xiiSceneDocument::CreatePrefabDocumentFromSelection(xiiStringView sFile, const xiiRTTI* pRootType, xiiDelegate<void(xiiAbstractObjectNode*)> adjustGraphNodeCB /* = {} */, xiiDelegate<void(xiiDocumentObject*)> adjustNewNodesCB /* = {} */, xiiDelegate<void(xiiAbstractObjectGraph& graph, xiiDynamicArray<xiiAbstractObjectNode*>& graphRootNodes)> finalizeGraphCB /* = {} */)
{
  ...
  auto finalizeGraph = [this, &varChildren](xiiAbstractObjectGraph& ref_graph, xiiDynamicArray<xiiAbstractObjectNode*>& ref_graphRootNodes) {
...
bool xiiSceneDocument::CopySelectedObjects(xiiAbstractObjectGraph& ref_graph, xiiStringBuilder& out_sMimeType) const
{
  out_sMimeType = "application/xiiEditor.xiiAbstractGraph";
  return CopySelectedObjects(ref_graph, nullptr);
}

bool xiiSceneDocument::CopySelectedObjects(xiiAbstractObjectGraph& ref_graph, xiiMap<xiiUuid, xiiUuid>* out_pParents) const
{
...
bool xiiSceneDocument::Paste(const xiiArrayPtr<PasteInfo>& info, const xiiAbstractObjectGraph& objectGraph, bool bAllowPickedPosition, xiiStringView sMimeType)
{
  const auto& ctxt = xiiQtEngineViewWidget::GetInteractionContext();
...
void xiiSceneDocument::UpdateAssetDocumentInfo(xiiAssetDocumentInfo* pInfo) const
{
  ...
  if (m_DocumentType == DocumentType::Prefab)
  {
    ...
    for (auto prop : pSettings->m_ExposedProperties)
    {
      ...
      const xiiExposedParameter* pSourceParameter = nullptr;
