...
xiiSceneViewContext::~xiiSceneViewContext() {}
...
void xiiSceneViewContext::SetupRenderTarget(xiiGALSwapChainHandle hSwapChain, const xiiGALRenderTargets* renderTargets, xiiUInt16 uiWidth, xiiUInt16 uiHeight)
{
  xiiEngineProcessViewContext::SetupRenderTarget(hSwapChain, renderTargets, uiWidth, uiHeight);
  xiiView* pView = nullptr;
  if (xiiRenderWorld::TryGetView(m_hView, pView))
  {
...
void xiiSceneViewContext::PickObjectAt(xiiUInt16 x, xiiUInt16 y)
{
  ...
  xiiView* pView = nullptr;
  if (xiiRenderWorld::TryGetView(m_hView, pView) && pView->IsRenderPassReadBackPropertyExisting("EditorPickingPass", "PickingMatrix"))
  {
    pView->SetRenderPassProperty("EditorPickingPass", "PickingPosition", xiiVec2(x, y));
    xiiVariant varMat = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingMatrix");

    if (varMat.IsA<xiiMat4>())
    {
      const xiiUInt32 uiPickingID = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingID").ConvertTo<xiiUInt32>();
      // const float fPickingDepth = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingDepth").ConvertTo<float>();
      res.m_vPickedNormal            = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingNormal").ConvertTo<xiiVec3>();
      res.m_vPickingRayStartPosition = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingRayStartPosition").ConvertTo<xiiVec3>();
      res.m_vPickedPosition          = pView->GetRenderPassReadBackProperty("EditorPickingPass", "PickingPosition").ConvertTo<xiiVec3>();
