...
void xiiGridRenderer::GetSupportedRenderDataTypes(xiiHybridArray<const xiiRTTI*, 8>& types) const
{
  types.PushBack(xiiGetStaticRTTI<xiiGridRenderData>());
}

void xiiGridRenderer::GetSupportedRenderDataCategories(xiiHybridArray<xiiRenderData::Category, 8>& categories) const
{
...
void xiiEditorGridExtractor::Extract(const xiiView& view, const xiiDynamicArray<const xiiGameObject*>& visibleObjects, xiiExtractedRenderData& extractedRenderData)
{
  ...
  if (cam->IsOrthographic())
  {
    ...
    plane1.SetFromNormalAndPoint(cam->GetCenterDirRight(), xiiVec3(0));
    plane2.SetFromNormalAndPoint(cam->GetCenterDirUp(), xiiVec3(0));
  ...
  extractedRenderData.AddRenderData(pRenderData, xiiDefaultRenderDataCategories::SimpleTransparent);
}
