...
void xiiSceneExportModifier_RemovePathNodeComponents::ModifyWorld(xiiWorld& world, const xiiUuid& documentGuid, bool bForExport)
{
  ...
  if (xiiPathComponentManager* pSiMan = world.GetComponentManager<xiiPathComponentManager>())
  {
    for (auto it = pSiMan->GetComponents(); it.IsValid(); it.Next())
    {
  ...
  if (xiiPathNodeComponentManager* pSiMan = world.GetComponentManager<xiiPathNodeComponentManager>())
  {
    for (auto it = pSiMan->GetComponents(); it.IsValid(); it.Next())
    {
