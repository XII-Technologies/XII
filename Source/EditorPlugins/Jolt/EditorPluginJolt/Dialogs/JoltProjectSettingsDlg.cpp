...
{
  if (xiiFileSystem::ExistsFile(g_szJoltConfigFile))
    return;
...
void xiiQtJoltProjectSettingsDlg::SetupTable()
{
  xiiQtScopedBlockSignals    s1(FilterTable);
  xiiQtScopedUpdatesDisabled s2(FilterTable);

  const xiiUInt32 uiLayers = m_Config.GetNumNamedGroups();
  ...
  for (xiiUInt32 r = 0; r < uiLayers; ++r)
  {
  ...
  for (xiiUInt32 r = 0; r < uiLayers; ++r)
  {
    for (xiiUInt32 c = 0; c < uiLayers; ++c)
    {
      ...
      connect(pCheck, &QCheckBox::clicked, this, &xiiQtJoltProjectSettingsDlg::onCheckBoxClicked);
...
xiiResult xiiQtJoltProjectSettingsDlg::Save()
{
...
xiiResult xiiQtJoltProjectSettingsDlg::Load()
{
...
void xiiQtJoltProjectSettingsDlg::onCheckBoxClicked(bool checked)
{
  QCheckBox* pCheck = qobject_cast<QCheckBox*>(sender());

  const xiiInt32 c = pCheck->property("column").toInt();
  const xiiInt32 r = pCheck->property("row").toInt();
...
void xiiQtJoltProjectSettingsDlg::on_DefaultButtons_clicked(QAbstractButton* pButton)
{
...
void xiiQtJoltProjectSettingsDlg::on_ButtonAddLayer_clicked()
{
  const xiiInt32 iNewIdx = m_Config.FindUnnamedGroup();

  if (iNewIdx < 0)
  {
    xiiQtUiServices::GetSingleton()->MessageBoxInformation("The maximum number of collision layers has been reached.");
    return;
  }
  ...
  while (true)
  {
    ...
    {
      xiiQtUiServices::GetSingleton()->MessageBoxWarning("A Collision Layer with the given name already exists.");
      continue;
    }
...
void xiiQtJoltProjectSettingsDlg::on_ButtonRemoveLayer_clicked()
{
  if (xiiQtUiServices::GetSingleton()->MessageBoxQuestion("Remove selected Collision Layer?", QMessageBox::StandardButton::Yes | QMessageBox::StandardButton::No, QMessageBox::StandardButton::No) == QMessageBox::StandardButton::No)
    return;
...
void xiiQtJoltProjectSettingsDlg::on_ButtonRenameLayer_clicked()
{
  const auto sel = FilterTable->selectionModel()->selectedRows();
  ...
  const xiiString sOldName  = m_Config.GetGroupName(iGroupIdx);
...
void xiiQtJoltProjectSettingsDlg::on_FilterTable_itemSelectionChanged()
{
  const auto sel = FilterTable->selectionModel()->selectedRows();
