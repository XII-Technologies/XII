...
xiiTransformStatus xiiPropertyAnimAssetDocument::InternalTransformAsset(xiiStreamWriter& stream, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags)
{
  const xiiPropertyAnimationTrackGroup* pProp = GetProperties();
...
void xiiPropertyAnimAssetDocument::SetPlayAnimation(bool play)
{
...
void xiiPropertyAnimAssetDocument::SetRepeatAnimation(bool repeat)
{
...
xiiStatus xiiPropertyAnimAssetDocument::CanAnimate(const xiiDocumentObject* pObject, const xiiAbstractProperty* pProp, xiiVariant index, xiiPropertyAnimTarget::Enum target) const
{
  ...
  if (!keys.Contains(key))
    return xiiStatus("No node name set or property is not reachable.");
...
xiiUuid xiiPropertyAnimAssetDocument::FindCurveCp(const xiiUuid& trackGuid, xiiInt64 tickX)
{
...
xiiUuid xiiPropertyAnimAssetDocument::InsertCurveCpAt(const xiiUuid& track, xiiInt64 tickX, double newPosY)
{
  xiiObjectCommandAccessor accessor(GetCommandHistory());
...
xiiUuid xiiPropertyAnimAssetDocument::FindGradientColorCp(const xiiUuid& trackGuid, xiiInt64 tickX)
{
  auto     pTrack = GetTrack(trackGuid);
  xiiInt32 iIndex = -1;
...
xiiUuid xiiPropertyAnimAssetDocument::InsertGradientColorCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, const xiiColorGammaUB& color)
{
  xiiObjectCommandAccessor accessor(GetCommandHistory());
  xiiObjectAccessorBase&   acc = accessor;
  ...
  XII_VERIFY(acc.SetValue(cpObject, "Tick", tickX).Succeeded(), "");
...
xiiUuid xiiPropertyAnimAssetDocument::FindGradientAlphaCp(const xiiUuid& trackGuid, xiiInt64 tickX)
{
  auto     pTrack = GetTrack(trackGuid);
  xiiInt32 iIndex = -1;
...
xiiUuid xiiPropertyAnimAssetDocument::InsertGradientAlphaCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, xiiUInt8 alpha)
{
  xiiObjectCommandAccessor accessor(GetCommandHistory());
  xiiObjectAccessorBase&   acc = accessor;
  ...
  const xiiDocumentObject* cpObject = GetObjectManager()->GetObject(newObjectGuid);
  XII_VERIFY(acc.SetValue(cpObject, "Tick", tickX).Succeeded(), "");
  XII_VERIFY(acc.SetValue(cpObject, "Alpha", alpha).Succeeded(), "");
  acc.FinishTransaction();
  return newObjectGuid;
}

xiiUuid xiiPropertyAnimAssetDocument::FindGradientIntensityCp(const xiiUuid& trackGuid, xiiInt64 tickX)
{
  auto     pTrack = GetTrack(trackGuid);
  xiiInt32 iIndex = -1;
...
xiiUuid xiiPropertyAnimAssetDocument::InsertGradientIntensityCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, float intensity)
{
  xiiObjectCommandAccessor accessor(GetCommandHistory());
  xiiObjectAccessorBase&   acc = accessor;
  ...
  const xiiDocumentObject* cpObject = GetObjectManager()->GetObject(newObjectGuid);
  XII_VERIFY(acc.SetValue(cpObject, "Tick", tickX).Succeeded(), "");
  XII_VERIFY(acc.SetValue(cpObject, "Intensity", intensity).Succeeded(), "");
  acc.FinishTransaction();
  return newObjectGuid;
}

xiiUuid xiiPropertyAnimAssetDocument::InsertEventTrackCpAt(xiiInt64 tickX, const char* szValue)
{
  xiiObjectCommandAccessor accessor(GetCommandHistory());
  xiiObjectAccessorBase&   acc = accessor;
