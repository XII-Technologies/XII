...
{
  typedef xiiSimpleAssetDocument<xiiPropertyAnimationTrackGroup, xiiGameObjectContextDocument> BaseClass;
...
public:
  ...
  xiiUuid FindCurveCp(const xiiUuid& trackGuid, xiiInt64 tickX);
  xiiUuid InsertCurveCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, double newPosY);

  xiiUuid FindGradientColorCp(const xiiUuid& trackGuid, xiiInt64 tickX);
  xiiUuid InsertGradientColorCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, const xiiColorGammaUB& color);

  xiiUuid FindGradientAlphaCp(const xiiUuid& trackGuid, xiiInt64 tickX);
  xiiUuid InsertGradientAlphaCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, xiiUInt8 alpha);

  xiiUuid FindGradientIntensityCp(const xiiUuid& trackGuid, xiiInt64 tickX);
  xiiUuid InsertGradientIntensityCpAt(const xiiUuid& trackGuid, xiiInt64 tickX, float intensity);
