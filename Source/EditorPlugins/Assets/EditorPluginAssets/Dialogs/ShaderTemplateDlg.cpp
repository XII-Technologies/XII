...
xiiQtShaderTemplateDlg::xiiQtShaderTemplateDlg(QWidget* parent, const xiiDocument* pSceneDoc) :
  QDialog(parent)
{
  setupUi(this);
  ...
  sSearchDir.AppendPath("ShaderTemplates/*.xiiShaderTemplate");
  ...
  for (it.StartSearch(sSearchDir, xiiFileSystemIteratorFlags::ReportFiles); it.IsValid(); it.Next())
  {
    ...
    if (file.Open(t.m_sPath).Succeeded())
    {
      ...
      for (xiiUInt32 ip1 = t.m_Vars.GetCount(); ip1 > 0; --ip1)
      {
        const xiiUInt32 i = ip1 - 1;

        xiiStringBuilder s = t.m_Vars[i];
        s.Trim(" \t");
        t.m_Vars[i] = s;
...
void xiiQtShaderTemplateDlg::on_Buttons_accepted()
{
  int idx = ShaderTemplate->currentIndex();
  if (idx < 0 || idx >= (int)m_Templates.GetCount())
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("No shader template selected.");
    return;
  }

  xiiStringBuilder relPath = DestinationFile->text().toUtf8().data();
  xiiStringBuilder absPath = relPath;

  if (!xiiQtEditorApp::GetSingleton()->MakeParentDataDirectoryRelativePathAbsolute(absPath, false))
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("The selected shader file path is invalid.");
    return;
  }

  relPath = absPath;

  if (!xiiQtEditorApp::GetSingleton()->MakePathDataDirectoryRelative(relPath))
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("The target shader file is not located inside a data directory of this project.");
    return;
  }
  ...
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("Could not create target shader file.");
    return;
  }

  xiiStringBuilder code = m_Templates[idx].m_sContent;
  ...
  pp.SetFileOpenFunction([code](const char* szAbsoluteFile, xiiDynamicArray<xiiUInt8>& FileContent, xiiTimestamp& out_FileModification) {
    FileContent.SetCountUninitialized(code.GetElementCount());
    xiiMemoryUtils::RawByteCopy(FileContent.GetData(), code.GetData(), code.GetElementCount());
    return XII_SUCCESS;
  });
  ...
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("Preparing the shader failed.");
    return;
  }
...
void xiiQtShaderTemplateDlg::on_Buttons_rejected()
{
  m_sResult.Clear();
  reject();
}

void xiiQtShaderTemplateDlg::on_Browse_clicked()
{
  ...
  QString sResult = QFileDialog::getSaveFileName(this, "Create Shader", sLastDir, "xiiShader (*.xiiShader)", nullptr);

  if (sResult.isEmpty())
    return;

  xiiStringBuilder tmp = sResult.toUtf8().data();
  if (!xiiQtEditorApp::GetSingleton()->MakePathDataDirectoryParentRelative(tmp))
  {
    xiiQtUiServices::GetSingleton()->MessageBoxWarning("The selected file is not located inside a data directory of this project.");
    return;
  }
...
void xiiQtShaderTemplateDlg::on_ShaderTemplate_currentIndexChanged(int idx)
{
  QTableWidget* pTable = TemplateVars;
  ...
  for (xiiUInt32 row = 0; row < m_Templates[idx].m_Vars.GetCount(); ++row)
  {
    ...
    else
    {
      ...
      for (xiiUInt32 idx2 = 0; idx2 < enumDef.m_Values.GetCount(); ++idx2)
      {
