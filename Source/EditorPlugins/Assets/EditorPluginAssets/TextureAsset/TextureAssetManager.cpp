...
xiiTextureAssetDocumentManager::xiiTextureAssetDocumentManager()
{
  ...
  xiiQtImageCache::GetSingleton()->RegisterTypeImage("Render Target", QPixmap(":/AssetIcons/Render_Target.png"));
}
...
void xiiTextureAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtTextureAssetDocumentWindow* pDocWnd = new xiiQtTextureAssetDocumentWindow(static_cast<xiiTextureAssetDocument*>(e.m_pDocument));
      }
    }
    break;
...
void xiiTextureAssetDocumentManager::InternalCreateDocument(
...
{
  xiiTextureAssetDocument* pDoc = new xiiTextureAssetDocument(szPath);
  out_pDocument                 = pDoc;

  if (xiiStringUtils::IsEqual(szDocumentTypeName, "Render Target"))
  {
