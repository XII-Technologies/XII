...
xiiCurve1DAssetDocument::xiiCurve1DAssetDocument(const char* szDocumentPath) :
  xiiSimpleAssetDocument<xiiCurveGroupData>(szDocumentPath, xiiAssetDocEngineConnection::None)
{
}
...
void xiiCurve1DAssetDocument::WriteResource(xiiStreamWriter& stream) const
{
  const xiiCurveGroupData* pProp = GetProperties();
...
xiiTransformStatus xiiCurve1DAssetDocument::InternalTransformAsset(xiiStreamWriter& stream, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags)
{
...
public:
  ...
  virtual void Patch(xiiGraphPatchContext& context, xiiAbstractObjectGraph* pGraph, xiiAbstractObjectNode* pNode) const override { pNode->RenameProperty("Control Points", "ControlPoints"); }
};
