...
xiiVisualScriptPin::xiiVisualScriptPin(Type type, const xiiVisualScriptPinDescriptor* pDescriptor, const xiiDocumentObject* pObject) :
...
const xiiString& xiiVisualScriptPin::GetTooltip() const
{
...
XII_BEGIN_DYNAMIC_REFLECTED_TYPE(xiiVisualScriptConnection, 1, xiiRTTINoAllocator)
XII_END_DYNAMIC_REFLECTED_TYPE;
...
void xiiVisualScriptNodeManager::InternalCreatePins(const xiiDocumentObject* pObject, NodeInternal& node)
{
  ...
  {
    auto pPin = XII_DEFAULT_NEW(xiiVisualScriptPin, xiiPin::Type::Input, &pinDesc, pObject);
    node.m_Inputs.PushBack(pPin);
  }

  for (const auto& pinDesc : pDesc->m_OutputPins)
  {
    auto pPin = XII_DEFAULT_NEW(xiiVisualScriptPin, xiiPin::Type::Output, &pinDesc, pObject);
...
const xiiRTTI* xiiVisualScriptNodeManager::GetConnectionType() const
{
  return xiiGetStaticRTTI<xiiVisualScriptConnection>();
}
...
const char* xiiVisualScriptNodeManager::GetTypeCategory(const xiiRTTI* pRtti) const
{
