...
xiiSurfaceAssetDocumentManager::xiiSurfaceAssetDocumentManager()
{
  ...
  xiiQtImageCache::GetSingleton()->RegisterTypeImage("Surface", QPixmap(":/AssetIcons/Surface.png"));
}
...
void xiiSurfaceAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtSurfaceAssetDocumentWindow* pDocWnd = new xiiQtSurfaceAssetDocumentWindow(e.m_pDocument);
      }
    }
    break;
...
void xiiSurfaceAssetDocumentManager::InternalCreateDocument(
...
{
  out_pDocument = new xiiSurfaceAssetDocument(szPath);
}
