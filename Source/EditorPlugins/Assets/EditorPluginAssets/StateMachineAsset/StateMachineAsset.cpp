...
// clang-format off
XII_BEGIN_DYNAMIC_REFLECTED_TYPE(xiiStateMachineAssetDocument, 3, xiiRTTINoAllocator)
XII_END_DYNAMIC_REFLECTED_TYPE;
// clang-format on

xiiStateMachineAssetDocument::xiiStateMachineAssetDocument(const char* szDocumentPath) :
  xiiAssetDocument(szDocumentPath, XII_DEFAULT_NEW(xiiStateMachineNodeManager), xiiAssetDocEngineConnection::None)
{
}

xiiTransformStatus xiiStateMachineAssetDocument::InternalTransformAsset(xiiStreamWriter& stream, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags)
{
  ...
  auto AddState = [&](const xiiDocumentObject* pObject) {
    ...
    {
      return xiiStatus(xiiFmt("State '{}' has no state type assigned", name));
    }
...
bool xiiStateMachineAssetDocument::Paste(const xiiArrayPtr<PasteInfo>& info, const xiiAbstractObjectGraph& objectGraph, bool bAllowPickedPosition, const char* szMimeType)
{
