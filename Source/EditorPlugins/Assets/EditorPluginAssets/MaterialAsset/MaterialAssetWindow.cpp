...
{
  s_hMaterialModelAction = XII_REGISTER_DYNAMIC_MENU("MaterialAsset.Model", xiiMaterialModelAction, ":/EditorFramework/Icons/Perspective.png");
}
...
void xiiMaterialAssetActions::MapActions(const char* szMapping, const char* szPath)
{
...
void xiiQtMaterialAssetDocumentWindow::OnVseConfigChanged(const char* filename, xiiDirectoryWatcherAction action, xiiDirectoryWatcherType type)
{
  if (type != xiiDirectoryWatcherType::File || !xiiPathUtils::HasExtension(filename, "DDL"))
    return;
  ...
  xiiVisualShaderTypeRegistry::GetSingleton()->UpdateNodeData(filename);
