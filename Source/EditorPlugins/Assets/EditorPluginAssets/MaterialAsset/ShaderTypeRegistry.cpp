...
namespace
{
  ...
  void AddAttributes(xiiShaderParser::ParameterDefinition& def, const xiiRTTI* pType, xiiHybridArray<xiiPropertyAttribute*, 2>& attributes)
  {
    if (def.m_sType.StartsWith_NoCase("texture"))
    {
      if (def.m_sType.IsEqual("Texture2D"))
      {
        attributes.PushBack(XII_DEFAULT_NEW(xiiCategoryAttribute, "Texture 2D"));
        attributes.PushBack(XII_DEFAULT_NEW(xiiAssetBrowserAttribute, "CompatibleAsset_Texture_2D"));
      }
      else if (def.m_sType.IsEqual("Texture3D"))
      {
        attributes.PushBack(XII_DEFAULT_NEW(xiiCategoryAttribute, "Texture 3D"));
        attributes.PushBack(XII_DEFAULT_NEW(xiiAssetBrowserAttribute, "CompatibleAsset_Texture_3D"));
      }
      else if (def.m_sType.IsEqual("TextureCube"))
      {
        attributes.PushBack(XII_DEFAULT_NEW(xiiCategoryAttribute, "Texture Cube"));
        attributes.PushBack(XII_DEFAULT_NEW(xiiAssetBrowserAttribute, "CompatibleAsset_Texture_Cube"));
      }
    }
    else if (def.m_sType.StartsWith_NoCase("permutation"))
    {
      attributes.PushBack(XII_DEFAULT_NEW(xiiCategoryAttribute, "Permutation"));
    }
    else
    {
      attributes.PushBack(XII_DEFAULT_NEW(xiiCategoryAttribute, "Constant"));
    }
    ...
    for (auto& attributeDef : def.m_Attributes)
    {
      if (attributeDef.m_sType.IsEqual("Default") && attributeDef.m_Values.GetCount() >= 1)
      {
        if (pType == xiiGetStaticRTTI<xiiColor>())
        {
          ...
          attributes.PushBack(XII_DEFAULT_NEW(xiiExposeColorAlphaAttribute));
        ...
        attributes.PushBack(XII_DEFAULT_NEW(xiiDefaultValueAttribute, attributeDef.m_Values[0]));
      }
      else if (attributeDef.m_sType.IsEqual("Clamp") && attributeDef.m_Values.GetCount() >= 2)
      {
        attributes.PushBack(XII_DEFAULT_NEW(xiiClampValueAttribute, attributeDef.m_Values[0], attributeDef.m_Values[1]));
      }
      else if (attributeDef.m_sType.IsEqual("Group"))
      {
        ...
        {
          attributes.PushBack(XII_DEFAULT_NEW(xiiGroupAttribute, attributeDef.m_Values[0].ConvertTo<xiiString>()));
        }
        else
        {
          attributes.PushBack(XII_DEFAULT_NEW(xiiGroupAttribute));
        }
      }
    }
  }
...
const xiiRTTI* xiiShaderTypeRegistry::GetShaderType(const char* szShaderPath)
{
  if (xiiStringUtils::IsNullOrEmpty(szShaderPath))
    return nullptr;

  xiiStringBuilder sShaderPath = szShaderPath;
  ...
  {
    xiiStringBuilder sAbsPath = szShaderPath;
    {
      if (!xiiQtEditorApp::GetSingleton()->MakeDataDirectoryRelativePathAbsolute(sAbsPath))
      {
        xiiLog::Warning("Can't make path absolute: '{0}'", szShaderPath);
        return nullptr;
      }
...
/// \brief Changes the base class of all shader types to xiiShaderTypeBase (version 1) and
...
public:
  ...
  virtual void Patch(xiiGraphPatchContext& context, xiiAbstractObjectGraph* pGraph, xiiAbstractObjectNode*) const override
  {
...
public:
  ...
  static void FixEnumString(xiiStringBuilder& sValue, const char* szName)
  {
    if (sValue.StartsWith(szName))
      sValue.Shrink(xiiStringUtils::GetCharacterCount(szName), 0);
