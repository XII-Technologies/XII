...
void xiiAnimatedMeshAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtAnimatedMeshAssetDocumentWindow* pDocWnd =
          new xiiQtAnimatedMeshAssetDocumentWindow(static_cast<xiiAnimatedMeshAssetDocument*>(e.m_pDocument));
      }
    }
    break;
...
void xiiAnimatedMeshAssetDocumentManager::InternalCreateDocument(
...
{
  out_pDocument = new xiiAnimatedMeshAssetDocument(szPath);
}
