...
void xiiSkeletonAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        new xiiQtSkeletonAssetDocumentWindow(static_cast<xiiSkeletonAssetDocument*>(e.m_pDocument)); // NOLINT: Not a memory leak
      }
    }
    break;
...
void xiiSkeletonAssetDocumentManager::InternalCreateDocument(xiiStringView sDocumentTypeName, xiiStringView sPath, bool bCreateNewDocument, xiiDocument*& out_pDocument, const xiiDocumentObject* pOpenContext)
{
  out_pDocument = new xiiSkeletonAssetDocument(sPath);
}
