...
xiiResult xiiMeshAssetDocumentManager::OpenPickedDocument(const xiiDocumentObject* pPickedComponent, xiiUInt32 uiPartIndex)
{
  ...
  xiiMeshAssetDocument* pMeshDoc =
    static_cast<xiiMeshAssetDocument*>(xiiQtEditorApp::GetSingleton()->OpenDocument(pSubAsset->m_pAssetInfo->m_sAbsolutePath, xiiDocumentFlags::None));
...
void xiiMeshAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtMeshAssetDocumentWindow* pDocWnd = new xiiQtMeshAssetDocumentWindow(static_cast<xiiMeshAssetDocument*>(e.m_pDocument));
      }
    }
    break;
...
{
  out_pDocument = new xiiMeshAssetDocument(szPath);
}
