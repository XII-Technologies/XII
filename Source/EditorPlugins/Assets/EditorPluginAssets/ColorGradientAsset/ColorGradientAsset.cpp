...
void xiiColorControlPoint::SetTickFromTime(xiiTime time, xiiInt64 fps)
{
...
void xiiAlphaControlPoint::SetTickFromTime(xiiTime time, xiiInt64 fps)
{
  const xiiUInt32 uiTicksPerStep = 4800 / fps;
  m_iTick                        = (xiiInt64)xiiMath::RoundToMultiple(time.GetSeconds() * 4800.0, (double)uiTicksPerStep);
}

void xiiIntensityControlPoint::SetTickFromTime(xiiTime time, xiiInt64 fps)
{
  const xiiUInt32 uiTicksPerStep = 4800 / fps;
  m_iTick                        = (xiiInt64)xiiMath::RoundToMultiple(time.GetSeconds() * 4800.0, (double)uiTicksPerStep);
}

xiiColorGradientAssetDocument::xiiColorGradientAssetDocument(const char* szDocumentPath) :
  xiiSimpleAssetDocument<xiiColorGradientAssetData>(szDocumentPath, xiiAssetDocEngineConnection::None)
{
}

void xiiColorGradientAssetDocument::WriteResource(xiiStreamWriter& stream) const
{
...
xiiTransformStatus xiiColorGradientAssetDocument::InternalTransformAsset(xiiStreamWriter& stream, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags)
{
