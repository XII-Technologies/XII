...
xiiLUTAssetDocumentManager::xiiLUTAssetDocumentManager()
{
  ...
  xiiQtImageCache::GetSingleton()->RegisterTypeImage("LUT", QPixmap(":/AssetIcons/LUT.png"));

  // xiiQtImageCache::GetSingleton()->RegisterTypeImage("LUT", QPixmap(":/AssetIcons/Render_Target.png"));
}
...
void xiiLUTAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtLUTAssetDocumentWindow* pDocWnd = new xiiQtLUTAssetDocumentWindow(static_cast<xiiLUTAssetDocument*>(e.m_pDocument));
      }
    }
    break;
...
{
  xiiLUTAssetDocument* pDoc = new xiiLUTAssetDocument(szPath);
  out_pDocument             = pDoc;
}
