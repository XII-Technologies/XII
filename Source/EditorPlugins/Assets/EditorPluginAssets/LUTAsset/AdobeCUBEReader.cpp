...
namespace
{
  bool GetVec3FromLine(xiiHybridArray<const xiiToken*, 32> line, xiiUInt32 skip, xiiVec3& out)
  {
    ...
    if ((line[skip + 0]->m_iType != xiiTokenType::Float && line[skip + 0]->m_iType != xiiTokenType::Integer) ||
        line[skip + 1]->m_iType != xiiTokenType::Whitespace ||
        (line[skip + 2]->m_iType != xiiTokenType::Float && line[skip + 2]->m_iType != xiiTokenType::Integer) ||
        line[skip + 3]->m_iType != xiiTokenType::Whitespace ||
        (line[skip + 4]->m_iType != xiiTokenType::Float && line[skip + 4]->m_iType != xiiTokenType::Integer))
    {
      return false;
    }

    double    res  = 0;
    xiiString sVal = line[skip + 0]->m_DataView;
...
xiiStatus xiiAdobeCUBEReader::ParseFile(xiiStreamReader& Stream, xiiLogInterface* pLog /*= nullptr*/)
{
  xiiString sContent;
