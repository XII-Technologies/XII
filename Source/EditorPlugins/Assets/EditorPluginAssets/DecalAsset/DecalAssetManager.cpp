...
xiiString xiiDecalAssetDocumentManager::GetAssetTableEntry(const xiiSubAsset* pSubAsset, const char* szDataDirectory, const xiiPlatformProfile* pAssetProfile) const
{
...
void xiiDecalAssetDocumentManager::OnDocumentManagerEvent(const xiiDocumentManager::Event& e)
{
  switch (e.m_Type)
  {
    case xiiDocumentManager::Event::Type::DocumentWindowRequested:
    {
      ...
      {
        xiiQtDecalAssetDocumentWindow* pDocWnd = new xiiQtDecalAssetDocumentWindow(static_cast<xiiDecalAssetDocument*>(e.m_pDocument));
      }
    }
    break;
...
void xiiDecalAssetDocumentManager::InternalCreateDocument(const char* szDocumentTypeName, const char* szPath, bool bCreateNewDocument, xiiDocument*& out_pDocument, const xiiDocumentObject* pOpenContext)
{
  out_pDocument = new xiiDecalAssetDocument(szPath);
}
...
xiiStatus xiiDecalAssetDocumentManager::GenerateDecalTexture(const xiiPlatformProfile* pAssetProfile)
{
  ...
  // find all decal assets, extract their file information to pass it along to TexConv
  {
    ...
    for (auto it = allAssets->GetIterator(); it.IsValid(); ++it)
    {
      ...
      XII_LOG_BLOCK("Decal", asset.m_pAssetInfo->m_sDataDirParentRelativePath);

      // does the document already exist and is it open ?
      bool         bWasOpen = false;
      xiiDocument* pDoc     = GetDocumentByPath(asset.m_pAssetInfo->m_sAbsolutePath);
      if (pDoc)
        bWasOpen = true;
      else
        pDoc = pEditorApp->OpenDocument(asset.m_pAssetInfo->m_sAbsolutePath, xiiDocumentFlags::None);

      if (pDoc == nullptr)
        return xiiStatus(xiiFmt("Could not open asset document '{0}'", asset.m_pAssetInfo->m_sDataDirParentRelativePath));
...
xiiString xiiDecalAssetDocumentManager::GetDecalTexturePath(const xiiPlatformProfile* pAssetProfile0) const
{
  ...
  GenerateOutputFilename(result, pAssetProfile, "xiiTextureAtlas", true);
