  ...
  // download the picking information from the GPU
  {
    if (m_uiWindowWidth != 0 && m_uiWindowHeight != 0)
    {
      ...
      /*
      {
        ...
        if (res.Failed())
          xiiLog::Debug("Inversion of View-Projection-Matrix failed. Picking results will be wrong.");
...
void xiiPickingRenderPass::ReadBackPropertiesSinglePick(xiiView* pView)
{
  ...
  xiiVec3 vPickedPosition(0);
  {
    ...
    vNormals[0] = xiiPlane(vPickedPosition, vOtherPos[0], vOtherPos[2]).m_vNormal;
    vNormals[1] = xiiPlane(vPickedPosition, vOtherPos[2], vOtherPos[1]).m_vNormal;
    vNormals[2] = xiiPlane(vPickedPosition, vOtherPos[1], vOtherPos[3]).m_vNormal;
    vNormals[3] = xiiPlane(vPickedPosition, vOtherPos[3], vOtherPos[0]).m_vNormal;
