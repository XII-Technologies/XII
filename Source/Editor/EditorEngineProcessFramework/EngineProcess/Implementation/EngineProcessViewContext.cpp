...
void xiiEngineProcessViewContext::SetViewID(xiiUInt32 id)
{
...
void xiiEngineProcessViewContext::SetupRenderTarget(xiiGALSwapChainHandle hSwapChain, const xiiGALRenderTargets* renderTargets, xiiUInt16 uiWidth, xiiUInt16 uiHeight)
{
  XII_LOG_BLOCK("xiiEngineProcessViewContext::SetupRenderTarget");
  XII_ASSERT_DEV(hSwapChain.IsInvalidated() || renderTargets == nullptr, "hSwapChain and renderTargetSetup are mutually exclusive.");
...
bool xiiEngineProcessViewContext::FocusCameraOnObject(xiiCamera& camera, const xiiBoundingBoxSphere& objectBounds, float fFov, const xiiVec3& vViewDir)
{
  ...
  bool    bChanged   = false;
  xiiVec3 vCameraPos = camera.GetCenterPosition();
  ...
  if (bChanged)
  {
    ...
    camera.SetCameraMode(xiiCameraMode::PerspectiveFixedFovX, fFov, 0.1f, 1000.0f);
    camera.LookAt(vNewCameraPos, vCenterPos, xiiVec3(0.0f, 0.0f, 1.0f));
  }
