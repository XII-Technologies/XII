...
xiiResult xiiProjectExport::ExportProject(const char* szTargetDirectory, const xiiPlatformProfile* pPlatformProfile, const xiiApplicationFileSystemConfig& dataDirs)
{
  ...
  // 3
  {
    // by default all DLLs are excluded by CommonBinaries.xiiExportFilter
    // we want to override this for all the runtime DLLs and indirect DLL dependencies
    // so we add those to the 'include filter'

    for (auto it : xiiQtEditorApp::GetSingleton()->GetPluginBundles().m_Plugins)
    {
