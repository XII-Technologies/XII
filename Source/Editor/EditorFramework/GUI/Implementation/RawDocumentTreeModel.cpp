...
xiiQtDocumentTreeModelAdapter::xiiQtDocumentTreeModelAdapter(const xiiDocumentObjectManager* pTree, const xiiRTTI* pType, const char* sChildProperty) :
  m_pTree(pTree), m_pType(pType), m_sChildProperty(sChildProperty)
{
...
bool xiiQtDocumentTreeModelAdapter::setData(const xiiDocumentObject* pObject, int row, int column, const QVariant& value, int role) const
{
  return false;
}

Qt::ItemFlags xiiQtDocumentTreeModelAdapter::flags(const xiiDocumentObject* pObject, int row, int column) const
{
  if (column == 0)
  {
...
xiiQtDummyAdapter::xiiQtDummyAdapter(const xiiDocumentObjectManager* pTree, const xiiRTTI* pType, const char* m_sChildProperty) :
  xiiQtDocumentTreeModelAdapter(pTree, pType, m_sChildProperty)
{
}

QVariant xiiQtDummyAdapter::data(const xiiDocumentObject* pObject, int row, int column, int role) const
{
  if (column == 0)
  {
    switch (role)
...
QVariant xiiQtNamedAdapter::data(const xiiDocumentObject* pObject, int row, int column, int role) const
{
  if (column == 0)
  {
    switch (role)
...
bool xiiQtNameableAdapter::setData(const xiiDocumentObject* pObject, int row, int column, const QVariant& value, int role) const
{
...
Qt::ItemFlags xiiQtNameableAdapter::flags(const xiiDocumentObject* pObject, int row, int column) const
{
  if (column == 0)
  {
...
void xiiQtDocumentTreeModel::AddAdapter(xiiQtDocumentTreeModelAdapter* adapter)
{
  XII_ASSERT_DEV(!m_Adapters.Contains(adapter->GetType()), "An adapter for the given type was already registered.");

  adapter->setParent(this);
  connect(adapter, &xiiQtDocumentTreeModelAdapter::dataChanged, this, [this](const xiiDocumentObject* pObject, QVector<int> roles) {
    if (!pObject)
      return;
...
QModelIndex xiiQtDocumentTreeModel::index(int row, int column, const QModelIndex& parent) const
{
  ...
  xiiVariant value = pObject->GetTypeAccessor().GetValue(pAdapter->GetChildProperty(), row);
  ...
  return createIndex(row, column, const_cast<xiiDocumentObject*>(pChild));
}
...
QVariant xiiQtDocumentTreeModel::data(const QModelIndex& index, int role) const
{
  // if (index.isValid())
  {
...
bool xiiQtDocumentTreeModel::canDropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent) const
{
...

  {
    ...
    QByteArray                             encodedData = data->data("application/xiiEditor.ObjectSelection");
...
bool xiiQtDocumentTreeModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent)
{
  if (!m_bAllowDragDrop)
    return false;
...
bool xiiQtDocumentTreeModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
  const xiiDocumentObject* pObject = (const xiiDocumentObject*)index.internalPointer();
  auto                     pType   = pObject->GetTypeAccessor().GetType();
  if (auto pAdapter = GetAdapter(pType))
