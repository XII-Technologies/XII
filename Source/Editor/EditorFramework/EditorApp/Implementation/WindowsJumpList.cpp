...
void xiiWindowsJumpList::UpdateJumpList(const xiiRecentFilesList& recentProjects)
{
  ...
  if (FAILED(hr))
  {
    xiiLog::Warning("Failed to create destination list: 0x{0:X}", static_cast<xiiUInt32>(hr));
    if (bComInitialized)
      CoUninitialize();
    return;
  }
  ...
  if (FAILED(hr))
  {
    xiiLog::Warning("Failed to begin destination list: 0x{0:X}", static_cast<xiiUInt32>(hr));
    pDestList->Release();
    if (bComInitialized)
      CoUninitialize();
    return;
  ...
  {
    xiiLog::Warning("Failed to get module file name");
    pDestList->AbortList();
    pDestList->Release();
    if (bComInitialized)
      CoUninitialize();
  ...
  if (FAILED(hr))
  {
    xiiLog::Warning("Failed to create object collection: 0x{0:X}", static_cast<xiiUInt32>(hr));
    pDestList->AbortList();
    pDestList->Release();
    if (bComInitialized)
      CoUninitialize();
  ...
  const xiiUInt32 uiMaxItems = xiiMath::Min<xiiUInt32>(5, fileList.GetCount());

  for (xiiUInt32 i = 0; i < uiMaxItems; ++i)
  {
    const auto&      recentFile   = fileList[i];
    xiiStringBuilder sProjectPath = recentFile.m_File;

    // Get project directory (remove /xiiProject suffix)
    if (sProjectPath.EndsWith_NoCase("/xiiProject"))
    {
      sProjectPath.Shrink(0, 11); // Remove "/xiiProject"
    }

    // Extract project name from path
    xiiStringBuilder sProjectName = sProjectPath.GetFileName();

    // Convert to wide strings
    xiiStringBuilder sArgumentsUtf8;
    sArgumentsUtf8.SetFormat("-project \"{}\"", recentFile.m_File);

    xiiStringWChar szProjectPath(sProjectPath.GetData());
    xiiStringWChar szArguments(sArgumentsUtf8.GetData());
    xiiStringWChar szProjectName(sProjectName.GetData());
  ...
  if (FAILED(hr))
  {
    xiiLog::Warning("Failed to commit destination list: 0x{0:X}", static_cast<xiiUInt32>(hr));
  }
