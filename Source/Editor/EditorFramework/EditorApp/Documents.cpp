...
void xiiQtEditorApp::OpenDocumentQueued(const char* szDocument, const xiiDocumentObject* pOpenContext /*= nullptr*/)
{
...
xiiDocument* xiiQtEditorApp::OpenDocument(const char* szDocument, xiiBitflags<xiiDocumentFlags> flags, const xiiDocumentObject* pOpenContext)
{
  ...
  if (xiiDocumentManager::FindDocumentTypeFromPath(szDocument, false, pTypeDesc).Failed())
  {
    xiiStringBuilder sTemp;
    sTemp.Format("The selected file extension '{0}' is not registered with any known type.\nCannot open file '{1}'", xiiPathUtils::GetFileExtension(szDocument), szDocument);
    xiiQtUiServices::MessageBoxWarning(sTemp);
    return nullptr;
  }

  // does the same document already exist and is open ?
  xiiDocument* pDocument = pTypeDesc->m_pManager->GetDocumentByPath(szDocument);
  if (!pDocument)
  {
    xiiStatus res = pTypeDesc->m_pManager->CanOpenDocument(szDocument);
...
xiiDocument* xiiQtEditorApp::CreateDocument(const char* szDocument, xiiBitflags<xiiDocumentFlags> flags, const xiiDocumentObject* pOpenContext)
{
  ...
  {
    xiiStatus res = xiiDocumentUtils::IsValidSaveLocationForDocument(szDocument, &pTypeDesc);
    if (res.Failed())
    {
      xiiStringBuilder s;
  ...
  {
    xiiStatus result = pTypeDesc->m_pManager->CreateDocument(pTypeDesc->m_sDocumentTypeName, szDocument, pDocument, flags, pOpenContext);
...
void xiiQtEditorApp::DocumentManagerEventHandler(const xiiDocumentManager::Event& r)
{
  switch (r.m_Type)
  {
    case xiiDocumentManager::Event::Type::AfterDocumentWindowRequested:
    {
      ...
      {
        xiiQtDocumentWindow* pWindow = xiiQtDocumentWindow::FindWindowByDocument(r.m_pDocument);
