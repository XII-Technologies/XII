...
xiiResult xiiQtEditorApp::CreateOrOpenProject(bool bCreate, const char* szFile)
{
  ...
  if (!IsInHeadlessMode())
  {
    xiiStringBuilder sdkDir;
    if (xiiFileSystem::FindFolderWithSubPath(sdkDir, szFile, "Data/Base", "xiiSdkRoot.txt").Succeeded())
    {
      sdkDir.MakeCleanPath();
      if (sdkDir != xiiFileSystem::GetSdkRootDirectory())
      {
        if (xiiQtUiServices::MessageBoxQuestion(xiiFmt("You are attempting to open a project that's located in a different SDK directory.\n\nSDK location: '{}'\nProject path: '{}'\n\nThis may make problems.\n\nContinue anyway?", xiiFileSystem::GetSdkRootDirectory(), szFile), QMessageBox::StandardButton::Yes | QMessageBox::StandardButton::No, QMessageBox::StandardButton::No) != QMessageBox::StandardButton::Yes)
        {
          return XII_FAILURE;
        }
      }
  ...
  xiiStringBuilder sProjectFile = szFile;
