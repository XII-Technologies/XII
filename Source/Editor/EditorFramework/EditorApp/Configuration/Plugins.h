...
struct XII_EDITORFRAMEWORK_DLL xiiPluginBundle
{
  ...
  xiiResult ReadBundleFromDDL(xiiOpenDdlReader& ddl);

  /// \brief Writes only the bundle's state to a DDL file.
  void WriteStateToDDL(xiiOpenDdlWriter& ddl, const char* szOwnName) const;

  /// \brief Reads only the bundle's state from a DDL file.
  void ReadStateFromDDL(xiiOpenDdlReader& ddl, const char* szOwnName);
...
struct XII_EDITORFRAMEWORK_DLL xiiPluginBundleSet
{
  ...
  void WriteStateToDDL(xiiOpenDdlWriter& ddl) const;

  /// \brief Reads the state of all bundles from a DDL file.
  void ReadStateFromDDL(xiiOpenDdlReader& ddl);
