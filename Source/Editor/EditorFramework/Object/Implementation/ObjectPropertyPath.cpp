...
xiiStatus xiiObjectPropertyPath::CreatePath(const xiiObjectPropertyPathContext& context, const xiiPropertyReference& prop, xiiStringBuilder& sObjectSearchSequence, xiiStringBuilder& sComponentType, xiiStringBuilder& sPropertyPath)
{
...

  {
    ...
    xiiStatus res                         = CreatePropertyPath(context2, prop, sPropertyPath);
    if (res.Failed())
      return res;
  }
...
xiiStatus xiiObjectPropertyPath::ResolvePath(const xiiObjectPropertyPathContext& context, xiiHybridArray<xiiPropertyReference, 1>& keys, const char* szObjectSearchSequence, const char* szComponentType, const char* szPropertyPath)
{
  XII_ASSERT_DEV(context.m_pAccessor && context.m_pContextObject && !context.m_sRootProperty.IsEmpty(), "All context fields must be valid.");
  keys.Clear();
  ...
  for (const xiiStringView& sName : names)
  {
    ...
    {
      visitor.Visit(pContext, false, [&output, &sName](const xiiDocumentObject* pObject) -> bool {
    ...
    if (pObject->GetType() == xiiGetStaticRTTI<xiiGameObject>())
    {
      ...
      else
      {
        ...
        for (xiiInt32 i = 0; i < iComponents; i++)
        {
          ...
          if (xiiStringUtils::IsEqual(szComponentType, pChild->GetType()->GetTypeName()))
          {
  ...
  xiiPropertyReference&               out_key)
{
  ...
  for (xiiUInt32 i = 0; i < parts.GetCount(); i++)
  {
    ...
    xiiAbstractProperty* pProperty = pObject->GetType()->FindPropertyByName(parts2[0]);
    if (!pProperty)
