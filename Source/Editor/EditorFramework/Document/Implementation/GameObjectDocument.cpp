...
xiiGameObjectDocument::xiiGameObjectDocument(
  ...
  xiiAssetDocument(szDocumentPath, pObjectManager, engineConnectionType)
{
...
void xiiGameObjectDocument::DetermineNodeName(const xiiDocumentObject* pObject, const xiiUuid& prefabGuid, xiiStringBuilder& out_Result, QIcon* out_pIcon /*= nullptr*/) const
{
  ...
  if (prefabGuid.IsValid())
  {
    ...
    if (pInfo)
    {
      xiiStringBuilder sPath = pInfo->m_pAssetInfo->m_sDataDirParentRelativePath;
  ...
  for (xiiInt32 i = 0; i < iComponents; i++)
  {
    ...
    if (!bHasIcon && out_pIcon != nullptr)
    {
      ...
      *out_pIcon = xiiQtUiServices::GetCachedIconResource(sIconName.GetData());
    }

    if (out_Result.IsEmpty())
    {
      // try to translate the component name, that will typically make it a nice clean name already
      out_Result = xiiTranslate(pChild->GetTypeAccessor().GetType()->GetTypeName());
...
void xiiGameObjectDocument::SetGlobalTransform(const xiiDocumentObject* pObject, const xiiTransform& t, xiiUInt8 transformationChanges) const
{
...
void xiiGameObjectDocument::SetGlobalTransformParentOnly(const xiiDocumentObject* pObject, const xiiTransform& t, xiiUInt8 transformationChanges) const
{
