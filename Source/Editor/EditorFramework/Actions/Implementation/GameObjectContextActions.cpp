...
void xiiGameObjectContextActions::MapToolbarActions(const char* szMapping, const char* szPath)
{
  ...
  xiiStringBuilder szSubPath(szPath, "/GameObjectContextCategory");
  pMap->MapAction(s_hPickContextScene, szSubPath, 1.0f);
}


void xiiGameObjectContextActions::MapContextMenuActions(const char* szMapping, const char* szPath)
{
  xiiActionMap* pMap = xiiActionMapManager::GetActionMap(szMapping);
  XII_ASSERT_DEV(pMap != nullptr, "The given mapping ('{0}') does not exist, mapping the actions failed!", szMapping);
