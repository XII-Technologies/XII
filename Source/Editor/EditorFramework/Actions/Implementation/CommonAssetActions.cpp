...
void xiiCommonAssetActions::RegisterActions()
{
  ...
  s_hSimulationSpeedMenu = XII_REGISTER_MENU_WITH_ICON("Common.Speed.Menu", ":/EditorPluginParticle/Icons/Speed16.png");
...
void xiiCommonAssetActions::MapActions(const char* szMapping, const char* szPath, xiiUInt32 stateMask)
{
  ...
  if (stateMask & xiiCommonAssetUiState::Pause)
  {
    pMap->MapAction(s_hPause, szSubPath, 0.5f);
  }

  if (stateMask & xiiCommonAssetUiState::Restart)
  {
    pMap->MapAction(s_hRestart, szSubPath, 1.0f);
  }

  if (stateMask & xiiCommonAssetUiState::Loop)
  {
    pMap->MapAction(s_hLoop, szSubPath, 2.0f);
  }

  if (stateMask & xiiCommonAssetUiState::SimulationSpeed)
  {
  ...
  if (stateMask & xiiCommonAssetUiState::Grid)
  {
    pMap->MapAction(s_hGrid, szSubPath, 4.0f);
  }

  if (stateMask & xiiCommonAssetUiState::Visualizers)
  {
