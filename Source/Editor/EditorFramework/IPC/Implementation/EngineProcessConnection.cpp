...
xiiResult xiiEditorEngineProcessConnection::WaitForDocumentMessage(const xiiUuid& assetGuid, const xiiRTTI* pMessageType, xiiTime timeout, xiiProcessCommunicationChannel::WaitForMessageCallback* pCallback /*= nullptr*/)
{
  if (!m_bProcessShouldBeRunning)
  {
...
bool xiiEditorEngineConnection::SendMessage(xiiEditorEngineDocumentMsg* pMessage)
{
  XII_WARNING_PUSH()
