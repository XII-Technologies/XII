  ...
  xiiUInt32          uiMemSize)
{
  ...
  m_pChannel->m_MessageEvent.AddEventHandler(xiiMakeDelegate(&xiiProcessCommunicationChannel::MessageFunc, this));
  m_pChannel->Connect();
...
void xiiEditorProcessCommunicationChannel::CloseConnection()
{
  if (m_pChannel)
  {
    m_pChannel->m_MessageEvent.RemoveEventHandler(xiiMakeDelegate(&xiiProcessCommunicationChannel::MessageFunc, this));
    XII_DEFAULT_DELETE(m_pChannel);
  }
  if (m_pClientProcess)
  {
