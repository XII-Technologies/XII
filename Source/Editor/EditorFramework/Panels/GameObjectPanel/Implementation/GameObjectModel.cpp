...
QVariant xiiQtGameObjectAdapter::data(const xiiDocumentObject* pObject, int row, int column, int role) const
{
  switch (role)
  {
    case Qt::DisplayRole:
...
bool xiiQtGameObjectAdapter::setData(const xiiDocumentObject* pObject, int row, int column, const QVariant& value, int role) const
{
  if (role == Qt::EditRole)
  {
    ...
    return xiiQtNameableAdapter::setData(pObject, row, column, QString::fromUtf8(sNewValue.GetData()), role);
  }
...
xiiQtGameObjectModel::~xiiQtGameObjectModel() {}
