...
xiiQtRemoteConnectionDlg::Address::Address()
{
  part[0] = 0;
  part[1] = 0;
  part[2] = 0;
...
void xiiQtRemoteConnectionDlg::Address::operator=(const Address& rhs)
{
  part[0] = rhs.part[0];
  part[1] = rhs.part[1];
  part[2] = rhs.part[2];
...
bool xiiQtRemoteConnectionDlg::Address::operator==(const Address& rhs) const
{
  return part[0] == rhs.part[0] && part[1] == rhs.part[1] && part[2] == rhs.part[2] && part[3] == rhs.part[3];
}


bool xiiQtRemoteConnectionDlg::Address::IsEmpty() const
{
  return part[0] == 0 && part[1] == 0 && part[2] == 0 && part[3] == 0;
}

xiiQtRemoteConnectionDlg::xiiQtRemoteConnectionDlg(QWidget* pParent) :
  QDialog(pParent)
{
  setupUi(this);
  ...
  {
    for (xiiInt32 i = 0; i < XII_ARRAY_SIZE(m_RecentAddresses); ++i)
    {
    ...
    for (xiiInt32 i = 0; i < XII_ARRAY_SIZE(m_RecentFsAddresses); ++i)
    {
...
xiiQtRemoteConnectionDlg::~xiiQtRemoteConnectionDlg() = default;

void xiiQtRemoteConnectionDlg::SetCurrentIP(const Address& addr)
{
...
void xiiQtRemoteConnectionDlg::SetCurrentFsIP(const Address& addr)
{
...
void xiiQtRemoteConnectionDlg::AddToRecentAddresses(Address* pRecentAddresses, const Address& addr)
{
  Address prev = addr;
  for (xiiInt32 i = 0; i < XII_ARRAY_SIZE(m_RecentAddresses); ++i)
  {
...
void xiiQtRemoteConnectionDlg::showEvent(QShowEvent* event)
{
  ...
  for (xiiInt32 i = 0; i < XII_ARRAY_SIZE(m_RecentAddresses); ++i)
  {
    if (!m_RecentAddresses[i].IsEmpty())
    {
      ...
      connect(pAction, &QAction::triggered, this, &xiiQtRemoteConnectionDlg::onRecentIPselected);
  ...
  for (xiiInt32 i = 0; i < XII_ARRAY_SIZE(m_RecentFsAddresses); ++i)
  {
    if (!m_RecentFsAddresses[i].IsEmpty())
    {
      ...
      connect(pAction, &QAction::triggered, this, &xiiQtRemoteConnectionDlg::onRecentFsIPselected);
...
void xiiQtRemoteConnectionDlg::on_ButtonConnect_clicked()
{
...
void xiiQtRemoteConnectionDlg::on_ButtonLaunchFS_clicked()
{
  xiiQtLaunchFileserveDlg dlg(this);
  dlg.exec();
}

QString xiiQtRemoteConnectionDlg::GetResultingAddress() const
{
...
QString xiiQtRemoteConnectionDlg::GetResultingFsAddress() const
{
...
void xiiQtRemoteConnectionDlg::onRecentIPselected()
{
...
void xiiQtRemoteConnectionDlg::onRecentFsIPselected()
{
  QAction* pAction = qobject_cast<QAction*>(sender());
  int      ip      = pAction->data().toInt();
