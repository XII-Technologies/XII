...
xiiQtWindowCfgDlg::xiiQtWindowCfgDlg(QWidget* pParent) :
  QDialog(pParent)
{
  setupUi(this);
  LoadDescs();
  ...
  m_ComboWnd->addItem("User Specific (xiiPlayer)");
...
void xiiQtWindowCfgDlg::FillUI(const xiiWindowCreationDesc& desc)
{
  ...
  m_ComboMode->setCurrentIndex(xiiMath::Clamp<xiiInt32>(desc.m_WindowMode, 0, 3));
...
void xiiQtWindowCfgDlg::GrabUI(xiiWindowCreationDesc& desc)
{
  ...
  desc.m_WindowMode        = (xiiWindowMode::Enum)(m_ComboMode->currentIndex());
...
void xiiQtWindowCfgDlg::UpdateUI()
{
...
void xiiQtWindowCfgDlg::LoadDescs()
{
...
#if XII_ENABLED(XII_MIGRATE_RUNTIMECONFIGS)
    xiiStringBuilder sOldPath = xiiApplicationServices::GetSingleton()->GetProjectPreferencesFolder();
    sOldPath.AppendPath("Window.ddl");
    sPath = xiiFileSystem::MigrateFileLocation(sOldPath, sPath);
#endif
...
void xiiQtWindowCfgDlg::SaveDescs()
{
  xiiStringBuilder sPath;
...

  {
    ...
    {
      xiiOSFile::DeleteFile(sPath).IgnoreResult();
    }
  }
}

void xiiQtWindowCfgDlg::on_m_ButtonBox_clicked(QAbstractButton* button)
{
...
void xiiQtWindowCfgDlg::on_m_ComboWnd_currentIndexChanged(int index)
{
  GrabUI(m_Descs[m_uiCurDesc]);
  m_uiCurDesc = index;
...
void xiiQtWindowCfgDlg::on_m_CheckOverrideDefault_stateChanged(int state)
{
