...
{
public:
  virtual void GetCreateableTypes(xiiHybridArray<const xiiRTTI*, 32>& Types) const override { Types.PushBack(xiiGetStaticRTTI<xiiPlatformProfile>()); }
};
...
public:
  ...
  virtual QVariant data(const xiiDocumentObject* pObject, int row, int column, int role) const override
  {
      ...
      if (role == Qt::DecorationRole)
      {
        ...
        switch (iPlatform)
        {
          case xiiProfileTargetPlatform::PC:
            return xiiQtUiServices::GetSingleton()->GetCachedIconResource(":EditorFramework/Icons/PlatformWindows16.png");

          case xiiProfileTargetPlatform::UWP:
            return xiiQtUiServices::GetSingleton()->GetCachedIconResource(":EditorFramework/Icons/PlatformWindows16.png"); // TODO: icon

          case xiiProfileTargetPlatform::Android:
            return xiiQtUiServices::GetSingleton()->GetCachedIconResource(":/EditorFramework/Icons/PlatformAndroid16.png");
        }
      }

      if (role == Qt::DisplayRole)
      {
        QString name = xiiQtNameableAdapter::data(pObject, row, column, role).toString();

        if (row == xiiAssetCurator::GetSingleton()->GetActiveAssetProfileIndex())
        {
          name += " (active)";
        }
    ...
    return xiiQtNameableAdapter::data(pObject, row, column, role);
  }
...
xiiQtAssetProfilesDlg::xiiQtAssetProfilesDlg(QWidget* parent) :
  QDialog(parent)
{
  setupUi(this);
