...
void UpdateInputDynamicEnumValues()
{
  ...
  xiiGameAppInputConfig::ReadFromDDL(file, Actions);

  auto& dynEnum = xiiDynamicStringEnum::CreateDynamicEnum("InputSet");
...
xiiQtInputConfigDlg::xiiQtInputConfigDlg(QWidget* parent) :
  QDialog(parent)
{
  setupUi(this);
  ...
  xiiQtEditorApp::GetSingleton()->GetKnownInputSlots(m_AllInputSlots);
  ...
  // to prevent losing data when some plugin is not loaded
  {
    for (const auto& action : m_Actions)
    {
      for (int i = 0; i < 3; ++i)
      {
        if (m_AllInputSlots.IndexOf(action.m_sInputSlotTrigger[i]) == xiiInvalidIndex)
...
void xiiQtInputConfigDlg::on_ButtonNewInputSet_clicked()
{
  ...
  {
    xiiQtUiServices::GetSingleton()->MessageBoxInformation("An Input Set with this name already exists.");
  }
  else
  {
    ...
    pItem->setIcon(0, xiiQtUiServices::GetSingleton()->GetCachedIconResource(":/EditorFramework/Icons/Input16.png"));
...
void xiiQtInputConfigDlg::on_ButtonNewAction_clicked()
{
...
void xiiQtInputConfigDlg::on_ButtonRemove_clicked()
{
  if (TreeActions->selectedItems().isEmpty())
    return;
...
void xiiQtInputConfigDlg::on_ButtonOk_clicked()
{
  GetActionsFromList();
  SaveActions();
...
void xiiQtInputConfigDlg::on_ButtonCancel_clicked()
{
  reject();
}

void xiiQtInputConfigDlg::on_ButtonReset_clicked()
{
  LoadActions();
  FillList();
  on_TreeActions_itemSelectionChanged();
...
void xiiQtInputConfigDlg::on_TreeActions_itemSelectionChanged()
{
...
void xiiQtInputConfigDlg::LoadActions()
{
  m_Actions.Clear();
  ...
  xiiGameAppInputConfig::ReadFromDDL(file, m_Actions);
}
...
void xiiQtInputConfigDlg::SaveActions()
{
  ...
  xiiGameAppInputConfig::WriteToDDL(file, m_Actions);

  if (file.Close().Failed())
    xiiLog::Error("Failed to save '{0}'.", sPath);
}

void xiiQtInputConfigDlg::FillList()
{
  xiiQtScopedBlockSignals    bs(TreeActions);
  xiiQtScopedUpdatesDisabled bu(TreeActions);

  m_InputSetToItem.Clear();
  TreeActions->clear();

  xiiSet<xiiString> InputSets;
...
void xiiQtInputConfigDlg::GetActionsFromList()
{
  m_Actions.Clear();
  ...
  for (int sets = 0; sets < TreeActions->topLevelItemCount(); ++sets)
  {
    ...
    const xiiString sSetName = pSetItem->text(0).toUtf8().data();

    for (int children = 0; children < pSetItem->childCount(); ++children)
    {
      xiiGameAppInputConfig& cfg = m_Actions.ExpandAndGetRef();
...
QTreeWidgetItem* xiiQtInputConfigDlg::CreateActionItem(QTreeWidgetItem* pParentItem, const xiiGameAppInputConfig& action)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    for (xiiUInt32 it = 0; it < m_AllInputSlots.GetCount(); ++it)
    {
