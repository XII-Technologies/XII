...
xiiQtSnapSettingsDlg::xiiQtSnapSettingsDlg(QWidget* parent) :
  QDialog(parent)
{
  setupUi(this);
  ...
  {
    TranslationSnap->addItem(xiiTranslate(m_Translation[i].m_szKey));
  ...
  {
    RotationSnap->addItem(xiiTranslate(m_Rotation[i].m_szKey));

    if (xiiSnapProvider::GetRotationSnapValue() == xiiAngle::Degree(m_Rotation[i].m_fValue))
      uiSelectedR = i;
  }

  for (xiiUInt32 i = 0; i < m_Scale.GetCount(); ++i)
  {
    ScaleSnap->addItem(xiiTranslate(m_Scale[i].m_szKey));
...
void xiiQtSnapSettingsDlg::QueryUI()
{
  ...
  xiiSnapProvider::SetRotationSnapValue(xiiAngle::Degree(m_Rotation[RotationSnap->currentIndex()].m_fValue));
