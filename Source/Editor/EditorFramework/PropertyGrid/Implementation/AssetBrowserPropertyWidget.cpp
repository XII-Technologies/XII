  ...
  else
  {
    ...
    if (xiiConversionUtils::IsStringUuid(sText))
    {
      ...
      if (pAsset)
      {
        ...
        sThumbnailPath = xiiAssetDocumentManager::GenerateResourceThumbnailPath(pAsset->m_pAssetInfo->m_sAbsolutePath);
      }
      else
        m_AssetGuid = xiiUuid();
    }
...
void xiiQtAssetPropertyWidget::OnOpenAssetDocument()
{
  xiiQtEditorApp::GetSingleton()->OpenDocumentQueued(
    xiiAssetCurator::GetSingleton()->GetSubAsset(m_AssetGuid)->m_pAssetInfo->m_sAbsolutePath, GetSelection()[0].m_pObject);
}
...
void xiiQtAssetPropertyWidget::OnOpenExplorer()
{
  ...
  if (m_AssetGuid.IsValid())
  {
    sPath = xiiAssetCurator::GetSingleton()->GetSubAsset(m_AssetGuid)->m_pAssetInfo->m_sAbsolutePath;
  }
  else
  {
