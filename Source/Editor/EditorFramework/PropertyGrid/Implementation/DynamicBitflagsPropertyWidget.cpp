...
xiiQtDynamicBitflagsPropertyWidget::xiiQtDynamicBitflagsPropertyWidget() :
  xiiQtStandardPropertyWidget()
{
  ...
  XII_VERIFY(connect(m_pMenu, SIGNAL(aboutToShow()), this, SLOT(on_Menu_aboutToShow())), "show connection failed");
  XII_VERIFY(connect(m_pMenu, SIGNAL(aboutToHide()), this, SLOT(on_Menu_aboutToHide())), "hide connection failed");
}
...
void xiiQtDynamicBitflagsPropertyWidget::on_Menu_aboutToHide()
{
  xiiInt64 iValue = 0;
  QString  sText;
...
void xiiQtDynamicBitflagsPropertyWidget::on_Menu_aboutToShow()
{
...
void xiiQtDynamicBitflagsPropertyWidget::OnInit()
{
  BuildMenu();
  FillInCheckedBoxes();
}

void xiiQtDynamicBitflagsPropertyWidget::InternalSetValue(const xiiVariant& value)
{
...
void xiiQtDynamicBitflagsPropertyWidget::SetAll(bool bChecked)
{
...
void xiiQtDynamicBitflagsPropertyWidget::ClearMenu()
{
  m_Constants.Clear();
  m_pMenu->clear();
}

void xiiQtDynamicBitflagsPropertyWidget::BuildMenu()
{
  XII_ASSERT_DEV(
    m_pProp->GetAttributeByType<xiiDynamicBitflagsAttribute>() != nullptr, "xiiQtDynamicBitflagsPropertyWidget was created without a xiiDynamicBitflagsAttribute!");

  const xiiDynamicBitflagsAttribute* pAttr = m_pProp->GetAttributeByType<xiiDynamicBitflagsAttribute>();

  const auto& dbitflags = xiiDynamicBitflags::GetDynamicBitflags(pAttr->GetDynamicBitflagsName());
  ...
  {
    xiiString sContantName = it.Value();
    xiiInt32  iConstant    = it.Key();

    QWidgetAction* pAction   = new QWidgetAction(m_pMenu);
    QCheckBox*     pCheckBox = new QCheckBox(xiiMakeQString(xiiTranslate(sContantName)), m_pMenu);
    pCheckBox->setCheckable(true);
    pCheckBox->setCheckState(Qt::Unchecked);
    pAction->setDefaultWidget(pCheckBox);

    m_Constants[(xiiInt64)iConstant] = pCheckBox;
  ...
  // sets all bits to clear or set
  {
    ...
    connect(m_pAllButton, &QPushButton::clicked, this, [this](bool bChecked) { SetAll(true); });
    ...
    connect(m_pClearButton, &QPushButton::clicked, this, [this](bool bChecked) { SetAll(false); });
    pClearAction->setDefaultWidget(m_pClearButton);
    m_pMenu->addAction(pClearAction);
  }
}

void xiiQtDynamicBitflagsPropertyWidget::FillInCheckedBoxes()
{
  if (m_iCurrentBitflags == 0)
    return;

  for (xiiUInt32 i = 0; i < 64; ++i) // at max 64 bits
  {
    xiiInt64 iValue = XII_BIT(i);
