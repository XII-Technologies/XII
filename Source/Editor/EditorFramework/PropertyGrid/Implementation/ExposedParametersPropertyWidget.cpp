...
bool xiiExposedParameterCommandAccessor::IsExposedProperty(const xiiDocumentObject* pObject, const xiiAbstractProperty* pProp)
{
  if (auto type = GetExposedParamsType(pObject))
  {
    ...
    return std::any_of(cbegin(props), cend(props), [pProp](const xiiAbstractProperty* prop) { return prop == pProp; });
  }
  return false;
}
...
void xiiQtExposedParametersPropertyWidget::OnInit()
{
...

  {
    ...
    m_pFixMeButton->setIcon(xiiQtUiServices::GetSingleton()->GetCachedIconResource(":/EditorFramework/Icons/Attention16.png"));
