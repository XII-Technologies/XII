...
public:
  ...
  xiiAssetDocument(const char* szDocumentPath, xiiDocumentObjectManager* pObjectManager, xiiAssetDocEngineConnection engineConnectionType);
  ~xiiAssetDocument();
  ...
  /// \brief Sends a message to the corresponding xiiEngineProcessDocumentContext on the engine process.
  void SendMessageToEngine(xiiEditorEngineDocumentMsg* pMessage) const;

  /// \brief Handles all messages received from the corresponding xiiEngineProcessDocumentContext on the engine process.
  virtual void HandleEngineMessage(const xiiEditorEngineDocumentMsg* pMsg);

  /// \brief Returns the xiiEditorEngineConnection for this document.
  xiiEditorEngineConnection* GetEditorEngineConnection() const { return m_pEngineConnection; }

  /// \brief Registers a sync object for this document. It will be mirrored to the xiiEngineProcessDocumentContext on the engine process.
...
protected:
  ...
  virtual xiiTransformStatus InternalTransformAsset(xiiStreamWriter& stream, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags) = 0;
  ...
  virtual xiiTransformStatus InternalTransformAsset(const char* szTargetFile, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags);
  ...
  /// \brief Override this function to generate a thumbnail. Only called if GetAssetFlags returns xiiAssetDocumentFlags::SupportsThumbnail.
  ...
  xiiString GetThumbnailFilePath() const;
...
protected:
  /// \brief Adds all prefab dependencies to the xiiAssetDocumentInfo object. Called automatically by UpdateAssetDocumentInfo()
  void AddPrefabDependencies(const xiiDocumentObject* pObject, xiiAssetDocumentInfo* pInfo) const;

  /// \brief Crawls through all asset properties of pObject and adds all string properties that have a xiiAssetBrowserAttribute as a dependency to
...
protected:
  xiiIPCObjectMirrorEditor m_Mirror;
