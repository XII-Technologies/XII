...
void xiiAssetTableWriter::NeedsReloadResource(const xiiUuid& assetGuid)
{
  ...
  if (asset.isValid())
  {
    ...
    xiiAssetDocumentManager* pManager = asset->m_pAssetInfo->GetManager();
    const xiiString&         sDocType = asset->m_pAssetInfo->m_pDocumentTypeDescriptor->m_sDocumentTypeName;
...
xiiUInt32 xiiAssetTableWriter::FindDataDir(const xiiSubAsset& asset)
{
  ...
  XII_REPORT_FAILURE("The data dir for an asset was not found.");
  return 0;
}
