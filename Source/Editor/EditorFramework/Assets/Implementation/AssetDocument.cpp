...
xiiAssetDocument::xiiAssetDocument(const char* szDocumentPath, xiiDocumentObjectManager* pObjectManager, xiiAssetDocEngineConnection engineConnectionType) :
  xiiDocument(szDocumentPath, pObjectManager)
{
...
xiiUInt64 xiiAssetDocument::GetDocumentHash() const
{
  ...
  typesSorted.Sort([](const xiiRTTI* a, const xiiRTTI* b) { return xiiStringUtils::Compare(a->GetTypeName(), b->GetTypeName()) < 0; });
  for (const xiiRTTI* pType : typesSorted)
  {
    uiHash                 = xiiHashingUtils::xxHash64(pType->GetTypeName(), std::strlen(pType->GetTypeName()), uiHash);
...
xiiTransformStatus xiiAssetDocument::InternalTransformAsset(const char* szTargetFile, const char* szOutputTag, const xiiPlatformProfile* pAssetProfile, const xiiAssetFileHeader& AssetHeader, xiiBitflags<xiiTransformFlags> transformFlags)
{
  ...
  xiiTransformStatus res = InternalTransformAsset(file, szOutputTag, pAssetProfile, AssetHeader, transformFlags);
...
xiiString xiiAssetDocument::GetThumbnailFilePath() const
{
  return static_cast<xiiAssetDocumentManager*>(GetDocumentManager())->GenerateResourceThumbnailPath(GetDocumentPath());
}

void xiiAssetDocument::InvalidateAssetThumbnail() const
{
...
void xiiAssetDocument::AppendThumbnailInfo(const char* szThumbnailFile, const ThumbnailInfo& thumbnailInfo) const
{
  ...
  {
    xiiLog::Error("Could not open file for writing: '{0}'", szThumbnailFile);
  }
}
...
void xiiAssetDocument::SendMessageToEngine(xiiEditorEngineDocumentMsg* pMessage /*= false*/) const
{
