...
xiiProcessTask::xiiProcessTask() :
  m_bProcessShouldBeRunning(false), m_bProcessCrashed(false), m_bWaiting(false), m_Status(XII_SUCCESS)
{
...
bool xiiProcessTask::GetNextAssetToProcess(xiiAssetInfo* pInfo, xiiUuid& out_guid, xiiStringBuilder& out_sAbsPath, xiiStringBuilder& out_sRelPath)
{
  bool bComplete = true;

  const xiiDocumentTypeDescriptor* pTypeDesc = nullptr;
  if (xiiDocumentManager::FindDocumentTypeFromPath(pInfo->m_sAbsolutePath, false, pTypeDesc).Succeeded())
  {
...
bool xiiProcessTask::GetNextAssetToProcess(xiiUuid& out_guid, xiiStringBuilder& out_sAbsPath, xiiStringBuilder& out_sRelPath)
{
...
{
  xiiStringBuilder sAssetRelPath;
  ...
  {
    xiiLog::Info(&xiiAssetProcessor::GetSingleton()->m_CuratorLog, "Processing '{0}'", sAssetRelPath);
    // Send and wait
