...
xiiQtAssetImportDlg::xiiQtAssetImportDlg(QWidget* parent, xiiDynamicArray<xiiAssetDocumentGenerator::ImportData>& allImports) :
  QDialog(parent), m_AllImports(allImports)
{
  setupUi(this);
...
xiiQtAssetImportDlg::~xiiQtAssetImportDlg() {}
...
void xiiQtAssetImportDlg::InitRow(xiiUInt32 uiRow)
{
  ...
  pCombo->addItem(xiiQtUiServices::GetSingleton()->GetCachedIconResource(":/GuiFoundation/Icons/No16.png"), "No Import");
  for (const auto& option : data2.m_ImportOptions)
  {
    QIcon icon = xiiQtUiServices::GetSingleton()->GetCachedIconResource(option.m_sIcon);
    pCombo->addItem(icon, xiiTranslate(option.m_sName.GetData()));
  }
  ...
  connect(table, &QTableWidget::cellChanged, this, &xiiQtAssetImportDlg::TableCellChanged);
  ...
  connect(pBrowse, &QToolButton::clicked, this, &xiiQtAssetImportDlg::BrowseButtonClicked);
  table->setCellWidget(uiRow, Columns::Browse, pBrowse);

  UpdateRow(uiRow);
}

void xiiQtAssetImportDlg::UpdateRow(xiiUInt32 uiRow)
{
  QTableWidget*           table = AssetTable;
  xiiQtScopedBlockSignals _1(table);
...
void xiiQtAssetImportDlg::QueryRow(xiiUInt32 uiRow)
{
  QTableWidget* table = AssetTable;
  ...
  xiiStringBuilder file = table->item(uiRow, Columns::GeneratedDoc)->text().toUtf8().data();
  file.ChangeFileExtension(option.m_pGenerator->GetDocumentExtension());
  option.m_sOutputFileParentRelative = file;
}

void xiiQtAssetImportDlg::UpdateAllRows()
{
  for (xiiUInt32 i = 0; i < m_AllImports.GetCount(); ++i)
  {
    UpdateRow(i);
...
{
  xiiAssetDocumentGenerator::ExecuteImport(m_AllImports);

  UpdateAllRows();
}

void xiiQtAssetImportDlg::TableCellChanged(int row, int column)
{
...
void xiiQtAssetImportDlg::BrowseButtonClicked(bool)
{
  QToolButton*    pButton = qobject_cast<QToolButton*>(sender());
  const xiiUInt32 uiRow   = pButton->property("row").toInt();
  ...
  else
  {
    ...
    QString result = QFileDialog::getSaveFileName(this, "Target Document", xiiToolsProject::GetSingleton()->GetProjectDirectory().GetData(),
                                                  filter.GetData(), nullptr, QFileDialog::Option::DontResolveSymlinks);

    if (result.isEmpty())
      return;

    xiiStringBuilder tmp = result.toUtf8().data();
    if (!xiiQtEditorApp::GetSingleton()->MakePathDataDirectoryParentRelative(tmp))
    {
      xiiQtUiServices::GetSingleton()->MessageBoxWarning("The selected file is not located in any of the project's data directories.");
      return;
    }
