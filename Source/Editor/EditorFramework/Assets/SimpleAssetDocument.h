...
public:
  xiiSimpleAssetDocument(xiiStringView sDocumentPath, xiiAssetDocEngineConnection engineConnectionType, bool bEnableDefaultLighting = false) :
    BaseClass(sDocumentPath, XII_DEFAULT_NEW(xiiSimpleDocumentObjectManager<PropertyType>), engineConnectionType), m_LightSettings(bEnableDefaultLighting)
  {
  ...
  xiiSimpleAssetDocument(xiiDocumentObjectManager* pObjectManager, xiiStringView sDocumentPath, xiiAssetDocEngineConnection engineConnectionType, bool bEnableDefaultLighting = false) :
    BaseClass(sDocumentPath, pObjectManager, engineConnectionType), m_LightSettings(bEnableDefaultLighting)
  {
    if (bEnableDefaultLighting)
    {
      xiiEditorPreferencesUser* pPreferences = xiiPreferences::QueryPreferences<xiiEditorPreferencesUser>();
