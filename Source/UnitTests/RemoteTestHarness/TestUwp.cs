...
namespace xiiUwpTestHarness
{
  ...
  class xiiTestUWP
  {
    ...
    public xiiTestUWP(string xiiWorkspace, string testOutputPath,
        string configuration, string platform, string project)
    {
      _xiiWorkspace = xiiWorkspace;
    ...
    private void DeployAppX(out string fullPackageName)
    {
      ...
      string absSlnPath = Path.Combine(_xiiWorkspace, "xiiVs2019.sln");
      ...
      // "C:\Program Files (x86)\Microsoft Visual Studio 14.0\Common7\IDE\devenv.com" "F:\Development\current_development\xiiEngine\build_uwp64\xiiEngine.sln" /Deploy "Dev|x64" /project CoreTest
      var deployProcessResults = xiiProcessHelper.RunExternalExe(devEnvPath,
    ...
    private void StartAppX(string fullPackageName, string args, out uint pid)
    {
      ...
      //
      // The following method seems to work just fine for xiiEngine projects if not for all AppX in general.
      //
      ...
      string appUserModelId = appFamilyName + "!App"; // Apply our knowledge of xiiEngine's appx manifests.
    ...
    public void ExecuteTest()
    {
      ...
      Func<xiiProcessHelper.ProcessResult> startFileserve = () =>
      {
        Console.WriteLine("Starting Fileserve ...");

        // 60s timeout for connect, 2s timeout for closing after connection loss.
        string args = string.Format("-specialdirs xiitest \"{0}\" -fs_start -fs_wait_timeout 120 -fs_close_timeout 4", _absTestOutputDirectory);
        return xiiProcessHelper.RunExternalExe(absFilerserveFilename, args, absBinDir, fileserveTimeoutMS);
      };

      // create a real time user mode session
      using (var session = new TraceEventSession("xiiETWMonitorSession"))
      {
 
        session.EnableProvider(new Guid("BFD4350A-BA77-463D-B4BE-E30374E42494")); //xiiLogProvider
        session.Source.Dynamic.AddCallbackForProviderEvent("xiiLogProvider", "LogMessge", delegate (TraceEvent data)
        {
          ...
          if (Type != (int)xiiLogMsgType.EndGroup)
          {
            Console.Out.WriteLine("".PadLeft(Indentation) + Text);
          }

          if (Type == (int)xiiLogMsgType.InfoMsg && Text == "All tests passed.")
          {
            Interlocked.Exchange(ref this._failed, 0);
            Interlocked.Exchange(ref this._finished, 1);
          }
          else if (Type == (int)xiiLogMsgType.InfoMsg && Text != null && Text.StartsWith("Tests failed: ") && Text.Contains("Tests passed: "))
          {
        ...
        using (Task<xiiProcessHelper.ProcessResult> runFileServe = Task.Factory.StartNew(startFileserve, System.Threading.Tasks.TaskCreationOptions.LongRunning))
        {
          ...
          if (!appXProcess.WaitForExit(5000))
          {
            ...
            try
            {
              ...
              xiiProcessHelper.RunExternalExe("procdump", args, absBinDir);
            }
            catch (Exception e)
            {
