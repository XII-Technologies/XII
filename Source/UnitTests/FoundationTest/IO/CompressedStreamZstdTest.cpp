...
XII_CREATE_SIMPLE_TEST(IO, CompressedStreamZstd)
{
  ...
  xiiCompressedStreamReaderZstd CompressedReader;
  xiiCompressedStreamWriterZstd CompressedWriter;
  ...
  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Compress Data")
  {
    ...
    for (xiiUInt32 i = 0; i < TestData.GetCount();)
    {
      ...
      if (bFlush)
      {
        ...
        XII_TEST_BOOL(CompressedWriter.Flush() == XII_SUCCESS);
      }
    ...
    const xiiUInt64 uiBytesWritten = CompressedWriter.GetWrittenBytes();

    XII_TEST_INT(uiUncompressed, TestData.GetCount() * sizeof(xiiUInt32));
    XII_TEST_BOOL(uiBytesWritten > uiCompressed);
    XII_TEST_BOOL(uiBytesWritten < uiUncompressed);

    const float fRatio = (float)uiUncompressed / (float)uiCompressed;
    XII_TEST_BOOL(fRatio >= fExpectedCompressionRatio);
  }
