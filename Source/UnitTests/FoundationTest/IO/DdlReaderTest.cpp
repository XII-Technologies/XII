...
static void WriteObjectToDDL(const xiiOpenDdlReaderElement* pElement, xiiOpenDdlWriter& ref_writer)
{
  ...
  if (pElement->IsCustomType())
  {
    ...
    while (pChild)
    {
      ...
      if (pChild->HasName())
      {
        xiiString                      sNameCopy = pChild->GetName();
        const xiiOpenDdlReaderElement* pChild2   = pElement->FindChild(sNameCopy);
  ...
  else
  {
    ...
    switch (type)
    {
      ...
      case xiiOpenDdlPrimitiveType::Custom:
      default:
...
static void WriteToDDL(const xiiOpenDdlReader& doc, xiiStreamWriter& ref_output)
{
  ...
  XII_ASSERT_DEV(pRoot != nullptr, "Invalid root");
...
static void TestEqual(const char* szOriginal, const char* szRecreation)
{
  ...
  do
  {
    ...
    if (cOrg != cAlt)
    {
      XII_TEST_FAILURE("String compare failed", "DDL Original and recreation don't match at character %u ('%c' -> '%c')", uiChar, cOrg, cAlt);
      return;
    }
...
static void TestDoc(const xiiOpenDdlReader& doc, const char* szOriginal)
{
  xiiStringBuilder recreation;
  WriteToString(doc, recreation);

  TestEqual(szOriginal, recreation);
}

XII_CREATE_SIMPLE_TEST(IO, DdlReader)
{
  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Basics and Comments")
  {
...
";
    ...
    XII_TEST_BOOL(!doc.HadFatalParsingError());

    auto pRoot = doc.GetRootElement();
    XII_TEST_INT(pRoot->GetNumChildObjects(), 1);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Structure")
  {
    const char* szTestData = "Node\n\
{\n\
	Name\n\
...
";
    ...
    XII_TEST_BOOL(pElement != nullptr);
    if (pElement)
    {
      XII_TEST_STRING(pElement->GetName(), "MyDoubles");
    }

    XII_TEST_BOOL(doc.FindElement("MyFloats") == nullptr);
    XII_TEST_BOOL(doc.FindElement("Node") == nullptr);

    TestDoc(doc, szTestData);
    XII_TEST_BOOL(!doc.HadFatalParsingError());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "All Primitives")
  {
...
";
  ...
  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Errors")
  {
    const char* szTestData = "\
...
";
    ...
    log.ExpectMessage("Unknown escape-sequence '\\2'", xiiLogMsgType::WarningMsg);

    xiiOpenDdlReader doc;
    XII_TEST_BOOL(doc.ParseDocument(stream).Succeeded()); // no fatal error
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Fatal Errors")
  {
    const char* szTestData = "\
string{\"s1\",\"back\\slash\"\n\
string{\"s\\2\",\"bla\"}\n\
