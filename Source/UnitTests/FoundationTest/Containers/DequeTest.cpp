#include <FoundationTest/FoundationTestPCH.h>

#include <Foundation/Configuration/Startup.h>
#include <Foundation/Types/UniquePtr.h>

namespace DequeTestDetail
{
  using st = xiiConstructionCounter;

  static xiiDeque<st> CreateArray(xiiUInt32 uiSize, xiiUInt32 uiOffset)
  {
    xiiDeque<st> a;
    a.SetCount(uiSize);

    for (xiiUInt32 i = 0; i < uiSize; ++i)
      a[i] = uiOffset + i;

    return a;
  }
} // namespace DequeTestDetail

XII_CREATE_SIMPLE_TEST(Containers, Deque)
{
  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Fill / Empty (Sawtooth)")
  {
    xiiDeque<xiiInt32> d;

    xiiUInt32 uiVal = 0;

    for (xiiInt32 i = 0; i < 10000; ++i)
      d.PushBack(uiVal++);

    // this is kind of the worst case scenario, as it will deallocate and reallocate chunks in every loop
    // the smaller the chunk size, the more allocations will happen
    for (xiiUInt32 s2 = 0; s2 < 10; ++s2)
    {
      for (xiiInt32 i = 0; i < 1000; ++i)
        d.PopBack();
      for (xiiInt32 i = 0; i < 1000; ++i)
        d.PushBack(uiVal++);
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Fill / Empty")
  {
    xiiDeque<xiiInt32> d;

    XII_TEST_BOOL(d.GetHeapMemoryUsage() == 0);

    for (xiiInt32 i = 0; i < 10000; ++i)
      d.PushBack(i);

    XII_TEST_BOOL(d.GetHeapMemoryUsage() > 10000 * sizeof(xiiInt32));

    for (xiiInt32 i = 0; i < 10000; ++i)
      d.PopFront();
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Queue Back")
  {
    xiiDeque<xiiInt32> d;

    d.PushBack(0);

    for (xiiInt32 i = 0; i < 10000; ++i)
    {
      d.PushBack(i);
      d.PopFront();
    }

    d.Compact();

    for (xiiInt32 i = 0; i < 10000; ++i)
    {
      d.PushBack(i);
      d.PopFront();
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Queue Front")
  {
    xiiDeque<xiiInt32> d;

    d.PushBack(0);

    for (xiiInt32 i = 0; i < 10000; ++i)
    {
      d.PushFront(i);
      d.PopBack();
    }

    d.Compact();

    for (xiiInt32 i = 0; i < 10000; ++i)
    {
      d.PushFront(i);
      d.PopBack();
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "POD Types")
  {
    xiiDeque<xiiInt32> d1;
    d1.SetCount(5120);

    d1.Compact();

    XII_TEST_BOOL(d1.GetCount() == 5120);


    d1.SetCount(1);
    d1.Compact();

    XII_TEST_BOOL(d1.GetHeapMemoryUsage() > 0);

    d1.Clear();

    d1.Compact();

    XII_TEST_BOOL(d1.GetHeapMemoryUsage() == 0);
  }

  xiiStartup::ShutdownCoreSystems();

  xiiStartup::StartupCoreSystems();

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "POD Types")
  {
    xiiDeque<xiiInt32> d1;
    d1.SetCount(1000);

    XII_TEST_BOOL(d1.GetCount() == 1000);
    d1.Clear();
    XII_TEST_BOOL(d1.IsEmpty());

    for (xiiInt32 i = 1; i < 1000; ++i)
    {
      d1.PushBack(i);

      XII_TEST_BOOL(d1.PeekBack() == i);
      XII_TEST_BOOL(d1.GetCount() == i);
      XII_TEST_BOOL(!d1.IsEmpty());
    }

    d1.Clear();
    XII_TEST_BOOL(d1.IsEmpty());

    for (xiiInt32 i = 1; i < 1000; ++i)
    {
      d1.PushFront(i);

      XII_TEST_BOOL(d1.PeekFront() == i);
      XII_TEST_BOOL(d1.GetCount() == i);
      XII_TEST_BOOL(!d1.IsEmpty());
    }

    d1.Clear();
    XII_TEST_BOOL(d1.IsEmpty());

    for (xiiInt32 i = 1; i <= 1000; ++i)
    {
      d1.PushFront(i);
      d1.PushBack(i);

      XII_TEST_BOOL(d1.PeekFront() == i);
      XII_TEST_BOOL(d1.PeekBack() == i);
      XII_TEST_BOOL(d1.GetCount() == i * 2);
      XII_TEST_BOOL(!d1.IsEmpty());
    }

    xiiDeque<xiiInt32> d2;
    d2 = d1;

    for (xiiInt32 i = 1000; i >= 1; --i)
    {
      XII_TEST_BOOL(d1.PeekFront() == i);
      XII_TEST_BOOL(d1.PeekBack() == i);
      XII_TEST_BOOL(d1.GetCount() == i * 2);
      XII_TEST_BOOL(!d1.IsEmpty());

      d1.PopFront();
      d1.PopBack();


      XII_TEST_BOOL(d2.PeekFront() == i);
      XII_TEST_BOOL(d2.PeekBack() == i);
      XII_TEST_BOOL(d2.GetCount() == i * 2);
      XII_TEST_BOOL(!d2.IsEmpty());

      d2.PopFront();
      d2.PopBack();
    }

    XII_TEST_BLOCK(xiiTestBlock::Enabled, "STL Iterator")
    {
      xiiDeque<xiiInt32> a1;
      for (xiiInt32 i = 0; i < 1000; ++i)
        a1.PushBack(1000 - i - 1);

      // STL sort
      std::sort(begin(a1), end(a1));

      xiiUInt32 prev = 0;
      for (xiiUInt32 val : a1)
      {
        XII_TEST_BOOL(prev <= val);
        prev = val;
      }

      // STL lower bound
      auto lb = std::lower_bound(begin(a1), end(a1), 400);
      XII_TEST_BOOL(*lb == a1[400]);
    }

    XII_TEST_BLOCK(xiiTestBlock::Enabled, "STL Reverse Iterator")
    {
      xiiDeque<xiiInt32> a1;
      for (xiiInt32 i = 0; i < 1000; ++i)
        a1.PushBack(1000 - i - 1);

      std::sort(rbegin(a1), rend(a1));

      // foreach
      xiiUInt32 prev = 1000;
      for (xiiUInt32 val : a1)
      {
        XII_TEST_BOOL(prev >= val);
        prev = val;
      }

      // const array
      const xiiDeque<xiiInt32>& a2 = a1;

      // STL lower bound
      auto lb2 = std::lower_bound(rbegin(a2), rend(a2), 400);
      XII_TEST_INT(*lb2, a2[1000 - 400 - 1]);
    }
  }

  xiiStartup::ShutdownCoreSystems();

  xiiStartup::StartupCoreSystems();

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Non-POD Types")
  {
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    {
      xiiDeque<DequeTestDetail::st> v1;
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

      {
        v1.PushBack(DequeTestDetail::st(3));
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1));

        v1.PushBack();
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(1, 0));

        v1.PopBack();
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1));
      }
      {
        v1.PushFront(DequeTestDetail::st(3));
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1));

        v1.PushFront();
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(1, 0));

        v1.PopFront();
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1));
      }

      XII_TEST_BOOL(v1.GetCount() == 2);

      v1.SetCount(12);
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(10, 0));

      {
        xiiDeque<DequeTestDetail::st> v2;
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

        v2 = v1;
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        v2.Clear();
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));

        xiiDeque<DequeTestDetail::st> v3(v1);
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        xiiDeque<DequeTestDetail::st> v4(v1);
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        v4.SetCount(0);
        XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));
      }

      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));
    }

    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "SortingPrimitives")
  {
    xiiDeque<xiiUInt32> list;

    list.Sort();

    for (xiiUInt32 i = 0; i < 245; i++)
    {
      list.PushBack(std::rand());
    }
    list.Sort();

    xiiUInt32 last = 0;
    for (xiiUInt32 i = 0; i < list.GetCount(); i++)
    {
      XII_TEST_BOOL(last <= list[i]);
      last = list[i];
    }
  }



  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Constructor")
  {
    xiiDeque<xiiInt32>            a1;
    xiiDeque<DequeTestDetail::st> a2;

    XII_TEST_BOOL(a1.GetCount() == 0);
    XII_TEST_BOOL(a2.GetCount() == 0);
    XII_TEST_BOOL(a1.IsEmpty());
    XII_TEST_BOOL(a2.IsEmpty());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Copy Constructor")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 32; ++i)
      a1.PushBack(rand() % 100000);

    xiiDeque<xiiInt32> a2 = a1;
    xiiDeque<xiiInt32> a3(a1);

    XII_TEST_BOOL(a1.GetCount() == a2.GetCount());
    XII_TEST_BOOL(a1.GetCount() == a3.GetCount());

    for (xiiUInt32 i = 0; i < a1.GetCount(); ++i)
    {
      XII_TEST_BOOL(a1[i] == a2[i]);
      XII_TEST_BOOL(a1[i] == a3[i]);
      XII_TEST_BOOL(a2[i] == a3[i]);
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Move Constructor / Operator")
  {
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    {
      // move constructor
      xiiDeque<DequeTestDetail::st> a1(DequeTestDetail::CreateArray(100, 20));

      XII_TEST_INT(a1.GetCount(), 100);
      for (xiiUInt32 i = 0; i < a1.GetCount(); ++i)
        XII_TEST_INT(a1[i].m_iData, 20 + i);

      // move operator
      a1 = DequeTestDetail::CreateArray(200, 50);

      XII_TEST_INT(a1.GetCount(), 200);
      for (xiiUInt32 i = 0; i < a1.GetCount(); ++i)
        XII_TEST_INT(a1[i].m_iData, 50 + i);
    }

    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "operator = / operator == / operator !=")
  {
    xiiDeque<xiiInt32> a1;
    xiiDeque<xiiInt32> a2;

    XII_TEST_BOOL(a1 == a2);

    for (xiiInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    XII_TEST_BOOL(a1 != a2);

    a2 = a1;

    XII_TEST_BOOL(a1.GetCount() == a2.GetCount());

    for (xiiUInt32 i = 0; i < a1.GetCount(); ++i)
      XII_TEST_BOOL(a1[i] == a2[i]);

    XII_TEST_BOOL(a1 == a2);
    XII_TEST_BOOL(a2 == a1);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Index operator")
  {
    xiiDeque<xiiInt32> a1;
    a1.SetCount(100);

    for (xiiInt32 i = 0; i < 100; ++i)
      a1[i] = i;

    for (xiiInt32 i = 0; i < 100; ++i)
      XII_TEST_INT(a1[i], i);

    xiiDeque<xiiInt32> ca1;
    ca1 = a1;

    for (xiiInt32 i = 0; i < 100; ++i)
      XII_TEST_INT(ca1[i], i);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "SetCount / GetCount / IsEmpty")
  {
    xiiDeque<xiiInt32> a1;

    XII_TEST_BOOL(a1.IsEmpty());

    for (xiiInt32 i = 0; i < 128; ++i)
    {
      a1.SetCount(i + 1);
      XII_TEST_INT(a1[i], 0); // default init
      a1[i] = i;
      XII_TEST_INT(a1[i], i);

      XII_TEST_INT(a1.GetCount(), i + 1);
      XII_TEST_BOOL(!a1.IsEmpty());
    }

    for (xiiInt32 i = 0; i < 128; ++i)
      XII_TEST_INT(a1[i], i);

    for (xiiInt32 i = 128; i >= 0; --i)
    {
      a1.SetCount(i);

      XII_TEST_INT(a1.GetCount(), i);

      for (xiiInt32 i2 = 0; i2 < i; ++i2)
        XII_TEST_INT(a1[i2], i2);
    }

    XII_TEST_BOOL(a1.IsEmpty());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "SetCountUninitialized")
  {
    xiiDeque<xiiInt32> a1;

    XII_TEST_BOOL(a1.IsEmpty());

    for (xiiInt32 i = 0; i < 128; ++i)
    {
      a1.SetCountUninitialized(i + 1);
      // no default init
      a1[i] = i;
      XII_TEST_INT(a1[i], i);

      XII_TEST_INT(a1.GetCount(), i + 1);
      XII_TEST_BOOL(!a1.IsEmpty());
    }

    for (xiiInt32 i = 0; i < 128; ++i)
      XII_TEST_INT(a1[i], i);

    for (xiiInt32 i = 128; i >= 0; --i)
    {
      a1.SetCountUninitialized(i);

      XII_TEST_INT(a1.GetCount(), i);

      for (xiiInt32 i2 = 0; i2 < i; ++i2)
        XII_TEST_INT(a1[i2], i2);
    }

    XII_TEST_BOOL(a1.IsEmpty());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "EnsureCount")
  {
    xiiDeque<xiiInt32> a1;

    XII_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(0);
    XII_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(1);
    XII_TEST_INT(a1.GetCount(), 1);

    a1.EnsureCount(2);
    XII_TEST_INT(a1.GetCount(), 2);

    a1.EnsureCount(1);
    XII_TEST_INT(a1.GetCount(), 2);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Clear")
  {
    xiiDeque<xiiInt32> a1;
    a1.Clear();

    a1.PushBack(3);
    a1.Clear();

    XII_TEST_BOOL(a1.IsEmpty());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Contains / IndexOf / LastIndexOf")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = -100; i < 100; ++i)
      XII_TEST_BOOL(!a1.Contains(i));

    for (xiiInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    for (xiiInt32 i = 0; i < 100; ++i)
    {
      XII_TEST_BOOL(a1.Contains(i));
      XII_TEST_INT(a1.IndexOf(i), i);
      XII_TEST_INT(a1.LastIndexOf(i), i);
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Insert")
  {
    xiiDeque<xiiInt32> a1;

    // always inserts at the front
    for (xiiInt32 i = 0; i < 100; ++i)
      a1.InsertAt(0, i);

    for (xiiInt32 i = 0; i < 100; ++i)
      XII_TEST_INT(a1[i], 99 - i);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "RemoveAndCopy")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 100; ++i)
      a1.PushBack(i % 2);

    while (a1.RemoveAndCopy(1))
    {
    }

    XII_TEST_BOOL(a1.GetCount() == 50);

    for (xiiUInt32 i = 0; i < a1.GetCount(); ++i)
      XII_TEST_INT(a1[i], 0);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "RemoveAndSwap")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAndSwap(9);
    a1.RemoveAndSwap(7);
    a1.RemoveAndSwap(5);
    a1.RemoveAndSwap(3);
    a1.RemoveAndSwap(1);

    XII_TEST_INT(a1.GetCount(), 5);

    for (xiiInt32 i = 0; i < 5; ++i)
      XII_TEST_BOOL(xiiMath::IsEven(a1[i]));
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "RemoveAtAndCopy")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAtAndCopy(9);
    a1.RemoveAtAndCopy(7);
    a1.RemoveAtAndCopy(5);
    a1.RemoveAtAndCopy(3);
    a1.RemoveAtAndCopy(1);

    XII_TEST_INT(a1.GetCount(), 5);

    for (xiiInt32 i = 0; i < 5; ++i)
      XII_TEST_INT(a1[i], i * 2);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "RemoveAtAndSwap")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAtAndSwap(9);
    a1.RemoveAtAndSwap(7);
    a1.RemoveAtAndSwap(5);
    a1.RemoveAtAndSwap(3);
    a1.RemoveAtAndSwap(1);

    XII_TEST_INT(a1.GetCount(), 5);

    for (xiiInt32 i = 0; i < 5; ++i)
      XII_TEST_BOOL(xiiMath::IsEven(a1[i]));
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "ExpandAndGetRef")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 20; ++i)
    {
      xiiInt32& intRef = a1.ExpandAndGetRef();
      intRef           = i * 5;
    }


    XII_TEST_BOOL(a1.GetCount() == 20);

    for (xiiInt32 i = 0; i < 20; ++i)
    {
      XII_TEST_INT(a1[i], i * 5);
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "PushBack / PopBack / PeekBack")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 10; ++i)
    {
      a1.PushBack(i);
      XII_TEST_INT(a1.PeekBack(), i);
    }

    for (xiiInt32 i = 9; i >= 0; --i)
    {
      XII_TEST_INT(a1.PeekBack(), i);
      a1.PopBack();
    }

    a1.PushBack(23);
    a1.PushBack(2);
    a1.PushBack(3);

    a1.PopBack(2);
    XII_TEST_INT(a1.PeekBack(), 23);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "PushFront / PopFront / PeekFront")
  {
    xiiDeque<xiiInt32> a1;

    for (xiiInt32 i = 0; i < 10; ++i)
    {
      a1.PushFront(i);
      XII_TEST_INT(a1.PeekFront(), i);
    }

    for (xiiInt32 i = 9; i >= 0; --i)
    {
      XII_TEST_INT(a1.PeekFront(), i);
      a1.PopFront();
    }

    a1.PushFront(23);
    a1.PushFront(2);
    a1.PushFront(3);

    a1.PopFront(2);
    XII_TEST_INT(a1.PeekFront(), 23);
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Construction / Destruction")
  {
    {
      XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

      xiiDeque<DequeTestDetail::st> a1;
      xiiDeque<DequeTestDetail::st> a2;

      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
      XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

      a1.PushBack(DequeTestDetail::st(1));
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a1.InsertAt(0, DequeTestDetail::st(2));
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a2 = a1;
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(2, 0)); // two copies

      a1.Clear();
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 2));

      a1.PushBack(DequeTestDetail::st(3));
      a1.PushBack(DequeTestDetail::st(4));
      a1.PushBack(DequeTestDetail::st(5));
      a1.PushBack(DequeTestDetail::st(6));

      XII_TEST_BOOL(DequeTestDetail::st::HasDone(8, 4)); // four temporaries

      a1.RemoveAndCopy(DequeTestDetail::st(3));
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(1, 2)); // one temporary, one destroyed

      a1.RemoveAndCopy(DequeTestDetail::st(3));
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(1, 1)); // one temporary, none destroyed

      a1.RemoveAtAndCopy(0);
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1)); // one destroyed

      a1.RemoveAtAndSwap(0);
      XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1)); // one destroyed
    }

    // tests the destructor of a2 and a1
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Reserve")
  {
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    xiiDeque<DequeTestDetail::st> a;

    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.Reserve(100);

    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.SetCount(10);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(10, 0));

    a.Reserve(100);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

    a.SetCount(100);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(90, 0));

    a.Reserve(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing had to be copied over

    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Compact")
  {
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    xiiDeque<DequeTestDetail::st> a;

    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    XII_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.SetCount(100);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.SetCount(10);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 190));

    // no reallocations and copying, if the memory is already available
    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(190, 0));

    a.SetCount(10);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 190));

    // now we remove the spare memory
    a.Compact();
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

    // this time the array needs to be relocated, and thus the already present elements need to be copied
    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(190, 0));

    // this does not deallocate memory
    a.Clear();
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 200));

    a.SetCount(100);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    // therefore no object relocation
    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.Clear();
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(0, 200));

    // this will deallocate ALL memory
    a.Compact();

    a.SetCount(100);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    // this time objects need to be relocated
    a.SetCount(200);
    XII_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "GetContiguousRange")
  {
    // deques allocate data in 4 KB chunks, so an integer deque will have 1024 ints per chunk

    xiiDeque<xiiInt32> d;

    for (xiiUInt32 i = 0; i < 100 * 1024; ++i)
      d.PushBack(i);

    xiiDynamicArray<xiiInt32> a;
    a.SetCountUninitialized(d.GetCount());

    xiiUInt32 uiArrayPos = 0;

    for (xiiUInt32 i = 0; i < 100; ++i)
    {
      const xiiUInt32 uiOffset = i * 1024 + i;

      const xiiUInt32 uiRange = d.GetContiguousRange(uiOffset);

      XII_TEST_INT(uiRange, 1024 - i);

      xiiMemoryUtils::Copy(&a[uiArrayPos], &d[uiOffset], uiRange);

      uiArrayPos += uiRange;
    }

    a.SetCountUninitialized(uiArrayPos);

    uiArrayPos = 0;

    for (xiiUInt32 i = 0; i < 100; ++i)
    {
      const xiiUInt32 uiOffset = i * 1024 + i;
      const xiiUInt32 uiRange  = 1024 - i;

      for (xiiUInt32 r = 0; r < uiRange; ++r)
      {
        XII_TEST_INT(a[uiArrayPos], uiOffset + r);
        ++uiArrayPos;
      }
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Swap")
  {
    xiiDeque<xiiInt32> a1, a2;

    xiiInt32 content1[] = {1, 2, 3, 4};
    xiiInt32 content2[] = {5, 6, 7, 8, 9};
    for (xiiInt32 i : content1)
    {
      a1.PushBack(i);
    }
    for (xiiInt32 i : content2)
    {
      a2.PushBack(i);
    }

    xiiInt32* a1Ptr = &a1[0];
    xiiInt32* a2Ptr = &a2[0];

    a1.Swap(a2);

    // The pointers should be simply swapped
    XII_TEST_BOOL(a2Ptr == &a1[0]);
    XII_TEST_BOOL(a1Ptr == &a2[0]);

    XII_TEST_INT(XII_ARRAY_SIZE(content1), a2.GetCount());
    XII_TEST_INT(XII_ARRAY_SIZE(content2), a1.GetCount());

    // The data should be swapped
    for (int i = 0; i < XII_ARRAY_SIZE(content1); ++i)
    {
      XII_TEST_INT(content1[i], a2[i]);
    }
    for (int i = 0; i < XII_ARRAY_SIZE(content2); ++i)
    {
      XII_TEST_INT(content2[i], a1[i]);
    }
  }

  XII_TEST_BLOCK(xiiTestBlock::Enabled, "Move PushBack / PushFront")
  {
    xiiDeque<xiiUniquePtr<xiiUInt32>> a1, a2;
    a1.PushBack(xiiUniquePtr<xiiUInt32>(XII_DEFAULT_NEW(xiiUInt32, 1)));
    a1.PushBack(xiiUniquePtr<xiiUInt32>(XII_DEFAULT_NEW(xiiUInt32, 2)));

    a2.PushFront(xiiUniquePtr<xiiUInt32>(XII_DEFAULT_NEW(xiiUInt32, 3)));
    a2.PushFront(xiiUniquePtr<xiiUInt32>(XII_DEFAULT_NEW(xiiUInt32, 4)));

    a1.Swap(a2);

    XII_TEST_INT(*a1[0].Borrow(), 4);
    XII_TEST_INT(*a1[1].Borrow(), 3);

    XII_TEST_INT(*a2[0].Borrow(), 1);
    XII_TEST_INT(*a2[1].Borrow(), 2);
  }
}
