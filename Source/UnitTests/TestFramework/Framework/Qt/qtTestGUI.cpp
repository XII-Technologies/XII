  ...
  QMainWindow(), m_pTestFramework(&testFramework)
{
  ...
  // Delegate
  m_pDelegate = new xiiQtTestDelegate(this);
  ...
  // Message Log Dock
  m_pMessageLogDock = new xiiQtLogMessageDock(this, &m_pTestFramework->GetTestResult());
...
void xiiQtTestGUI::closeEvent(QCloseEvent* e)
{
...
////////////////////////////////////////////////////////////////////////
// xiiQtTestGUI public slots
////////////////////////////////////////////////////////////////////////

void xiiQtTestGUI::on_actionAssertOnTestFail_triggered(bool bChecked)
{
  TestSettings settings       = m_pTestFramework->GetSettings();
...
void xiiQtTestGUI::on_actionOpenHTMLOutput_triggered(bool bChecked)
{
  TestSettings settings             = m_pTestFramework->GetSettings();
...
void xiiQtTestGUI::on_actionKeepConsoleOpen_triggered(bool bChecked)
{
  TestSettings settings       = m_pTestFramework->GetSettings();
...
void xiiQtTestGUI::on_actionShowMessageBox_triggered(bool bChecked)
{
  TestSettings settings      = m_pTestFramework->GetSettings();
...
void xiiQtTestGUI::on_actionDisableSuccessfulTests_triggered(bool bChecked)
{
  TestSettings settings                  = m_pTestFramework->GetSettings();
...
void xiiQtTestGUI::on_actionSaveTestSettingsAs_triggered()
{
  xiiStringBuilder tmp;
  xiiStringView    defaultDir = xiiPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
...
void xiiQtTestGUI::on_actionSaveTestOrderAs_triggered()
{
  xiiStringBuilder tmp;
  xiiStringView    defaultDir = xiiPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
  QString          dir        = defaultDir.IsValid() ? defaultDir.GetData(tmp) : QString();
...
void xiiQtTestGUI::on_actionRunTests_triggered()
{
  ...
  while (m_pTestFramework->RunTestExecutionLoop() == xiiTestAppRun::Continue)
  {
    UpdateButtonStates();
...
void xiiQtTestGUI::on_actionAbort_triggered()
{
  m_bAbort = true;
}

void xiiQtTestGUI::on_actionQuit_triggered()
{
  close();
}

void xiiQtTestGUI::on_actionEnableOnlyThis_triggered()
{
...
void xiiQtTestGUI::on_actionEnableOnlyFailed_triggered()
{
  m_pTestFramework->SetAllFailedTestsEnabledStatus();
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void xiiQtTestGUI::on_actionEnableAllChildren_triggered()
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
  if (!CurrentIndex.isValid())
    return;
...
void xiiQtTestGUI::on_actionEnableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(true);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void xiiQtTestGUI::on_actionDisableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(false);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void xiiQtTestGUI::on_actionExpandAll_triggered()
{
  testTreeView->expandAll();
}

void xiiQtTestGUI::on_actionCollapseAll_triggered()
{
  testTreeView->collapseAll();
}

void xiiQtTestGUI::onTestFrameworkTestResultReceived(qint32 iTestIndex, qint32 iSubTestIndex)
{
  ...
  // Update status bar
  const xiiUInt32 uiTestCount            = m_uiTestsEnabledCount;
  const xiiUInt32 uiFailed               = m_pTestFramework->GetTestsFailedCount();
  const xiiUInt32 uiPassed               = m_pTestFramework->GetTestsPassedCount();
  const xiiUInt32 uiErrors               = m_pTestFramework->GetTotalErrorCount();
  ...
  {
    fSubTestPercentage =
      (float)m_pTestFramework->GetTestResult().GetSubTestCount(m_pTestFramework->GetCurrentTestIndex(), xiiTestResultQuery::Executed) /
...
void xiiQtTestGUI::onTestTreeViewCustomContextMenuRequested(const QPoint& pnt)
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
...
void xiiQtTestGUI::onSelectionModelCurrentRowChanged(const QModelIndex& index)
{
  if (!index.isValid())
  {
  ...
  const xiiTestResultData*   pTestResult = pEntry->GetTestResult();
...
void xiiQtTestGUI::on_actionOpenTestDataFolder_triggered()
{
...
void xiiQtTestGUI::on_actionOpenOutputFolder_triggered()
{
...
void xiiQtTestGUI::on_actionOpenHTMLFile_triggered()
{
...
void xiiQtTestGUI::on_actionUpdateReferenceImages_triggered()
{
  m_pTestFramework->UpdateReferenceImages();
}

////////////////////////////////////////////////////////////////////////
// xiiQtTestGUI private functions
////////////////////////////////////////////////////////////////////////

void xiiQtTestGUI::UpdateButtonStates()
{
...
void xiiQtTestGUI::SaveGUILayout()
{
  QSettings Settings;
...
void xiiQtTestGUI::LoadGUILayout()
{
  QSettings Settings;
  Settings.beginGroup("MainWindow");
...
void xiiQtTestGUI::SetCheckStateRecursive(const QModelIndex& index, bool bChecked)
{
  m_pModel->setData(index, bChecked ? Qt::Checked : Qt::Unchecked, Qt::CheckStateRole);

  xiiInt32 iChildren = m_pModel->rowCount(index);
...
void xiiQtTestGUI::EnableAllParents(const QModelIndex& index)
{
...
////////////////////////////////////////////////////////////////////////
// xiiQtTestGUI public static functions
////////////////////////////////////////////////////////////////////////
...
void xiiQtTestGUI::SetDarkTheme()
{
  ...
  palette.setColor(QPalette::Window, QColor(50, 50, 50, 255));
  palette.setColor(QPalette::Shadow, QColor(0, 0, 0, 255));
  palette.setColor(QPalette::Highlight, QColor(103, 141, 178, 255));
  palette.setColor(QPalette::HighlightedText, QColor(255, 255, 255, 255));
  palette.setColor(QPalette::Link, QColor(0, 0, 238, 255));
  palette.setColor(QPalette::LinkVisited, QColor(82, 24, 139, 255));
  palette.setColor(QPalette::AlternateBase, QColor(25, 25, 25, 255));
  QBrush NoRoleBrush(QColor(0, 0, 0, 255), Qt::NoBrush);
  palette.setBrush(QPalette::NoRole, NoRoleBrush);
  palette.setColor(QPalette::ToolTipBase, QColor(255, 255, 220, 255));
  palette.setColor(QPalette::ToolTipText, QColor(0, 0, 0, 255));
...
XII_STATICLINK_FILE(TestFramework, TestFramework_Framework_Qt_qtTestGUI);
