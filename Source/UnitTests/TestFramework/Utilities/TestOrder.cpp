...
{
  return xiiStringUtils::Compare_NoCase(lhs.m_szTestName, rhs.m_szTestName) < 0;
}
...
{
  return xiiStringUtils::Compare_NoCase(lhs.m_szSubTestName, rhs.m_szSubTestName) < 0;
}
...
void LoadTestOrder(const char* szFile, std::deque<xiiTestEntry>& ref_allTests)
{
  ...
  for (xiiTestEntry& test : ref_allTests)
  {
  ...
  while (!feof(pFile))
  {
    ...
    if (szTestName[0] != '\0')
    {
      if (!bIndented)
      {
        ...
        if (xiiStringUtils::IsEqual_NoCase("Settings", szTestName))
        {
          iLastMainTest = -1;
        }
        // Are we in a test block?
        for (xiiUInt32 t = 0; t < ref_allTests.size(); ++t)
        {
          strcpy(szOtherName, ref_allTests[t].m_szTestName);
          StripWhitespaces(szOtherName);

          if (xiiStringUtils::IsEqual_NoCase(szOtherName, szTestName))
          {
            iLastMainTest                 = t;
      ...
      else
      {
        ...
        if (iLastMainTest >= 0)
        {
          for (xiiUInt32 t = 0; t < ref_allTests[iLastMainTest].m_SubTests.size(); ++t)
          {
...
void LoadTestSettings(const char* szFile, TestSettings& ref_testSettings)
{
  ...
  while (!feof(pFile))
  {
    ...
    if (szTestName[0] != '\0')
    {
      ...
      {
        // Are we in the settings block?
        bInSettings = xiiStringUtils::IsEqual_NoCase("Settings", szTestName);
      }
      else
      {
        ...
        if (bInSettings)
        {
          if (xiiStringUtils::IsEqual_NoCase("AssertOnTestFail", szTestName))
          {
            ref_testSettings.m_AssertOnTestFail = bIsOff ? AssertOnTestFail::DoNotAssert : AssertOnTestFail::AssertIfDebuggerAttached;
          }
          else if (xiiStringUtils::IsEqual_NoCase("OpenHtmlOutputOnError", szTestName))
          {
            ref_testSettings.m_bOpenHtmlOutputOnError = !bIsOff;
          }
          else if (xiiStringUtils::IsEqual_NoCase("KeepConsoleOpen", szTestName))
          {
            ref_testSettings.m_bKeepConsoleOpen = !bIsOff;
          }
          else if (xiiStringUtils::IsEqual_NoCase("ShowMessageBox", szTestName))
          {
            ref_testSettings.m_bShowMessageBox = !bIsOff;
          }
          else if (xiiStringUtils::IsEqual_NoCase("DisableSuccessfulTests", szTestName))
          {
