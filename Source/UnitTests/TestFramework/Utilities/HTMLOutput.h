  ...
  static void OutputToHTML(xiiTestOutput::Enum Type, const char* szMsg)
  {
    ...
    switch (Type)
    {
      case xiiTestOutput::StartOutput:
      {
        ...
        xiiTestFramework::GetInstance()->CreateOutputFolder();

        std::string sOutputFile = std::string(xiiTestFramework::GetInstance()->GetAbsOutputPath()) + "/UnitTestsLog.htm";
        const char* szTestName  = xiiTestFramework::GetInstance()->GetTestName();
...
std::ofstream xiiOutputToHTML::htmlFile;
