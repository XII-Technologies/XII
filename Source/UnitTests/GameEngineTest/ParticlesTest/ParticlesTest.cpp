...
static xiiGameEngineTestParticles s_GameEngineTestParticles;

const char* xiiGameEngineTestParticles::GetTestName() const
{
  return "Particle Tests";
}

xiiGameEngineTestApplication* xiiGameEngineTestParticles::CreateApplication()
{
  m_pOwnApplication = XII_DEFAULT_NEW(xiiGameEngineTestApplication_Particles);
  return m_pOwnApplication;
}

void xiiGameEngineTestParticles::SetupSubTests()
{
...
xiiResult xiiGameEngineTestParticles::InitializeSubTest(xiiInt32 iIdentifier)
{
  ...
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/Billboards.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else if (iIdentifier == SubTests::PullAlongBehavior)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/PullAlong.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else if (iIdentifier == SubTests::DistanceEmitter)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/DistanceEmitter.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else if (iIdentifier == SubTests::SharedInstances)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/SharedInstances.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else if (iIdentifier == SubTests::EventReactionEffect)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/EventReactionEffect.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else if (iIdentifier == SubTests::LocalSpaceSim)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/LocalSpaceSim.xiiObjectGraph");
    return XII_SUCCESS;
  }
  else
  {
...
xiiTestAppRun xiiGameEngineTestParticles::RunSubTest(xiiInt32 iIdentifier, xiiUInt32 uiInvocationCount)
{
  ++m_iFrame;
...
xiiUInt32 xiiGameEngineTestParticles::GetImageCompareThreshold(xiiInt32 iIdentifier)
{
...
xiiGameEngineTestApplication_Particles::xiiGameEngineTestApplication_Particles() :
  xiiGameEngineTestApplication("Particles")
{
}

//////////////////////////////////////////////////////////////////////////

void xiiGameEngineTestApplication_Particles::SetupSceneSubTest(const char* szFile)
{
  LoadScene(szFile).IgnoreResult();
}

void xiiGameEngineTestApplication_Particles::SetupParticleSubTest(const char* szFile)
{
  LoadScene("Particles/AssetCache/Common/Particles1.xiiObjectGraph").IgnoreResult();
  ...
  {
    xiiParticleComponent* pEffect;
    m_pWorld->GetOrCreateComponentManager<xiiParticleComponentManager>()->CreateComponent(pObject, pEffect);
    pEffect->SetParticleEffectFile(szFile);
    pEffect->m_uiRandomSeed = 42;
  }
}

xiiTestAppRun xiiGameEngineTestApplication_Particles::ExecParticleSubTest(xiiInt32 iCurFrame)
{
  ...
  {
    case 15:
      XII_TEST_IMAGE(0, m_uiImageCompareThreshold);
      break;

    case 30:
      XII_TEST_IMAGE(1, m_uiImageCompareThreshold);
      break;

    case 60:
      XII_TEST_IMAGE(2, m_uiImageCompareThreshold);
      return xiiTestAppRun::Quit;
  }
