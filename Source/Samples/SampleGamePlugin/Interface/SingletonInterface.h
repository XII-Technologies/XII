...
public:
  ...
  virtual void Print(const xiiFormatString& text) = 0;
};
// END-DOCS-CODE-SNIPPET

// BEGIN-DOCS-CODE-SNIPPET: singleton-impl-declaration
/// \brief Implementation of the PrintInterface, just forwards the text to xiiLog::Info()
///
...
{
  XII_DECLARE_SINGLETON_OF_INTERFACE(PrintImplementation, PrintInterface);

public:
  PrintImplementation();

  virtual void Print(const xiiFormatString& text) override;

private:
  // needed for the startup system to be able to call the private function below
  XII_MAKE_SUBSYSTEM_STARTUP_FRIEND(SampleGamePluginStartupGroup, SampleGamePluginMainStartup);
