...
///       xiiJniObject activity = attachment.GetActivity();
///
///       // Perform Java calls
///       xiiJniClass myClassType = activity.Call<xiiJniObject>("getClassLoader").Call<xiiJniClass>("loadClass", xiiJniString("com.myproject.MyClass"));
///     }
///   \endcode
class XII_FOUNDATION_DLL xiiJniAttachment
{
public:
  /// \brief Constructor.
  xiiJniAttachment();
  ...
  static xiiJniObject GetActivity();
  ...
  static xiiJniErrorState GetLastError();
  ...
  ///
  /// If an exception occurred, no other Java method may be called until ClearPendingException has been called.
  static xiiJniObject GetPendingException();
  ...
  static void SetLastError(xiiJniErrorState state);
...
private:
  ...
  xiiJniAttachment& operator=(const xiiJniAttachment&);
};

/// \brief Describes the ownership handling of the local JNI reference.
enum class xiiJniOwnerShip
{
...
class XII_FOUNDATION_DLL xiiJniObject
{
public:
  ...
  /// \param ownerShip How the object handle should be managed. See xiiJniOwnerShip.
  inline xiiJniObject(jobject object, xiiJniOwnerShip ownerShip);

  /// \brief Copy constructor. Both instances will reference the same Java object.
  inline xiiJniObject(const xiiJniObject& other);

  /// \brief Move constructor.
  inline xiiJniObject(xiiJniObject&& other);

  /// \brief Assignment operator.
  inline xiiJniObject& operator=(const xiiJniObject& other);

  /// \brief Move assignment operator.
  inline xiiJniObject& operator=(xiiJniObject&& other);

  /// \brief Destructor.
  inline virtual ~xiiJniObject();

  /// \brief Compares if the two objects reference the same Java object.
  /// \param other The object to compare to.
  ///
  /// This method returns true if two xiiJniObjects reference the same Java object.
  ///
  ...
  ///     xiiJniObject player = myClassInstance.Call<xiiJniObject>("getPlayer", 0);
  ///
  ///     // Call the second overload
  ///     xiiJniObject player = myClassInstance.Call<xiiJniObject>("getPlayer", xiiJniString("player1"));
  ///
  ...
  ///     xiiJniObject player = myClassInstance.Call<xiiJniObject>("getPlayerById", 0);
  ///
  ...
  ///     //
  ///     xiiJniObject player = myClassInstance.Call<xiiJniObject>("getPlayerById", jlong(0));
  ///
  ...
  ///     xiiJniObject boxedParam = xiiJniClass("java/lang/Integer").CreateInstance(param);
  ///
  ///     // .. Call the method...
  ///     xiiJniObject boxedResult = myClassInstance.Call<xiiJniObject>("SomeMethodWithBoxedTypes", boxedParam);
  ///
  ...
  ///     xiiJniObject objectField = myClassInstance.GetField<xiiJniObject>("ObjectField");
  ///   \endcode
  ...
  ///     myClassInstance.SetField("ObjectField", xiiJniString("SomeString");
  ///   \endcode
  template <typename T>
...
/// as 3 bytes each.
class XII_FOUNDATION_DLL xiiJniString : public xiiJniObject
{
public:
  ...
  xiiJniString(const char* str);

  /// \brief Constructs a String from a JNI string  handle.
  ///
  /// \param string The JNI string handle.
  /// \param ownerShip How the object handle should be managed. See xiiJniObject::OwnerShip.
  xiiJniString(jstring string, xiiJniOwnerShip ownerShip);

  /// \brief Copy constructor. Both instances will reference the same Java String.
  xiiJniString(const xiiJniString& other);

  /// \brief Move constructor.
  xiiJniString(xiiJniString&& other);

  /// \brief Assignment operator. Both instances will reference the same Java String.
  xiiJniString& operator=(const xiiJniString& other);

  /// \brief Move assignment operator.
  xiiJniString& operator=(xiiJniString&& other);

  /// \brief Destructor.
  virtual ~xiiJniString();
...
class XII_FOUNDATION_DLL xiiJniClass : public xiiJniObject
{
public:
  ...
  ///
  /// If the class could not be found, isNull() will return true and xiiJniAttachment::getLastError will return xiiJniAttachment::CLASS_NOT_FOUND.
  ///
  /// In order to load classes from the application package, you will have to use the activity's class loader instead.
  /// For example:
  /// \code
  ///   xiiJniObject classLoader = attachment.GetActivity().Call<xiiJniObject>("getClassLoader");
  ///   xiiJniClass myClass = classLoader.Call<xiiJniClass>("loadClass", xiiJniString("com.myproject.MyClass"));
  /// \endcode
  xiiJniClass(const char* className);

  /// \brief Constructs a Class from a JNI class handle.
  ///
  /// \param clazz The JNI class handle.
  /// \param ownerShip How the object handle should be managed. See xiiJniObject::OwnerShip.
  xiiJniClass(jclass clazz, xiiJniOwnerShip ownerShip);

  /// \brief Copy constructor. Both instances will reference the same Java class.
  xiiJniClass(const xiiJniClass& other);

  /// \brief Move constructor.
  xiiJniClass(xiiJniClass&& other);

  /// \brief Assignment operator. Both instances will reference the same Java class.
  xiiJniClass& operator=(const xiiJniClass& other);

  /// \brief Move assignment operator.
  xiiJniClass& operator=(xiiJniClass&& other);
  ...
  ///     xiiJniObject myClassInstance = myClassType.CreateInstance(true, xiiJniString("some string"), 12345);
  ///   \endcode
  ///
  /// See xiiJniObject::Call for more information on overload resolution and argument conversion.
  template <typename... Args>
  xiiJniObject CreateInstance(const Args&... args) const;
  ...
  bool IsAssignableFrom(const xiiJniClass& other) const;
  ...
  /// \param args The function arguments to pass.
  ///
  /// See xiiJniObject::Call() for details on argument handling.
  ///
  /// \example
  ///   \code
  ///     // To call a static method of a type directly:
  ///     xiiJniClass myClassType;
  ///     myClassType.CallStatic("SomeStaticMethod");
  ///     xiiJniObject result = myClassType.CallStatic<xiiJniObject>("SomeStaticMethodReturningObject");
  ///
  ///     // To call a static method of the type of a class instance:
  ///     xiiJniObject myClassInstance;
  ///     myClassInstance.getClass().CallStatic("SomeStaticMethod");
  ///     xiiJniObject result = myClassInstance.GetClass().CallStatic<xiiJniObject>("SomeStaticMethodReturningObject");
  ///   \endcode
  template <typename Ret = void, typename... Args>
