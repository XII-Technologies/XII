...
namespace xiiUwpUtils
{
  ...
  void xiiWinRtIterateIMapView(const ComPtr<ABI::Windows::Foundation::Collections::IMapView<KeyType, ValueType>>& pMapView, const Callback& callback)
  {
    ...
    XII_ASSERT_DEBUG(pIterable != nullptr, "Invalid map iteration interface");
  ...
  HRESULT xiiWinRtPutCompleted(ObjectType& object, Callback callback)
  {
  ...
  void RetrieveStatics(const WCHAR* szRuntimeClassName, ComPtr<T>& out_Ptr)
  {
    ...
    {
      XII_REPORT_FAILURE("Failed to retrieve activation factory (statics) for '{0}'", xiiStringUtf8(szRuntimeClassName).GetData());
      out_Ptr = nullptr;
    }
  }
  ...
  xiiMat4 XII_FOUNDATION_DLL ConvertMat4(const ABI::Windows::Foundation::Numerics::Matrix4x4& in);

  xiiVec3 XII_FOUNDATION_DLL ConvertVec3(const ABI::Windows::Foundation::Numerics::Vector3& in);
  void XII_FOUNDATION_DLL    ConvertVec3(const xiiVec3& in, ABI::Windows::Foundation::Numerics::Vector3& out);

  xiiQuat XII_FOUNDATION_DLL ConvertQuat(const ABI::Windows::Foundation::Numerics::Quaternion& in);
  void XII_FOUNDATION_DLL    ConvertQuat(const xiiQuat& in, ABI::Windows::Foundation::Numerics::Quaternion& out);

  xiiUuid XII_FOUNDATION_DLL ConvertGuid(const GUID& in);
  void XII_FOUNDATION_DLL    ConvertGuid(const xiiUuid& in, GUID& out);
} // namespace xiiUwpUtils
