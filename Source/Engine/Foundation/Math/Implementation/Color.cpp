...
void xiiColor::GetHSV(float& out_hue, float& out_sat, float& out_value) const
{
  ...
  out_value = xiiMath::Max(r2, g2, b2); // Value

  if (out_value < xiiMath::SmallEpsilon<float>())
  {
    out_hue   = 0.0f;
    out_sat   = 0.0f;
    out_value = 0.0f;
...
void xiiColor::SetHSV(float hue, float sat, float val)
{
  XII_ASSERT_DEBUG(hue <= 360 && hue >= 0, "HSV 'hue' is in invalid range.");
  XII_ASSERT_DEBUG(sat <= 1 && val >= 0, "HSV 'saturation' is in invalid range.");
  XII_ASSERT_DEBUG(val >= 0, "HSV 'value' is in invalid range.");

  float c = sat * val;
  float x = c * (1.0f - xiiMath::Abs(xiiMath::Mod(hue / 60.0f, 2) - 1.0f));
  float m = val - c;
...
void xiiColor::ScaleRGB(float factor)
{
  r *= factor;
  g *= factor;
  b *= factor;
...
void xiiColor::ApplyHdrExposureValue(float ev)
{
  const float factor = xiiMath::Pow2(ev);
  r *= factor;
  g *= factor;
  b *= factor;
}
...
float xiiColor::GammaToLinear(float gamma)
{
  return gamma <= 0.04045f ? (gamma / 12.92f) : (xiiMath::Pow((gamma + 0.055f) / 1.055f, 2.4f));
}

float xiiColor::LinearToGamma(float linear)
{
  // assuming we have linear color (not CIE xyY or CIE XYZ)
  return linear <= 0.0031308f ? (12.92f * linear) : (1.055f * xiiMath::Pow(linear, 1.0f / 2.4f) - 0.055f);
}

xiiVec3 xiiColor::GammaToLinear(const xiiVec3& gamma)
{
  return xiiVec3(GammaToLinear(gamma.x), GammaToLinear(gamma.y), GammaToLinear(gamma.z));
}

xiiVec3 xiiColor::LinearToGamma(const xiiVec3& linear)
{
  // assuming we have linear color (not CIE xyY or CIE XYZ)
  return xiiVec3(LinearToGamma(linear.x), LinearToGamma(linear.y), LinearToGamma(linear.z));
}
