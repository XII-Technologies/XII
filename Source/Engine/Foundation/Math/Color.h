...
/// \brief xiiColor represents and RGBA color in linear color space. Values are stored as float, allowing HDR values and full precision color
/// modifications.
///
...
///   * If you need to use preserve memory and LDR values are sufficient, you should use xiiColorGammaUB. This format uses 8 Bit per pixel
...
public:
  ...
  static xiiVec3 GammaToLinear(const xiiVec3& gamma); // [tested]
  /// \brief Helper function to convert a float RGB color value from linear space to gamma color space.
  static xiiVec3 LinearToGamma(const xiiVec3& gamma); // [tested]
...
public:
  ...
  xiiColor WithAlpha(float alpha) const;
};
