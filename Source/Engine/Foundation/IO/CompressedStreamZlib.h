...
public:
  ...
  /// one.
  void SetInputStream(xiiStreamReader* pInputStream, xiiUInt64 uiInputSize);
...
private:
  xiiUInt64                 m_uiRemainingInputSize = 0;
...
/// \brief A stream reader that will decompress data that was stored using the xiiCompressedStreamWriterZlib.
///
...
class XII_FOUNDATION_DLL xiiCompressedStreamReaderZlib : public xiiStreamReader
{
public:
  /// \brief Takes an input stream as the source from which to read the compressed data.
  xiiCompressedStreamReaderZlib(xiiStreamReader* pInputStream); // [tested]

  ~xiiCompressedStreamReaderZlib(); // [tested]
...
class XII_FOUNDATION_DLL xiiCompressedStreamWriterZlib : public xiiStreamWriter
{
public:
  ...
  xiiCompressedStreamWriterZlib(xiiStreamWriter* pOutputStream, Compression ratio = Compression::Default); // [tested]

  /// \brief Calls CloseStream() internally.
  ~xiiCompressedStreamWriterZlib(); // [tested]
