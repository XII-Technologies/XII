...
/// The xiiFileSystem provides high-level functionality to manage files in a virtual file system.
///
...
/// Different xiiDataDirectoryType's can implement these different 'decoding methods', i.e. they handle how to actually
...
/// The default xiiFileReader and xiiFileWriter implement a buffering policy, i.e. they use an internal cache to only
...
/// reader/writer class.
/// How to mount data directories (i.e. with which xiiDataDirectoryType) is defined by the 'DataDirFactories', which
/// are functions that create xiiDataDirectoryType's. This way one can mount the same data directory (e.g. "MyTestDir")
...
///
/// Additionally xiiFileSystem also broadcasts events about which files are (about to be) accessed.
...
///
/// All operations that go through the xiiFileSystem are protected by a mutex, which means that opening, closing, deleting
...
class XII_FOUNDATION_DLL xiiFileSystem
{
public:
  ...
  static xiiEventSubscriptionID RegisterEventHandler(xiiEvent<const FileEvent&>::Handler handler);

  /// Unregisters a previously registered Event Handler.
  static void UnregisterEventHandler(xiiEvent<const FileEvent&>::Handler handler);
  static void UnregisterEventHandler(xiiEventSubscriptionID subscriptionId);
...
public:
  ...
  /// on a single thread with no other thread accessing anything in xiiFileSystem simultaneously.
  ///@{
  ...
  using xiiDataDirFactory = xiiDataDirectoryType* (*)(xiiStringView, xiiStringView, xiiStringView, xiiDataDirUsage);
  ...
  static xiiResult AddDataDirectory(xiiStringView sDataDirectory, xiiStringView sGroup = {}, xiiStringView sRootName = {}, xiiDataDirUsage usage = xiiDataDirUsage::ReadOnly); // [tested]
  ...
  static const xiiDataDirectoryInfo* FindDataDirectoryWithRoot(xiiStringView sRootName);
  ...
  static const xiiDataDirectoryInfo& GetDataDirectoryInfo(xiiUInt32 uiDataDirIndex);

  /// Calls xiiDataDirectoryType::ReloadExternalConfigs() on all active data directories.
  ...
  ///
  /// Additionally the 'redirection file' feature of xiiFileSystem::FindFolderWithSubPath() is used
  /// to allow finding a relocated SDK folder. To do that, place a file called 'xiiSdkRoot.txt'
  ...
  /// ">user/" - Resolves to what xiiOSFile::GetUserDataFolder() returns.
  /// ">temp/" - Resolves to what xiiOSFile::GetTempDataFolder() returns.
...
public:
  ...
  /// Tries to get the xiiFileStats for the given file.
  ...
  static xiiResult ResolvePath(xiiStringView sPath, xiiStringBuilder* out_pAbsolutePath, xiiStringBuilder* out_pDataDirRelativePath, const xiiDataDirectoryInfo** out_pDataDir = nullptr); // [tested]

  /// Starts at szStartDirectory and goes up until it finds a folder that contains the given sub folder structure.
  ///
  /// Returns XII_FAILURE if nothing is found. Otherwise \a result is the absolute path to the existing folder that has a given sub-folder.
  ///
  ...
  static xiiStringView MigrateFileLocation(xiiStringView sOldLocation, xiiStringView sNewLocation);

private:
  friend class xiiDataDirectoryReaderWriterBase;
  friend class xiiFileReaderBase;
  friend class xiiFileWriterBase;

  /// This is used by the actual file readers (like xiiFileReader) to get an abstract file reader.
  ///
  ...
  /// This is used by the actual file writers (like xiiFileWriter) to get an abstract file writer.
  ///
...
private:
  XII_MAKE_SUBSYSTEM_STARTUP_FRIEND(Foundation, FileSystem);
...
private:
  struct Factory
  {
    ...
    float             m_fPriority;
    xiiDataDirFactory m_Factory;
  };

  struct FileSystemData
  {
    xiiHybridArray<Factory, 4>               m_DataDirFactories;
    xiiHybridArray<xiiDataDirectoryInfo, 16> m_DataDirectories;
  ...
  static xiiDataDirectoryInfo* GetDataDirForRoot(const xiiString& sRoot);

  static void CleanUpRootName(xiiStringBuilder& sRoot);

  static xiiString                    s_sSdkRootDir;
  static xiiMap<xiiString, xiiString> s_SpecialDirectories;
  static FileSystemData*              s_pData;
};

/// Describes the type of events that are broadcast by the xiiFileSystem.
struct xiiFileSystem::FileEventType
{
  enum Enum
  {
...
struct xiiFileSystem::FileEvent
{
