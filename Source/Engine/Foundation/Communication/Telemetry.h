...
{
public:
  /// \brief The port over which xiiTelemetry will connect.
  static xiiUInt16 s_uiPort /* = 1040*/;

  /// \brief Defines how the xiiTelemetry system was configured.
  enum ConnectionMode
  {
  ...
  static xiiResult ConnectToServer(const char* szConnectTo = nullptr);
  ...
  static xiiTime GetPingToServer() { return s_PingToServer; }
  ...
  static xiiUInt32 GetServerID() { return s_uiServerID; }
  ...
  ///
  /// If no message for the given system is available, XII_FAILURE is returned.
  ...
  static void RemoveEventHandler(xiiEventTelemetry::Handler handler) { s_TelemetryEvents.RemoveEventHandler(handler); }
...
private:
  ...
  static xiiResult OpenConnection(ConnectionMode Mode, const char* szConnectTo = nullptr);

  static void Transmit(TransmitMode tm, const void* pData, xiiUInt32 uiDataBytes);

  static void Send(TransmitMode tm, xiiUInt32 uiSystemID, xiiUInt32 uiMsgID, const void* pData, xiiUInt32 uiDataBytes);
  static void Send(TransmitMode tm, xiiUInt32 uiSystemID, xiiUInt32 uiMsgID, xiiStreamReader& Stream, xiiInt32 iDataBytes = -1);
  static void Send(TransmitMode tm, xiiTelemetryMessage& msg);

  friend class xiiTelemetryThread;

  static void FlushOutgoingQueues();

  static void           InitializeAsServer();
  static xiiResult      InitializeAsClient(const char* szConnectTo);
  static ConnectionMode s_ConnectionMode;

  static xiiUInt32 s_uiApplicationID;
  static xiiUInt32 s_uiServerID;

  static xiiString s_sServerName;
  static xiiString s_sServerIP;

  static bool s_bConnectedToServer;
  static bool s_bConnectedToClient;
  static bool s_bAllowNetworkUpdate;

  static void QueueOutgoingMessage(TransmitMode tm, xiiUInt32 uiSystemID, xiiUInt32 uiMsgID, const void* pData, xiiUInt32 uiDataBytes);
  ...
  struct MessageQueue
  {
    ...
    void*                   m_pPassThrough;
    xiiUInt32               m_uiMaxQueuedOutgoing;

    MessageDeque m_IncomingQueue;
    MessageDeque m_OutgoingQueue;
  };

  static xiiMap<xiiUInt64, MessageQueue> s_SystemMessages;

  static xiiEventTelemetry s_TelemetryEvents;

private:
  static xiiMutex s_TelemetryMutex;
