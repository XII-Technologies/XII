...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand('type synthetic add -x "^xiiDynamicArray<" --python-class xiiEngine.xiiDynamicArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^xiiHybridArray<" --python-class xiiEngine.xiiHybridArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^xiiHybridString<" --python-class xiiEngine.xiiHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add xiiStringBuilder --python-class xiiEngine.xiiHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add xiiStringView --python-class xiiEngine.xiiStringViewSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^xiiEnum<" --python-class xiiEngine.xiiEnumSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^xiiArrayPtr<" --python-class xiiEngine.xiiArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add xiiByteArrayPtr --python-class xiiEngine.xiiArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add xiiConstByteArrayPtr --python-class xiiEngine.xiiArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^xiiMap<" --python-class xiiEngine.xiiMapSynthProvider')

    debugger.HandleCommand('type summary add -x "^xiiHybridString<" --python-function xiiEngine.xiiHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add xiiStringBuilder --python-function xiiEngine.xiiHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add xiiStringView --python-function xiiEngine.xiiStringView_SummaryProvider')
    debugger.HandleCommand('type summary add -x "^xiiEnum<" --python-function xiiEngine.xiiEnum_SummaryProvider')
...
class xiiDynamicArraySynthProvider:
...
class xiiHybridArraySynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
class xiiHybridStringSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
def xiiHybridString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "xiiHybridString_SummaryProvider"
...
class xiiStringViewSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        logger = lldb.formatters.Logger.Logger()
        logger >> "xiiStringViewSynthProvider::update"
        self.valid = False
        try:
...
def xiiStringView_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "xiiStringView_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return '"' + make_string(content) + '"'
    except Exception as inst:
        logger >> str(inst)
...
class xiiEnumSynthProvider:
    ...
    def update(self):
        self.value = "?"
        logger = lldb.formatters.Logger.Logger()
        logger >> "xiiEnumSynthProvider::Update"
        try:
...
def xiiEnum_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "xiiEnum_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return make_string(content)
    except Exception as inst:
        logger >> str(inst)
...
class xiiArrayPtrSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
