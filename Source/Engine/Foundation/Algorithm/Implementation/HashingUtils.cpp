...
// static
xiiUInt32 xiiHashingUtils::MurmurHash32(const void* pKey, size_t uiSizeInByte, xiiUInt32 uiSeed /*= 0*/)
{
  constexpr xiiUInt32 m = xiiInternal::MURMUR_M;
  constexpr xiiUInt32 r = xiiInternal::MURMUR_R;

  // Initialize the hash to a 'random' value
  xiiUInt32 h = uiSeed ^ (xiiUInt32)uiSizeInByte;

#if XII_ENABLED(XII_PLATFORM_ARCH_ARM)
  ...
  if (reinterpret_cast<size_t>(pKey) % 4 != 0)
  {
    ...
    while (uiSizeInByte >= 4)
    {
      ...
      pKey = xiiMemoryUtils::AddByteOffset(pKey, sizeof(k));
...
#endif
  {
    const xiiUInt32* pData = static_cast<const xiiUInt32*>(pKey);
  ...
  const xiiUInt8* pData2 = reinterpret_cast<const xiiUInt8*>(pKey);
...
// static
xiiUInt64 xiiHashingUtils::MurmurHash64(const void* pKey, size_t uiSizeInByte, xiiUInt64 uiSeed /*= 0*/)
{
  constexpr xiiUInt64 m = 0xc6a4a7935bd1e995ULL;
  constexpr xiiUInt64 r = 47;

  xiiUInt64 h = uiSeed ^ (uiSizeInByte * m);

#if XII_ENABLED(XII_PLATFORM_ARCH_ARM)
  if (reinterpret_cast<size_t>(pKey) % 8 != 0)
  {
    const void* pData = static_cast<const xiiUInt8*>(pKey);
    const void* pEnd  = xiiMemoryUtils::AddByteOffset(pKey, (uiSizeInByte / 8) * 8);

    while (pData != pEnd)
    {
      xiiUInt64 k = 0;
      memcpy(&k, pData, sizeof(k));
      pData = xiiMemoryUtils::AddByteOffset(pData, sizeof(k));
...
#endif
  {
    const xiiUInt64* pData = static_cast<const xiiUInt64*>(pKey);
    const xiiUInt64* pEnd  = pData + (uiSizeInByte / 8);
  ...
  {
    case 7:
      h ^= xiiUInt64(pData2[6]) << 48;
    case 6:
      h ^= xiiUInt64(pData2[5]) << 40;
    case 5:
      h ^= xiiUInt64(pData2[4]) << 32;
    case 4:
      h ^= xiiUInt64(pData2[3]) << 24;
    case 3:
      h ^= xiiUInt64(pData2[2]) << 16;
    case 2:
      h ^= xiiUInt64(pData2[1]) << 8;
    case 1:
      h ^= xiiUInt64(pData2[0]);
      h *= m;
  };
...
static constexpr xiiUInt32 uiCRC32Table[256] = {0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3, 0x0EDB8832,
...
// static
xiiUInt32 xiiHashingUtils::CRC32Hash(const void* pKey, size_t uiSizeInBytes)
{
...
// static
xiiUInt64 xiiHashingUtils::xxHash64(const void* pKey, size_t uiSizeInByte, xiiUInt64 uiSeed /*= 0*/)
{
  return XXH64(pKey, uiSizeInByte, uiSeed);
}

XII_STATICLINK_FILE(Foundation, Foundation_Algorithm_Implementation_HashingUtils);
