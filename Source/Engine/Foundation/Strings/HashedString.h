...
public:
  ...
  typedef xiiMap<xiiUInt64, HashedData, xiiCompareHelper<xiiUInt64>, xiiStaticAllocatorWrapper> StringStorage;
  ...
  /// If you need to create an object to compare xiiHashedString objects against, prefer to use xiiTempHashedString. It will only compute
  /// the strings hash value, but does not require any thread synchronization.
  void Assign(xiiStringView szString); // [tested]
...
template <size_t N>
xiiHashedString xiiMakeHashedString(const char (&szString)[N]);
...
public:
  ...
  template <size_t N>
  xiiTempHashedString(const char (&szString)[N]); // [tested]

  template <size_t N>
  xiiTempHashedString(char (&szString)[N]) = delete;
  ...
  /// be slow.
  xiiTempHashedString(xiiStringView szString); // [tested]
  ...
  void operator=(xiiStringView szString); // [tested]
...
// For xiiFormatString
XII_FOUNDATION_DLL xiiStringView BuildString(char* tmp, xiiUInt32 uiLength, const xiiHashedString& arg);
