...
public:
  ...
  static void GetCharacterAndElementCount(const char* szUtf8, xiiUInt32& uiCharacterCount, xiiUInt32& uiElementCount,
                                          const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  ///  I.e. they will be truncated at a character boundary.
  /// Returns the number of bytes that were copied into szDest, excluding the terminating \0
  static xiiUInt32 CopyN(char* szDest, xiiUInt32 uiDstSize, const char* szSource, xiiUInt32 uiCharsToCopy,
                         const char* pSourceEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  static bool IsEqual(const char* pString1, const char* pString2, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                      const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  static bool IsEqualN(const char* pString1, const char* pString2, xiiUInt32 uiCharsToCompare, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                       const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]

  /// \brief Returns true, if the two given strings are identical (case-insensitive).
  static bool IsEqual_NoCase(const char* pString1, const char* pString2, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                             const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  static xiiInt32 Compare(const char* pString1, const char* pString2, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                          const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  /// Returns 0 for equal strings.
  /// Works with UTF-8 strings as well.
  static xiiInt32 CompareN(const char* pString1, const char* pString2, xiiUInt32 uiCharsToCompare, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                           const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  /// Returns 0 for equal strings.
  /// Works with UTF-8 strings as well.
  static xiiInt32 Compare_NoCase(const char* pString1, const char* pString2, const char* pString1End = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                                 const char* pString2End = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  /// Formatting works exactly like printf, except that it additionally supports outputting binary with the 'b' modifier and it will
  /// output float NaN and Infinity as proper text.
  static xiiInt32 vsnprintf(char* szDst, xiiUInt32 uiDstSize, const char* szFormat, va_list ap); // [tested]

  /// \brief Returns true if szString starts with the string given in szStartsWith.
  static bool StartsWith(const char* szString, const char* szStartsWith, const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                         const char* szStartsWithEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]

  /// \brief Returns true if szString starts with the string given in szStartsWith. Ignores case.
  static bool StartsWith_NoCase(const char* szString, const char* szStartsWith, const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                                const char* szStartsWithEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]

  /// \brief Returns true if szString ends with the string given in szEndsWith.
  static bool EndsWith(const char* szString, const char* szEndsWith, const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                       const char* szEndsWithEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]

  /// \brief Returns true if szString ends with the string given in szEndsWith. Ignores case.
  static bool EndsWith_NoCase(const char* szString, const char* szEndsWith, const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>(),
                              const char* szEndsWithEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  static void Trim(const char*& pString, const char*& pStringEnd, const char* szTrimCharsStart,
                   const char* szTrimCharsEnd); // [tested] via xiiStringView and xiiStringBuilder
  ...
  static const char* FindWholeWord(const char* szString, const char* szSearchFor, XII_CHARACTER_FILTER IsDelimiterCB,
                                   const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]

  /// \brief Searches szString for the word szSearchFor. If IsDelimiterCB returns true for both characters in front and back of the word,
  /// the position is returned. Otherwise nullptr. Ignores case.
  static const char* FindWholeWord_NoCase(const char* szString, const char* szSearchFor, XII_CHARACTER_FILTER IsDelimiterCB,
                                          const char* pStringEnd = xiiUnicodeUtils::GetMaxStringEnd<char>()); // [tested]
  ...
  static void OutputFormattedInt(char* szOutputBuffer, xiiUInt32 uiBufferSize, xiiUInt32& uiWritePos, xiiInt64 value, xiiUInt8 uiWidth, bool bPadZeros, xiiUInt8 uiBase);
  /// \brief [internal] Prefer to use snprintf.
  static void OutputFormattedUInt(char* szOutputBuffer, xiiUInt32 uiBufferSize, xiiUInt32& uiWritePos, xiiUInt64 value, xiiUInt8 uiWidth, bool bPadZeros, xiiUInt8 uiBase, bool bUpperCase);
  /// \brief [internal] Prefer to use snprintf.
  static void OutputFormattedFloat(char* szOutputBuffer, xiiUInt32 uiBufferSize, xiiUInt32& uiWritePos, double value, xiiUInt8 uiWidth, bool bPadZeros, xiiInt8 iPrecision, bool bScientific, bool bRemoveTrailingZeroes = false);
