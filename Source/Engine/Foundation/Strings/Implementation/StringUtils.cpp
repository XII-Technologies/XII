...
xiiUInt32 xiiStringUtils::ToUpperChar(xiiUInt32 uiWc)
{
  if (uiWc >= 'a' && uiWc <= 'z')
  {
    uiWc -= 'a' - 'A';
...
//  Authors: $(WEB digitalmars.com, Walter Bright), Jonathan M Davis, and Kenji Hara
//  Source: $(PHOBOSSRC std/_uni.d)
xiiUInt32 xiiStringUtils::ToLowerChar(xiiUInt32 uiWc)
{
  if (uiWc >= 'A' && uiWc <= 'Z')
  {
    uiWc += 'a' - 'A';
...
xiiUInt32 xiiStringUtils::ToUpperString(char* pString, const char* pStringEnd)
{
  ...
  *pWriteStart = '\0';
  ...
  const xiiUInt32 uiNewStringLength = (xiiUInt32)(pWriteStart - pString);
  return uiNewStringLength;
}

xiiUInt32 xiiStringUtils::ToLowerString(char* pString, const char* pStringEnd)
{
  char*       pWriteStart = pString;
  const char* pReadStart  = pString;
...
xiiInt32 xiiStringUtils::Compare(const char* pString1, const char* pString2, const char* pString1End, const char* pString2End)
{
  ...
  const char* pString2End)
{
  ...
  while ((*pString1 != '\0') && (*pString2 != '\0') && (uiCharsToCompare > 0) && (pString1 < pString1End) && (pString2 < pString2End))
  {
    ...
    if (!xiiUnicodeUtils::IsUtf8ContinuationByte(*pString1))
      --uiCharsToCompare;
...
xiiInt32 xiiStringUtils::Compare_NoCase(const char* pString1, const char* pString2, const char* pString1End, const char* pString2End)
{
  XII_STRINGCOMPARE_HANDLE_NULL_PTRS(pString1, pString2, 0, -1, 1, pString1End, pString2End);
  ...
  while ((*pString1 != '\0') && (*pString2 != '\0') && (pString1 < pString1End) && (pString2 < pString2End))
  {
    ...
    const xiiUInt32 uiChar2 = xiiUnicodeUtils::DecodeUtf8ToUtf32(pString2);

    const xiiInt32 iComparison = CompareChars_NoCase(uiChar1, uiChar2);
...
xiiUInt32 xiiStringUtils::Copy(char* szDest, xiiUInt32 uiDstSize, const char* szSource, const char* pSourceEnd)
{
  XII_ASSERT_DEBUG(szDest != nullptr && uiDstSize > 0, "Invalid output buffer.");
  ...
  xiiUInt32 uiSourceLen   = static_cast<xiiUInt32>((pSourceEnd == xiiUnicodeUtils::GetMaxStringEnd<char>()) ? strlen(szSource) : pSourceEnd - szSource);
  xiiUInt32 uiBytesToCopy = xiiMath::Min(uiDstSize - 1, uiSourceLen);
...
// Check if we just copied half a UTF-8 character
  ...
  if (uiBytesToCopy > 0)
  {
    ...
    while (!xiiUnicodeUtils::IsUtf8StartByte(*szUtf8StartByte) && szUtf8StartByte > szDest)
    {
      szUtf8StartByte--;
    }
...
xiiUInt32 xiiStringUtils::CopyN(char* szDest, xiiUInt32 uiDstSize, const char* szSource, xiiUInt32 uiCharsToCopy, const char* pSourceEnd)
{
  ...
  while (uiDstSize > 0)
  {
    ...
    if (!xiiUnicodeUtils::IsUtf8ContinuationByte(*szSource))
    {
...
bool xiiStringUtils::StartsWith_NoCase(const char* szString, const char* szStartsWith, const char* pStringEnd, const char* szStartsWithEnd)
{
  if (IsNullOrEmpty(szStartsWith, szStartsWithEnd))
    return true;
  if (IsNullOrEmpty(szString, pStringEnd))
...
bool xiiStringUtils::EndsWith(const char* szString, const char* szEndsWith, const char* pStringEnd, const char* szEndsWithEnd)
{
  ...
  const xiiUInt32 uiLength1 = xiiStringUtils::GetStringElementCount(szString, pStringEnd);
  const xiiUInt32 uiLength2 = xiiStringUtils::GetStringElementCount(szEndsWith, szEndsWithEnd);
...
bool xiiStringUtils::EndsWith_NoCase(const char* szString, const char* szEndsWith, const char* pStringEnd, const char* szEndsWithEnd)
{
  if (IsNullOrEmpty(szEndsWith, szEndsWithEnd))
    return true;
  if (IsNullOrEmpty(szString, pStringEnd))
...
const char* xiiStringUtils::FindSubString(const char* szSource, const char* szStringToFind, const char* pSourceEnd, const char* szStringToFindEnd)
{
...
const char* xiiStringUtils::FindSubString_NoCase(const char* szSource, const char* szStringToFind, const char* pSourceEnd, const char* szStringToFindEnd)
{
  // Handle nullptr-pointer strings
  if ((IsNullOrEmpty(szSource)) || (IsNullOrEmpty(szStringToFind)))
    return nullptr;
...
const char* xiiStringUtils::FindLastSubString(const char* szSource, const char* szStringToFind, const char* szStartSearchAt, const char* pSourceEnd, const char* szStringToFindEnd)
{
  // Handle nullptr-pointer strings
  if ((IsNullOrEmpty(szSource)) || (IsNullOrEmpty(szStringToFind)))
    return nullptr;
  ...
  if (szStartSearchAt == nullptr)
    szStartSearchAt = szSource + xiiStringUtils::GetStringElementCount(szSource, pSourceEnd);
...
const char* xiiStringUtils::FindLastSubString_NoCase(const char* szSource, const char* szStringToFind, const char* szStartSearchAt, const char* pSourceEnd, const char* szStringToFindEnd)
{
  // Handle nullptr-pointer strings
  if ((IsNullOrEmpty(szSource)) || (IsNullOrEmpty(szStringToFind)))
    return nullptr;
...
const char* xiiStringUtils::FindWholeWord(const char* szString, const char* szSearchFor, XII_CHARACTER_FILTER isDelimiterCB, const char* pStringEnd)
{
  // Handle nullptr-pointer strings
  if ((IsNullOrEmpty(szString)) || (IsNullOrEmpty(szSearchFor)))
    return nullptr;

  const xiiUInt32 uiSearchedWordLength = GetStringElementCount(szSearchFor);
  ...
  while ((*pCurPos != '\0') && (pCurPos < pStringEnd))
  {
    if (StartsWith(pCurPos, szSearchFor, pStringEnd)) // yay, we found a substring, now make sure it is a 'whole word'
    {
      if (((szString == pCurPos) || // the start of the string is always a word delimiter
           (isDelimiterCB(
             xiiUnicodeUtils::ConvertUtf8ToUtf32(pPrevPos) /* front */))) &&                                 // make sure the character before this substring is a word delimiter
          ((pCurPos + uiSearchedWordLength >= pStringEnd) ||                                                 // the end of the string is also always a delimiter
           (isDelimiterCB(xiiUnicodeUtils::ConvertUtf8ToUtf32(pCurPos + uiSearchedWordLength) /* back */)))) // and the character after it, as well
        return pCurPos;
    }
...
xiiResult xiiStringUtils::FindUIntAtTheEnd(const char* szString, xiiUInt32& out_uiValue, xiiUInt32* pStringLengthBeforeUInt /*= nullptr*/)
{
  ...
  {
    xiiInt64 iResult = 0;
    if (xiiConversionUtils::StringToInt64(szNumberStart, iResult).Failed() || iResult < 0 || iResult > 0xFFFFFFFFu)
    {
      return XII_FAILURE;
    }

    out_uiValue = static_cast<xiiUInt32>(iResult);

    if (pStringLengthBeforeUInt != nullptr)
    {
      *pStringLengthBeforeUInt = static_cast<xiiUInt32>(szNumberStart - szString);
    }
...
const char* xiiStringUtils::SkipCharacters(const char* szString, XII_CHARACTER_FILTER skipCharacterCB, bool bAlwaysSkipFirst)
{
  XII_ASSERT_DEBUG(szString != nullptr, "Invalid string");

  while (*szString != '\0')
  {
    if (!bAlwaysSkipFirst && !skipCharacterCB(xiiUnicodeUtils::ConvertUtf8ToUtf32(szString)))
      break;
...
const char* xiiStringUtils::FindWordEnd(const char* szString, XII_CHARACTER_FILTER isDelimiterCB, bool bAlwaysSkipFirst)
{
  XII_ASSERT_DEBUG(szString != nullptr, "Invalid string");

  while (*szString != '\0')
  {
    if (!bAlwaysSkipFirst && isDelimiterCB(xiiUnicodeUtils::ConvertUtf8ToUtf32(szString)))
      break;
...
void xiiStringUtils::Trim(const char*& ref_pString, const char*& ref_pStringEnd, const char* szTrimCharsStart, const char* szTrimCharsEnd)
{
  bool bTrimmed = false;
  UpdateStringEnd(ref_pString, ref_pStringEnd);
  xiiStringView view(ref_pString, ref_pStringEnd);
  xiiStringView trimFront(szTrimCharsStart);
  xiiStringView trimEnd(szTrimCharsEnd);
  ...
  {
    bTrimmed = false;
    for (xiiUInt32 needle : trimFront)
    {
...
bool xiiStringUtils::IsWhiteSpace(xiiUInt32 c)
{
  // ASCII range of useless characters (32 is actually SPACE)
  return (c >= 1 && c <= 32);
}

bool xiiStringUtils::IsWordDelimiter_English(xiiUInt32 uiChar)
{
...
bool xiiStringUtils::IsIdentifierDelimiter_C_Code(xiiUInt32 uiChar)
{
  if ((uiChar >= 'a') && (uiChar <= 'z'))
    return false;
  if ((uiChar >= 'A') && (uiChar <= 'Z'))
...
bool xiiStringUtils::IsValidIdentifierName(const char* pString, const char* pStringEnd /*= xiiMaxStringEnd*/)
{
  if (IsNullOrEmpty(pString, pStringEnd))
    return false;

  xiiUInt32 cur = xiiUnicodeUtils::ConvertUtf8ToUtf32(pString);
  ...
  while (*pString != '\0' && pString < pStringEnd)
  {
    ...
    if (xiiStringUtils::IsIdentifierDelimiter_C_Code(cur))
      return false;
  }
...
XII_STATICLINK_FILE(Foundation, Foundation_Strings_Implementation_StringUtils);
