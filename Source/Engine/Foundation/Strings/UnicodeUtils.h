...
public:
  ...
  static xiiUInt32 GetUtf8SequenceLength(char uiFirstByte); // [tested]

  /// \brief Converts the UTF-8 character that starts at pFirstChar into a UTF-32 character.
  static xiiUInt32 ConvertUtf8ToUtf32(const char* pFirstChar); // [tested]

  /// \brief Computes how many bytes the character would require, if encoded in UTF-8.
  static xiiUInt32 GetSizeForCharacterInUtf8(xiiUInt32 uiCharacter); // [tested]
  ...
  static void MoveToNextUtf8(const char*& szUtf8, xiiUInt32 uiNumCharacters = 1); // [tested]

  /// \brief Moves the given string pointer ahead to the next Utf8 character sequence.
  ///
  /// The string may point to an invalid position (in between a character sequence).
  /// It may not point to a zero terminator already.
  static void MoveToNextUtf8(const char*& szUtf8, const char* szUtf8End, xiiUInt32 uiNumCharacters = 1); // [tested]
  ...
  static void MoveToPriorUtf8(const char*& szUtf8, xiiUInt32 uiNumCharacters = 1); // [tested]
  ...
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomLE(const xiiUInt16*& szUtf16); // [tested]
  ...
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomBE(const xiiUInt16*& szUtf16); // [tested]
  ...
  static xiiUInt32 DecodeUtf8ToUtf32(ByteIterator& szUtf8Iterator); // [tested]
  ...
  template <typename UInt16Iterator>
  static xiiUInt32 DecodeUtf16ToUtf32(UInt16Iterator& szUtf16Iterator); // [tested]
  ...
  /// Writes the result to the desired container type (xiiString or xiiStringBuilder).
  ///
