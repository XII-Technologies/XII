...
public:
  ...
  static xiiUInt32 GetUtf8SequenceLength(char iFirstByte); // [tested]
  ...
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomLE(const xiiUInt16*& ref_pUtf16); // [tested]
  ...
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomBE(const xiiUInt16*& ref_pUtf16); // [tested]
  ...
  static xiiUInt32 DecodeUtf8ToUtf32(ByteIterator& ref_szUtf8Iterator); // [tested]
  ...
  template <typename UInt16Iterator>
  static xiiUInt32 DecodeUtf16ToUtf32(UInt16Iterator& ref_szUtf16Iterator); // [tested]
  ...
  static xiiUInt32 DecodeWCharToUtf32(WCharIterator& ref_szWCharIterator); // [tested]

  /// Encodes the given Utf32 character to Utf8 and writes as many bytes to the output iterator, as necessary.
  template <typename ByteIterator>
  static void EncodeUtf32ToUtf8(xiiUInt32 uiUtf32, ByteIterator& ref_szUtf8Output); // [tested]

  /// Encodes the given Utf32 character to Utf16 and writes as many bytes to the output iterator, as necessary.
  template <typename UInt16Iterator>
  static void EncodeUtf32ToUtf16(xiiUInt32 uiUtf32, UInt16Iterator& ref_szUtf16Output); // [tested]

  /// Encodes the given Utf32 character to wchar_t and writes as many bytes to the output iterator, as necessary.
  template <typename WCharIterator>
  static void EncodeUtf32ToWChar(xiiUInt32 uiUtf32, WCharIterator& ref_szWCharOutput); // [tested]
