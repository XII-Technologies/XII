...
#define DEFINE_TERNARY_OP(name, code)                                                     \
  ...
  DEFINE_UNARY_OP(ExpF, r->f = xiiSimdMath::Exp(a->f));
  DEFINE_UNARY_OP(LnF, r->f = xiiSimdMath::Ln(a->f));
  DEFINE_UNARY_OP(Log2F, r->f = xiiSimdMath::Log2(a->f));
  DEFINE_UNARY_OP(Log2I, r->i = xiiSimdMath::Log2i(a->i));
  DEFINE_UNARY_OP(Log10F, r->f = xiiSimdMath::Log10(a->f));
  DEFINE_UNARY_OP(Pow2F, r->f = xiiSimdMath::Pow2(a->f));

  DEFINE_UNARY_OP(SinF, r->f = xiiSimdMath::Sin(a->f));
  DEFINE_UNARY_OP(CosF, r->f = xiiSimdMath::Cos(a->f));
  DEFINE_UNARY_OP(TanF, r->f = xiiSimdMath::Tan(a->f));

  DEFINE_UNARY_OP(ASinF, r->f = xiiSimdMath::ASin(a->f));
  DEFINE_UNARY_OP(ACosF, r->f = xiiSimdMath::ACos(a->f));
  DEFINE_UNARY_OP(ATanF, r->f = xiiSimdMath::ATan(a->f));
  ...
  DEFINE_UNARY_OP(FToI, r->i = xiiSimdVec4i::Truncate(a->f));
  ...
  DEFINE_TERNARY_OP(SelF, r->f = xiiSimdVec4f::Select(a->b, b->f, c->f));
  DEFINE_TERNARY_OP(SelI, r->i = xiiSimdVec4i::Select(a->b, b->i, c->i));
  DEFINE_TERNARY_OP(SelB, r->b = xiiSimdVec4b::Select(a->b, b->b, c->b));
  ...
  void StoreOutput(RegisterType* r, RegisterType* pRe, xiiProcessingStream& ref_output, xiiUInt32 uiNumRemainderInstances)
  {
    ...
    if (uiNumRemainderInstances > 0)
    {
      ...
      for (xiiUInt32 i = 0; i < uiNumRemainderInstances; ++i)
      {
  ...
  void VM_LoadF_4(const ByteCodeType*& pByteCode, ExecutionContext& context)
  {
    ...
    {
      LoadInput<xiiSimdVec4f, float, float>(reinterpret_cast<xiiSimdVec4f*>(r), reinterpret_cast<xiiSimdVec4f*>(re), input, uiNumRemainderInstances);
    }
    else
    {
      XII_ASSERT_DEBUG(input.GetDataType() == xiiProcessingStream::DataType::Half, "Unsupported input type '{}' for LoadF instruction", xiiProcessingStream::GetDataTypeName(input.GetDataType()));
      LoadInput<xiiSimdVec4f, float, xiiFloat16>(reinterpret_cast<xiiSimdVec4f*>(r), reinterpret_cast<xiiSimdVec4f*>(re), input, uiNumRemainderInstances);
    }
  }
  ...
  void VM_StoreF_4(const ByteCodeType*& pByteCode, ExecutionContext& context)
  {
    ...
    if (output.GetDataType() == xiiProcessingStream::DataType::Float)
    {
      StoreOutput<xiiSimdVec4f, float, float>(reinterpret_cast<xiiSimdVec4f*>(r), reinterpret_cast<xiiSimdVec4f*>(re), output, uiNumRemainderInstances);
    }
    else
    {
      XII_ASSERT_DEBUG(output.GetDataType() == xiiProcessingStream::DataType::Half, "Unsupported input type '{}' for StoreF instruction", xiiProcessingStream::GetDataTypeName(output.GetDataType()));
      StoreOutput<xiiSimdVec4f, float, xiiFloat16>(reinterpret_cast<xiiSimdVec4f*>(r), reinterpret_cast<xiiSimdVec4f*>(re), output, uiNumRemainderInstances);
    }
  }
  ...
  void VM_Call(const ByteCodeType*& pByteCode, ExecutionContext& context)
  {
    ...
    xiiHybridArray<xiiArrayPtr<const xiiExpression::Register>, 32> inputs;
    inputs.Reserve(uiNumArgs);
    for (xiiUInt32 uiArgIndex = 0; uiArgIndex < uiNumArgs; ++uiArgIndex)
    {
      DEFINE_OP_REGISTER(x);
      inputs.PushBack(xiiMakeArrayPtr(x, context.m_uiNumSimd4Instances));
    }
