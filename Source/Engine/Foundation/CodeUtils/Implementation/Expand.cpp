...
xiiResult xiiPreprocessor::Expand(const TokenStream& Tokens, TokenStream& Output)
{
...
xiiResult xiiPreprocessor::ExpandOnce(const TokenStream& Tokens, TokenStream& Output)
{
  const bool bIsOutermost = m_CurrentFileStack.PeekBack().m_iExpandDepth == 0;

  ++m_CurrentFileStack.PeekBack().m_iExpandDepth;

  for (xiiUInt32 uiCurToken = 0; uiCurToken < Tokens.GetCount();)
  {
    XII_ASSERT_DEV(Tokens[uiCurToken]->m_iType < s_iMacroParameter0, "Implementation error");
    ...
    if (bIsOutermost)
    {
      ...
      m_CurrentFileStack.PeekBack().m_iCurrentLine     = (xiiInt32)Tokens[uiCurToken]->m_uiLine;
    }

    // if it is no identifier, it cannot be a macro -> just pass it through
    if (Tokens[uiCurToken]->m_iType != xiiTokenType::Identifier)
    {
    ...
    const xiiUInt32 uiIdentifierToken = uiCurToken;
    ...
    else
    {
      ...
      for (xiiUInt32 i = uiIdentifierToken; i < uiCurToken; ++i)
...
void xiiPreprocessor::OutputNotExpandableMacro(MacroDefinition& Macro, TokenStream& Output)
{
  const xiiStringView& sMacroName = Macro.m_MacroIdentifier->m_DataView;

  XII_ASSERT_DEV(Macro.m_bCurrentlyExpanding, "Implementation Error.");

  xiiToken* pNewToken        = AddCustomToken(Macro.m_MacroIdentifier, sMacroName);
  pNewToken->m_uiCustomFlags = TokenFlags::NoFurtherExpansion;

  Output.PushBack(pNewToken);
}

xiiResult xiiPreprocessor::ExpandObjectMacro(MacroDefinition& Macro, TokenStream& Output, const xiiToken* pMacroToken)
{
  ...
  if (pMacroToken->m_DataView.IsEqual("__FILE__"))
  {
    ...
    xiiToken* pNewToken = AddCustomToken(pMacroToken, sName);
    pNewToken->m_iType  = xiiTokenType::String1;
...
void xiiPreprocessor::PassThroughFunctionMacro(MacroDefinition& Macro, const MacroParameters& Parameters, TokenStream& Output)
{
  OutputNotExpandableMacro(Macro, Output);
...
xiiToken* xiiPreprocessor::CreateStringifiedParameter(xiiUInt32 uiParam, const xiiToken* pParamToken, const MacroDefinition& Macro)
{
  xiiStringBuilder sStringifiedParam;
  ...
  else
  {
    ...
    if (Macro.m_bHasVarArgs && uiParam + 1 == Macro.m_uiNumParameters)
    {
      ...
      for (xiiUInt32 i = uiParam; i < m_MacroParamStack.PeekBack()->GetCount(); ++i)
      {
  ...
  xiiToken* pStringifiedToken = AddCustomToken(pParamToken, sStringifiedParam);
  pStringifiedToken->m_iType  = xiiTokenType::String1;
  return pStringifiedToken;
}

xiiResult xiiPreprocessor::InsertStringifiedParameters(const TokenStream& Tokens, TokenStream& Output, const MacroDefinition& Macro)
{
  xiiInt32 iConsecutiveHashes  = 0;
  bool     bLastTokenWasHash   = false;
  bool     bStringifyParameter = false;

  for (xiiUInt32 i = 0; i < Tokens.GetCount(); ++i)
  {
    const bool bTokenIsHash = (Tokens[i]->m_iType == xiiTokenType::NonIdentifier && xiiString(Tokens[i]->m_DataView) == "#");
    ...
    else
    {
      ...
      bStringifyParameter = xiiMath::IsOdd(iConsecutiveHashes);
    }

    if (bStringifyParameter && Tokens[i]->m_iType < s_iMacroParameter0 && Tokens[i]->m_iType != xiiTokenType::Whitespace)
    {
    ...
    {
      const xiiUInt32 uiParam = Tokens[i]->m_iType - s_iMacroParameter0;

      bStringifyParameter = false;

      xiiToken* pStringifiedToken = CreateStringifiedParameter(uiParam, Tokens[i], Macro);

      // remove all whitespace and the last # at the end of the output
      while (true)
      {
        if (Output.PeekBack()->m_iType == xiiTokenType::Whitespace)
          Output.PopBack();
        else
        {
          Output.PopBack();
    ...
    if (!bTokenIsHash && (Tokens[i]->m_iType != xiiTokenType::BlockComment) && (Tokens[i]->m_iType != xiiTokenType::LineComment) && (Tokens[i]->m_iType != xiiTokenType::Whitespace))
      iConsecutiveHashes = 0;
  }
...
void xiiPreprocessor::MergeTokens(const xiiToken* pFirst, const xiiToken* pSecond, TokenStream& Output, const MacroDefinition& Macro)
{
  if (pFirst != nullptr && pFirst->m_iType >= s_iMacroParameter0)
  {
    xiiUInt32 uiParam = pFirst->m_iType - s_iMacroParameter0;
    ...
    if (uiParam < m_MacroParamStack.PeekBack()->GetCount())
    {
      ...
      for (xiiUInt32 i = 1; i < (*m_MacroParamStack.PeekBack())[uiParam].GetCount(); ++i)
  ...
  {
    const xiiUInt32 uiParam = pSecond->m_iType - s_iMacroParameter0;
  ...
  if (pFirst == nullptr || pSecond == nullptr || (pFirst->m_iType != xiiTokenType::Identifier && pFirst->m_iType != xiiTokenType::Integer) || (pSecond->m_iType != xiiTokenType::Identifier && pSecond->m_iType != xiiTokenType::Integer))
  {
    if (pFirst != nullptr)
  ...
  xiiStringBuilder sMerged;
  sMerged.Append(pFirst->m_DataView);
  sMerged.Append(pSecond->m_DataView);

  const xiiToken* pMergedToken = AddCustomToken(pFirst, sMerged);

  Output.PushBack(pMergedToken);
}

xiiResult xiiPreprocessor::ConcatenateParameters(const TokenStream& Tokens, TokenStream& Output, const MacroDefinition& Macro)
{
  ...
  {
    xiiUInt32 uiConcatToken = uiCurToken;

    // do this extra check for whitespace here, because 'Accept' would just skip it, but we want the whitespace in our output
    if (Tokens[uiCurToken]->m_iType != xiiTokenType::Whitespace && Tokens[uiCurToken]->m_iType != xiiTokenType::Newline && Accept(Tokens, uiCurToken, "#", "#", &uiConcatToken))
    {
      ...
      while (!Output.IsEmpty() && (Output.PeekBack()->m_iType == xiiTokenType::Whitespace || Output.PeekBack()->m_iType == xiiTokenType::Newline))
        Output.PopBack();
      ...
      const xiiToken* pFirstToken = Output.PeekBack();
      Output.PopBack();
      ...
      const xiiToken* pSecondToken = Tokens[uiCurToken];
...
xiiResult xiiPreprocessor::InsertParameters(const TokenStream& Tokens, TokenStream& Output, const MacroDefinition& Macro)
{
  ...
  for (xiiUInt32 i = 0; i < Concatenated.GetCount(); ++i)
  {
    if (Concatenated[i]->m_iType >= s_iMacroParameter0)
    {
      const xiiUInt32 uiParam = Concatenated[i]->m_iType - s_iMacroParameter0;
...
xiiResult xiiPreprocessor::ExpandFunctionMacro(MacroDefinition& Macro, const MacroParameters& Parameters, TokenStream& Output, const xiiToken* pMacroToken)
{
  // when the macro is already being expanded, just pass the macro name through, but flag it as not to be expanded further
  if (Macro.m_bCurrentlyExpanding)
  {
  ...
  for (xiiUInt32 i = 0; i < Parameters.GetCount(); ++i)
  {
...
xiiResult xiiPreprocessor::ExpandMacroParam(const xiiToken& MacroToken, xiiUInt32 uiParam, TokenStream& Output, const MacroDefinition& Macro)
{
  XII_ASSERT_DEV(!m_MacroParamStack.IsEmpty(), "Implementation error.");

  const MacroParameters& ParamsExpanded = *m_MacroParamStackExpanded.PeekBack();

  if (uiParam >= ParamsExpanded.GetCount())
  {
    xiiToken* pWhitespace = AddCustomToken(&MacroToken, "");
    pWhitespace->m_iType  = xiiTokenType::Whitespace;
  ...
  else if (uiParam + 1 == Macro.m_uiNumParameters && Macro.m_bHasVarArgs)
  {
    ...
    for (xiiUInt32 i = uiParam; i < ParamsExpanded.GetCount(); ++i)
    {
...
XII_STATICLINK_FILE(Foundation, Foundation_CodeUtils_Implementation_Expand);
