...
/// or GetFloatOption()
class XII_FOUNDATION_DLL xiiCommandLineUtils
{
public:
  enum ArgMode
  {
  ...
  static void SplitCommandLineString(const char* commandString, bool addExecutableDir, xiiDynamicArray<xiiString>& outArgs, xiiDynamicArray<const char*>& outArgsV);

  /// \brief Initializes xiiCommandLineUtils from the parameter arguments that were passed to the application.
  void SetCommandLine(xiiUInt32 argc, const char** argv, ArgMode mode = UseArgcArgv); // [tested]
