...
{
  xiiOSFile status;
  if (status.Open("/proc/self/status", xiiFileOpenMode::Read).Failed())
  {
    return false;
  }
  ...
  char      buffer[2048];
  xiiUInt64 numBytesRead = status.Read(buffer, XII_ARRAY_SIZE(buffer));
  status.Close();


  xiiStringView contents(buffer, numBytesRead);
...
void xiiSystemInformation::Initialize()
{
  ...
  xiiUInt64 uiPageCount = sysconf(_SC_PHYS_PAGES);
  xiiUInt64 uiPageSize  = sysconf(_SC_PAGE_SIZE);
...
{
  return static_cast<xiiUInt64>(sysconf(_SC_AVPHYS_PAGES)) * static_cast<xiiUInt64>(sysconf(_SC_PAGESIZE));
}
