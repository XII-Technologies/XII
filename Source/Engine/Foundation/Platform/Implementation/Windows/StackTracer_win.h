...
#ifdef XII_STACKTRACER_WIN_INL_H_INCLUDED
#  error "This file must not be included twice."
#endif

#define XII_STACKTRACER_WIN_INL_H_INCLUDED
...
namespace
{
  ...
  struct StackTracerImplementation
  {
    ...
    {
      xiiMemoryUtils::ZeroFill(this, 1);

      kernel32Dll = LoadLibraryW(L"kernel32.dll");
      XII_ASSERT_DEV(kernel32Dll != nullptr, "StackTracer could not load kernel32.dll");
...
void xiiStackTracer::OnPluginEvent(const xiiPluginEvent& e)
{
  ...
  if (e.m_EventType == xiiPluginEvent::BeforeUnloading)
  {
    SymbolInitialize();
  }
  ...
  if (false) // e.m_EventType == xiiPluginEvent::AfterLoading)
  {
    ...
    xiiMemoryUtils::ZeroFill(&moduleInfo, 1);
...
void xiiStackTracer::ResolveStackTrace(const xiiArrayPtr<void*>& trace, PrintFunc printFunc)
{
  ...
  if (s_pImplementation->symbolFromAddress != nullptr && s_pImplementation->lineFromAdress != nullptr)
  {
    ...
    for (xiiUInt32 i = 0; i < uiNumTraceEntries; i++)
    {
      ...
      char finalStr[1024];
      wcstombs(finalStr, str, XII_ARRAY_SIZE(finalStr));
