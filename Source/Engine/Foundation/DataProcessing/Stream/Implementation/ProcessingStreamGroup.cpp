...
xiiProcessingStreamGroup::xiiProcessingStreamGroup()
{
  Clear();
}
...
void xiiProcessingStreamGroup::Clear()
{
  ClearProcessors();
  ...
  for (xiiProcessingStream* pStream : m_DataStreams)
  {
    XII_DEFAULT_DELETE(pStream);
  }

  m_DataStreams.Clear();
}

void xiiProcessingStreamGroup::AddProcessor(xiiProcessingStreamProcessor* pProcessor)
{
  XII_ASSERT_DEV(pProcessor != nullptr, "Stream processor may not be null!");

  if (pProcessor->m_pStreamGroup != nullptr)
  {
    xiiLog::Debug("Stream processor is already assigned to a stream group!");
    return;
  }
...
void xiiProcessingStreamGroup::RemoveProcessor(xiiProcessingStreamProcessor* pProcessor)
{
  m_Processors.RemoveAndCopy(pProcessor);
  pProcessor->GetDynamicRTTI()->GetAllocator()->Deallocate(pProcessor);
}

void xiiProcessingStreamGroup::ClearProcessors()
{
  m_bStreamAssignmentDirty = true;

  for (xiiProcessingStreamProcessor* pProcessor : m_Processors)
  {
    pProcessor->GetDynamicRTTI()->GetAllocator()->Deallocate(pProcessor);
  }
...
void xiiProcessingStreamGroup::SetSize(xiiUInt64 uiNumElements)
{
...
/// processors).
void xiiProcessingStreamGroup::RemoveElement(xiiUInt64 uiElementIndex)
{
  if (m_PendingRemoveIndices.Contains(uiElementIndex))
    return;

  XII_ASSERT_DEBUG(uiElementIndex < m_uiNumActiveElements, "Element which should be removed is outside of active element range!");
...
void xiiProcessingStreamGroup::InitializeElements(xiiUInt64 uiNumElements)
{
  m_uiPendingNumberOfElementsToSpawn += uiNumElements;
}

void xiiProcessingStreamGroup::Process()
{
  ...
  for (xiiProcessingStreamProcessor* pStreamProcessor : m_Processors)
  {
...
void xiiProcessingStreamGroup::RunPendingDeletions()
{
  xiiStreamGroupElementRemovedEvent e;
  ...
  while (!m_PendingRemoveIndices.IsEmpty())
  {
    ...
    const xiiUInt64 uiLastActiveElementIndex = m_uiNumActiveElements - 1;

    const xiiUInt64 uiElementToRemove = m_PendingRemoveIndices.PeekBack();
    m_PendingRemoveIndices.PopBack();

    XII_ASSERT_DEBUG(uiElementToRemove < m_uiNumActiveElements, "Invalid index to remove");
    ...
    for (xiiUInt32 i = 0; i < m_PendingRemoveIndices.GetCount(); ++i)
    {
...
void xiiProcessingStreamGroup::EnsureStreamAssignmentValid()
{
...
void xiiProcessingStreamGroup::RunPendingSpawns()
{
  ...
  {
    m_uiPendingNumberOfElementsToSpawn = xiiMath::Min(m_uiPendingNumberOfElementsToSpawn, m_uiNumElements - m_uiNumActiveElements);

    if (m_uiPendingNumberOfElementsToSpawn)
    {
      for (xiiProcessingStreamProcessor* pSpawner : m_Processors)
      {
    ...
    m_uiHighestNumActiveElements = xiiMath::Max(m_uiNumActiveElements, m_uiHighestNumActiveElements);
...
{
  XII_ALWAYS_INLINE bool Less(const xiiProcessingStreamProcessor* a, const xiiProcessingStreamProcessor* b) const { return a->m_fPriority < b->m_fPriority; }
};

void xiiProcessingStreamGroup::SortProcessorsByPriority()
{
  ProcessorComparer cmp;
  m_Processors.Sort(cmp);
}

XII_STATICLINK_FILE(Foundation, Foundation_DataProcessing_Stream_Implementation_ProcessingStreamGroup);
