...
public:
  ...
  XII_ALWAYS_INLINE ptrdiff_t operator-(const const_iterator_base& rhs) const { return m_uiIndex - rhs.m_uiIndex; }

  XII_ALWAYS_INLINE const_iterator_base operator+(ptrdiff_t rhs) const { return const_iterator_base(*m_pArray, m_uiIndex + rhs); }
  XII_ALWAYS_INLINE const_iterator_base operator-(ptrdiff_t rhs) const { return const_iterator_base(*m_pArray, m_uiIndex - rhs); }

  XII_ALWAYS_INLINE void operator+=(ptrdiff_t rhs) { m_uiIndex += rhs; }
  XII_ALWAYS_INLINE void operator-=(ptrdiff_t rhs) { m_uiIndex -= rhs; }
  ...
  XII_ALWAYS_INLINE const T& operator[](size_t index) const
  {
    if (reverse)
      return (*m_pArray)[m_pArray->GetCount() - static_cast<xiiUInt32>(m_uiIndex + index) - 1];
    else
      return (*m_pArray)[static_cast<xiiUInt32>(m_uiIndex + index)];
  }
...
public:
  ...
  XII_ALWAYS_INLINE iterator_base operator+(ptrdiff_t rhs) const { return iterator_base(*this->m_pArray, this->m_uiIndex + rhs); }
  XII_ALWAYS_INLINE iterator_base operator-(ptrdiff_t rhs) const { return iterator_base(*this->m_pArray, this->m_uiIndex - rhs); }
  ...
  XII_ALWAYS_INLINE T& operator[](size_t index) const
  {
    if (reverse)
      return (*this->m_pArray)[this->m_pArray->GetCount() - static_cast<xiiUInt32>(this->m_uiIndex + index) - 1];
    else
      return (*this->m_pArray)[static_cast<xiiUInt32>(this->m_uiIndex + index)];
  }
};
...
public:
  ...
  XII_ALWAYS_INLINE ptrdiff_t operator-(const const_reverse_pointer_iterator& rhs) const { return rhs.m_pPtr - m_pPtr; }

  XII_ALWAYS_INLINE const_reverse_pointer_iterator operator+(ptrdiff_t rhs) const { return const_reverse_pointer_iterator(m_pPtr - rhs); }
  XII_ALWAYS_INLINE const_reverse_pointer_iterator operator-(ptrdiff_t rhs) const { return const_reverse_pointer_iterator(m_pPtr + rhs); }

  XII_ALWAYS_INLINE void operator+=(ptrdiff_t rhs) { m_pPtr -= rhs; }
  XII_ALWAYS_INLINE void operator-=(ptrdiff_t rhs) { m_pPtr += rhs; }
  ...
  XII_ALWAYS_INLINE const T& operator[](ptrdiff_t index) const { return *(m_pPtr - index); }
...
public:
  ...
  XII_ALWAYS_INLINE reverse_pointer_iterator operator+(ptrdiff_t rhs) const { return reverse_pointer_iterator(this->m_pPtr - rhs); }
  XII_ALWAYS_INLINE reverse_pointer_iterator operator-(ptrdiff_t rhs) const { return reverse_pointer_iterator(this->m_pPtr + rhs); }

  XII_ALWAYS_INLINE T& operator*() const { return *(this->m_pPtr); }
  XII_ALWAYS_INLINE T* operator->() const { return this->m_pPtr; }
  XII_ALWAYS_INLINE T& operator[](ptrdiff_t index) const { return *(this->m_pPtr - index); }
};
