...
xiiIdTableBase<IdType, ValueType>::ConstIterator::ConstIterator(const xiiIdTableBase<IdType, ValueType>& idTable) :
  m_IdTable(idTable), m_CurrentIndex(0), m_CurrentCount(0)
{
...
template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE bool xiiIdTableBase<IdType, ValueType>::ConstIterator::IsValid() const
{
...
template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE IdType xiiIdTableBase<IdType, ValueType>::ConstIterator::Id() const
{
  return m_IdTable.m_pEntries[m_CurrentIndex].id;
}

template <typename IdType, typename ValueType>
XII_FORCE_INLINE const ValueType& xiiIdTableBase<IdType, ValueType>::ConstIterator::Value() const
{
  return m_IdTable.m_pEntries[m_CurrentIndex].value;
}

template <typename IdType, typename ValueType>
void xiiIdTableBase<IdType, ValueType>::ConstIterator::Next()
{
  ++m_CurrentCount;
...
template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE void xiiIdTableBase<IdType, ValueType>::ConstIterator::operator++()
{
  Next();
}
...
template <typename IdType, typename ValueType>
xiiIdTableBase<IdType, ValueType>::Iterator::Iterator(const xiiIdTableBase<IdType, ValueType>& idTable) :
  ConstIterator(idTable)
{
}

template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE ValueType& xiiIdTableBase<IdType, ValueType>::Iterator::Value()
{
...
template <typename IdType, typename ValueType>
xiiIdTableBase<IdType, ValueType>::~xiiIdTableBase()
{
...
template <typename IdType, typename ValueType>
void xiiIdTableBase<IdType, ValueType>::operator=(const xiiIdTableBase<IdType, ValueType>& rhs)
{
  Clear();
  ...
  for (IndexType i = 0; i < rhs.m_Capacity; ++i)
  {
    ...
    {
      xiiMemoryUtils::CopyConstruct(&entry.value, rhs.m_pEntries[i].value, 1);
    }
  }
...
template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE typename xiiIdTableBase<IdType, ValueType>::IndexType xiiIdTableBase<IdType, ValueType>::GetCount() const
{
  return m_Count;
}

template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE bool xiiIdTableBase<IdType, ValueType>::IsEmpty() const
{
  return m_Count == 0;
}

template <typename IdType, typename ValueType>
void xiiIdTableBase<IdType, ValueType>::Clear()
{
  for (IndexType i = 0; i < m_Capacity; ++i)
  {
    Entry& entry = m_pEntries[i];
...
template <typename IdType, typename ValueType>
IdType xiiIdTableBase<IdType, ValueType>::Insert(const ValueType& value)
{
  Reserve(m_Count + 1);
  ...
  xiiMemoryUtils::CopyConstruct(&entry.value, value, 1);
...
template <typename IdType, typename ValueType>
IdType xiiIdTableBase<IdType, ValueType>::Insert(ValueType&& value)
{
  Reserve(m_Count + 1);
  ...
  xiiMemoryUtils::MoveConstruct<ValueType>(&entry.value, std::move(value));
...
template <typename IdType, typename ValueType>
XII_FORCE_INLINE bool xiiIdTableBase<IdType, ValueType>::TryGetValue(const IdType id, ValueType& out_value) const
{
...
template <typename IdType, typename ValueType>
XII_FORCE_INLINE bool xiiIdTableBase<IdType, ValueType>::TryGetValue(const IdType id, ValueType*& out_pValue) const
{
  const IndexType index = id.m_InstanceIndex;
  if (index < m_Capacity && m_pEntries[index].id.IsIndexAndGenerationEqual(id))
  {
...
template <typename IdType, typename ValueType>
XII_FORCE_INLINE bool xiiIdTableBase<IdType, ValueType>::Contains(const IdType id) const
{
  const IndexType index = id.m_InstanceIndex;
...
template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE typename xiiIdTableBase<IdType, ValueType>::Iterator xiiIdTableBase<IdType, ValueType>::GetIterator()
{
  return Iterator(*this);
}

template <typename IdType, typename ValueType>
XII_ALWAYS_INLINE typename xiiIdTableBase<IdType, ValueType>::ConstIterator xiiIdTableBase<IdType, ValueType>::GetIterator() const
{
...
template <typename IdType, typename ValueType>
bool xiiIdTableBase<IdType, ValueType>::IsFreelistValid() const
{
...
// private methods
template <typename IdType, typename ValueType>
void xiiIdTableBase<IdType, ValueType>::SetCapacity(IndexType uiCapacity)
{
  Entry* pNewEntries = XII_NEW_RAW_BUFFER(m_pAllocator, Entry, (size_t)uiCapacity);
  ...
  for (IndexType i = 0; i < m_Capacity; ++i)
  {
    ...
    if (m_pEntries[i].id.m_InstanceIndex == i)
    {
      xiiMemoryUtils::RelocateConstruct(&pNewEntries[i].value, &m_pEntries[i].value, 1);
    }
  }
...
template <typename IdType, typename ValueType>
inline void xiiIdTableBase<IdType, ValueType>::InitializeFreelist(IndexType uiStart, IndexType uiEnd)
{
...
xiiIdTable<IdType, V, A>::xiiIdTable() :
  xiiIdTableBase<IdType, V>(A::GetAllocator())
{
}
...
template <typename IdType, typename V, typename A>
xiiIdTable<IdType, V, A>::xiiIdTable(const xiiIdTable<IdType, V, A>& other) :
  xiiIdTableBase<IdType, V>(other, A::GetAllocator())
{
}

template <typename IdType, typename V, typename A>
xiiIdTable<IdType, V, A>::xiiIdTable(const xiiIdTableBase<IdType, V>& other) :
  xiiIdTableBase<IdType, V>(other, A::GetAllocator())
{
}

template <typename IdType, typename V, typename A>
void xiiIdTable<IdType, V, A>::operator=(const xiiIdTable<IdType, V, A>& rhs)
{
  xiiIdTableBase<IdType, V>::operator=(rhs);
}

template <typename IdType, typename V, typename A>
void xiiIdTable<IdType, V, A>::operator=(const xiiIdTableBase<IdType, V>& rhs)
{
  xiiIdTableBase<IdType, V>::operator=(rhs);
}
