...
xiiAbstractObjectNode* xiiAbstractObjectGraph::Clone(xiiAbstractObjectGraph& cloneTarget, const xiiAbstractObjectNode* pRootNode, FilterFunction filter) const
{
  cloneTarget.Clear();
...
void xiiAbstractObjectGraph::ReMapNodeGuidsToMatchGraph(xiiAbstractObjectNode* root, const xiiAbstractObjectGraph& rhsGraph, const xiiAbstractObjectNode* rhsRoot)
{
  xiiHashTable<xiiUuid, xiiUuid> guidMap;
  XII_ASSERT_DEV(xiiStringUtils::IsEqual(root->GetType(), rhsRoot->GetType()), "Roots must have the same type to be able re-map guids!");
...
void xiiAbstractObjectGraph::FindTransitiveHull(const xiiUuid& rootGuid, xiiSet<xiiUuid>& reachableNodes) const
{
...
xiiAbstractObjectNode* xiiAbstractObjectGraph::CopyNodeIntoGraph(const xiiAbstractObjectNode* pNode, FilterFunction& filter)
{
  auto pNewNode = AddNode(pNode->GetGuid(), pNode->GetType(), pNode->GetTypeVersion(), pNode->GetNodeName());
...
void xiiAbstractObjectGraph::ApplyDiff(xiiDeque<xiiAbstractGraphDiffOperation>& Diff)
{
...
void xiiAbstractObjectGraph::MergeDiffs(const xiiDeque<xiiAbstractGraphDiffOperation>& lhs, const xiiDeque<xiiAbstractGraphDiffOperation>& rhs, xiiDeque<xiiAbstractGraphDiffOperation>& out) const
{
  struct Prop
  {
    Prop() {}
  ...
  for (const xiiAbstractGraphDiffOperation& op : rhs)
  {
    ...
    else if (op.m_Operation == xiiAbstractGraphDiffOperation::Op::NodeAdded)
    {
      ...
      {
        xiiAbstractGraphDiffOperation& leftOp = out[added[op.m_Node]];
