...
/// \brief This class is used to convert between native C++ objects and xiiAbstractObjectGraphs. It is used by the xiiDocument system to serialize native objects into documents and deserialize them back.
...
/// \brief This struct is used to store the information about an object that is currently being converted. It is stored in the xiiRttiConverterContext and allows to look up objects by their guid and vice versa.
class XII_FOUNDATION_DLL xiiRttiConverterContext
{
public:
  /// \brief Clears all stored information about objects. This is called when the converter context is reused for multiple conversions. Override this to clear any additional data that your implementation may store.
  virtual void Clear();

  /// \brief Generates a guid for a new object. Default implementation generates stable guids derived from parentGuid + property name + index and ignores the address of pObject.
  ...
  /// \brief Creates a new object for the given guid and type. Default implementation uses the allocator from the RTTI type to create a new instance of the object. Override this to implement custom object creation logic, e.g. to reuse existing objects or to create objects that cannot be allocated through RTTI.
  virtual xiiInternal::NewInstance<void> CreateObject(const xiiUuid& guid, const xiiRTTI* pRtti);

  /// \brief Deletes the object with the given guid. Default implementation uses the allocator from the RTTI type to delete the object. Override this to implement custom object deletion logic, e.g. to reuse existing objects or to delete objects that cannot be allocated through RTTI.
  virtual void DeleteObject(const xiiUuid& guid);

  /// \brief Registers an object with the given guid, type and pointer. This is called by the default implementation of CreateObject after creating a new object. Override this to store additional information about the object or to implement custom registration logic.
  virtual void RegisterObject(const xiiUuid& guid, const xiiRTTI* pRtti, void* pObject);

  /// \brief Unregisters the object with the given guid. This is called by the default implementation of DeleteObject after deleting an object. Override this to clear any additional information about the object or to implement custom unregistration logic.
  virtual void UnregisterObject(const xiiUuid& guid);

  /// \brief Returns the object for the given guid. Default implementation looks up the guid in a hash table and returns the corresponding object pointer and type. Override this to implement custom lookup logic, e.g. to look up objects that are not currently instantiated, but only exist as graph nodes.
  virtual xiiRttiConverterObject GetObjectByGUID(const xiiUuid& guid) const;

  /// \brief Returns the guid for the given object pointer. Default implementation looks up the object pointer in a hash table and returns the corresponding guid. Override this to implement custom lookup logic, e.g. to look up guids for objects that are not currently instantiated, but only exist as graph nodes.
  virtual xiiUuid GetObjectGUID(const xiiRTTI* pRtti, const void* pObject) const;

  /// \brief Returns the type for the given type name. Default implementation looks up the type name in a hash table and returns the corresponding RTTI type. Override this to implement custom lookup logic, e.g. to look up types that are not currently registered in the RTTI system.
  virtual const xiiRTTI* FindTypeByName(xiiStringView sName) const;

  /// \brief Returns the type for the given type name hash. Default implementation looks up the type name hash in a hash table and returns the corresponding RTTI type. Override this to implement custom lookup logic, e.g. to look up types that are not currently registered in the RTTI system.
  template <typename T>
