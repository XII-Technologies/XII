...
///
/// Retrieve the system configuration by using xiiSystemInformation::Get(). If you use the system configuration in startup code
/// make sure to add the correct dependency to the system "SystemInformation" in "Foundation".
class XII_FOUNDATION_DLL xiiSystemInformation
{
public:
  /// Returns the installed physical memory in bytes
  xiiUInt64 GetInstalledMainMemory() const { return m_uiInstalledMainMemory; }
  ...
  xiiUInt32 GetMemoryPageSize() const { return m_uiMemoryPageSize; }

  /// Returns the CPU core count of the system.
  xiiUInt32 GetCPUCoreCount() const { return m_uiCPUCoreCount; }
  ...
  const xiiCpuFeatures& GetCpuFeatures() const { return m_CpuFeatures; }
...
public:
  ...
  static const xiiSystemInformation& Get()
  {
...
private:
  ...
  xiiCpuFeatures m_CpuFeatures;

  static void Initialize();

  static xiiSystemInformation s_SystemInformation;
};
