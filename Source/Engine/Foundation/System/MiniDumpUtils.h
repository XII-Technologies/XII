...
struct XII_FOUNDATION_DLL xiiMiniDumpUtils
{
  ...
  static xiiStatus WriteExternalProcessMiniDump(const char* szDumpFile, xiiUInt32 uiProcessID);
  ...
  static xiiStatus LaunchMiniDumpTool(const char* szDumpFile);

#if XII_ENABLED(XII_PLATFORM_WINDOWS_DESKTOP)
  /// \brief Windows-specific implementation for writing a mini-dump of the running process.
  ///
  /// \sa WriteProcessMiniDump()
  static xiiStatus WriteOwnProcessMiniDump(const char* szDumpFile, struct _EXCEPTION_POINTERS* pExceptionInfo);
