...
///
/// Once a waiting thread is woken up, the signal state is cleared automatically, unless the xiiThreadSignal uses 'ManualReset' mode.
...
///
/// If an already raised xiiThreadSignal is raised again, this has no effect.
class XII_FOUNDATION_DLL xiiThreadSignal
{
  XII_DISALLOW_COPY_AND_ASSIGN(xiiThreadSignal);
...
private:
  ...
  mutable xiiConditionVariable m_ConditionVariable;
};
