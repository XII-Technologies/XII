/// Copyright (c) Theophilus Eriata. All Rights Reserved.

#pragma once

#include <Foundation/Basics.h>
#include <Foundation/Utilities/EnumerableClass.h>

/// Describes the different stages during startup and shutdown
struct xiiStartupStage
{
  enum Enum
  {
    None = -1,
    BaseSystems,      ///< In this stage the absolute base functionality is started. This should only be used by the Foundation library.
    CoreSystems,      ///< In this stage the core functionality is being started / shut down
    HighLevelSystems, ///< In this stage the higher level functionality, which depends on a rendering context, is being started / shut down

    ENUM_COUNT
  };
};

/// Base class for all subsystems.
///
/// xiiStartup will initialize and shut down all instances of this class, according to their dependencies etc.
/// If you have a subsystem that is a non-static class, just derive from this base class and override the
/// virtual functions as required.
/// If you have a subsystem that is implemented in a purely static way (there is no class instance),
/// just use the macros XII_BEGIN_SUBSYSTEM_DECLARATION, XII_END_SUBSYSTEM_DECLARATION etc.
/// Those macros will create a wrapper object (derived from xiiSubSystem) to handle initialization.
class XII_FOUNDATION_DLL xiiSubSystem : public xiiEnumerable<xiiSubSystem>
{
  XII_DECLARE_ENUMERABLE_CLASS(xiiSubSystem);
  XII_DISALLOW_COPY_AND_ASSIGN(xiiSubSystem);

public:
  xiiSubSystem()
  {
    for (xiiInt32 i = 0; i < xiiStartupStage::ENUM_COUNT; ++i)
      m_bStartupDone[i] = false;
  }

  virtual ~xiiSubSystem() = default;

  /// Returns the name of the subsystem. Must be overridden.
  virtual xiiStringView GetSubSystemName() const = 0;

  /// Returns the name of the group to which this subsystem belongs. Must be overridden.
  virtual xiiStringView GetGroupName() const = 0;

  /// Returns a series of strings with the names of the subsystem, which this subsystem depends on. nullptr indicates the last entry.
  /// Must be overridden.
  virtual xiiStringView GetDependency(xiiInt32 iDep)
  {
    XII_IGNORE_UNUSED(iDep);
    return {};
  }

  /// Returns the plugin name to which this subsystem belongs.
  xiiStringView GetPluginName() const { return m_sPluginName; }

  /// Returns whether the given startup stage has been done on this subsystem.
  bool IsStartupPhaseDone(xiiStartupStage::Enum stage) const { return m_bStartupDone[stage]; }

private:
  // only the startup system may access the following functionality
  friend class xiiStartup;

  /// This will be called to initialize the subsystems base components. Can be overridden to handle this event.
  virtual void OnBaseSystemsStartup() {}

  /// This will be called to initialize the subsystems core components. Can be overridden to handle this event.
  virtual void OnCoreSystemsStartup() {}

  /// This will be called to shut down the subsystems core components. Can be overridden to handle this event.
  virtual void OnCoreSystemsShutdown() {}

  /// This will be called to initialize the subsystems engine / rendering components. Can be overridden to handle this event.
  virtual void OnHighLevelSystemsStartup() {}

  /// This will be called to shut down the subsystems engine / rendering components. Can be overridden to handle this event.
  virtual void OnHighLevelSystemsShutdown() {}

  /// Set by xiiStartup to store to which plugin this subsystem belongs.
  xiiStringView m_sPluginName;

  /// Stores which startup phase has been done already.
  bool m_bStartupDone[xiiStartupStage::ENUM_COUNT];
};

#include <Foundation/Configuration/StaticSubSystem.h>
