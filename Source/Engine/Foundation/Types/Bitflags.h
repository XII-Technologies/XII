...
/// \brief Similar to xiiBitflags but without type-safety.
///
...
template <typename T>
struct xiiTypelessBitflags
{
public:
  using StorageType = T;

  /// \brief Initializes the flags zero.
  XII_ALWAYS_INLINE xiiTypelessBitflags() // [tested]
    :
    m_Value(0)
  {
  }

  XII_ALWAYS_INLINE xiiTypelessBitflags(StorageType flags) // [tested]
  {
    m_Value = flags;
  }

  XII_ALWAYS_INLINE void operator=(StorageType flags) // [tested]
  {
    m_Value = flags;
  }
  ...
  /// \brief Returns whether all the given flags are set.
  XII_ALWAYS_INLINE bool AreAllSet(const xiiTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == rhs.m_Value;
  }

  /// \brief Returns whether none of the given flags is set.
  XII_ALWAYS_INLINE bool AreNoneSet(const xiiTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == 0;
  }
  ...
  /// \brief Sets the given flag.
  XII_ALWAYS_INLINE void Add(const xiiTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }

  /// \brief Removes the given flag.
  XII_ALWAYS_INLINE void Remove(const xiiTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value &= (~rhs.m_Value);
  }

  /// \brief Toggles the state of the given flag.
  XII_ALWAYS_INLINE void Toggle(const xiiTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value ^= rhs.m_Value;
  }
  ...
  /// \brief Returns an object that has the flags of \a this and \a rhs combined.
  XII_ALWAYS_INLINE xiiTypelessBitflags<StorageType> operator|(const xiiTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return xiiTypelessBitflags<StorageType>(m_Value | rhs.m_Value);
  }
  ...
  /// \brief Modifies \a this to also contain the bits from \a rhs.
  XII_ALWAYS_INLINE void operator|=(const xiiTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }
