...
xiiGALTextureDX11::xiiGALTextureDX11(const xiiGALTextureCreationDescription& Description) :
  xiiGALTexture(Description), m_pDXTexture(nullptr), m_pDXStagingTexture(nullptr), m_pExisitingNativeObject(Description.m_pExisitingNativeObject)
{
}

xiiGALTextureDX11::~xiiGALTextureDX11() {}

XII_DEFINE_AS_POD_TYPE(D3D11_SUBRESOURCE_DATA);

xiiResult xiiGALTextureDX11::InitPlatform(xiiGALDevice* pDevice, xiiArrayPtr<xiiGALSystemMemoryDescription> pInitialData)
{
  ...
  if (m_pExisitingNativeObject != nullptr)
  {
    ...
    {
      xiiResult res = CreateStagingTexture(pDXDevice);
      if (res == XII_FAILURE)
      {
  ...
  switch (m_Description.m_Type)
  {
    ...
    case xiiGALTextureType::TextureCubeArray:
    {
      ...
      if (m_Description.m_bCreateRenderTarget || m_Description.m_bAllowDynamicMipGeneration)
        Tex2DDesc.BindFlags |= xiiGALResourceFormat::IsDepthFormat(m_Description.m_Format) ? D3D11_BIND_DEPTH_STENCIL : D3D11_BIND_RENDER_TARGET;
      ...
      xiiHybridArray<D3D11_SUBRESOURCE_DATA, 16> InitialData;
    ...
    case xiiGALTextureType::Texture3D:
    {
      ...
      if (m_Description.m_bCreateRenderTarget)
        Tex3DDesc.BindFlags |= xiiGALResourceFormat::IsDepthFormat(m_Description.m_Format) ? D3D11_BIND_DEPTH_STENCIL : D3D11_BIND_RENDER_TARGET;
...
xiiResult xiiGALTextureDX11::DeInitPlatform(xiiGALDevice* pDevice)
{
...
XII_STATICLINK_FILE(RendererDX11, RendererDX11_Resources_Implementation_TextureDX11);
