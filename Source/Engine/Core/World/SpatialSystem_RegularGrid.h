...
private:
  ...
  void FindVisibleObjects(const xiiFrustum& frustum, const QueryParams& queryParams, xiiDynamicArray<const xiiGameObject*>& out_Objects, xiiSpatialSystem::IsOccludedFunc IsOccluded) const override;

  xiiUInt64 GetNumFramesSinceVisible(const xiiSpatialDataHandle& hData) const override;
  ...
  struct Stats;
  using CellCallback = xiiDelegate<xiiVisitorExecution::Enum(const Cell&, const QueryParams&, Stats&, void*)>;
  void ForEachCellInBoxInMatchingGrids(const xiiSimdBBox& box, const QueryParams& queryParams, CellCallback noFilterCallback, CellCallback filterByTagsCallback, void* pUserData) const;
