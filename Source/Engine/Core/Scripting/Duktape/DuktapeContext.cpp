...
xiiDuktapeContext::~xiiDuktapeContext()
{
  DestroyContext();
}

void xiiDuktapeContext::EnableModuleSupport(duk_c_function pModuleSearchFunction)
{
...
void xiiDuktapeContext::InitializeContext()
{
  XII_ASSERT_ALWAYS(m_pContext == nullptr, "Duktape context should be null");

  m_pContext = duk_create_heap(DukAlloc, DukRealloc, DukFree, this, FatalErrorHandler);

  XII_ASSERT_ALWAYS(m_pContext != nullptr, "Duktape context could not be created");
}

void xiiDuktapeContext::DestroyContext()
{
...
void* xiiDuktapeContext::DukAlloc(void* pUserData, size_t size)
{
  XII_ASSERT_DEBUG(size > 0, "Invalid allocation");

  xiiDuktapeContext* pDukWrapper = reinterpret_cast<xiiDuktapeContext*>(pUserData);

  return pDukWrapper->m_Allocator.Allocate(size, 8);
}

void* xiiDuktapeContext::DukRealloc(void* pUserData, void* pPointer, size_t size)
{
  xiiDuktapeContext* pDukWrapper = reinterpret_cast<xiiDuktapeContext*>(pUserData);
...
void xiiDuktapeContext::DukFree(void* pUserData, void* pPointer)
{
  if (pPointer == nullptr)
    return;
