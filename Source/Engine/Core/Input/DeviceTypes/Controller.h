...
class XII_CORE_DLL xiiInputDeviceController : public xiiInputDevice
{
  XII_ADD_DYNAMIC_REFLECTION(xiiInputDeviceController, xiiInputDevice);
...
public:
  ...
  void SetVibrationStrength(xiiUInt8 uiVirtual, Motor::Enum eMotor, float fValue);

  /// \brief Returns the amount of (constant) vibration that is currently set on this controller.
  float GetVibrationStrength(xiiUInt8 uiVirtual, Motor::Enum eMotor);
  ...
  void AddVibrationTrack(xiiUInt8 uiVirtual, Motor::Enum eMotor, float* fVibrationTrackValue, xiiUInt32 uiSamples, float fScalingFactor = 1.0f);
