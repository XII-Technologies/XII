...
///
/// All input handling is usually handled through the xiiInputManager class.
...
///
/// By deriving from xiiInputDevice you can extend what hardware the engine supports. The derived class should override
...
class XII_CORE_DLL xiiInputDevice : public xiiEnumerable<xiiInputDevice, xiiReflectedClass>
{
  XII_DECLARE_ENUMERABLE_CLASS_WITH_BASE(xiiInputDevice, xiiReflectedClass);
...
protected:
  ...
  static void RegisterInputSlot(const char* szName, const char* szDefaultDisplayName, xiiBitflags<xiiInputSlotFlags> SlotFlags); // [tested]
