...
struct XII_CORE_DLL xiiCollectionEntry
{
  ...
  xiiString       m_sResourceID;             ///< The ID / path to the resource to load.
  xiiHashedString m_sAssetTypeName;
...
///
/// \note Once xiiCollectionResource::PreloadResources() has been triggered, the resource will hold a handle to all referenced resources,
...
///       should get freed up.
class XII_CORE_DLL xiiCollectionResource : public xiiResource
{
  XII_ADD_DYNAMIC_REFLECTION(xiiCollectionResource, xiiResource);
  XII_RESOURCE_DECLARE_COMMON_CODE(xiiCollectionResource);
  XII_RESOURCE_DECLARE_CREATEABLE(xiiCollectionResource, xiiCollectionResourceDescriptor);
...
public:
  ...
  /// \brief Puts every resource for which a resource type could be found into the preload queue of the xiiResourceManager
  ///
  /// This has to be called manually (or triggered by an xiiCollectionComponent).
...
private:
  ...
  xiiCollectionResourceDescriptor            m_Collection;
  xiiDynamicArray<xiiTypelessResourceHandle> m_PreloadedResources;
};
