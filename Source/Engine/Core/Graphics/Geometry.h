...
class XII_CORE_DLL xiiGeometry
{
public:
  ...
  struct Polygon
  {
    ...
    xiiVec3                      m_vNormal;
    xiiHybridArray<xiiUInt32, 4> m_Vertices;
  };
  ...
  struct Line
  {
    ...
    xiiUInt32 m_uiStartVertex;
    xiiUInt32 m_uiEndVertex;
  };
  ...
  xiiDeque<Vertex>& GetVertices() { return m_Vertices; }

  /// \brief Returns the entire polygon data.
  xiiDeque<Polygon>& GetPolygons() { return m_Polygons; }

  /// \brief Returns the entire line data.
  xiiDeque<Line>& GetLines() { return m_Lines; }

  /// \brief Returns the entire vertex data.
  const xiiDeque<Vertex>& GetVertices() const { return m_Vertices; }

  /// \brief Returns the entire polygon data.
  const xiiDeque<Polygon>& GetPolygons() const { return m_Polygons; }

  /// \brief Returns the entire line data.
  const xiiDeque<Line>& GetLines() const { return m_Lines; }

  /// \brief Clears all data.
  void Clear();

  /// \brief Adds a vertex, returns the index to the added vertex.
  xiiUInt32 AddVertex(const xiiVec3& vPos, const xiiVec3& vNormal, const xiiVec2& vTexCoord, const xiiColor& color, const xiiVec4U16& boneIndices = xiiVec4U16::ZeroVector(), const xiiColorLinearUB& boneWeights = xiiColorLinearUB(255, 0, 0, 0));

  /// \brief Adds a vertex, returns the index to the added vertex. Position and normal are transformed with the given matrix.
  xiiUInt32 AddVertex(const xiiVec3& vPos, const xiiVec3& vNormal, const xiiVec2& vTexCoord, const xiiColor& color, const xiiVec4U16& boneIndices, const xiiColorLinearUB& boneWeights, const xiiMat4& mTransform)
  {
  ...
  xiiUInt32 AddVertex(const xiiVec3& vPos, const xiiVec3& vNormal, const xiiVec2& vTexCoord, const xiiColor& color, const xiiUInt16 boneIndex, const xiiMat4& mTransform)
  {
    return AddVertex(mTransform.TransformPosition(vPos), mTransform.TransformDirection(vNormal).GetNormalized(), vTexCoord, color, xiiVec4U16(boneIndex, 0, 0, 0), xiiColorLinearUB(255, 0, 0, 0));
  }

  /// \brief Adds a vertex with a single bone index, returns the index to the added vertex. Position and normal are transformed with the given matrix.
  xiiUInt32 AddVertex(const xiiVec3& vPos, const xiiVec3& vNormal, const xiiVec2& vTexCoord, const GeoOptions& options)
  {
    return AddVertex(options.m_Transform.TransformPosition(vPos), options.m_Transform.TransformDirection(vNormal).GetNormalized(), vTexCoord, options.m_Color, xiiVec4U16(options.m_uiBoneIndex, 0, 0, 0), xiiColorLinearUB(255, 0, 0, 0));
  }
  ...
  void SetAllVertexBoneIndices(const xiiVec4U16& boneIndices, xiiUInt32 uiFirstVertex = 0);
  ...
  void SetAllVertexTexCoord(const xiiVec2& texCoord, xiiUInt32 uiFirstVertex = 0);
  ...
  void AddRectXY(const xiiVec2& size, xiiUInt32 uiTesselationX = 1, xiiUInt32 uiTesselationY = 1, const GeoOptions& options = GeoOptions());
  ...
  void AddLineBox(const xiiVec3& size, const GeoOptions& options = GeoOptions());
  ...
  void AddLineBoxCorners(const xiiVec3& size, float fCornerFraction, const GeoOptions& options = GeoOptions());
  ...
  void AddPyramid(const xiiVec3& size, bool bCap, const GeoOptions& options = GeoOptions());
  ...
  void AddSphere(float fRadius, xiiUInt16 uiSegments, xiiUInt16 uiStacks, const GeoOptions& options = GeoOptions());
  ...
  void AddTexturedRamp(const xiiVec3& size, const GeoOptions& options = GeoOptions());

  /// \brief Generates a straight stair mesh along the X axis. The number of steps determines the step height and depth.
  void AddStairs(const xiiVec3& size, xiiUInt32 uiNumSteps, xiiAngle curvature, bool bSmoothSloped, const GeoOptions& options = GeoOptions());

  void AddArch(const xiiVec3& size, xiiUInt32 uiNumSegments, float fThickness, xiiAngle angle, bool bMakeSteps, bool bSmoothBottom, bool bSmoothTop, bool bCapTopAndBottom, const GeoOptions& options = GeoOptions());
...
private:
  void TransformVertices(const xiiMat4& mTransform, xiiUInt32 uiFirstVertex);

  xiiDeque<Vertex>  m_Vertices;
  xiiDeque<Polygon> m_Polygons;
  xiiDeque<Line>    m_Lines;
};
