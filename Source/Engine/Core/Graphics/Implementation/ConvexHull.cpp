...
xiiConvexHullGenerator::xiiConvexHullGenerator()  = default;
xiiConvexHullGenerator::~xiiConvexHullGenerator() = default;

xiiResult xiiConvexHullGenerator::ComputeCenterAndScale(const xiiArrayPtr<const xiiVec3> vertices)
{
  if (vertices.IsEmpty())
    return XII_FAILURE;
  ...
  const xiiVec3 ext = box.GetHalfExtents();

  const double minExt = xiiMath::Min(ext.x, ext.y, ext.z);

  if (minExt <= 0.000001)
    return XII_FAILURE;

  const double maxExt = xiiMath::Max(ext.x, ext.y, ext.z);
...
xiiResult xiiConvexHullGenerator::StoreNormalizedVertices(const xiiArrayPtr<const xiiVec3> vertices)
{
  struct Comparer
  {
    XII_ALWAYS_INLINE bool Less(const xiiVec3d& a, const xiiVec3d& b) const
    {
  ...
  xiiSet<xiiVec3d, Comparer> used;

  m_Vertices.Clear();
  m_Vertices.Reserve(vertices.GetCount());

  for (xiiVec3 v : vertices)
  {
    xiiVec3d norm;
...
void xiiConvexHullGenerator::StoreTriangle(xiiUInt16 i, xiiUInt16 j, xiiUInt16 k)
{
  Triangle& triangle = m_Triangles.ExpandAndGetRef();

  XII_ASSERT_DEBUG((i < j) && (i < k) && (j < k), "Invalid Triangle");

  xiiVec3d tangent1 = m_Vertices[k] - m_Vertices[i];
  xiiVec3d tangent2 = m_Vertices[j] - m_Vertices[i];
  ...
  if (triangle.m_bIsDegenerate)
  {
    ...
    const xiiVec3d orth = m_Vertices[i] - m_vInside;
    tangent2            = tangent1.CrossRH(orth);
    triangle.m_vNormal  = tangent1.CrossRH(tangent2);

    XII_ASSERT_DEBUG(!triangle.m_vNormal.IsZero(0.0000001), "Normal is still invalid");
  }
  ...
  const xiiUInt32 uiMaxVertices = m_Vertices.GetCount();
...
xiiResult xiiConvexHullGenerator::InitializeHull()
{
  xiiVec3d  minV  = m_Vertices[0];
  xiiVec3d  maxV  = m_Vertices[0];
  xiiUInt32 minIx = 0;
  xiiUInt32 minIy = 0;
  xiiUInt32 minIz = 0;
  xiiUInt32 maxIx = 0;
  ...
  for (xiiUInt32 i = 0; i < m_Vertices.GetCount(); ++i)
  {
  ...
  const xiiVec3d extents = maxV - minV;
  xiiUInt32      uiMainAxis1;
  xiiUInt32      uiMainAxis2;
  ...
  if (testIdx.GetCount() < 4)
  {
    ...
    xiiUInt32 uiMaxVts = xiiMath::Min(50U, m_Vertices.GetCount());
    for (xiiUInt32 i = 0; i < uiMaxVts; ++i)
    {
      testIdx.PushBack(i);
    }
  }

  xiiVec3d planePoints[3];
  planePoints[0] = m_Vertices[testIdx[0]];
  planePoints[1] = m_Vertices[testIdx[1]];

  double    maxDist = 0;
  xiiUInt32 uiIx1 = 0xFFFFFFFF, uiIx2 = 0xFFFFFFFF;

  for (xiiUInt32 i = 2; i < testIdx.GetCount(); ++i)
  {
    ...
    for (xiiUInt32 j = 2; j < testIdx.GetCount(); ++j)
    {
      if (i == j)
        continue;

      const double thisDist = xiiMath::Abs(p.GetDistanceTo(m_Vertices[testIdx[j]]));
      if (thisDist > maxDist)
      {
        maxDist = thisDist;
        uiIx1   = testIdx[i];
  ...
  for (int i = 0; i < 4; ++i)
  {
    if (i > 0)
    {
      XII_ASSERT_DEBUG(testIdx[i - 1] != testIdx[i], "Same index used twice");
    }

    xiiMath::Swap(m_Vertices[i], m_Vertices[testIdx[i]]);
  }

  // precompute the 'inside' position
  {
    m_vInside.SetZero();
    for (xiiUInt32 v = 0; v < 4; ++v)
      m_vInside += m_Vertices[v];
    m_vInside /= 4.0;
  }

  // construct the hull as containing only the first four points
  for (xiiUInt16 i = 0; i < 4; i++)
  {
    for (xiiUInt16 j = i + 1; j < 4; j++)
    {
      for (xiiUInt16 k = j + 1; k < 4; k++)
      {
        StoreTriangle(i, j, k);
      }
    }
...
xiiResult xiiConvexHullGenerator::ComputeHull()
{
  const xiiUInt32 uiMaxVertices = m_Vertices.GetCount();
  ...
  m_Edges.Clear();
  m_Edges.SetCount(xiiMath::Square(uiMaxVertices));
  m_Triangles.Clear();
  m_Triangles.Reserve(512);

  XII_SUCCEED_OR_RETURN(InitializeHull());

  // Add the points to the hull, one at a time.
  for (xiiUInt32 vtxId = 4; vtxId < uiMaxVertices; ++vtxId)
  {
    if (IsInside(vtxId))
      continue;
...
bool xiiConvexHullGenerator::IsInside(xiiUInt32 vtxId) const
{
  const xiiVec3d pos = m_Vertices[vtxId];

  const xiiInt32 iNumTriangles = m_Triangles.GetCount();
  for (xiiInt32 j = 0; j < iNumTriangles; j++)
  {
...
void xiiConvexHullGenerator::RemoveVisibleFaces(xiiUInt32 vtxId)
{
  const xiiUInt32 uiMaxVertices = m_Vertices.GetCount();
  const xiiVec3d  pos           = m_Vertices[vtxId];

  xiiInt32 iNumTriangles = m_Triangles.GetCount();
  for (xiiInt32 j = 0; j < iNumTriangles; j++)
  {
    const auto& tri = m_Triangles[j];
    ...
    const xiiUInt16 vtx0 = tri.m_uiVertexIdx[0];
    const xiiUInt16 vtx1 = tri.m_uiVertexIdx[1];
    const xiiUInt16 vtx2 = tri.m_uiVertexIdx[2];
...
void xiiConvexHullGenerator::PatchHole(xiiUInt32 vtxId)
{
  XII_ASSERT_DEBUG(vtxId < 0xFFFFu, "Vertex Id is larger than 16 bits can address.");
  const xiiUInt32 uiMaxVertices = m_Vertices.GetCount();

  const xiiUInt32 uiNumFaces = m_Triangles.GetCount();
  for (xiiUInt32 j = 0; j < uiNumFaces; j++)
  {
    const auto& tri = m_Triangles[j];

    for (xiiInt32 a = 0; a < 3; a++)
    {
      for (xiiInt32 b = a + 1; b < 3; b++)
      {
        const xiiUInt16 vtxA = tri.m_uiVertexIdx[a];
        const xiiUInt16 vtxB = tri.m_uiVertexIdx[b];

        if (m_Edges[vtxA * uiMaxVertices + vtxB].GetSize() == 2)
          continue;

        StoreTriangle(vtxA, vtxB, static_cast<xiiUInt16>(vtxId));
      }
    }
  }
}

bool xiiConvexHullGenerator::PruneFlatVertices(double fNormalThreshold)
{
  struct VertexNormals
  {
    xiiVec3d m_vNormals[2];
    xiiInt32 m_iDifferentNormals = 0;
  };

  xiiDynamicArray<VertexNormals> VtxNorms;
  VtxNorms.SetCount(m_Vertices.GetCount());

  xiiUInt32 uiNumVerticesRemaining = 0;
  ...
  for (const auto& tri : m_Triangles)
  {
    ...
    const xiiVec3d planeNorm = tri.m_vNormal;
  ...
  xiiDynamicArray<xiiVec3d> remaining;
...
bool xiiConvexHullGenerator::PruneDegenerateTriangles(double fMaxCosAngle)
{
  bool bChanged = false;

  xiiDynamicBitfield discardVtx;
  discardVtx.SetCount(m_Vertices.GetCount(), false);

  for (const auto& tri : m_Triangles)
  {
    const xiiUInt32 idx0 = tri.m_uiVertexIdx[0];
    const xiiUInt32 idx1 = tri.m_uiVertexIdx[1];
    const xiiUInt32 idx2 = tri.m_uiVertexIdx[2];
    const xiiVec3d  v0   = m_Vertices[idx0];
    const xiiVec3d  v1   = m_Vertices[idx1];
    const xiiVec3d  v2   = m_Vertices[idx2];
    const xiiVec3d  e0   = (v1 - v0).GetNormalized();
    const xiiVec3d  e1   = (v2 - v1).GetNormalized();
    const xiiVec3d  e2   = (v0 - v2).GetNormalized();
  ...
  if (bChanged)
  {
    for (xiiUInt32 n = discardVtx.GetCount(); n > 0; --n)
    {
...
bool xiiConvexHullGenerator::PruneSmallTriangles(double fMaxEdgeLen)
{
  bool bChanged = false;
  ...
  for (const auto& tri : m_Triangles)
  {
    ...
    if (len0 < fMaxEdgeLen && len1 < fMaxEdgeLen && len2 < fMaxEdgeLen)
    {
      ...
      const xiiVec3d center = (v0 + v1 + v2) / 3.0;
    ...
    if (len0 < fMaxEdgeLen)
    {
      ...
      const xiiVec3d center = (v0 + v1) / 2.0;
      m_Vertices.PushBack(center);
    ...
    if (len1 < fMaxEdgeLen)
    {
      ...
      const xiiVec3d center = (v2 + v1) / 2.0;
      m_Vertices.PushBack(center);
    ...
    if (len2 < fMaxEdgeLen)
    {
      ...
      const xiiVec3d center = (v0 + v2) / 2.0;
      m_Vertices.PushBack(center);
...
xiiResult xiiConvexHullGenerator::ProcessVertices(const xiiArrayPtr<const xiiVec3> vertices)
{
  xiiUInt32       uiFirstVertex      = 0;
  xiiUInt32       uiNumVerticesLeft  = vertices.GetCount();
  const xiiUInt32 uiVerticesPerBatch = 1000;

  xiiDynamicArray<xiiVec3> workingSet;

  while (uiNumVerticesLeft > 0)
  {
    RetrieveVertices(workingSet);

    const xiiUInt32                  uiAdd = xiiMath::Min(uiNumVerticesLeft, uiVerticesPerBatch);
    const xiiArrayPtr<const xiiVec3> range = vertices.GetSubArray(uiFirstVertex, uiAdd);
    workingSet.PushBackRange(range);

    uiFirstVertex += uiAdd;
    uiNumVerticesLeft -= uiAdd;

    XII_SUCCEED_OR_RETURN(StoreNormalizedVertices(workingSet));

    if (m_Vertices.GetCount() >= 16384)
      return XII_FAILURE;

    XII_SUCCEED_OR_RETURN(ComputeHull());
  }
...
xiiResult xiiConvexHullGenerator::Build(const xiiArrayPtr<const xiiVec3> vertices)
{
  m_Vertices.Clear();

  XII_SUCCEED_OR_RETURN(ComputeCenterAndScale(vertices));

  XII_SUCCEED_OR_RETURN(ProcessVertices(vertices));
  ...
  while (prune)
  {
    ...
    if (PruneDegenerateTriangles(xiiMath::Cos(m_MinTriangleAngle)))
    {
      XII_SUCCEED_OR_RETURN(ComputeHull());
      prune = true;
    }

    if (PruneFlatVertices(xiiMath::Cos(m_FlatVertexNormalThreshold)))
    {
      XII_SUCCEED_OR_RETURN(ComputeHull());
      prune = true;
    }
...
void xiiConvexHullGenerator::Retrieve(xiiDynamicArray<xiiVec3>& out_Vertices, xiiDynamicArray<Face>& out_Faces)
{
  ...
  for (const auto& tri : m_Triangles)
  {
    ...
    for (int v = 0; v < 3; ++v)
    {
      const xiiUInt32 orgIdx = tri.m_uiVertexIdx[v];
      ...
      face.m_uiVertexIdx[v] = static_cast<xiiUInt16>(it.Value());
    }

    if (tri.m_bFlip)
    {
      xiiMath::Swap(face.m_uiVertexIdx[1], face.m_uiVertexIdx[2]);
    }
  }
}
