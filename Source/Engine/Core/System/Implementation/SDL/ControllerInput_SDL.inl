...
namespace
{
  ...
  SDL_GameController*                 GameControllers[xiiControllerInputSDL::MaxControllers];
...
XII_CHECK_AT_COMPILETIME(XII_ARRAY_SIZE(szControllerName) >= xiiControllerInputSDL::MaxControllers);
...
void xiiControllerInputSDL::UpdateInputSlotValues()
{
  ...
  for (xiiInt32 iPhysical = 0; iPhysical < MaxControllers; ++iPhysical)
  {
    ...
    {
      bool bIsGameController = (SDL_IsGameController(iPhysical) == SDL_TRUE);

      SDL_GameController* pGameController = SDL_GameControllerOpen(iPhysical);
      if (GameControllers[iPhysical] != pGameController)
      {
        SDL_GameControllerClose(GameControllers[iPhysical]);
        GameControllers[iPhysical] = pGameController;

        xiiLog::Dev("New instance of game controller opened for '{0}'", iPhysical);
      }

      bIsAvailable[iPhysical] = bIsGameController && (GameControllers[iPhysical] != nullptr);
  ...
  for (xiiUInt8 uiVirtual = 0; uiVirtual < MaxControllers; ++uiVirtual)
  {
    ...
    for (size_t buttonIndex = 0; buttonIndex < XII_ARRAY_SIZE(g_ControllerButtonMappings); buttonIndex++)
    {
      ...
      xiiUInt8                      buttonState = SDL_GameControllerGetButton(GameControllers[iPhysical], mapping.sdlButton);
      SetValue(iPhysical, mapping.xiiName, (buttonState == 1) ? 1.0f : 0.0f);
    }

    // Update game controller trigger buttons
    float lTriggerState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_TRIGGERLEFT));
    float rTriggerState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_TRIGGERRIGHT));
    SetValue(iPhysical, "left_trigger", lTriggerState);
    SetValue(iPhysical, "right_trigger", rTriggerState);

    // Update game controller left stick
    float lXState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_LEFTX));
    float lYState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_LEFTY));
    ...
    float rXState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_RIGHTX));
    float rYState = static_cast<float>(SDL_GameControllerGetAxis(GameControllers[iPhysical], SDL_CONTROLLER_AXIS_RIGHTY));
...
void xiiControllerInputSDL::ApplyVibration(xiiUInt8 uiPhysicalController, Motor::Enum eMotor, float fStrength)
{
  ...
  if (SDL_GameControllerHasRumble(GameControllers[uiPhysicalController]) == SDL_FALSE)
  {
    xiiLog::Info("Connected controller at '{}' does not have Rumble support.", uiPhysicalController);
    return;
  }
  ...
  switch (eMotor)
  {
    ...
    {
      xiiInt32 result = SDL_GameControllerRumble(GameControllers[uiPhysicalController], static_cast<xiiUInt16>(fStrength), 0, 100);
      if (result < 0)
    ...
    {
      xiiInt32 result = SDL_GameControllerRumble(GameControllers[uiPhysicalController], 0, static_cast<xiiUInt16>(fStrength), 100);
      if (result < 0)
        xiiLog::Info("Encountered an error setting rumble on SDL Controller '{0}'.", SDL_GetError());
    }
    break;
