...
/// \brief The central class for managing all types derived from xiiResource
...
public:
  ...
  ///
  /// \sa RestoreResource()
  static void UpdateResourceWithCustomLoader(const xiiTypelessResourceHandle& hResource, xiiUniquePtr<xiiResourceTypeLoader>&& loader);
...
public:
  /// \brief Acquires a resource pointer from a handle. Prefer to use xiiResourceLock, which wraps BeginAcquireResource / EndAcquireResource
  ///
...
public:
  ...
  /// \brief Calls xiiResource::ResetResource() on all resources.
  ///
  ...
  /// \brief Calls xiiResource::UpdateContent() to fill the resource with 'low resolution' data
  ///
...
public:
  ...
  /// \brief [internal] Used by xiiResource to register a cleanup function to be called at resource manager shutdown.
