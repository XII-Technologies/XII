...
xiiGALBlendStateD3D12::xiiGALBlendStateD3D12(xiiSharedPtr<xiiGALDeviceD3D12> pDeviceD3D12, const xiiGALBlendStateCreationDescription& creationDescription) :
  xiiGALBlendState(std::move(pDeviceD3D12), creationDescription)
{
}
...
xiiResult xiiGALBlendStateD3D12::InitPlatform()
{
  ...
  for (xiiUInt32 uiAttachmentIndex = 0; uiAttachmentIndex < m_Description.m_RenderTargets.GetCount(); ++uiAttachmentIndex)
  {
    ...
    D3D12_RENDER_TARGET_BLEND_DESC&     rtAttachmentState = m_BlendState.RenderTarget[uiAttachmentIndex];
