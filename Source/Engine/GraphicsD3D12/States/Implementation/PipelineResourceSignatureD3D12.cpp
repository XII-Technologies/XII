...
xiiGALPipelineResourceSignatureD3D12::xiiGALPipelineResourceSignatureD3D12(const xiiGALPipelineResourceSignatureCreationDescription& creationDescription) :
...
xiiResult xiiGALPipelineResourceSignatureD3D12::InitPlatform(xiiGALDevice* pDevice)
{
  ...
  pDeviceD3D12->GetDevice()->CreatePipelineResourceSignature(pipelineResourceSignatureDescription, &m_pPipelineResourceSignature);

  return (m_pPipelineResourceSignature != nullptr) ? XII_SUCCESS : XII_FAILURE;
}

xiiResult xiiGALPipelineResourceSignatureD3D12::DeInitPlatform(xiiGALDevice* pDevice)
{
