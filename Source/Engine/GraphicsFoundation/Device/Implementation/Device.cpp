...
xiiSharedPtr<xiiGALTexture> xiiGALDevice::CreateTexture(const xiiGALTextureCreationDescription& description, const xiiGALTextureData* pInitialData /* = nullptr*/)
{
  ...
  if (description.m_MiscFlags.IsSet(xiiGALMiscTextureFlags::Memoryless))
  {
    ...
    XII_GAL_DEVICE_CHECK(description.m_Usage == xiiGALResourceUsage::Default, "Memoryless attachment requires xiiGALResourceUsage::Default.");
  ...
  if (description.m_BindFlags.IsSet(xiiGALBindFlags::ShadingRate))
  {
    ...
    XII_GAL_DEVICE_CHECK(description.m_Usage == xiiGALResourceUsage::Default || description.m_Usage == xiiGALResourceUsage::Immutable, "Shading rate textures only allow xiiGALResourceUsage::Default or xiiGALResourceUsage::Immutable.");
...
xiiSharedPtr<xiiGALTextureView> xiiGALDevice::CreateTextureView(xiiGALTextureViewCreationDescription& description)
{
  XII_GAL_DEVICE_LOCK_AND_CHECK();

  XII_GAL_DEVICE_CHECK(description.m_pTexture != nullptr, "The texture handle given for texture view creation is invalid.");

  const auto& textureDescription = description.m_pTexture->GetDescription();

  XII_GAL_DEVICE_CHECK(description.m_ViewType > xiiGALTextureViewType::Undefined && description.m_ViewType < xiiGALTextureViewType::ENUM_COUNT, "The texture view type is invalid.");
  XII_GAL_DEVICE_CHECK(description.m_uiMostDetailedMip < textureDescription.m_uiMipLevels, "The most detailed mip ({0}) is out of range. The texture has only {1} mip level (s).", description.m_uiMostDetailedMip, textureDescription.m_uiMipLevels);
  XII_GAL_DEVICE_CHECK((description.m_uiMipLevelCount == XII_GAL_REMAINING_MIP_LEVELS) || ((description.m_uiMostDetailedMip + description.m_uiMipLevelCount) <= textureDescription.m_uiMipLevels), "The most detailed mip ({0}) and the number of mip levels in the view ({1}) is out of range. The texture has only {2} mip level (s).", description.m_uiMostDetailedMip, description.m_uiMipLevelCount, textureDescription.m_uiMipLevels);
  ...
  {
    XII_GAL_DEVICE_CHECK(description.m_uiFirstArrayOrDepthSlice < textureDescription.m_uiArraySizeOrDepth, "The first array slice ({0}) is out of range. The texture has only ({1}) slice (s)", description.m_uiFirstArrayOrDepthSlice, textureDescription.m_uiArraySizeOrDepth);
    XII_GAL_DEVICE_CHECK((description.m_uiArrayOrDepthSlicesCount == XII_GAL_REMAINING_ARRAY_SLICES) || ((description.m_uiFirstArrayOrDepthSlice + description.m_uiArrayOrDepthSlicesCount) <= textureDescription.m_uiArraySizeOrDepth), "The first array slice ({0}) and the number of array slice (s) ({1}) are out of range. The texture has only ({2}) slice (s)", description.m_uiFirstArrayOrDepthSlice, description.m_uiArrayOrDepthSlicesCount, textureDescription.m_uiArraySizeOrDepth);
  }
  else if (!textureDescription.Is3D())
  {
    XII_GAL_DEVICE_CHECK(description.m_uiFirstArrayOrDepthSlice == 0U, "For non-array texture, the First Array or Depth Slice must be zero.");
  }
  ...
  if (description.m_ResourceDimension == xiiGALResourceDimension::Undefined)
  {
    if (textureDescription.m_Type == xiiGALResourceDimension::TextureCube || textureDescription.m_Type == xiiGALResourceDimension::TextureCubeArray)
    {
      switch (description.m_ViewType)
      {
        ...
        default:
          XII_GAL_DEVICE_CHECK(false, "Unexpected view type.");
      }
    }
    else
    {
  ...
  switch (textureDescription.m_Type)
  {
    case xiiGALResourceDimension::Texture1D:
    {
      ...
      XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture 1D view, only xiiGALResourceDimension Texture1D is allowed.");
    }
    break;
    case xiiGALResourceDimension::Texture1DArray:
    {
      ...
      XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture 1D Array view, only xiiGALResourceDimension Texture1D or Texture1DArray is allowed.");
    }
    break;
    case xiiGALResourceDimension::Texture2D:
    {
      ...
      XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture 2D view, only xiiGALResourceDimension Texture2D or Texture2DArray is allowed.");
    }
    break;
    case xiiGALResourceDimension::Texture2DArray:
    {
      const bool bIsValid = description.m_ResourceDimension == xiiGALResourceDimension::Texture2D || description.m_ResourceDimension == xiiGALResourceDimension::Texture2DArray;
      XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture 2D Array view, only xiiGALResourceDimension Texture2D or Texture2DArray is allowed.");
    }
    break;
    case xiiGALResourceDimension::Texture3D:
    {
      const bool bIsValid = description.m_ResourceDimension == xiiGALResourceDimension::Texture3D;
      XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture 3D view, only xiiGALResourceDimension Texture3D is allowed.");
    }
    break;
    case xiiGALResourceDimension::TextureCube:
    {
      if (description.m_ViewType == xiiGALTextureViewType::ShaderResource)
      {
        ...
        XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture Cube shader resource view, only xiiGALResourceDimension Texture2D or Texture2DArray or TextureCube is allowed.");
      }
      else
      {
        const bool bIsValid = description.m_ResourceDimension == xiiGALResourceDimension::Texture2D || description.m_ResourceDimension == xiiGALResourceDimension::Texture2DArray;
        XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture Cube non-shader resource view, only xiiGALResourceDimension Texture2D or Texture2DArray is allowed.");
      }
    }
    break;
    case xiiGALResourceDimension::TextureCubeArray:
    {
      if (description.m_ViewType == xiiGALTextureViewType::ShaderResource)
      {
        ...
        XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture Cube Array shader resource view, only xiiGALResourceDimension Texture2D or Texture2DArray or TextureCube TextureCubeArray is allowed.");
      }
      else
      {
        const bool bIsValid = description.m_ResourceDimension == xiiGALResourceDimension::Texture2D || description.m_ResourceDimension == xiiGALResourceDimension::Texture2DArray;
        XII_GAL_DEVICE_CHECK(bIsValid, "Incorrect texture view type for a Texture Cube Array non-shader resource view, only xiiGALResourceDimension Texture2D or Texture2DArray is allowed.");
      }
    }
    break;

    default:
      XII_GAL_DEVICE_CHECK(false, "Encountered an unexpected view type.");
  }
  ...
  switch (description.m_ResourceDimension)
  {
    case xiiGALResourceDimension::TextureCube:
    {
      XII_GAL_DEVICE_CHECK(description.m_ViewType == xiiGALTextureViewType::ShaderResource, "Unexpected view type, a Shader Resource view is expected.");
      XII_GAL_DEVICE_CHECK((description.m_uiArrayOrDepthSlicesCount == 6U) || (description.m_uiArrayOrDepthSlicesCount == 0U) || (description.m_uiArrayOrDepthSlicesCount == XII_GAL_REMAINING_ARRAY_SLICES), "Texture Cube Shader Resource view is expected to have 6 array slices. {0} are provided.", description.m_uiArrayOrDepthSlicesCount);
    }
    break;
    case xiiGALResourceDimension::TextureCubeArray:
    {
      XII_GAL_DEVICE_CHECK(description.m_ViewType == xiiGALTextureViewType::ShaderResource, "Unexpected view type, a Shader Resource view is expected.");
      XII_GAL_DEVICE_CHECK((description.m_uiArrayOrDepthSlicesCount == XII_GAL_REMAINING_ARRAY_SLICES) || ((description.m_uiArrayOrDepthSlicesCount % 6U) == 0U), "The number of slices in Texture Cube Array Shader Resource view is expected to be a multiple of 6. {0} are provided.", description.m_uiArrayOrDepthSlicesCount);
    }
    break;
    case xiiGALResourceDimension::Texture1D:
    case xiiGALResourceDimension::Texture2D:
    {
      XII_GAL_DEVICE_CHECK((description.m_uiArrayOrDepthSlicesCount == XII_GAL_REMAINING_ARRAY_SLICES) || (description.m_uiArrayOrDepthSlicesCount <= 1U), "The number of slices in the view ({0}) must be 1 (or 0) for non-array Texture 1D/2D views.", description.m_uiArrayOrDepthSlicesCount);
    }
    break;
    case xiiGALResourceDimension::Texture1DArray:
    case xiiGALResourceDimension::Texture2DArray:
    ...
    case xiiGALResourceDimension::Texture3D:
    {
      ...
      XII_GAL_DEVICE_CHECK((description.m_uiFirstArrayOrDepthSlice + description.m_uiArrayOrDepthSlicesCount) <= uiMipDepth, "The first depth slice ({0}) and the number of slices in the view description ({1}), specify more slices than the target 3D texture mip level has ({2}).", description.m_uiFirstArrayOrDepthSlice, description.m_uiArrayOrDepthSlicesCount, uiMipDepth);
    }
    break;

    default:
      XII_GAL_DEVICE_CHECK(false, "Unexpected texture dimension.");
  }

  XII_GAL_DEVICE_CHECK(!xiiGALTextureUtilities::GetResourceFormatProperties(description.m_Format).m_bIsTypeless, "The texture view format ({0}) cannot be typeless.", description.m_Format.GetValue());

  if (description.m_Flags.IsSet(xiiGALTextureViewFlags::AllowMipGeneration))
  {
    XII_GAL_DEVICE_CHECK(textureDescription.m_MiscFlags.IsSet(xiiGALMiscTextureFlags::GenerateMips), "The xiiGALTextureViewFlags::AllowMipGeneration flag can only be set if the texture was created with the xiiGALMiscTextureFlags::GenerateMips flag.");
    XII_GAL_DEVICE_CHECK(description.m_ViewType == xiiGALTextureViewType::ShaderResource, "The xiiGALTextureViewFlags::AllowMipGeneration flag can only used with the xiiGALTextureViewType::ShaderResource view type.");
  }

  if (description.m_ViewType == xiiGALTextureViewType::ShadingRate)
  {
    XII_GAL_DEVICE_CHECK(textureDescription.m_BindFlags.IsSet(xiiGALBindFlags::ShadingRate), "To create a xiiGALTextureViewType::ShadingRate, the texture must be created with the xiiGALBindFlags::ShadingRate flag.");
  }
  ...
  {
    XII_GAL_DEVICE_CHECK(false, "Non-identity texture component swizzle is only supported for Shader Resource views.");
  }
  ...
  return CreateTextureViewPlatform(description.m_pTexture, description);
}
