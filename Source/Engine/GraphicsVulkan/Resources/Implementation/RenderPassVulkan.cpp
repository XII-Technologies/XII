...
xiiGALRenderPassVulkan::xiiGALRenderPassVulkan(const xiiGALRenderPassCreationDescription& creationDescription) :
...
xiiResult xiiGALRenderPassVulkan::InitPlatform(xiiGALDevice* pDevice)
{
  ...
  for (xiiUInt32 i = 0; i < uiSubpassDependencyCount; ++i)
  {
    ...
    /// \todo GraphicsVulkan: Update dependency description with pipeline stage and access flags.
  ...
  pDeviceVulkan->GetDevice()->CreateRenderPass(renderPassDescription, &m_pRenderPass);

  return (m_pRenderPass != nullptr) ? XII_SUCCESS : XII_FAILURE;
}

xiiResult xiiGALRenderPassVulkan::DeInitPlatform(xiiGALDevice* pDevice)
{
