...
void xiiDynamicQuadtree::CreateTree(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, float fMinNodeSize)
{
  ...
  m_uiAddIDTopLevel = 0;
  for (xiiUInt32 i = 0; i < m_uiMaxTreeDepth; ++i)
    m_uiAddIDTopLevel += xiiMath::Pow(4, i);
}
...
xiiResult xiiDynamicQuadtree::InsertObject(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, xiiInt32 iObjectType, xiiInt32 iObjectInstance, xiiDynamicTreeObject* out_pObject, bool bOnlyIfInside)
{
  ...
  m_BBox.m_vMin.y = xiiMath::Min(m_BBox.m_vMin.y, vCenter.y - vHalfExtents.y);
  m_BBox.m_vMax.y = xiiMath::Max(m_BBox.m_vMax.y, vCenter.y + vHalfExtents.y);


  xiiDynamicTree::xiiObjectData oData;
                    ...
                    xiiMath::Pow(4, m_uiMaxTreeDepth - 1), out_pObject))
  {
    if (!bOnlyIfInside)
    {
      xiiDynamicTree::xiiMultiMapKey mmk;
      mmk.m_uiKey     = 0;
...
void xiiDynamicQuadtree::RemoveObject(xiiDynamicTreeObject obj)
{
  m_NodeMap.Remove(obj);
}

void xiiDynamicQuadtree::RemoveObject(xiiInt32 iObjectType, xiiInt32 iObjectInstance)
{
  for (xiiDynamicTreeObject it = m_NodeMap.GetIterator(); it.IsValid(); ++it)
  {
...
void xiiDynamicQuadtree::RemoveObjectsOfType(xiiInt32 iObjectType)
{
  for (xiiDynamicTreeObject it = m_NodeMap.GetIterator(); it.IsValid();)
  {
    if (it.Value().m_iObjectType == iObjectType)
    {
      xiiDynamicTreeObject itold = it;
      ++it;
...
bool xiiDynamicQuadtree::InsertObject(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, const xiiDynamicTree::xiiObjectData& Obj, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiDynamicTreeObject* out_Object)
{
  ...
  if (uiAddID > 0)
  {
    ...
    const xiiUInt32 uiNodeIDBase    = uiNodeID + 1;
    const xiiUInt32 uiAddIDChild    = uiAddID - uiSubAddID;
    const xiiUInt32 uiSubAddIDChild = uiSubAddID / 4;
...
void xiiDynamicQuadtree::FindVisibleObjects(const xiiFrustum& Viewfrustum, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
  ...
  const xiiVolumePosition::Enum pos = Viewfrustum.GetObjectPosition(&v[0], 8);

  // stop traversal, if node is outside view-frustum
  if (pos == xiiVolumePosition::Outside)
    return;
  ...
  xiiDynamicTreeObjectConst it1 = m_NodeMap.LowerBound(mmk);
  ...
  if (pos == xiiVolumePosition::Inside)
  {
    ...
    while (it1.IsValid())
    {
      ...
      xiiDynamicTreeObjectConst temp = it1;
      ++it1;
  ...
  else if (pos == xiiVolumePosition::Intersecting)
  {
    ...
    if (uiAddID > 0)
    {
      ...
      const xiiUInt32 uiNodeIDBase    = uiNodeID + 1;
      const xiiUInt32 uiAddIDChild    = uiAddID - uiSubAddID;
      const xiiUInt32 uiSubAddIDChild = uiSubAddID >> 2;
...
bool xiiDynamicQuadtree::FindObjectsInRange(const xiiVec3& vPoint, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
  if (vPoint.x < minx)
    return true;
  if (vPoint.x > maxx)
...

  {
    ...
    {
      xiiDynamicTree::xiiMultiMapKey mmk2;
      mmk2.m_uiKey = uiNodeID + 1;

      const xiiDynamicTreeObjectConst itlast = m_NodeMap.LowerBound(mmk2);
...
bool xiiDynamicQuadtree::FindObjectsInRange(const xiiVec3& vPoint, float fRadius, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
...
XII_STATICLINK_FILE(Utilities, Utilities_DataStructures_Implementation_DynamicQuadtree);
