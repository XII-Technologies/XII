...
void xiiDynamicOctree::CreateTree(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, float fMinNodeSize)
{
  ...
  for (xiiUInt32 i = 0; i < m_uiMaxTreeDepth; ++i)
    m_uiAddIDTopLevel += xiiMath::Pow(8, i);
}
...
bool xiiDynamicOctree::InsertObject(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, const xiiDynamicTree::xiiObjectData& Obj, float minx, float maxx, float miny, float maxy, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiDynamicTreeObject* out_Object)
{
  ...
  if (uiAddID > 0)
  {
    ...
    const xiiUInt32 uiSubAddIDChild = uiSubAddID >> 3;
...
void xiiDynamicOctree::FindVisibleObjects(const xiiFrustum& Viewfrustum, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny, float maxy, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
  xiiVec3 v[8];
  ...
  xiiVolumePosition::Enum pos = Viewfrustum.GetObjectPosition(&v[0], 8);
...
void xiiDynamicOctree::RemoveObject(xiiDynamicTreeObject obj)
{
  m_NodeMap.Remove(obj);
}

void xiiDynamicOctree::RemoveObject(xiiInt32 iObjectType, xiiInt32 iObjectInstance)
{
...
void xiiDynamicOctree::RemoveObjectsOfType(xiiInt32 iObjectType)
{
...
bool xiiDynamicOctree::FindObjectsInRange(const xiiVec3& vPoint, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny, float maxy, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
  if (vPoint.x < minx)
    return true;
  if (vPoint.x > maxx)
...
bool xiiDynamicOctree::FindObjectsInRange(const xiiVec3& vPoint, float fRadius, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny, float maxy, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const
{
...
XII_STATICLINK_FILE(Utilities, Utilities_DataStructures_Implementation_DynamicOctree);
