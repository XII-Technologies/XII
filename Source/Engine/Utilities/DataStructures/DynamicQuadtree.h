...
class XII_UTILITIES_DLL xiiDynamicQuadtree
{
...
private:
  ...
  bool InsertObject(const xiiVec3& vCenter, const xiiVec3& vHalfExtents, const xiiDynamicTree::xiiObjectData& Obj, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiDynamicTreeObject* out_Object);

  /// Recursively checks which nodes are visible and calls the callback for each object at those nodes.
  void FindVisibleObjects(const xiiFrustum& Viewfrustum, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const;

  /// Recursively checks in which node a point is located and calls the callback for all objects at those nodes.
  bool FindObjectsInRange(const xiiVec3& vPoint, XII_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz, xiiUInt32 uiNodeID, xiiUInt32 uiAddID, xiiUInt32 uiSubAddID, xiiUInt32 uiNextNodeID) const;
