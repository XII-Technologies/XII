...
xiiResult xiiOBJLoader::LoadOBJ(const char* szFile, bool bIgnoreMaterials)
{
  ...
  while (sText.IsValid())
  {
    ...
    {
      xiiVec3 v(0.0f);
      xiiConversionUtils::ExtractFloatsFromString(sLine.GetStartPointer(), 3, &v.x);
    ...
    else if (sFirst.IsEqual_NoCase("vt")) // line declares a texture coordinate
    {
      ...
      xiiVec3 v(0.0f);
      xiiConversionUtils::ExtractFloatsFromString(sLine.GetStartPointer(), 3, &v.x); // reads up to three texture-coordinates
    ...
    else if (sFirst.IsEqual_NoCase("f")) // line declares a face
    {
      ...
      while (sLine.IsValid())
      {
        ...
        if (xiiConversionUtils::StringToInt(sLine.GetStartPointer(), id, &szCurPos).Failed())
