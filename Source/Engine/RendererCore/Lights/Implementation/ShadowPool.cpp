...
// clang-format off
XII_BEGIN_SUBSYSTEM_DECLARATION(RendererCore, ShadowPool)
...
xiiUInt32 xiiShadowPool::AddDirectionalLight(const xiiDirectionalLightComponent* pDirLight, const xiiView* pReferenceView)
{
  ...
  for (xiiUInt32 i = 0; i < uiNumCascades; ++i)
  {
    ...
    // Setup camera
    {
      ...
      xiiVec3 shadowCameraPos         = center - vForward * fCameraToCenterDistance;
...
void xiiShadowPool::OnExtractionEvent(const xiiRenderWorldExtractionEvent& e)
{
  ...
  {
    xiiDebugRenderer::DrawInfoText(debugContext, xiiDebugRenderer::ScreenPlacement::TopLeft, "ShadowPoolStats", "Shadow Pool Stats:", xiiColor::LightSteelBlue);
    xiiDebugRenderer::DrawInfoText(debugContext, xiiDebugRenderer::ScreenPlacement::TopLeft, "ShadowPoolStats", "Details (Name: Size - Atlas Offset)", xiiColor::LightSteelBlue);
  }
  ...
  for (auto& sorted : s_SortedShadowData)
  {
    ...
    for (xiiUInt32 uiViewIndex = 0; uiViewIndex < shadowData.m_Views.GetCount(); ++uiViewIndex)
    {
      ...
      if (cvar_RenderingShadowsShowPoolStats)
      {
        xiiDebugRenderer::DrawInfoText(debugContext, xiiDebugRenderer::ScreenPlacement::TopLeft, "ShadowPoolStats", xiiFmt("{0}: {1} - {2}x{3}", pShadowView->GetName(), atlasRect.width, atlasRect.x, atlasRect.y), xiiColor::LightSteelBlue);
  ...
  if (cvar_RenderingShadowsShowPoolStats)
  {
    xiiDebugRenderer::DrawInfoText(debugContext, xiiDebugRenderer::ScreenPlacement::TopLeft, "ShadowPoolStats", xiiFmt("Atlas Utilization: {0}%%", xiiArgF(100.0 * (double)uiUsedAtlasSize / uiTotalAtlasSize, 2)), xiiColor::LightSteelBlue);
  }
#endif
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Lights_Implementation_ShadowPool);
