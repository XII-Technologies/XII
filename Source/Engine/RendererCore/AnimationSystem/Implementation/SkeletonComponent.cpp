...
void xiiSkeletonComponent::BuildColliderVisualization(xiiMsgAnimationPoseUpdated& msg)
{
  ...
  for (const auto& geo : pSkeleton->GetDescriptor().m_Geometry)
  {
    ...
    if (geo.m_Type == xiiSkeletonJointGeometryType::Sphere)
    {
      ...
      shape.m_Shape.SetElements(xiiVec3::ZeroVector(), geo.m_Transform.m_vScale.z);
      shape.m_Color = hlS;
    }
    ...
    if (geo.m_Type == xiiSkeletonJointGeometryType::Box)
    {
      ...
      shape.m_Transform = st;
      shape.m_Shape.SetCenterAndHalfExtents(xiiVec3::ZeroVector(), ext);
      shape.m_Color = hlS;
    }
...
XII_STATICLINK_FILE(RendererCore, RendererCore_AnimationSystem_Implementation_SkeletonComponent);
