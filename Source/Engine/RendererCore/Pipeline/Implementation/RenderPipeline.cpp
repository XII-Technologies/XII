...
const xiiRenderPipelinePassConnection* xiiRenderPipeline::GetInputConnection(xiiRenderPipelinePass* pPass, xiiHashedString sInputPinName) const
{
...
const xiiRenderPipelinePassConnection* xiiRenderPipeline::GetOutputConnection(xiiRenderPipelinePass* pPass, xiiHashedString sOutputPinName) const
{
  auto it = m_Connections.Find(pPass);
  if (!it.IsValid())
    return nullptr;
...
// clang-format off
XII_BEGIN_SUBSYSTEM_DECLARATION(RendererCore, SwRasterizer)
...
void xiiRenderPipeline::FindVisibleObjects(const xiiView& view)
{
  ...
  limitedFrustum.AccessPlane(xiiFrustum::PlaneType::FarPlane).SetFromNormalAndPoint(farPlane.m_vNormal, view.GetCullingCamera()->GetCenterPosition() + farPlane.m_vNormal * cvar_SpatialCullingOcclusionFarPlane.GetValue()); // only use occluders closer than this
  ...
  if (bRecordStats)
  {
    ...
    xiiDebugRenderer::DrawInfoText(hView, xiiDebugRenderer::ScreenPlacement::TopLeft, "VisCulling", "Visibility Culling Stats", xiiColor::LimeGreen);

    sb.Format("Total Num Objects: {0}", stats.m_uiTotalNumObjects);
    xiiDebugRenderer::DrawInfoText(hView, xiiDebugRenderer::ScreenPlacement::TopLeft, "VisCulling", sb, xiiColor::LimeGreen);
    ...
    xiiDebugRenderer::DrawInfoText(hView, xiiDebugRenderer::ScreenPlacement::TopLeft, "VisCulling", sb, xiiColor::AntiqueWhite);
  }
#endif
}
...
void xiiRenderPipeline::CreateDgmlGraph(xiiDGMLGraph& ref_graph)
{
  ...
  for (xiiUInt32 i = 0; i < m_TextureUsage.GetCount(); ++i)
  {
    ...
    for (const xiiRenderPipelinePassConnection* pCon : data.m_UsedBy)
    {
      ...
      if (!xiiReflectionUtils::EnumerationToString(xiiGetStaticRTTI<xiiGALResourceFormat>(), pCon->m_Desc.m_Format, sFormat, xiiReflectionUtils::EnumConversionMode::ValueNameOnly))
      {
...
void xiiRenderPipeline::PreviewOcclusionBuffer(const xiiRasterizerView& rasterizer, const xiiView& view)
{
  ...
  else
  {
    ...
    d.m_DescGAL.m_Format   = xiiGALResourceFormat::RGBAByteNormalized;

    xiiGALSystemMemoryDescription content[1];
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_RenderPipeline);
