...
// clang-format off
XII_BEGIN_SUBSYSTEM_DECLARATION(RendererCore, RenderData)
...
// static
xiiRenderData::Category xiiRenderData::FindCategory(const char* szCategoryName)
{
...
void xiiRenderData::UpdateRendererTypes()
{
  ...
  {
    if (!pRtti->IsDerivedFrom<xiiRenderer>() || pRtti->GetTypeFlags().IsAnySet(xiiTypeFlags::Abstract) || !pRtti->GetAllocator()->CanAllocate())
      continue;
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_RenderData);
