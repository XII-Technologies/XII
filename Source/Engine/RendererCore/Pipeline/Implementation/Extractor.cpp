  ...
  const xiiView& view, const xiiDynamicArray<const xiiGameObject*>& visibleObjects, xiiExtractedRenderData& extractedRenderData)
{
  ...
  if (cvar_SpatialExtractionShowStats && bIsMainView)
  {
    ...
    xiiDebugRenderer::DrawInfoText(hView, xiiDebugRenderer::ScreenPlacement::TopLeft, "ExtractionStats", "Extraction Stats:");

    sb.Format("Num Cached Render Data: {0}", m_uiNumCachedRenderData);
    xiiDebugRenderer::DrawInfoText(hView, xiiDebugRenderer::ScreenPlacement::TopLeft, "ExtractionStats", sb);
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_Extractor);
