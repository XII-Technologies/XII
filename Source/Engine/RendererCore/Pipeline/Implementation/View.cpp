...
xiiView::xiiView()
{
  m_pExtractTask = XII_DEFAULT_NEW(xiiDelegateTask<void>, "", xiiMakeDelegate(&xiiView::ExtractData, this));
}
...
void xiiView::ApplyProperty(xiiReflectedClass* pClass, PropertyValue& data, const char* szTypeName)
{
  xiiAbstractProperty* pAbstractProperty = pClass->GetDynamicRTTI()->FindPropertyByName(data.m_sPropertyName);
  ...
  xiiReflectionUtils::SetMemberPropertyValue(static_cast<xiiAbstractMemberProperty*>(pAbstractProperty), pClass, data.m_Value);
}

XII_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_View);
