...
xiiResourceLoadDesc xiiRenderPipelineResource::UpdateContent(xiiStreamReader* Stream)
{
  ...
  if (sAbsFilePath.HasExtension("xiiRenderPipelineBin"))
  {
    ...
    (*Stream) >> uiVersion;
    XII_ASSERT_DEV(uiVersion == 1, "Unknown xiiRenderPipelineBin version {0}", uiVersion);
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_RenderPipelineResource);
