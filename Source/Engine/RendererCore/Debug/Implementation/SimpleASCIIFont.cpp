...
static void CopyCharacter(xiiUInt32* pImage, xiiInt32 c, const char* szChar)
{
  xiiInt32 x = ((xiiInt32)c % 16) * 16;
  xiiInt32 y = ((xiiInt32)c / 16) * 16;

  pImage += (y + 3) * 256; // 3 pixel boundary at the top
  pImage += x + 3;         // 3 pixel boundary at the left side

  for (xiiUInt32 i1 = 0; i1 < 10; ++i1)
  {
    for (xiiUInt32 i2 = 0; i2 < 10; ++i2)
    {
...
void xiiGraphicsUtils::CreateSimpleASCIIFontTexture(xiiImage& ref_img, bool bSetEmptyToUnknown)
{
  ...
  Img.SetWidth(256);
  Img.SetHeight(128);
  Img.SetImageFormat(xiiImageFormat::R8G8B8A8_UNORM);
  Img.AllocateImageData();

  // pre-fill with black
  {
    xiiUInt32* pPixelData = Img.GetPixelPointer<xiiUInt32>();
  ...
  class VariableReader : public xiiStreamReader
  {
  public:
    virtual xiiUInt64 ReadBytes(void* pReadBuffer, xiiUInt64 uiBytesToRead) override
    {
      const xiiUInt64 uiSize = xiiMath::Min(uiBytesToRead, m_uiSize);
      m_uiSize -= uiSize;

      if (uiSize > 0)
      {
        xiiMemoryUtils::Copy((xiiUInt8*)pReadBuffer, m_pData, (size_t)uiSize);
        m_pData += uiSize;
      }
...
XII_STATICLINK_FILE(RendererCore, RendererCore_Debug_Implementation_SimpleASCIIFont);
