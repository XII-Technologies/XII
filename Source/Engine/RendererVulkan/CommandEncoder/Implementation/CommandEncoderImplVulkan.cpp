...
xiiGALCommandEncoderImplVulkan::xiiGALCommandEncoderImplVulkan(xiiGALDeviceVulkan& device) :
  m_GALDeviceVulkan(device)
{
  m_vkDevice = device.GetVulkanDevice();
}

xiiGALCommandEncoderImplVulkan::~xiiGALCommandEncoderImplVulkan() = default;

void xiiGALCommandEncoderImplVulkan::Reset()
{
  XII_ASSERT_DEBUG(!m_bRenderPassActive, "Render pass was not closed");
...
void xiiGALCommandEncoderImplVulkan::MarkDirty()
{
  XII_ASSERT_DEBUG(!m_bRenderPassActive, "Render pass was not closed");
...
void xiiGALCommandEncoderImplVulkan::SetCurrentCommandBuffer(vk::CommandBuffer* commandBuffer, xiiPipelineBarrierVulkan* pipelineBarrier)
{
...
void xiiGALCommandEncoderImplVulkan::SetShaderPlatform(const xiiGALShader* pShader)
{
  if (pShader != nullptr)
  {
    m_PipelineDesc.m_pCurrentShader = static_cast<const xiiGALShaderVulkan*>(pShader);
    m_ComputeDesc.m_pCurrentShader  = m_PipelineDesc.m_pCurrentShader;
    m_bPipelineStateDirty           = true;
  }
}

void xiiGALCommandEncoderImplVulkan::SetConstantBufferPlatform(xiiUInt32 uiSlot, const xiiGALBuffer* pBuffer)
{
  // \todo Check if the device supports the slot index?
  m_pBoundConstantBuffers[uiSlot] = pBuffer != nullptr ? static_cast<const xiiGALBufferVulkan*>(pBuffer) : nullptr;
  m_bDescriptorsDirty             = true;
}

void xiiGALCommandEncoderImplVulkan::SetSamplerStatePlatform(xiiGALShaderStage::Enum Stage, xiiUInt32 uiSlot, const xiiGALSamplerState* pSamplerState)
{
  // \todo Check if the device supports the stage / the slot index
  m_pBoundSamplerStates[Stage][uiSlot] = pSamplerState != nullptr ? static_cast<const xiiGALSamplerStateVulkan*>(pSamplerState) : nullptr;
  m_bDescriptorsDirty                  = true;
}

void xiiGALCommandEncoderImplVulkan::SetResourceViewPlatform(xiiGALShaderStage::Enum Stage, xiiUInt32 uiSlot, const xiiGALResourceView* pResourceView)
{
  auto& boundShaderResourceViews = m_pBoundShaderResourceViews[Stage];
  boundShaderResourceViews.EnsureCount(uiSlot + 1);
  boundShaderResourceViews[uiSlot] = pResourceView != nullptr ? static_cast<const xiiGALResourceViewVulkan*>(pResourceView) : nullptr;
  m_bDescriptorsDirty              = true;
}

void xiiGALCommandEncoderImplVulkan::SetUnorderedAccessViewPlatform(xiiUInt32 uiSlot, const xiiGALUnorderedAccessView* pUnorderedAccessView)
{
  m_pBoundUnoderedAccessViews.EnsureCount(uiSlot + 1);
  m_pBoundUnoderedAccessViews[uiSlot] = pUnorderedAccessView != nullptr ? static_cast<const xiiGALUnorderedAccessViewVulkan*>(pUnorderedAccessView) : nullptr;
  m_bDescriptorsDirty                 = true;
}

// Query functions

void xiiGALCommandEncoderImplVulkan::BeginQueryPlatform(const xiiGALQuery* pQuery)
{
  auto pVulkanQuery = static_cast<const xiiGALQueryVulkan*>(pQuery);
...
void xiiGALCommandEncoderImplVulkan::EndQueryPlatform(const xiiGALQuery* pQuery)
{
  auto pVulkanQuery = static_cast<const xiiGALQueryVulkan*>(pQuery);
...
xiiResult xiiGALCommandEncoderImplVulkan::GetQueryResultPlatform(const xiiGALQuery* pQuery, xiiUInt64& uiQueryResult)
{
  auto       pVulkanQuery = static_cast<const xiiGALQueryVulkan*>(pQuery);
  vk::Result result       = m_vkDevice.getQueryPoolResults(pVulkanQuery->GetPool(), pVulkanQuery->GetID(), 1u, sizeof(xiiUInt64), &uiQueryResult, 0, vk::QueryResultFlagBits::e64);

  return result == vk::Result::eSuccess ? XII_SUCCESS : XII_FAILURE;
}

void xiiGALCommandEncoderImplVulkan::InsertTimestampPlatform(xiiGALTimestampHandle hTimestamp)
{
...
void xiiGALCommandEncoderImplVulkan::ClearUnorderedAccessViewPlatform(const xiiGALUnorderedAccessView* pUnorderedAccessView, xiiVec4 clearValues)
{
...
void xiiGALCommandEncoderImplVulkan::ClearUnorderedAccessViewPlatform(const xiiGALUnorderedAccessView* pUnorderedAccessView, xiiVec4U32 clearValues)
{
...
void xiiGALCommandEncoderImplVulkan::CopyBufferPlatform(const xiiGALBuffer* pDestination, const xiiGALBuffer* pSource)
{
  auto pDestinationVulkan = static_cast<const xiiGALBufferVulkan*>(pDestination);
  auto pSourceVulkan      = static_cast<const xiiGALBufferVulkan*>(pSource);

  XII_ASSERT_DEV(pSource->GetSize() != pDestination->GetSize(), "Source and destination buffer sizes mismatch!");
...
void xiiGALCommandEncoderImplVulkan::CopyBufferRegionPlatform(const xiiGALBuffer* pDestination, xiiUInt32 uiDestOffset, const xiiGALBuffer* pSource, xiiUInt32 uiSourceOffset, xiiUInt32 uiByteCount)
{
  auto pDestinationVulkan = static_cast<const xiiGALBufferVulkan*>(pDestination);
  auto pSourceVulkan      = static_cast<const xiiGALBufferVulkan*>(pSource);
...
void xiiGALCommandEncoderImplVulkan::UpdateBufferPlatform(const xiiGALBuffer* pDestination, xiiUInt32 uiDestOffset, xiiArrayPtr<const xiiUInt8> pSourceData, xiiGALUpdateMode::Enum updateMode)
{
  XII_CHECK_ALIGNMENT_16(pSourceData.GetPtr());

  auto pVulkanDestination = static_cast<const xiiGALBufferVulkan*>(pDestination);
  ...
  switch (updateMode)
  {
    ...
    {
      xiiVulkanAllocation alloc = pVulkanDestination->GetAllocation();
      void*               pData = nullptr;
      VK_ASSERT_DEV(xiiMemoryAllocatorVulkan::MapMemory(alloc, &pData));
      XII_ASSERT_DEV(pData, "Implementation error");
      xiiMemoryUtils::Copy(xiiMemoryUtils::AddByteOffset((xiiUInt8*)pData, uiDestOffset), pSourceData.GetPtr(), pSourceData.GetCount());
      xiiMemoryAllocatorVulkan::UnmapMemory(alloc);
    }
    break;
    case xiiGALUpdateMode::CopyToTempStorage:
    {
      ...
      XII_ASSERT_DEBUG(!m_bRenderPassActive, "Vulkan does not support copying buffers while a render pass is active. TODO: Fix high level render code to make this impossible.");
...
void xiiGALCommandEncoderImplVulkan::CopyTexturePlatform(const xiiGALTexture* pDestination, const xiiGALTexture* pSource)
{
  if (m_bRenderPassActive)
  {
    m_pCommandBuffer->endRenderPass();
  ...
  auto destination = static_cast<const xiiGALTextureVulkan*>(pDestination->GetParentResource());
  auto source      = static_cast<const xiiGALTextureVulkan*>(pSource->GetParentResource());

  const xiiGALTextureCreationDescription& destDesc = pDestination->GetDescription();
  const xiiGALTextureCreationDescription& srcDesc  = pSource->GetDescription();

  XII_ASSERT_DEBUG(xiiGALResourceFormat::IsDepthFormat(destDesc.m_Format) == xiiGALResourceFormat::IsDepthFormat(srcDesc.m_Format), "");
  XII_ASSERT_DEBUG(destDesc.m_uiArraySize == srcDesc.m_uiArraySize, "");
  XII_ASSERT_DEBUG(destDesc.m_uiMipLevelCount == srcDesc.m_uiMipLevelCount, "");
  XII_ASSERT_DEBUG(destDesc.m_uiWidth == srcDesc.m_uiWidth, "");
  XII_ASSERT_DEBUG(destDesc.m_uiHeight == srcDesc.m_uiHeight, "");
  XII_ASSERT_DEBUG(destDesc.m_uiDepth == srcDesc.m_uiDepth, "");

  vk::ImageAspectFlagBits imageAspect = xiiGALResourceFormat::IsDepthFormat(destDesc.m_Format) ? vk::ImageAspectFlagBits::eDepth : vk::ImageAspectFlagBits::eColor;
  ...
  xiiHybridArray<vk::ImageCopy, 14> imageCopies;

  for (xiiUInt32 i = 0; i < destDesc.m_uiMipLevelCount; ++i)
  {
...
void xiiGALCommandEncoderImplVulkan::CopyTextureRegionPlatform(const xiiGALTexture* pDestination, const xiiGALTextureSubresource& DestinationSubResource, const xiiVec3U32& DestinationPoint, const xiiGALTexture* pSource, const xiiGALTextureSubresource& SourceSubResource, const xiiBoundingBoxu32& Box)
{
  auto destination = static_cast<const xiiGALTextureVulkan*>(pDestination->GetParentResource());
  auto source      = static_cast<const xiiGALTextureVulkan*>(pSource->GetParentResource());
  ...
  xiiVec3U32 extent = Box.m_vMax - Box.m_vMin;
...
void xiiGALCommandEncoderImplVulkan::UpdateTexturePlatform(const xiiGALTexture* pDestination, const xiiGALTextureSubresource& DestinationSubResource, const xiiBoundingBoxu32& DestinationBox, const xiiGALSystemMemoryDescription& data)
{
  const xiiGALTextureVulkan* pDestVulkan = static_cast<const xiiGALTextureVulkan*>(pDestination);
  ...
  xiiStagingBufferVulkan stagingBuffer = m_GALDeviceVulkan.GetStagingBufferPool().AllocateBuffer(0, uiTotalSize);
  ...
  xiiMemoryUtils::RawByteCopy(pData, data.m_pData, uiTotalSize);
...
void xiiGALCommandEncoderImplVulkan::ResolveTexturePlatform(const xiiGALTexture* pDestination, const xiiGALTextureSubresource& DestinationSubResource, const xiiGALTexture* pSource, const xiiGALTextureSubresource& SourceSubResource)
{
  auto pVulkanDestination = static_cast<const xiiGALTextureVulkan*>(pDestination->GetParentResource());
  auto pVulkanSource      = static_cast<const xiiGALTextureVulkan*>(pSource->GetParentResource());
  ...
  resolveRegion.extent.width                  = xiiMath::Min(destDesc.m_uiWidth, srcDesc.m_uiWidth);
  resolveRegion.extent.height                 = xiiMath::Min(destDesc.m_uiHeight, srcDesc.m_uiHeight);
  resolveRegion.extent.depth                  = xiiMath::Min(destDesc.m_uiDepth, srcDesc.m_uiDepth);
  ...
  m_pPipelineBarrier->Flush();
  if (srcDesc.m_SampleCount != xiiGALMSAASampleCount::None)
  {
  ...
  else
  {
    ...
    vk::Offset3D sourceMipLevelEndOffset = {(xiiInt32)sourceMipLevelSize.width, (xiiInt32)sourceMipLevelSize.height, (xiiInt32)sourceMipLevelSize.depth};
    vk::Extent3D dstMipLevelSize         = pVulkanDestination->GetMipLevelSize(DestinationSubResource.m_uiMipLevel);
    vk::Offset3D dstMipLevelEndOffset    = {(xiiInt32)sourceMipLevelSize.width, (xiiInt32)sourceMipLevelSize.height, (xiiInt32)sourceMipLevelSize.depth};
...
void xiiGALCommandEncoderImplVulkan::CopyImageToBuffer(const xiiGALTextureVulkan* pSource, const xiiGALBufferVulkan* pDestination)
{
  const xiiGALTextureCreationDescription& textureDesc = pSource->GetDescription();
  const vk::ImageAspectFlags              imageAspect = pSource->GetAspectMask();

  xiiHybridArray<xiiGALTextureVulkan::SubResourceOffset, 8> subResourceOffsets;
  const xiiUInt32                                           uiBufferSize = pSource->ComputeSubResourceOffsets(subResourceOffsets);

  xiiHybridArray<vk::BufferImageCopy, 8> imageCopy;
  const xiiUInt32                        arraySize = textureDesc.m_Type == xiiGALTextureType::TextureCube ? textureDesc.m_uiArraySize * 6 : textureDesc.m_uiArraySize;
  const xiiUInt32                        mipLevels = textureDesc.m_uiMipLevelCount;

  for (xiiUInt32 uiLayer = 0; uiLayer < arraySize; uiLayer++)
  {
    for (xiiUInt32 uiMipLevel = 0; uiMipLevel < mipLevels; uiMipLevel++)
    {
      ...
      const xiiGALTextureVulkan::SubResourceOffset& offset             = subResourceOffsets[uiSubresourceIndex];
...
void xiiGALCommandEncoderImplVulkan::ReadbackTexturePlatform(const xiiGALTexture* pTexture)
{
  ...
  XII_ASSERT_DEV(!m_bRenderPassActive, "Can't readback within a render pass");

  const xiiGALTextureVulkan*              pVulkanTexture     = static_cast<const xiiGALTextureVulkan*>(pTexture->GetParentResource());
  const xiiGALTextureCreationDescription& textureDesc        = pVulkanTexture->GetDescription();
  const vk::ImageAspectFlagBits           imageAspect        = xiiGALResourceFormat::IsDepthFormat(textureDesc.m_Format) ? vk::ImageAspectFlagBits::eDepth : vk::ImageAspectFlagBits::eColor;
  const bool                              bMSAASourceTexture = textureDesc.m_SampleCount != xiiGALMSAASampleCount::None;
  XII_ASSERT_DEV(!bMSAASourceTexture, "MSAA read-back not implemented!");
  const xiiGALTextureVulkan::StagingMode stagingMode = pVulkanTexture->GetStagingMode();
  XII_ASSERT_DEV(stagingMode != xiiGALTextureVulkan::StagingMode::None, "No staging resource available for read-back");

  if (stagingMode == xiiGALTextureVulkan::StagingMode::Buffer)
  {
    const xiiGALBufferVulkan* pStagingBuffer = static_cast<const xiiGALBufferVulkan*>(m_GALDeviceVulkan.GetBuffer(pVulkanTexture->GetStagingBuffer()));
    CopyImageToBuffer(pVulkanTexture, pStagingBuffer);
  }
  else
  {
    // Render to texture
    const xiiGALTextureVulkan* pStagingTexture = static_cast<const xiiGALTextureVulkan*>(m_GALDeviceVulkan.GetTexture(pVulkanTexture->GetStagingTexture()));
    const bool                 bSourceIsDepth  = xiiConversionUtilsVulkan::IsDepthFormat(pVulkanTexture->GetImageFormat());
    ...
    xiiImageCopyVulkan copy(m_GALDeviceVulkan);

    const bool bStereoSupport = m_GALDeviceVulkan.GetCapabilities().m_bVertexShaderRenderTargetArrayIndex || m_GALDeviceVulkan.GetCapabilities().m_bShaderStageSupported[xiiGALShaderStage::GeometryShader];
    if (bStereoSupport)
    {
      copy.Init(pVulkanTexture, pStagingTexture, xiiShaderUtils::xiiBuiltinShaderType::CopyImageArray);
      const xiiUInt32 arraySize = textureDesc.m_Type == xiiGALTextureType::TextureCube ? textureDesc.m_uiArraySize * 6 : textureDesc.m_uiArraySize;
      const xiiUInt32 mipLevels = textureDesc.m_uiMipLevelCount;
      for (xiiUInt32 uiMipLevel = 0; uiMipLevel < textureDesc.m_uiMipLevelCount; uiMipLevel++)
      {
    ...
    {
      copy.Init(pVulkanTexture, pStagingTexture, xiiShaderUtils::xiiBuiltinShaderType::CopyImage);
      const xiiUInt32 arraySize = textureDesc.m_Type == xiiGALTextureType::TextureCube ? textureDesc.m_uiArraySize * 6 : textureDesc.m_uiArraySize;
      const xiiUInt32 mipLevels = textureDesc.m_uiMipLevelCount;
    ...
    if (stagingMode == xiiGALTextureVulkan::StagingMode::TextureAndBuffer)
    {
      // Copy to buffer
      const xiiGALBufferVulkan* pStagingBuffer = static_cast<const xiiGALBufferVulkan*>(m_GALDeviceVulkan.GetBuffer(pVulkanTexture->GetStagingBuffer()));
      CopyImageToBuffer(pVulkanTexture, pStagingBuffer);
...
void xiiGALCommandEncoderImplVulkan::CopyTextureReadbackResultPlatform(const xiiGALTexture* pTexture, xiiArrayPtr<xiiGALTextureSubresource> SourceSubResource, xiiArrayPtr<xiiGALSystemMemoryDescription> TargetData)
{
  //#TODO_VULKAN readback fence
  auto                                    pVulkanTexture = static_cast<const xiiGALTextureVulkan*>(pTexture->GetParentResource());
  const xiiGALTextureCreationDescription& textureDesc    = pVulkanTexture->GetDescription();
  const xiiGALTextureVulkan::StagingMode  stagingMode    = pVulkanTexture->GetStagingMode();
  XII_ASSERT_DEV(stagingMode != xiiGALTextureVulkan::StagingMode::None, "No staging resource available for read-back");

  if (stagingMode == xiiGALTextureVulkan::StagingMode::Texture)
  {
    const xiiGALTextureVulkan* pStagingTexture = static_cast<const xiiGALTextureVulkan*>(m_GALDeviceVulkan.GetTexture(pVulkanTexture->GetStagingTexture()));
    ...
    void* pData = nullptr;
    xiiMemoryAllocatorVulkan::MapMemory(pStagingTexture->GetAllocation(), &pData);
    ...
    for (xiiUInt32 i = 0; i < uiSubResources; i++)
    {
      ...
      xiiUInt8* pSubResourceData = reinterpret_cast<xiiUInt8*>(pData) + subResourceLayout.offset;

      if (subResourceLayout.rowPitch == memDesc.m_uiRowPitch)
      {
        const xiiUInt32 uiMemorySize = xiiGALResourceFormat::GetBitsPerElement(textureDesc.m_Format) *
      ...
      {
        // Copy row by row
        const xiiUInt32 uiHeight = GetMipSize(textureDesc.m_uiHeight, subRes.m_uiMipLevel);
        for (xiiUInt32 y = 0; y < uiHeight; ++y)
        {
          const void* pSource = xiiMemoryUtils::AddByteOffset(pSubResourceData, y * subResourceLayout.rowPitch);
          void*       pDest   = xiiMemoryUtils::AddByteOffset(memDesc.m_pData, y * memDesc.m_uiRowPitch);

          memcpy(pDest, pSource, xiiGALResourceFormat::GetBitsPerElement(textureDesc.m_Format) * GetMipSize(textureDesc.m_uiWidth, subRes.m_uiMipLevel) / 8);
        }
      }
    }

    xiiMemoryAllocatorVulkan::UnmapMemory(pStagingTexture->GetAllocation());
  }
  else // One of the buffer variants.
  {
    ...
    xiiHybridArray<xiiGALTextureVulkan::SubResourceOffset, 8> subResourceOffsets;
    const xiiUInt32                                           uiBufferSize = pVulkanTexture->ComputeSubResourceOffsets(subResourceOffsets);


    const xiiUInt32 uiSubResources = SourceSubResource.GetCount();

    void* pData = nullptr;
    xiiMemoryAllocatorVulkan::MapMemory(pStagingBuffer->GetAllocation(), &pData);

    const xiiUInt32 uiMipLevels = textureDesc.m_uiMipLevelCount;
    for (xiiUInt32 i = 0; i < uiSubResources; i++)
    {
      const xiiGALTextureSubresource&              subRes             = SourceSubResource[i];
      const xiiUInt32                              uiSubresourceIndex = subRes.m_uiMipLevel + subRes.m_uiArraySlice * uiMipLevels;
      const xiiGALTextureVulkan::SubResourceOffset offset             = subResourceOffsets[uiSubresourceIndex];
      const xiiGALSystemMemoryDescription&         memDesc            = TargetData[i];
      const auto                                   blockExtent        = vk::blockExtent(stagingFormat);
      const xiiUInt8                               uiBlockSize        = vk::blockSize(stagingFormat);

      const xiiUInt32 uiRowPitch = offset.m_uiRowLength * blockExtent[0] * uiBlockSize;

      xiiUInt8* pSubResourceData = reinterpret_cast<xiiUInt8*>(pData) + offset.m_uiOffset;
      ...
      else
      {
        ...
        for (xiiUInt32 y = 0; y < uiHeight; ++y)
        {
          const void* pSource = xiiMemoryUtils::AddByteOffset(pSubResourceData, y * uiRowPitch);
          void*       pDest   = xiiMemoryUtils::AddByteOffset(memDesc.m_pData, y * memDesc.m_uiRowPitch);
    ...
    xiiMemoryAllocatorVulkan::UnmapMemory(pStagingBuffer->GetAllocation());
  }
}

void xiiGALCommandEncoderImplVulkan::GenerateMipMapsPlatform(const xiiGALResourceView* pResourceView)
{
  const xiiGALResourceViewVulkan* pVulkanResourceView = static_cast<const xiiGALResourceViewVulkan*>(pResourceView);
  if (m_bRenderPassActive)
  {
    m_pCommandBuffer->endRenderPass();
    m_bRenderPassActive = false;
  ...
  const xiiGALTextureVulkan* pVulkanTexture = static_cast<const xiiGALTextureVulkan*>(pVulkanResourceView->GetResource()->GetParentResource());
  ...
  else
  {
    if (bSupportsBlit)
    {
      ...
      for (xiiUInt32 uiMipLevel = viewRange.baseMipLevel; uiMipLevel < (viewRange.baseMipLevel + viewRange.levelCount - 1); uiMipLevel++)
      {
        {
        ...
        vk::Offset3D destinationMipLevelEndOffset = {(xiiInt32)destinationMipLevelSize.width, (xiiInt32)destinationMipLevelSize.height, (xiiInt32)destinationMipLevelSize.depth};
    ...
    else
    {
      ...
      if (bStereoSupport)
      {
        copy.Init(pVulkanTexture, pVulkanTexture, xiiShaderUtils::xiiBuiltinShaderType::DownscaleImageArray);
        for (xiiUInt32 uiMipLevel = viewRange.baseMipLevel; uiMipLevel < (viewRange.baseMipLevel + viewRange.levelCount - 1); uiMipLevel++)
        {
          vk::Extent3D sourceMipLevelSize           = pVulkanTexture->GetMipLevelSize(uiMipLevel);
          vk::Offset3D sourceMipLevelEndOffset      = {(xiiInt32)sourceMipLevelSize.width, (xiiInt32)sourceMipLevelSize.height, (xiiInt32)sourceMipLevelSize.depth};
          ...
          vk::Extent3D mipLevelSize = pVulkanTexture->GetMipLevelSize(0);
          copy.Copy({0, 0, 0}, sourceLayers, {0, 0, 0}, destinationLayers, {(xiiUInt32)destinationMipLevelSize.width, (xiiUInt32)destinationMipLevelSize.height, (xiiUInt32)destinationMipLevelSize.depth});
        }
      }
      else
      {
        copy.Init(pVulkanTexture, pVulkanTexture, xiiShaderUtils::xiiBuiltinShaderType::DownscaleImage);
        const xiiUInt32 arraySize = textureDesc.m_Type == xiiGALTextureType::TextureCube ? textureDesc.m_uiArraySize * 6 : textureDesc.m_uiArraySize;
        const xiiUInt32 mipLevels = textureDesc.m_uiMipLevelCount;

        for (xiiUInt32 uiLayer = viewRange.baseArrayLayer; uiLayer < (viewRange.baseArrayLayer + viewRange.layerCount); uiLayer++)
        {
          for (xiiUInt32 uiMipLevel = viewRange.baseMipLevel; uiMipLevel < (viewRange.baseMipLevel + viewRange.levelCount - 1); uiMipLevel++)
          {
            vk::Extent3D sourceMipLevelSize           = pVulkanTexture->GetMipLevelSize(uiMipLevel);
...
void xiiGALCommandEncoderImplVulkan::FlushPlatform()
{
...
void xiiGALCommandEncoderImplVulkan::PushMarkerPlatform(const char* szMarker)
{
  ...
  xiiMemoryUtils::Copy(markerInfo.color.data(), markerColor, XII_ARRAY_SIZE(markerColor));
  markerInfo.pLabelName = szMarker;

  m_pCommandBuffer->beginDebugUtilsLabelEXT(markerInfo);
}

void xiiGALCommandEncoderImplVulkan::PopMarkerPlatform()
{
  m_pCommandBuffer->endDebugUtilsLabelEXT();
}

void xiiGALCommandEncoderImplVulkan::InsertEventMarkerPlatform(const char* szMarker)
{
  constexpr float        markerColor[4] = {1, 1, 1, 1};
  vk::DebugUtilsLabelEXT markerInfo     = {};
  xiiMemoryUtils::Copy(markerInfo.color.data(), markerColor, XII_ARRAY_SIZE(markerColor));
...
void xiiGALCommandEncoderImplVulkan::BeginRendering(const xiiGALRenderingSetup& renderingSetup)
{
  m_PipelineDesc.m_renderPass        = xiiResourceCacheVulkan::RequestRenderPass(renderingSetup);
  m_PipelineDesc.m_uiAttachmentCount = renderingSetup.m_RenderTargetSetup.GetRenderTargetCount();
  xiiSizeU32 size;
  m_frameBuffer = xiiResourceCacheVulkan::RequestFrameBuffer(m_PipelineDesc.m_renderPass, renderingSetup.m_RenderTargetSetup, size, m_PipelineDesc.m_msaa, m_uiLayers);

  SetScissorRectPlatform(xiiRectU32(size.width, size.height));

  {
    ...
    if (m_bHasDepth)
    {
      ...
      const xiiGALRenderTargetViewVulkan* pRenderTargetView = static_cast<const xiiGALRenderTargetViewVulkan*>(m_GALDeviceVulkan.GetRenderTargetView(renderingSetup.m_RenderTargetSetup.GetDepthStencilTarget()));
    ...
    for (xiiUInt32 i = 0; i < uiColorCount; i++)
    {
      ...
      xiiColor        col        = renderingSetup.m_ClearColor;
      colorClear.color.setFloat32({col.r, col.g, col.b, col.a});

      const xiiGALRenderTargetViewVulkan* pRenderTargetView = static_cast<const xiiGALRenderTargetViewVulkan*>(m_GALDeviceVulkan.GetRenderTargetView(renderingSetup.m_RenderTargetSetup.GetRenderTarget(i)));
...
void xiiGALCommandEncoderImplVulkan::EndRendering()
{
  if (!m_bClearSubmitted)
  {
    m_pPipelineBarrier->Flush();
  ...
  m_depthMask                 = {};
  m_uiLayers                  = 0;
  m_PipelineDesc.m_msaa       = xiiGALMSAASampleCount::None;
  m_PipelineDesc.m_renderPass = nullptr;
  m_frameBuffer               = nullptr;
}

void xiiGALCommandEncoderImplVulkan::ClearPlatform(const xiiColor& ClearColor, xiiUInt32 uiRenderTargetClearMask, bool bClearDepth, bool bClearStencil, float fDepthClear, xiiUInt8 uiStencilClear)
{
  ...
  xiiHybridArray<vk::ClearAttachment, 8> attachments;
...
void xiiGALCommandEncoderImplVulkan::DrawPlatform(xiiUInt32 uiVertexCount, xiiUInt32 uiStartVertex)
{
  FlushDeferredStateChanges();
...
void xiiGALCommandEncoderImplVulkan::DrawIndexedPlatform(xiiUInt32 uiIndexCount, xiiUInt32 uiStartIndex)
{
  FlushDeferredStateChanges();
...
void xiiGALCommandEncoderImplVulkan::DrawIndexedInstancedPlatform(xiiUInt32 uiIndexCountPerInstance, xiiUInt32 uiInstanceCount, xiiUInt32 uiStartIndex)
{
  FlushDeferredStateChanges();
...
void xiiGALCommandEncoderImplVulkan::DrawIndexedInstancedIndirectPlatform(const xiiGALBuffer* pIndirectArgumentBuffer, xiiUInt32 uiArgumentOffsetInBytes)
{
  FlushDeferredStateChanges();

  m_pCommandBuffer->drawIndexedIndirect(static_cast<const xiiGALBufferVulkan*>(pIndirectArgumentBuffer)->GetVkBuffer(), uiArgumentOffsetInBytes, 1, 0);
}

void xiiGALCommandEncoderImplVulkan::DrawInstancedPlatform(xiiUInt32 uiVertexCountPerInstance, xiiUInt32 uiInstanceCount, xiiUInt32 uiStartVertex)
{
  FlushDeferredStateChanges();
...
void xiiGALCommandEncoderImplVulkan::DrawInstancedIndirectPlatform(const xiiGALBuffer* pIndirectArgumentBuffer, xiiUInt32 uiArgumentOffsetInBytes)
{
  FlushDeferredStateChanges();

  m_pCommandBuffer->drawIndirect(static_cast<const xiiGALBufferVulkan*>(pIndirectArgumentBuffer)->GetVkBuffer(), uiArgumentOffsetInBytes, 1, 0);
}

void xiiGALCommandEncoderImplVulkan::DrawAutoPlatform()
{
  //FlushDeferredStateChanges();
...
void xiiGALCommandEncoderImplVulkan::BeginStreamOutPlatform()
{
  FlushDeferredStateChanges();
}

void xiiGALCommandEncoderImplVulkan::EndStreamOutPlatform()
{
  XII_ASSERT_NOT_IMPLEMENTED;
}

void xiiGALCommandEncoderImplVulkan::SetIndexBufferPlatform(const xiiGALBuffer* pIndexBuffer)
{
  if (m_pIndexBuffer != pIndexBuffer)
  {
    m_pIndexBuffer      = static_cast<const xiiGALBufferVulkan*>(pIndexBuffer);
    m_bIndexBufferDirty = true;
  }
}

void xiiGALCommandEncoderImplVulkan::SetVertexBufferPlatform(xiiUInt32 uiSlot, const xiiGALBuffer* pVertexBuffer)
{
  XII_ASSERT_DEV(uiSlot < XII_GAL_MAX_VERTEX_BUFFER_COUNT, "Invalid slot index");
  vk::Buffer buffer = pVertexBuffer != nullptr ? static_cast<const xiiGALBufferVulkan*>(pVertexBuffer)->GetVkBuffer() : nullptr;
...
void xiiGALCommandEncoderImplVulkan::SetVertexDeclarationPlatform(const xiiGALVertexDeclaration* pVertexDeclaration)
{
  if (m_PipelineDesc.m_pCurrentVertexDecl != pVertexDeclaration)
  {
    m_PipelineDesc.m_pCurrentVertexDecl = static_cast<const xiiGALVertexDeclarationVulkan*>(pVertexDeclaration);
    m_bPipelineStateDirty               = true;
  }
}

void xiiGALCommandEncoderImplVulkan::SetPrimitiveTopologyPlatform(xiiGALPrimitiveTopology::Enum Topology)
{
...
void xiiGALCommandEncoderImplVulkan::SetBlendStatePlatform(const xiiGALBlendState* pBlendState, const xiiColor& BlendFactor, xiiUInt32 uiSampleMask)
{
  //#TODO_VULKAN BlendFactor / uiSampleMask ?
  if (m_PipelineDesc.m_pCurrentBlendState != pBlendState)
  {
    m_PipelineDesc.m_pCurrentBlendState = pBlendState != nullptr ? static_cast<const xiiGALBlendStateVulkan*>(pBlendState) : nullptr;
    m_bPipelineStateDirty               = true;
  }
}

void xiiGALCommandEncoderImplVulkan::SetDepthStencilStatePlatform(const xiiGALDepthStencilState* pDepthStencilState, xiiUInt8 uiStencilRefValue)
{
  //#TODO_VULKAN uiStencilRefValue ?
  if (m_PipelineDesc.m_pCurrentDepthStencilState != pDepthStencilState)
  {
    m_PipelineDesc.m_pCurrentDepthStencilState = pDepthStencilState != nullptr ? static_cast<const xiiGALDepthStencilStateVulkan*>(pDepthStencilState) : nullptr;
    m_bPipelineStateDirty                      = true;
  }
}

void xiiGALCommandEncoderImplVulkan::SetRasterizerStatePlatform(const xiiGALRasterizerState* pRasterizerState)
{
  if (m_PipelineDesc.m_pCurrentRasterizerState != pRasterizerState)
  {
    m_PipelineDesc.m_pCurrentRasterizerState = pRasterizerState != nullptr ? static_cast<const xiiGALRasterizerStateVulkan*>(pRasterizerState) : nullptr;
...
void xiiGALCommandEncoderImplVulkan::SetViewportPlatform(const xiiRectFloat& rect, float fMinDepth, float fMaxDepth)
{
...
void xiiGALCommandEncoderImplVulkan::SetScissorRectPlatform(const xiiRectU32& rect)
{
...
void xiiGALCommandEncoderImplVulkan::SetStreamOutBufferPlatform(xiiUInt32 uiSlot, const xiiGALBuffer* pBuffer, xiiUInt32 uiOffset)
{
  XII_ASSERT_NOT_IMPLEMENTED;
}
...
void xiiGALCommandEncoderImplVulkan::BeginCompute()
{
  m_bClearSubmitted     = true;
...
void xiiGALCommandEncoderImplVulkan::EndCompute()
{
  m_bInsideCompute = false;
}

void xiiGALCommandEncoderImplVulkan::DispatchPlatform(xiiUInt32 uiThreadGroupCountX, xiiUInt32 uiThreadGroupCountY, xiiUInt32 uiThreadGroupCountZ)
{
  FlushDeferredStateChanges();
  m_pCommandBuffer->dispatch(uiThreadGroupCountX, uiThreadGroupCountY, uiThreadGroupCountZ);
}

void xiiGALCommandEncoderImplVulkan::DispatchIndirectPlatform(const xiiGALBuffer* pIndirectArgumentBuffer, xiiUInt32 uiArgumentOffsetInBytes)
{
  FlushDeferredStateChanges();
  m_pCommandBuffer->dispatchIndirect(static_cast<const xiiGALBufferVulkan*>(pIndirectArgumentBuffer)->GetVkBuffer(), uiArgumentOffsetInBytes);
}

//////////////////////////////////////////////////////////////////////////

#if 0
static void SetShaderResources(xiiGALShaderStage::Enum stage, ID3D11DeviceContext* pContext, xiiUInt32 uiStartSlot, xiiUInt32 uiNumSlots,
...
static void SetConstantBuffers(xiiGALShaderStage::Enum stage, ID3D11DeviceContext* pContext, xiiUInt32 uiStartSlot, xiiUInt32 uiNumSlots,
...
static void SetSamplers(xiiGALShaderStage::Enum stage, ID3D11DeviceContext* pContext, xiiUInt32 uiStartSlot, xiiUInt32 uiNumSlots,
...
void xiiGALCommandEncoderImplVulkan::FlushDeferredStateChanges()
{
  ...
  if (m_bPipelineStateDirty)
  {
    ...
    {
      xiiLog::Error("No shader set");
      return;
    }
    const xiiGALShaderVulkan::DescriptorSetLayoutDesc& descriptorLayoutDesc = m_PipelineDesc.m_pCurrentShader->GetDescriptorSetLayout();

    m_LayoutDesc.m_layout   = xiiResourceCacheVulkan::RequestDescriptorSetLayout(descriptorLayoutDesc);
    m_PipelineDesc.m_layout = xiiResourceCacheVulkan::RequestPipelineLayout(m_LayoutDesc);
    m_ComputeDesc.m_layout  = m_PipelineDesc.m_layout;

    vk::Pipeline pipeline;
    if (m_bInsideCompute)
    {
      pipeline = xiiResourceCacheVulkan::RequestComputePipeline(m_ComputeDesc);
    }
    else
    {
      pipeline = xiiResourceCacheVulkan::RequestGraphicsPipeline(m_PipelineDesc);
    }
  ...
  if (!m_bInsideCompute && m_bViewportDirty)
  {
    ...
    {
      vk::Rect2D noScissor({int(m_viewport.x), int(m_viewport.y + m_viewport.height)}, {xiiUInt32(m_viewport.width), xiiUInt32(-m_viewport.height)});
  ...
  {
    //#TODO_VULKAN we always create a new descriptor set as we don't know if a buffer was modified since the last draw call (xiiGALBufferVulkan::DiscardBuffer).
    // Need to figure out a fast check if any buffer or buffer of a resource view was discarded.
    m_bDescriptorsDirty = false;

    m_DescriptorWrites.Clear();
    vk::DescriptorSet descriptorSet = xiiDescriptorSetPoolVulkan::CreateDescriptorSet(m_LayoutDesc.m_layout);

    xiiArrayPtr<const xiiGALShaderVulkan::BindingMapping> bindingMapping = m_PipelineDesc.m_pCurrentShader->GetBindingMapping();
    const xiiUInt32                                       uiCount        = bindingMapping.GetCount();
    for (xiiUInt32 i = 0; i < uiCount; i++)
    {
      const xiiGALShaderVulkan::BindingMapping& mapping = bindingMapping[i];
      ...
      {
        case xiiGALShaderVulkan::BindingMapping::ConstantBuffer:
        {
          const xiiGALBufferVulkan* pBuffer = m_pBoundConstantBuffers[mapping.m_uiSource];
          write.pBufferInfo                 = &pBuffer->GetBufferInfo();
        }
        break;
        case xiiGALShaderVulkan::BindingMapping::ResourceView:
        {
          const xiiGALResourceViewVulkan* pResourceView = nullptr;
          ...
          if (!pResourceView)
          {
            xiiStringBuilder bla    = mapping.m_sName;
            bool             bDepth = bla.FindSubString_NoCase("shadow") != nullptr || bla.FindSubString_NoCase("depth");
            pResourceView           = xiiFallbackResourcesVulkan::GetFallbackResourceView(mapping.m_descriptorType, mapping.m_xiiType, bDepth);
          }
          if (!pResourceView->GetDescription().m_hTexture.IsInvalidated())
          {
            write.pImageInfo = &pResourceView->GetImageInfo(xiiShaderResourceType::IsArray(mapping.m_xiiType));

            const xiiGALTextureVulkan* pTexture = static_cast<const xiiGALTextureVulkan*>(pResourceView->GetResource()->GetParentResource());
        ...
        break;
        case xiiGALShaderVulkan::BindingMapping::UAV:
        {
          const xiiGALUnorderedAccessViewVulkan* pUAV = m_pBoundUnoderedAccessViews[mapping.m_uiSource];
          if (!pUAV->GetDescription().m_hTexture.IsInvalidated())
          {
            write.pImageInfo = &pUAV->GetImageInfo();

            const xiiGALTextureVulkan* pTexture = static_cast<const xiiGALTextureVulkan*>(pUAV->GetResource()->GetParentResource());
        ...
        break;
        case xiiGALShaderVulkan::BindingMapping::Sampler:
        {
          const xiiGALSamplerStateVulkan* pSampler = m_pBoundSamplerStates[mapping.m_stage][mapping.m_uiSource];
    ...
    xiiDescriptorSetPoolVulkan::UpdateDescriptorSet(descriptorSet, m_DescriptorWrites);
