...
struct xiiVulkanAllocationCreateFlags
{
...
struct xiiVulkanAllocationCreateInfo
{
  xiiBitflags<xiiVulkanAllocationCreateFlags> m_flags;
  xiiEnum<xiiVulkanMemoryUsage>               m_usage;
  const char*                                 m_pUserData = nullptr;
};

/// \brief Subset of VmaAllocationInfo. Duplicated for abstraction purposes.
struct xiiVulkanAllocationInfo
{
...
class XII_RENDERERVULKAN_DLL xiiMemoryAllocatorVulkan
{
public:
  static vk::Result Initialize(vk::PhysicalDevice physicalDevice, vk::Device device, vk::Instance instance);
  static void       DeInitialize();

  static vk::Result CreateImage(const vk::ImageCreateInfo& imageCreateInfo, const xiiVulkanAllocationCreateInfo& allocationCreateInfo, vk::Image& out_image, xiiVulkanAllocation& out_alloc, xiiVulkanAllocationInfo* pAllocInfo = nullptr);
  static void       DestroyImage(vk::Image& image, xiiVulkanAllocation& alloc);

  static vk::Result CreateBuffer(const vk::BufferCreateInfo& bufferCreateInfo, const xiiVulkanAllocationCreateInfo& allocationCreateInfo, vk::Buffer& out_buffer, xiiVulkanAllocation& out_alloc, xiiVulkanAllocationInfo* pAllocInfo = nullptr);
  static void       DestroyBuffer(vk::Buffer& buffer, xiiVulkanAllocation& alloc);

  static xiiVulkanAllocationInfo GetAllocationInfo(xiiVulkanAllocation alloc);
  static void                    SetAllocationUserData(xiiVulkanAllocation alloc, const char* pUserData);

  static vk::Result MapMemory(xiiVulkanAllocation alloc, void** pData);
  static void       UnmapMemory(xiiVulkanAllocation alloc);
  static vk::Result FlushAllocation(xiiVulkanAllocation alloc, vk::DeviceSize offset = 0, vk::DeviceSize size = VK_WHOLE_SIZE);
  static vk::Result InvalidateAllocation(xiiVulkanAllocation alloc, vk::DeviceSize offset = 0, vk::DeviceSize size = VK_WHOLE_SIZE);
