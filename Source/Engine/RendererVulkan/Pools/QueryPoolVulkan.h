...
class XII_RENDERERVULKAN_DLL xiiQueryPoolVulkan
{
public:
  ...
  void Initialize(xiiGALDeviceVulkan* pDevice, xiiUInt32 uiValidBits);
  void DeInitialize();
  ...
  void InsertTimestamp(vk::CommandBuffer commandBuffer, xiiGALTimestampHandle hTimestamp, vk::PipelineStageFlagBits pipelineStage = vk::PipelineStageFlagBits::eBottomOfPipe);
  ...
  xiiResult GetTimestampResult(xiiGALTimestampHandle hTimestamp, xiiTime& result, bool bForce = false);
...
private:
  ...
  static constexpr xiiUInt32 s_uiRetainFrames = 4;
  static constexpr xiiUInt32 s_uiPoolSize     = 256;

  struct TimestampPool
  {
    vk::QueryPool             m_pool;
    bool                      m_bReady = false;
    xiiDynamicArray<uint64_t> m_queryResults;
  };

  struct FramePool
  {
    xiiUInt64                         m_uiNextIndex    = 0;
    xiiUInt64                         m_uiFrameCounter = 0;
    xiiHybridArray<TimestampPool*, 2> m_pools;
  };
  ...
  xiiUInt64 m_uiValidBitsMask     = 0;
  xiiTime   m_gpuToCpuDelta;

  // Current active data.
  FramePool*          m_pCurrentFrame     = nullptr;
  xiiUInt64           m_uiFirstFrameIndex = 0;
  xiiDeque<FramePool> m_pendingFrames;

  // Pools.
  xiiHybridArray<vk::QueryPool, 8>  m_resetPools;
  xiiHybridArray<TimestampPool*, 8> m_freePools;
};
