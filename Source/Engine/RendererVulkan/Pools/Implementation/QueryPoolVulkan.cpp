...
void xiiQueryPoolVulkan::Initialize(xiiGALDeviceVulkan* pDevice, xiiUInt32 uiValidBits)
{
  m_pDevice = pDevice;
  m_device  = pDevice->GetVulkanDevice();
  XII_ASSERT_DEV(pDevice->GetPhysicalDeviceProperties().limits.timestampComputeAndGraphics, "Timestamps not supported by hardware.");
  m_fNanoSecondsPerTick = pDevice->GetPhysicalDeviceProperties().limits.timestampPeriod;

  m_uiValidBitsMask = uiValidBits == 64 ? xiiMath::MaxValue<xiiUInt64>() : ((1ull << uiValidBits) - 1);
}

void xiiQueryPoolVulkan::DeInitialize()
{
  for (FramePool& framePool : m_pendingFrames)
  {
    for (xiiUInt32 i = 0; i < framePool.m_pools.GetCount(); i++)
    {
  ...
  for (TimestampPool* pPool : m_freePools)
  {
    ...
    XII_DEFAULT_DELETE(pPool);
  }
  m_freePools.Clear();
}

void xiiQueryPoolVulkan::Calibrate()
{
  ...
  xiiTime systemTS;
  xiiThreadUtils::Sleep(xiiTime::Milliseconds(100)); // Waiting for 100ms should be enough for the GPU to have gotten stuck on the event right?
  m_device.setEvent(event);
  systemTS = xiiTime::Now();

  VK_ASSERT_DEV(m_device.waitForFences(1, &fence, true, xiiMath::MaxValue<xiiUInt64>()));
...
void xiiQueryPoolVulkan::BeginFrame(vk::CommandBuffer commandBuffer)
{
  xiiUInt64 uiCurrentFrame = m_pDevice->GetCurrentFrame();
  xiiUInt64 uiSafeFrame    = m_pDevice->GetSafeFrame();
  ...
  for (FramePool& framePool : m_pendingFrames)
  {
    ...
    for (xiiUInt32 i = 0; i < framePool.m_pools.GetCount(); i++)
    {
      ...
      if (!pPool->m_bReady)
      {
        xiiUInt32 uiQueryCount = s_uiPoolSize;
        if (i + 1 == m_pendingFrames[0].m_pools.GetCount())
          uiQueryCount = framePool.m_uiNextIndex % s_uiPoolSize;
        if (uiQueryCount > 0)
        {
          vk::Result res = m_device.getQueryPoolResults(pPool->m_pool, 0, uiQueryCount, s_uiPoolSize * sizeof(xiiUInt64), pPool->m_queryResults.GetData(), sizeof(xiiUInt64), vk::QueryResultFlagBits::e64);
...
xiiGALTimestampHandle xiiQueryPoolVulkan::GetTimestamp()
{
  const xiiUInt64 uiPoolIndex = m_pCurrentFrame->m_uiNextIndex / s_uiPoolSize;
  if (uiPoolIndex == m_pCurrentFrame->m_pools.GetCount())
  {
    m_pCurrentFrame->m_pools.PushBack(GetFreePool());
  }

  xiiGALTimestampHandle res = {m_pCurrentFrame->m_uiNextIndex, m_pCurrentFrame->m_uiFrameCounter};
  m_pCurrentFrame->m_uiNextIndex++;
  return res;
}

void xiiQueryPoolVulkan::InsertTimestamp(vk::CommandBuffer commandBuffer, xiiGALTimestampHandle hTimestamp, vk::PipelineStageFlagBits pipelineStage)
{
  for (vk::QueryPool pool : m_resetPools)
  {
    commandBuffer.resetQueryPool(pool, 0, s_uiPoolSize);
  ...
  m_resetPools.Clear();
  const xiiUInt32 uiPoolIndex  = (xiiUInt32)hTimestamp.m_uiIndex / s_uiPoolSize;
  const xiiUInt32 uiQueryIndex = (xiiUInt32)hTimestamp.m_uiIndex % s_uiPoolSize;
  XII_ASSERT_DEBUG(hTimestamp.m_uiFrameCounter == m_pCurrentFrame->m_uiFrameCounter, "Timestamps must be created and used in the same frame!");

  commandBuffer.writeTimestamp(pipelineStage, m_pCurrentFrame->m_pools[uiPoolIndex]->m_pool, uiQueryIndex);
}

xiiResult xiiQueryPoolVulkan::GetTimestampResult(xiiGALTimestampHandle hTimestamp, xiiTime& result, bool bForce)
{
  if (hTimestamp.m_uiFrameCounter >= m_uiFirstFrameIndex)
  {
    const xiiUInt32 uiFrameIndex = static_cast<xiiUInt32>(hTimestamp.m_uiFrameCounter - m_uiFirstFrameIndex);
    const xiiUInt32 uiPoolIndex  = (xiiUInt32)hTimestamp.m_uiIndex / s_uiPoolSize;
    const xiiUInt32 uiQueryIndex = (xiiUInt32)hTimestamp.m_uiIndex % s_uiPoolSize;
    ...
    if (pPool->m_bReady)
    {
      result = xiiTime::Nanoseconds(m_fNanoSecondsPerTick * pPool->m_queryResults[uiQueryIndex]) + m_gpuToCpuDelta;
      return XII_SUCCESS;
    }
    else if (bForce)
    {
      vk::Result res = m_device.getQueryPoolResults(pPool->m_pool, uiQueryIndex, 1, sizeof(xiiUInt64), &pPool->m_queryResults[uiQueryIndex], sizeof(xiiUInt64), vk::QueryResultFlagBits::e64);
      result         = xiiTime::Nanoseconds(m_fNanoSecondsPerTick * pPool->m_queryResults[uiQueryIndex]) + m_gpuToCpuDelta;
      return res == vk::Result::eSuccess ? XII_SUCCESS : XII_FAILURE;
    }
    return XII_FAILURE;
  }
  else
...
xiiQueryPoolVulkan::TimestampPool* xiiQueryPoolVulkan::GetFreePool()
{
  ...
  TimestampPool* pPool = XII_DEFAULT_NEW(TimestampPool);
  pPool->m_queryResults.SetCount(s_uiPoolSize, 0);
