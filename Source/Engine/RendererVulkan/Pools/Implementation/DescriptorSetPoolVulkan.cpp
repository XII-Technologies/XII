...
vk::DescriptorPool                      xiiDescriptorSetPoolVulkan::s_currentPool;
xiiHybridArray<vk::DescriptorPool, 4>   xiiDescriptorSetPoolVulkan::s_freePools;
vk::Device                              xiiDescriptorSetPoolVulkan::s_device;
xiiHashTable<vk::DescriptorType, float> xiiDescriptorSetPoolVulkan::s_descriptorWeights;

void xiiDescriptorSetPoolVulkan::Initialize(vk::Device device)
{
  s_device                                                     = device;
...
void xiiDescriptorSetPoolVulkan::DeInitialize()
{
...
xiiHashTable<vk::DescriptorType, float>& xiiDescriptorSetPoolVulkan::AccessDescriptorPoolWeights()
{
  return s_descriptorWeights;
}

vk::DescriptorSet xiiDescriptorSetPoolVulkan::CreateDescriptorSet(vk::DescriptorSetLayout layout)
{
  ...
  if (bPoolExhausted)
  {
    xiiGALDeviceVulkan* pDevice = static_cast<xiiGALDeviceVulkan*>(xiiGALDevice::GetDefaultDevice());
...
void xiiDescriptorSetPoolVulkan::UpdateDescriptorSet(vk::DescriptorSet descriptorSet, xiiArrayPtr<vk::WriteDescriptorSet> update)
{
  s_device.updateDescriptorSets(update.GetCount(), update.GetPtr(), 0, nullptr);
}

void xiiDescriptorSetPoolVulkan::ReclaimPool(vk::DescriptorPool& descriptorPool)
{
  s_device.resetDescriptorPool(descriptorPool);
  s_freePools.PushBack(descriptorPool);
}

vk::DescriptorPool xiiDescriptorSetPoolVulkan::GetNewPool()
{
  if (s_freePools.IsEmpty())
  {
    xiiHybridArray<vk::DescriptorPoolSize, 20> poolSizes;
    for (auto weight : s_descriptorWeights)
    {
      if (static_cast<xiiUInt32>(weight.Value() * s_uiPoolBaseSize) > 0)
        poolSizes.PushBack(vk::DescriptorPoolSize(weight.Key(), static_cast<xiiUInt32>(weight.Value() * s_uiPoolBaseSize)));
    }
