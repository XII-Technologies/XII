...
namespace
{
  xiiResult GetAlternativeFormat(vk::Format& format, vk::ComponentMapping& componentMapping)
  {
    switch (format)
    {
  ...
  xiiGALResourceFormat::Enum GetResourceFormat(vk::Format& format)
  {
    switch (format)
    {
      case vk::Format::eR8G8B8A8Srgb:
      ...
      default:
        return xiiGALResourceFormat::ENUM_COUNT;
    }
  }
} // namespace
...
void xiiGALSwapChainVulkan::AcquireNextRenderTarget(xiiGALDevice* pDevice)
{
  ...
  XII_ASSERT_DEV(!m_currentPipelineImageAvailableSemaphore, "Pipeline semaphores leaked");
  m_currentPipelineImageAvailableSemaphore = xiiSemaphorePoolVulkan::RequestSemaphore();
  ...
  {
    xiiLog::Warning("Swap-chain does not match the target window size and should be recreated.");
  }

#ifdef VK_LOG_LAYOUT_CHANGES
  xiiLog::Warning("AcquireNextRenderTarget {}", xiiArgP(static_cast<void*>(m_swapChainImages[m_uiCurrentSwapChainImage])));
#endif

  m_RenderTargets.m_hRTs[0] = m_swapChainTextures[m_uiCurrentSwapChainImage];
}

void xiiGALSwapChainVulkan::PresentRenderTarget(xiiGALDevice* pDevice)
{
  XII_PROFILE_SCOPE("PresentRenderTarget");

  auto pVulkanDevice = static_cast<xiiGALDeviceVulkan*>(pDevice);
  {
    auto                       view     = xiiGALDevice::GetDefaultDevice()->GetDefaultRenderTargetView(m_RenderTargets.m_hRTs[0]);
    const xiiGALTextureVulkan* pTexture = static_cast<const xiiGALTextureVulkan*>(pVulkanDevice->GetTexture(m_swapChainTextures[m_uiCurrentSwapChainImage]));
  ...
  vk::Semaphore currentPipelineRenderFinishedSemaphore = xiiSemaphorePoolVulkan::RequestSemaphore();
...
#ifdef VK_LOG_LAYOUT_CHANGES
    xiiLog::Warning("PresentInfoKHR {}", xiiArgP(m_swapChainImages[m_uiCurrentSwapChainImage]));
#endif
...
xiiResult xiiGALSwapChainVulkan::UpdateSwapChain(xiiGALDevice* pDevice, xiiEnum<xiiGALPresentMode> newPresentMode)
{
  XII_ASSERT_DEBUG(!m_currentPipelineImageAvailableSemaphore, "UpdateSwapChain must not be called between AcquireNextRenderTarget and PresentRenderTarget.");
  m_currentPresentMode = newPresentMode;
  return CreateSwapChainInternal();
}

xiiGALSwapChainVulkan::xiiGALSwapChainVulkan(const xiiGALWindowSwapChainCreationDescription& Description) :
  xiiGALWindowSwapChain(Description), m_vulkanSwapChain(nullptr)
{
}
...
{
  m_pVulkanDevice      = static_cast<xiiGALDeviceVulkan*>(pDevice);
  ...
  xiiWindowHandle windowHandle = m_WindowDesc.m_pWindow->GetNativeWindowHandle();
  switch (windowHandle.type)
  {
    case xiiWindowHandle::Type::Invalid:
      xiiLog::Error("Invalid native window handle for window \"{}\"", m_WindowDesc.m_pWindow);
      return XII_FAILURE;
    case xiiWindowHandle::Type::GLFW:
    {
      VkSurfaceKHR glfwSurface = VK_NULL_HANDLE;
      VK_SUCCEED_OR_RETURN_XII_FAILURE(glfwCreateWindowSurface(m_pVulkanDevice->GetVulkanInstance(), windowHandle.glfwWindow, nullptr, &glfwSurface));
      m_vulkanSurface = glfwSurface;
    }
    break;
    case xiiWindowHandle::Type::XCB:
    {
      vk::XcbSurfaceCreateInfoKHR surfaceCreateInfo = {};
      XII_ASSERT_DEV(windowHandle.xcbWindow.m_pConnection != nullptr && windowHandle.xcbWindow.m_Window != 0, "Invalid xcb handle");
  ...
  VkSurfaceKHR glfwSurface = VK_NULL_HANDLE;
  VK_SUCCEED_OR_RETURN_XII_FAILURE(glfwCreateWindowSurface(m_pVulkanDevice->GetVulkanInstance(), m_WindowDesc.m_pWindow->GetNativeWindowHandle(), nullptr, &glfwSurface));
  m_vulkanSurface = glfwSurface;
#else
#  error Platform not supported
  ...
  if (!m_vulkanSurface)
  {
    xiiLog::Error("Failed to create Vulkan surface for window \"{}\"", m_WindowDesc.m_pWindow);
    return XII_FAILURE;
  }
  ...
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanPhysicalDevice().getSurfaceSupportKHR(m_pVulkanDevice->GetGraphicsQueue().m_uiQueueFamily, m_vulkanSurface, &surfaceSupported));

  if (!surfaceSupported)
  {
    xiiLog::Error("Vulkan device does not support surfaces");
...
xiiResult xiiGALSwapChainVulkan::CreateSwapChainInternal()
{
  uint32_t uiPresentModes = 0;
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanPhysicalDevice().getSurfacePresentModesKHR(m_vulkanSurface, &uiPresentModes, nullptr));
  xiiHybridArray<vk::PresentModeKHR, 4> presentModes;
  presentModes.SetCount(uiPresentModes);
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanPhysicalDevice().getSurfacePresentModesKHR(m_vulkanSurface, &uiPresentModes, presentModes.GetData()));
  ...
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanPhysicalDevice().getSurfaceFormatsKHR(m_vulkanSurface, &uiNumSurfaceFormats, nullptr));
  std::vector<vk::SurfaceFormatKHR> supportedFormats;
  supportedFormats.resize(uiNumSurfaceFormats);
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanPhysicalDevice().getSurfaceFormatsKHR(m_vulkanSurface, &uiNumSurfaceFormats, supportedFormats.data()));
  ...
  if (!formatFound)
  {
    xiiStringBuilder backBufferFormatNice = "<unknown>";
    xiiReflectionUtils::EnumerationToString(xiiGetStaticRTTI<xiiGALResourceFormat>(), m_WindowDesc.m_BackBufferFormat, backBufferFormatNice);
    xiiLog::Error("The requested back buffer format {} mapping to the vulkan format {} is not supported on this system.", backBufferFormatNice, vk::to_string(desiredFormat).c_str());
    xiiLog::Info("Available formats are:");
    for (vk::SurfaceFormatKHR& supportedFormat : supportedFormats)
    {
      xiiLog::Info("  format: {}  color space: {}", vk::to_string(supportedFormat.format).c_str(), vk::to_string(supportedFormat.colorSpace).c_str());
    }
    return XII_FAILURE;
  }
  ...
  swapChainCreateInfo.imageExtent.width          = xiiMath::Clamp(swapChainCreateInfo.imageExtent.width, surfaceCapabilities.minImageExtent.width, surfaceCapabilities.maxImageExtent.width);
  swapChainCreateInfo.imageExtent.height         = xiiMath::Clamp(swapChainCreateInfo.imageExtent.height, surfaceCapabilities.minImageExtent.height, surfaceCapabilities.maxImageExtent.height);
  ...
  swapChainCreateInfo.minImageCount = xiiMath::Max(m_WindowDesc.m_bDoubleBuffered ? 2u : 1u, surfaceCapabilities.minImageCount);
  if (surfaceCapabilities.maxImageCount != 0)
    swapChainCreateInfo.minImageCount = xiiMath::Min(swapChainCreateInfo.minImageCount, surfaceCapabilities.maxImageCount);

  swapChainCreateInfo.presentMode  = xiiConversionUtilsVulkan::GetPresentMode(m_currentPresentMode, presentModes);
  ...
  {
    xiiLog::Error("Failed to create Vulkan swap chain!");
    return XII_FAILURE;
  }

  xiiUInt32 uiSwapChainImages = 0;
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanDevice().getSwapchainImagesKHR(m_vulkanSwapChain, &uiSwapChainImages, nullptr));
  m_swapChainImages.SetCount(uiSwapChainImages);
  VK_SUCCEED_OR_RETURN_XII_FAILURE(m_pVulkanDevice->GetVulkanDevice().getSwapchainImagesKHR(m_vulkanSwapChain, &uiSwapChainImages, m_swapChainImages.GetData()));

  XII_ASSERT_DEV(uiSwapChainImages < 4, "If we have more than 3 swap chain images we can't hold ontp fences owned by xiiDeviceVulkan::PerFrameData anymore as that reclaims all fences once it reuses the frame data (which is 4 right now). Thus, we can't safely pass in the fence in xiiGALSwapChainVulkan::PresentRenderTarget as it will be reclaimed before we use it.");
  m_swapChainImageInUseFences.SetCount(uiSwapChainImages);

  for (xiiUInt32 i = 0; i < uiSwapChainImages; i++)
  {
    ...
    m_swapChainTextures.PushBack(m_pVulkanDevice->CreateTextureInternal(TexDesc, xiiArrayPtr<xiiGALSystemMemoryDescription>(), desiredFormat));
  }
  m_CurrentSize             = xiiSizeU32(swapChainCreateInfo.imageExtent.width, swapChainCreateInfo.imageExtent.height);
  m_RenderTargets.m_hRTs[0] = m_swapChainTextures[0];
  return XII_SUCCESS;
}

void xiiGALSwapChainVulkan::DestroySwapChainInternal(xiiGALDeviceVulkan* pVulkanDevice)
{
  xiiUInt32 uiSwapChainImages = m_swapChainTextures.GetCount();
  for (xiiUInt32 i = 0; i < uiSwapChainImages; i++)
  {
...
XII_STATICLINK_FILE(RendererVulkan, RendererVulkan_Device_Implementation_SwapChainVulkan);
