...
xiiInitContextVulkan::xiiInitContextVulkan(xiiGALDeviceVulkan* pDevice) :
  m_pDevice(pDevice)
{
  xiiProxyAllocator& allocator = m_pDevice->GetAllocator();
  m_pPipelineBarrier           = XII_NEW(&allocator, xiiPipelineBarrierVulkan);
  m_pCommandBufferPool         = XII_NEW(&allocator, xiiCommandBufferPoolVulkan);
  m_pCommandBufferPool->Initialize(m_pDevice->GetVulkanDevice(), m_pDevice->GetGraphicsQueue().m_uiQueueFamily);
  m_pStagingBufferPool = XII_NEW(&allocator, xiiStagingBufferPoolVulkan);
  m_pStagingBufferPool->Initialize(m_pDevice);
}

xiiInitContextVulkan::~xiiInitContextVulkan()
{
  m_pCommandBufferPool->DeInitialize();
  m_pStagingBufferPool->DeInitialize();
}

vk::CommandBuffer xiiInitContextVulkan::GetFinishedCommandBuffer()
{
  XII_LOCK(m_Lock);
...
void xiiInitContextVulkan::EnsureCommandBufferExists()
{
  if (!m_currentCommandBuffer)
  {
...
void xiiInitContextVulkan::TextureDestroyed(const xiiGALTextureVulkan* pTexture)
{
  XII_LOCK(m_Lock);
  m_pPipelineBarrier->TextureDestroyed(pTexture);
}

void xiiInitContextVulkan::InitTexture(const xiiGALTextureVulkan* pTexture, vk::ImageCreateInfo& createInfo, xiiArrayPtr<xiiGALSystemMemoryDescription> pInitialData)
{
  XII_LOCK(m_Lock);
  ...
  if (pTexture->GetDescription().m_pExisitingNativeObject == nullptr)
  {
    ...
    xiiDynamicArray<xiiUInt8>                      tempData;
    xiiDynamicArray<xiiGALSystemMemoryDescription> initialData;
    if (pInitialData.IsEmpty())
    {
      ...
      for (xiiUInt32 uiLayer = 0; uiLayer < createInfo.arrayLayers; uiLayer++)
      {
        for (xiiUInt32 uiMipLevel = 0; uiMipLevel < createInfo.mipLevels; uiMipLevel++)
        {
          const xiiUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * createInfo.mipLevels;
          XII_ASSERT_DEBUG(initialData.GetCount() == uiSubresourceIndex, "");
          ...
          xiiGALSystemMemoryDescription data;
    ...
    for (xiiUInt32 uiLayer = 0; uiLayer < createInfo.arrayLayers; uiLayer++)
    {
      ...
      {
        const xiiUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * createInfo.mipLevels;
        XII_ASSERT_DEBUG(uiSubresourceIndex < pInitialData.GetCount(), "Not all data provided in the intial texture data.");
        const xiiGALSystemMemoryDescription& subResourceData = pInitialData[uiSubresourceIndex];

        vk::ImageSubresourceLayers subresourceLayers;
        // We do not support stencil uploads right now.
        subresourceLayers.aspectMask     = xiiConversionUtilsVulkan::IsDepthFormat(pTexture->GetImageFormat()) ? vk::ImageAspectFlagBits::eDepth : vk::ImageAspectFlagBits::eColor;
