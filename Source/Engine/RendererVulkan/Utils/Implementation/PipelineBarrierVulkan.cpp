...
namespace
{
  vk::ImageSubresourceRange CreateSubRange(const vk::ImageSubresourceRange& fullRange, xiiUInt32 uiLayer, xiiUInt32 uiMipLevel)
  {
...
void xiiPipelineBarrierVulkan::SetCommandBuffer(vk::CommandBuffer* pCommandBuffer)
{
  m_pCommandBuffer = pCommandBuffer;
}

void xiiPipelineBarrierVulkan::Flush()
{
  if (m_srcStageMask || m_dstStageMask)
  {
    ...
    {
      xiiLog::Warning("Layout Changed {}: {} -> {}", xiiArgP(img.image), vk::to_string(img.oldLayout).c_str(), vk::to_string(img.newLayout).c_str());
    }
#endif
...
void xiiPipelineBarrierVulkan::Submit()
{
  Flush();
  ...
  for (auto it = m_imageState.GetIterator(); it.IsValid();)
  {
    ...
    const xiiUInt32 uiCount = it.Value().m_subElementLayout.GetCount();
...
bool xiiPipelineBarrierVulkan::IsDirty(vk::Image image, const vk::ImageSubresourceRange& subResources) const
{
...
bool xiiPipelineBarrierVulkan::IsDirty() const
{
  return m_srcStageMask || m_dstStageMask;
}

bool xiiPipelineBarrierVulkan::IsDirty(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::AccessFlags dstAccess)
{
...
void xiiPipelineBarrierVulkan::AccessBuffer(const xiiGALBufferVulkan* pBuffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  SubBufferState subState;
  subState.m_offset     = offset;
  subState.m_length     = length;
...
void xiiPipelineBarrierVulkan::EnsureImageLayout(const xiiGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  EnsureImageLayout(pTexture, pTexture->GetFullRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void xiiPipelineBarrierVulkan::EnsureImageLayout(const xiiGALRenderTargetViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const xiiGALTextureVulkan*>(pTextureView->GetTexture()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void xiiPipelineBarrierVulkan::EnsureImageLayout(const xiiGALResourceViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const xiiGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void xiiPipelineBarrierVulkan::EnsureImageLayout(const xiiGALUnorderedAccessViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const xiiGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void xiiPipelineBarrierVulkan::EnsureImageLayout(const xiiGALTextureVulkan* pTexture, vk::ImageSubresourceRange subResources, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  const vk::ImageSubresourceRange fullRange = pTexture->GetFullRange();
  if (xiiGALResourceFormat::IsStencilFormat(pTexture->GetDescription().m_Format))
  {
  ...
  if (pState)
  {
    ...
    if (bIsFullRange)
    {
      ...
      else
      {
        ...
        const xiiUInt32        uiLastLayer = subResources.baseArrayLayer + subResources.layerCount;
        const xiiUInt32        uiLastLevel = subResources.baseMipLevel + subResources.levelCount;
        for (xiiUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
        {
          for (xiiUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
          {
            const xiiUInt32  uiSubresourceIndex = uiMipLevel + uiLayer * fullRange.levelCount;
...
void xiiPipelineBarrierVulkan::SetInitialImageState(const xiiGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  XII_ASSERT_DEBUG(!it.IsValid(), "Can't set initial state, texture is already tracked.");
...
void xiiPipelineBarrierVulkan::TextureDestroyed(const xiiGALTextureVulkan* pTexture)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  if (it.IsValid())
  {
    ...
    if (it.Value().m_dirty.IsAnyBitSet())
    {
      ...
      for (xiiInt32 i = ((xiiInt32)m_imageBarriers.GetCount()) - 1; i >= 0; --i)
      {
...
void xiiPipelineBarrierVulkan::BufferDestroyed(const xiiGALBufferVulkan* pBuffer)
{
...
void xiiPipelineBarrierVulkan::FullBarrier()
{
  vk::MemoryBarrier memoryBarrier;
...
bool xiiPipelineBarrierVulkan::AddBufferBarrierInternal(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
...
bool xiiPipelineBarrierVulkan::IsDirtyInternal(const BufferState& state, const SubBufferState& subState) const
{
  const xiiUInt32 uiCount = state.m_subBufferState.GetCount();
  for (xiiUInt32 i = 0; i < uiCount; i++)
  {
...
bool xiiPipelineBarrierVulkan::AddImageBarrierInternal(vk::Image image, const vk::ImageSubresourceRange& subResources, vk::ImageLayout srcLayout, vk::AccessFlags srcAccess, vk::ImageLayout dstLayout, vk::AccessFlags dstAccess, bool bDiscardSource)
{
...
bool xiiPipelineBarrierVulkan::IsDirtyInternal(const ImageState& state, const vk::ImageSubresourceRange& subResources) const
{
  ...
  {
    xiiUInt32 uiLevelCount = state.m_pTexture->GetDescription().m_uiMipLevelCount;
    ...
    for (xiiUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
    {
      for (xiiUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
      {
        const xiiUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * uiLevelCount;
