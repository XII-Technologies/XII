...
public:
  ...
  static void ParseShaderResources(xiiStringView sShaderStageSource, xiiDynamicArray<xiiShaderResourceDefinition>& out_resources);

  /// \brief Delegate to creates a new declaration and register binding for a specific shader xiiShaderResourceDefinition.
  ...
  static xiiResult MergeShaderResourceBindings(const xiiShaderProgramData& spd, xiiHashTable<xiiHashedString, xiiGALShaderResourceDescription>& out_bindings, xiiLogInterface* pLog);

  /// \brief Makes sure that bindings fulfills the basic requirements that xiiEngine has for resource bindings in a shader, e.g. that each binding has a set / slot set.
