...
struct XII_GRAPHICSCORE_DLL xiiParticleSimulationPassData
{
  ...
  xiiRGBufferHandle m_hParticleStateWrite;     ///< Buffer handle for writing the next particle state. This is bound as a write-only resource in simulation passes and contains the updated state of all particles at the end of the pass. The exact layout of this buffer is defined by the particle graph and should be compatible with the shader code used for simulation.
  xiiRGBufferHandle m_hAliveIndexBuffer;       ///< Buffer handle for the alive index buffer, which contains the indices of currently alive particles. This is bound as a read-write resource in simulation passes and is updated by the GPU based on particle births and deaths during the pass.
  xiiRGBufferHandle m_hDeadIndexBuffer;        ///< Buffer handle for the dead index buffer, which contains the indices of currently dead particles. This is bound as a read-write resource in simulation passes and is updated by the GPU based on particle births and deaths during the pass.
  xiiRGBufferHandle m_hCountersBuffer;         ///< Buffer handle for the counters buffer, which contains various counters related to the particle system (e.g., alive count, dead count, spawn request count, event count). This is bound as a read-write resource in simulation passes and is updated by the GPU based on particle behavior during the pass.
  xiiRGBufferHandle m_hEventBuffer;            ///< Buffer handle for the event buffer, which contains events emitted by particles during simulation passes. This is bound as a read-write resource in simulation passes when event support is enabled, and is updated by the GPU based on particle behavior during the pass.
  xiiRGBufferHandle m_hSortKeyBuffer;          ///< Buffer handle for the sort key buffer, which contains keys used for sorting particles (e.g., by depth or by ID). This is bound as a read-write resource in simulation passes when sorting is enabled, and is updated by the GPU based on particle behavior during the pass.
  xiiRGBufferHandle m_hNeighborPairBuffer;     ///< Buffer handle for the neighbor pair buffer, which contains pairs of neighboring particles when GPU-based neighbor searching is enabled. This is bound as a read-write resource in simulation passes when neighbor searching is enabled, and is updated by the GPU based on particle positions during the pass.
  xiiRGBufferHandle m_hDrawIndirectBuffer;     ///< Buffer handle for the draw indirect buffer, which contains the arguments for indirect draw calls. This is bound as a read-write resource in simulation passes when indirect drawing is enabled, and is updated by the GPU based on the alive count and other factors during the pass.
  xiiRGBufferHandle m_hDispatchIndirectBuffer; ///< Buffer handle for the dispatch indirect buffer, which contains the arguments for indirect dispatch calls (e.g., for culling or sorting). This is bound as a read-write resource in simulation passes when async compute or GPU culling is enabled, and is updated by the GPU based on the alive count and other factors during the pass.
