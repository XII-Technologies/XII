...
public:
  ...
  xiiArrayPtr<xiiPerInstanceData> GetOrCreateInstanceData(const xiiComponent* pOwnerComponent, bool bDynamic, xiiGALDynamicBufferHandle& out_hBuffer, xiiInstanceDataOffset& inout_instanceDataOffset, xiiUInt32 uiCount = 1) const;
  ...
  static void FillPerInstanceData(xiiPerInstanceData& out_perInstanceData, const xiiGameObject* pObject, const xiiTransform& globalTransform, xiiUInt32 uiUniqueID = 0, const xiiColor& color = xiiColor::White, const xiiVec4& vCustomData = xiiVec4(0, 1, 0, 1), float fBoundingSphereRadius = 1.0f, xiiUInt32 uiRandomSeed = 0);

  /// \brief Helper function that combines GetOrCreateInstanceData and FillPerInstanceData.
  xiiGALDynamicBufferHandle GetOrCreateInstanceDataAndFill(const xiiComponent& ownerComponent, bool bDynamic, const xiiTransform& globalTransform, xiiInstanceDataOffset& inout_instanceDataOffset, xiiUInt32 uiUniqueID = 0, const xiiColor& color = xiiColor::White, const xiiVec4& vCustomData = xiiVec4(0, 1, 0, 1)) const;
  ...
  /// This function is thread-safe and is typically called in an xiiMsgExtractRenderData message handler.
  template <typename T>
  xiiArrayPtr<T> GetOrCreateCustomInstanceData(xiiUInt32 uiCustomDataIndex, const xiiComponent* pOwnerComponent, xiiGALDynamicBufferHandle& out_hBuffer, xiiCustomInstanceDataOffset& inout_instanceDataOffset, xiiUInt32 uiCount = 1) const;
  ...
  template <typename T>
  xiiGALDynamicBufferHandle GetOrCreateCustomInstanceDataAndFill(xiiUInt32 uiCustomDataIndex, const xiiComponent& ownerComponent, xiiCustomInstanceDataOffset& inout_instanceDataOffset, const T& data) const;

  /// \brief Returns the underlying dynamic buffer for the given custom instance data buffer index.
  xiiGALDynamicBufferHandle GetCustomInstanceDataBuffer(xiiUInt32 uiCustomDataIndex) const;
  ...
  xiiGALDynamicBufferHandle GetSkinningDataBuffer() const;

private:
  xiiByteArrayPtr GetOrCreateCustomInstanceData(xiiUInt32 uiCustomDataIndex, xiiUInt32 uiStructByteSize, const xiiComponent* pOwnerComponent, xiiGALDynamicBufferHandle& out_hBuffer, xiiCustomInstanceDataOffset& inout_instanceDataOffset, xiiUInt32 uiCount) const;
  ...
  xiiHybridArray<xiiGALDynamicBufferHandle, 16> m_Buffers;
  xiiDynamicArray<xiiDelegate<void()>> m_BeforeUploadCallbacks;

  struct ExtractionData
  {
    xiiHybridArray<xiiGALDynamicBuffer*, 16> m_pBuffers;
  };
