...
namespace
{
  struct ShadowCascadeSetupData
  {
    ...
    xiiUInt32         m_uiActiveCascades = 0;
    xiiVec3           m_vLightDir        = xiiVec3(0.0f, -1.0f, 0.0f);
...
static void SetupShadowCascadeSetup(xiiView& view, ShadowCascadeSetupData& data, xiiRGBuilder& builder)
{
  ...
  // Read active cascade count and light direction from extracted render data.
  data.m_uiActiveCascades = 3u; // default; overridden by extracted light data
  ...
  const xiiExtractedRenderData* pExtracted = view.GetExtractedRenderData();
  if (pExtracted)
  {
    xiiArrayPtr<xiiRenderData* const> lights = pExtracted->GetRenderData(xiiDefaultRenderDataCategories::Light);
    for (xiiRenderData* pRD : lights)
    {
      if (const auto* pDir = xiiDynamicCast<const xiiDirectionalLightRenderData*>(pRD))
      {
        data.m_vLightDir        = -pDir->m_GlobalTransform.GetColumn(2).GetAsVec3().GetNormalized();
  ...
  desc.m_uiElementByteStride = sizeof(xiiShadowCascadeConstants);
  desc.m_uiSize              = desc.m_uiElementByteStride;
  desc.m_BindFlags           = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
  desc.m_Mode                = xiiGALBufferMode::Structured;
  data.m_hCascadeMatrices    = builder.WriteBuffer(xiiRGBlackboardKeys::k_ShadowCascadeMatrices, desc, xiiGALResourceStateFlags::UnorderedAccess);

  xiiView::EnsureComputePipeline(sp.m_pCascadeSetupPipeline, "Shaders/Pipeline/ShadowCascadeSetup.xiiShader");
...
static void ExecuteShadowCascadeSetup(xiiView& view, const ShadowCascadeSetupData& data, xiiRGPassContext& ctx)
{
  ...
  {
    // CPU-upload the cascade constant block inline.
    xiiGALMapHelper<xiiShadowCascadeConstants> pConsts(cmd, ctx.GetBuffer(data.m_hCascadeMatrices), xiiGALMapType::Write, xiiGALMapFlags::Discard);
    pConsts->ActiveCascadeCount = data.m_uiActiveCascades;
    ...
    // (Full implementation would call xiiMath::ComputeCascadeViewProjection; simplified for now.)
    for (xiiUInt32 i = 0; i < 4; ++i)
      pConsts->CascadeViewProjection[i] = xiiMat4::MakeIdentity();
  }

  cmd.SetPipelineState(sp.m_pCascadeSetupPipeline);
  cmd.ResolveAndSetUnorderedAccessBufferView("g_CascadeOut", ctx.GetBuffer(data.m_hCascadeMatrices)->GetDefaultView(xiiGALBufferViewType::UnorderedAccess), xiiGALShaderType::Compute);
...
static void SetupDirShadow(xiiView& view, DirShadowData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  xiiRGBufferHandle hCascades;
  bb.TryGetValue(xiiMakeHashedString(xiiRGBlackboardKeys::k_ShadowCascadeMatrices), hCascades);
  data.m_hCascadeMatrices = builder.ReadBuffer(hCascades, xiiGALResourceStateFlags::ShaderResource);
...
static void ExecuteDirShadow(xiiView& view, const DirShadowData& data, xiiRGPassContext& ctx)
{
  ...
  {
    cmd.BeginDebugGroup(xiiStringView("Cascade{}").GetData()); // placeholder label
    ...
    cmd.SetViewports({{0.0f, 0.0f, static_cast<float>(k_uiDirAtlasWidth), static_cast<float>(k_uiDirAtlasHeight), 0.0f, 1.0f}}, k_uiDirAtlasWidth, k_uiDirAtlasHeight);
    ...
    if (sp.m_pShadowDepthPipeline)
    {
      cmd.SetPipelineState(sp.m_pShadowDepthPipeline);
      // cascade index uploaded via push constant / cbuffer update.
      cmd.DrawIndexedIndirect(ctx.GetBuffer(data.m_hShadowCasterCommands), uiCascade * 20u /*stride per cascade DrawIndexedIndirectArguments*/);
    }
