...
static void SetupForwardOpaque(xiiView& view, ForwardOpaqueData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  auto readBuf = [&](xiiRGBufferHandle& h, const char* k) {
    xiiRGBufferHandle b2;
    bb.TryGetValue(xiiMakeHashedString(k), b2);
    if (b2.IsValid()) h = builder.ReadBuffer(b2, xiiGALResourceStateFlags::ShaderResource);
  };
  ...
  {
    xiiRGBufferHandle h;
    bb.TryGetValue(xiiMakeHashedString(xiiRGBlackboardKeys::k_DrawIndirectCommands), h);
