...
#define READ_TEX(hOut, key) \
  do { xiiRGTextureHandle _h; bb.TryGetValue(xiiMakeHashedString(key), _h); if (_h.IsValid()) hOut = builder.ReadTexture(_h, xiiGALResourceStateFlags::ShaderResource); } while(false)

#define WRITE_TEX_HDR(hOut, key, w, h) \
  do { \
    xiiGALTextureCreationDescription _d; _d.m_TextureType = xiiGALTextureType::Texture2D; _d.m_Format = xiiGALTextureFormat::RGBA16Float; \
    _d.m_uiWidth = (w); _d.m_uiHeight = (h); _d.m_uiMipLevels = 1u; \
    _d.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource; _d.m_Usage = xiiGALResourceUsage::Default; \
    hOut = builder.WriteTexture(key, _d, xiiGALResourceStateFlags::UnorderedAccess); \
  } while(false)

#define WRITE_TEX_R8(hOut, key, w, h) \
  do { \
    xiiGALTextureCreationDescription _d; _d.m_TextureType = xiiGALTextureType::Texture2D; _d.m_Format = xiiGALTextureFormat::R8Unorm; \
    _d.m_uiWidth = (w); _d.m_uiHeight = (h); _d.m_uiMipLevels = 1u; \
    _d.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource; _d.m_Usage = xiiGALResourceUsage::Default; \
    hOut = builder.WriteTexture(key, _d, xiiGALResourceStateFlags::UnorderedAccess); \
  } while(false)
...
#define BIND_SRV_TEX(slot, h) \
  do { if ((h).IsValid()) cmd.ResolveAndSetShaderResourceView(slot, ctx.GetTexture(h)->GetDefaultView(xiiGALTextureViewType::ShaderResource), xiiGALShaderType::Compute); } while(false)

#define BIND_SRV_BUF(slot, h) \
  do { if ((h).IsValid()) cmd.ResolveAndSetShaderResourceBufferView(slot, ctx.GetBuffer(h)->GetDefaultView(xiiGALBufferViewType::ShaderResource), xiiGALShaderType::Compute); } while(false)

#define BIND_UAV_TEX(slot, h) \
  do { if ((h).IsValid()) cmd.ResolveAndSetUnorderedAccessView(slot, ctx.GetTexture(h)->GetDefaultView(xiiGALTextureViewType::UnorderedAccess), xiiGALShaderType::Compute); } while(false)
...
namespace { struct ParticleSimData { xiiRGBufferHandle m_hParticleState; xiiUInt32 m_uiParticleCount = 0; }; }
...
static void SetupSSDecal(xiiView& view, SSDecalData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  xiiGALBufferCreationDescription desc;
  desc.m_uiElementByteStride = 4u; desc.m_uiSize = 4u * 1024u * 16u;
  desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess; desc.m_Mode = xiiGALBufferMode::Structured;
...
namespace
{
  struct SSGIData { xiiRGTextureHandle m_hSceneDepth, m_hGBufNormal, m_hHDRIn, m_hSSGIOut; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct SSRefractData { xiiRGTextureHandle m_hSceneDepth, m_hHDRIn, m_hHDROut, m_hGBufNormal; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct PlanarReflData { xiiRGTextureHandle m_hPlanarTarget; xiiUInt32 m_W=1920u, m_H=1080u; };
}
...
static void SetupPlanarRefl(xiiView& view, PlanarReflData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  if (!sp.m_pPlanarReflectionTarget)
  {
    ...
    desc.m_uiWidth     = data.m_W / 2u; desc.m_uiHeight = data.m_H / 2u;
...
void xiiView::BuildStage9_ScreenSpace(xiiRenderGraph& graph, const xiiRenderGraphBlackboard& blackboard)
{
  ...
  graph.AddPass<SSGIData>("SSGI", xiiGALCommandQueueFlags::Compute,
    [self, &bb](SSGIData& d, xiiRGBuilder& b) { SetupSSGI(*self, d, b, bb); },
    [self](const SSGIData& d, xiiRGPassContext& c) { ExecuteSSGI(*self, d, c); });

  graph.AddPass<SSRefractData>("SSRefraction", xiiGALCommandQueueFlags::Compute,
    [self, &bb](SSRefractData& d, xiiRGBuilder& b) { SetupSSRefraction(*self, d, b, bb); },
    [self](const SSRefractData& d, xiiRGPassContext& c) { ExecuteSSRefraction(*self, d, c); });

  graph.AddPass<PlanarReflData>("PlanarReflections", xiiGALCommandQueueFlags::Graphics,
    [self, &bb](PlanarReflData& d, xiiRGBuilder& b) { SetupPlanarRefl(*self, d, b, bb); },
    [self](const PlanarReflData& d, xiiRGPassContext& c) { ExecutePlanarRefl(*self, d, c); });
}
...
namespace
{
  struct LumHistogramData { xiiRGTextureHandle m_hHDRIn; xiiRGBufferHandle m_hHistogram; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct AutoExposureData { xiiRGBufferHandle m_hHistogram, m_hExposure; };
  struct TAAData          { xiiRGTextureHandle m_hHDRIn, m_hVelocity, m_hHistory, m_hTAAOut; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct UpscaleData      { xiiRGTextureHandle m_hTAAIn, m_hUpscaled; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct SharpenData      { xiiRGTextureHandle m_hUpscaled, m_hSharpened; xiiUInt32 m_W=1920u, m_H=1080u; };
}
...
static void SetupLumHistogram(xiiView& view, LumHistogramData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  READ_TEX(data.m_hHDRIn, xiiRGBlackboardKeys::k_HDRSceneColor);
  xiiGALBufferCreationDescription desc;
  desc.m_uiElementByteStride = 4u; desc.m_uiSize = 4u * 256u;
  desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
  desc.m_Mode = xiiGALBufferMode::Structured;
...
static void SetupAutoExposure(xiiView& view, AutoExposureData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  xiiRGBufferHandle hHist; bb.TryGetValue(xiiMakeHashedString(xiiRGBlackboardKeys::k_LuminanceHistogram), hHist);
  ...
  {
    xiiGALBufferCreationDescription desc;
    desc.m_uiElementByteStride = 4u; desc.m_uiSize = 4u; // single float EV100
    desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
    desc.m_Mode = xiiGALBufferMode::Structured;
...
static void SetupTAA(xiiView& view, TAAData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  if (!tp.m_pTAAHistoryBuffer)
  {
    ...
    desc.m_TextureType = xiiGALTextureType::Texture2D;
    desc.m_Format      = xiiGALTextureFormat::RGBA16Float;
    desc.m_uiWidth = data.m_W; desc.m_uiHeight = data.m_H; desc.m_uiMipLevels = 1u;
    desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
    desc.m_Usage = xiiGALResourceUsage::Default;
...
static void SetupUpscale(xiiView& view, UpscaleData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  xiiGALTextureCreationDescription desc;
  desc.m_TextureType = xiiGALTextureType::Texture2D; desc.m_Format = xiiGALTextureFormat::RGBA16Float;
  desc.m_uiWidth = uiNW; desc.m_uiHeight = uiNH; desc.m_uiMipLevels = 1u;
  desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
  desc.m_Usage = xiiGALResourceUsage::Default;
...
void xiiView::BuildStage10_Temporal(xiiRenderGraph& graph, const xiiRenderGraphBlackboard& blackboard)
{
  ...
  graph.AddPass<LumHistogramData>("LuminanceHistogram", xiiGALCommandQueueFlags::Compute,
    [self, &bb](LumHistogramData& d, xiiRGBuilder& b) { SetupLumHistogram(*self, d, b, bb); },
    [self](const LumHistogramData& d, xiiRGPassContext& c) { ExecuteLumHistogram(*self, d, c); });

  graph.AddPass<AutoExposureData>("AutoExposure", xiiGALCommandQueueFlags::Compute,
    [self, &bb](AutoExposureData& d, xiiRGBuilder& b) { SetupAutoExposure(*self, d, b, bb); },
    [self](const AutoExposureData& d, xiiRGPassContext& c) { ExecuteAutoExposure(*self, d, c); });

  graph.AddPass<TAAData>("TAA", xiiGALCommandQueueFlags::Compute,
    [self, &bb](TAAData& d, xiiRGBuilder& b) { SetupTAA(*self, d, b, bb); },
    [self](const TAAData& d, xiiRGPassContext& c) { ExecuteTAA(*self, d, c); });

  graph.AddPass<UpscaleData>("Upscale", xiiGALCommandQueueFlags::Compute,
    [self, &bb](UpscaleData& d, xiiRGBuilder& b) { SetupUpscale(*self, d, b, bb); },
    [self](const UpscaleData& d, xiiRGPassContext& c) { ExecuteUpscale(*self, d, c); });
}
...
namespace
{
  struct BloomData      { xiiRGTextureHandle m_hHDRIn, m_hExposure, m_hBloom; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct ColorGradeData { xiiRGTextureHandle m_hHDRIn, m_hExposure, m_hBloom, m_hGraded; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct ToneMappingData{ xiiRGTextureHandle m_hGraded, m_hLDROut; xiiUInt32 m_W=1920u, m_H=1080u; };
  struct PostFxData     { xiiRGTextureHandle m_hIn, m_hOut; xiiUInt32 m_W=1920u, m_H=1080u; const char* m_szShader=""; const char* m_szLabel=""; };
}
...
static void SetupToneMapping(xiiView& view, ToneMappingData& data, xiiRGBuilder& builder, const xiiRenderGraphBlackboard& bb)
{
  ...
  xiiGALTextureCreationDescription desc;
  desc.m_TextureType = xiiGALTextureType::Texture2D; desc.m_Format = xiiGALTextureFormat::RGBA8Unorm;
  desc.m_uiWidth = data.m_W; desc.m_uiHeight = data.m_H; desc.m_uiMipLevels = 1u;
  desc.m_BindFlags = xiiGALBindFlags::UnorderedAccess | xiiGALBindFlags::ShaderResource;
  desc.m_Usage = xiiGALResourceUsage::Default;
...
void xiiView::BuildStage11_PostProcess(xiiRenderGraph& graph, const xiiRenderGraphBlackboard& blackboard)
{
  ...
  graph.AddPass<BloomData>("Bloom", xiiGALCommandQueueFlags::Compute,
    [self, &bb](BloomData& d, xiiRGBuilder& b) { SetupBloom(*self, d, b, bb); },
    [self](const BloomData& d, xiiRGPassContext& c) { ExecuteBloom(*self, d, c); });

  graph.AddPass<ColorGradeData>("ColorGrading", xiiGALCommandQueueFlags::Compute,
    [self, &bb](ColorGradeData& d, xiiRGBuilder& b) { SetupColorGrade(*self, d, b, bb); },
    [self](const ColorGradeData& d, xiiRGPassContext& c) { ExecuteColorGrade(*self, d, c); });

  graph.AddPass<ToneMappingData>("ToneMapping", xiiGALCommandQueueFlags::Compute,
    [self, &bb](ToneMappingData& d, xiiRGBuilder& b) { SetupToneMapping(*self, d, b, bb); },
    [self](const ToneMappingData& d, xiiRGPassContext& c) { ExecuteToneMapping(*self, d, c); });
}
