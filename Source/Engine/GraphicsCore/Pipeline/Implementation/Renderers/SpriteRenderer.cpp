...
#include <GraphicsCore/Pipeline/Renderers/SpriteRenderer.h>
...
void xiiSpriteRenderer::GetSupportedRenderDataTypes(xiiDynamicArray<const xiiRTTI*>& out_types) const
{
  out_types.PushBack(xiiGetStaticRTTI<xiiSpriteRenderData>());
}
...
void xiiSpriteRenderer::DeleteSpriteDataBuffer(xiiSharedPtr<xiiGALBuffer> pBuffer) const
{
  xiiGPUResourcePool::GetDefaultInstance()->ReturnBuffer(std::move(pBuffer));
}
