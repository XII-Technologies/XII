...
void xiiPopulateAccelerationStructurePass(xiiAccelerationStructurePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  if (!bRTSupported)
    return; // No-op â€” downstream RT passes will also bail out when they find no TLAS handle.
  ...
  auto [pData, hPass] = graph.AddPass<ASPassData>(
    ...
    [](ASPassData& data, xiiRGBuilder& builder)
    {
      ...
      builder.SetPassAllowMerge(false);   // Must not be merged â€” explicit barrier semantics.
    },
    [&passData](const ASPassData& data, xiiRGPassContext& context)
    {
      ...
      // Pass 32: BLAS scheduling â€” CPU-side policy (refit vs full-rebuild per mesh).
      // Pass 33: BLAS build/refit â€” issued as batched BuildAccelerationStructure commands.
      cmd.BuildAccelerationStructures(); // Submits all pending BLAS builds enqueued this frame.

      // Pass 34: TLAS build â€” single top-level instance buffer covering all BLASes.
      cmd.BuildTopLevelAccelerationStructure();

      // Pass 35: SBT update â€” only re-records hit groups that changed this frame.
      cmd.UpdateShaderBindingTable();

      // Pass 45: Compaction deferred query â€” issued here if any BLAS flagged for compaction.
...
// ----- Moved from AmbientOcclusionPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/AmbientOcclusionPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateAmbientOcclusionPass(xiiAmbientOcclusionPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<AOData>(
    ...
    [hDepth, hNR, hHiZ, hVelocity, uiW, uiH, &passData, makeAO](AOData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiSlices = passData.m_uiSliceCount; data.uiSteps = passData.m_uiStepsPerSlice; data.fRadius = passData.m_fRadius;
    },
    [](const AOData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("GTAO + Denoise");
      // Pass 29: GTAO.xiiShader â€” horizon-based AO integration with passData.m_uiSliceCount * passData.m_uiStepsPerSlice samples.
...
// ----- Moved from AtmosphereCompositePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/AtmosphereCompositePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateAtmosphereCompositePass(xiiAtmosphereCompositePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<AtmCompData>(
    ...
    [hDepth, hTransmittance, hMultiScatter, hDirectLight, skyDesc, uiW, uiH, &passData](AtmCompData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.fSunSolidAngle = passData.m_fSunSolidAngle;
    },
...
// ----- Moved from AtmosphereLUTPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/AtmosphereLUTPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateAtmosphereLUTPass(xiiAtmosphereLUTPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<AtmLUTData>(
    ...
    [consts](const AtmLUTData& data, xiiRGPassContext& context)
    {
      ...
      // AtmosphereTransmittance.xiiShader: 256x64 compute â€” precomputed optical depth integral.
...
void xiiPopulateBloomPass(xiiBloomPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  bloomDesc.m_uiWidth = uiW; bloomDesc.m_uiHeight = uiH; bloomDesc.m_uiMipLevels = passData.m_uiMipLevels;
  ...
  auto [pData, hPass] = graph.AddPass<BloomData>(
    ...
    [hInput, bloomDesc, uiW, uiH, &passData](BloomData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiMips = passData.m_uiMipLevels;
      data.fThreshold = passData.m_fThreshold; data.fKnee = passData.m_fKnee; data.fIntensity = passData.m_fIntensity;
    },
    [](const BloomData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("Bloom");
      // Pass 57: BloomPrefilter.xiiShader â€” soft-knee bright-pass at full resolution.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      // Pass 58: BloomDownsample.xiiShader â€” 13-tap filter per mip level.
      ...
      // Pass 59: BloomUpsample.xiiShader â€” tent filter upsample chain + intensity blend.
...
// ----- Moved from ClusterGridAndLightListPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ClusterGridAndLightListPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateClusterGridAndLightListPass(xiiClusterGridAndLightListPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<ClusterData>(
    ...
    [hDepth, descDescs, gridDesc, idxDesc, &passData](ClusterData& data, xiiRGBuilder& builder)
    {
      ...
      data.cX = passData.m_uiClusterCountX; data.cY = passData.m_uiClusterCountY; data.cZ = passData.m_uiClusterCountZ;
    },
    [](const ClusterData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("Cluster Grid + Light List");
      // ClusterBuild.xiiShader: pass 21 â€” builds frustum-space cluster AABB descriptors.
      cmd.Dispatch(data.cX, data.cY, data.cZ);
      // LightAssignment.xiiShader: pass 22 â€” assigns active lights to overlapping clusters.
...
// ----- Moved from CoarseFrustumCullPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/CoarseFrustumCullPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from ColorGradingPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ColorGradingPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateColorGradingPass(xiiColorGradingPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<ColorGradData>(
    ...
    [hLDR, gradedDesc, uiW, uiH, &passData](ColorGradData& data, xiiRGBuilder& builder)
    {
      ...
      data.fVignette = passData.m_fVignetteStrength; data.fGrain = passData.m_fGrainStrength;
      data.fSat = passData.m_fSaturation; data.fContrast = passData.m_fContrast;
    },
...
// ----- Moved from ContactShadowPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ContactShadowPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateContactShadowPass(xiiContactShadowPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<ContactShadowData>(
    ...
    [hDepth, cDesc, uiW, uiH, &passData](ContactShadowData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiWidth = uiW; data.uiHeight = uiH; data.uiSteps = passData.m_uiRaySteps; data.fMaxDist = passData.m_fMaxRayDistance;
    },
...
void xiiPopulateDecalPass(xiiDecalPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<DecalPassData>(
    ...
    [hDepth, hAlbedo, hNormal, tileDesc, uiW, uiH, &passData](DecalPassData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiTileSize = passData.m_uiTileSize;
    },
    [](const DecalPassData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("Decal Classification + Resolve");
      // Pass 23: DecalClassification.xiiShader â€” builds per-tile decal index list.
      ...
      // Pass 24: DecalResolve.xiiShader â€” applies decal attributes to GBuffer targets.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from DirectionalShadowRenderPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/DirectionalShadowRenderPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateDirectionalShadowRenderPass(xiiDirectionalShadowRenderPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  atlasDesc.m_uiWidth = passData.m_uiAtlasSize; atlasDesc.m_uiHeight = passData.m_uiAtlasSize; atlasDesc.m_uiMipLevels = 1u;
  ...
  auto [pData, hPass] = graph.AddPass<DirShadowData>(
    ...
    [](const DirShadowData& data, xiiRGPassContext& context)
    {
      ...
      // Indirect draws per cascade into shadow atlas tiles â€” ShadowDepth.xiiShader.
...
// ----- Moved from DrawCommandBuildPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/DrawCommandBuildPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateDrawCommandBuildPass(xiiDrawCommandBuildPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<DrawCmdBuildData>(
    ...
    [](const DrawCmdBuildData& data, xiiRGPassContext& context)
    {
      ...
      // Uses prefix sum for compact packing â€” no CPU readback needed.
      cmd.Dispatch((data.uiInstanceCount + 63u) / 64u, 1u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from DynamicResolutionPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
...
void xiiPopulateDynamicResolutionPass(xiiDynamicResolutionPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  // This introduces a 1-frame latency between scale computation and resource sizing â€” acceptable.
...
// ----- Moved from EmissiveAuxPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/EmissiveAuxPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateEmissiveAuxPass(xiiEmissiveAuxPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  // Emissive target already created by GBufferBasePass â€” read+write to append.
  ...
  // hEmissive handle remains the same â€” no need to re-publish (same texture resource).
}

// ----- Moved from ExposurePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ExposurePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateExposurePass(xiiExposurePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<ExposureData>(
    ...
    [consts](const ExposureData& data, xiiRGPassContext& context)
    {
      ...
      // Pass 53: ExposureHistogram.xiiShader â€” atomic histogram of log2(luminance) per pixel.
      cmd.Dispatch((data.uiW + 15u) / 16u, (data.uiH + 15u) / 16u, 1u);
      // Pass 54: ExposureAdaptation.xiiShader â€” reads histogram, emits adapted exposure.
      cmd.Dispatch(1u, 1u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from GBufferBasePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/GBufferBasePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateGBufferBasePass(xiiGBufferBasePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<GBufferData>(
    ...
    [](const GBufferData& data, xiiRGPassContext& context)
    {
      ...
      // GBufferBase.xiiShader: indirect MRT draw â€” albedo+alpha/oct-normal/roughness-metallic-AO/emissive.
...
// ----- Moved from HiZOcclusionCullPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/HiZOcclusionCullPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from InstanceTransformPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/InstanceTransformPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from LightingCombinePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/LightingCombinePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateLightingCombinePass(xiiLightingCombinePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<LightingData>(
    ...
    [=, &passData](LightingData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH;
      data.bRTShadows = passData.m_bEnableRTShadows; data.bContactShadows = passData.m_bEnableContactShadows;
      data.bRTRefl = passData.m_bEnableRTReflections; data.bSSR = passData.m_bEnableSSRFallback;
      builder.SetPassAllowMerge(false);
    },
    [](const LightingData& data, xiiRGPassContext& context)
    {
      ...
      // Pass 46: DirectLighting.xiiShader â€” GGX BRDF + cluster lights + cascade shadows.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      // Pass 47: IndirectLighting.xiiShader â€” AO-modulated RT GI / IBL + RT/SSR reflections.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from LocalLightShadowPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/LocalLightShadowPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateLocalLightShadowPass(xiiLocalLightShadowPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<LocalShadowData>(
    ...
    [](const LocalShadowData& data, xiiRGPassContext& context)
    {
      ...
      // Pass 18: LocalLightShadowAtlasAllocation.xiiShader â€” deterministic atlas tile assignment.
      cmd.Dispatch(1u, 1u, 1u);
      // Pass 19: ShadowDepth.xiiShader â€” indirect draws per shadowed light into allocated tiles.
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from LodAndMeshletPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/LodAndMeshletPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from MainDepthPrepassPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/MainDepthPrepassPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateMainDepthPrepassPass(xiiMainDepthPrepassPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<MainDepthData>(
    ...
    [](const MainDepthData& data, xiiRGPassContext& context)
    {
      ...
      // Indirect depth-only draw per material bin â€” DepthPrepass.xiiShader (VS only, no PS).
...
// ----- Moved from MotionVectorPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/MotionVectorPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from NormalRoughnessPrepassPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/NormalRoughnessPrepassPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from OccluderDepthPrepass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/OccluderDepthPrepass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateOccluderDepthPrepass(xiiOccluderDepthPrepass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<OccluderDepthData>(
    ...
    [](const OccluderDepthData& data, xiiRGPassContext& context)
    {
      ...
      // Draw occluder meshes via dedicated low-poly draw list â€” geometry-only, no alpha test.
...
// ----- Moved from OpaqueCompositePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/OpaqueCompositePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from ParticleVFXPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ParticleVFXPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateParticleVFXPass(xiiParticleVFXPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<ParticleData>(
    ...
    [hDepth, hHDRScene, uiW, uiH, &passData](ParticleData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiMaxParticles = passData.m_uiMaxParticles;
      builder.SetPassAllowMerge(false);
    },
    [](const ParticleData& data, xiiRGPassContext& context)
    {
      ...
      // Simulate: ParticleSimulate.xiiShader â€” position + velocity integration.
      cmd.Dispatch((data.uiMaxParticles + 255u) / 256u, 1u, 1u);
      // Render: Particle.xiiShader â€” billboard quads, depth-tested, soft particle edge fading.
...
// ----- Moved from PerFrameBufferUploadPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
...
// Shared CPU/GPU layout â€” must match Orchestration/PerFrameUploadData.h exactly.
...
// ----- Moved from ReadbackAndTelemetryPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ReadbackAndTelemetryPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from RTGlobalIlluminationPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/RTGlobalIlluminationPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateRTGlobalIlluminationPass(xiiRTGlobalIlluminationPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<RTGIData>(
    ...
    [hDepth, hNR, hVelocity, giDesc, uiW, uiH, &passData](RTGIData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiRPP = passData.m_uiRaysPerPixel; data.uiCandidates = passData.m_uiReservoirCandidateCount;
    },
    [](const RTGIData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("RT Global Illumination");
      // Pass 42: RTGIFinalGather.xiiShader (TraceRays) â€” ReSTIR reservoir resampling initial candidates.
      cmd.TraceRays(data.uiW, data.uiH, 1u);
      // Pass 43: RTGITemporal.xiiShader â€” temporal reservoir reuse + firefly clamp.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      // Pass 44: RTGIDenoise.xiiShader â€” diffuse/specular indirect denoiser.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from RTReflectionPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/RTReflectionPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateRTReflectionPass(xiiRTReflectionPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<RTReflData>(
    ...
    [hDepth, hNR, hVelocity, hSSR, reflDesc, uiW, uiH, &passData](RTReflData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiMaxRays = passData.m_uiMaxRaysPerPixel; data.fMaxRoughness = passData.m_fMaxRoughnessForRT;
    },
    [](const RTReflData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("RT Reflections");
      // Pass 39: RTReflection.xiiShader (TraceRays) â€” glossy reflection rays, budget by roughness.
      cmd.TraceRays(data.uiW, data.uiH, 1u);
      // Pass 40: RTReflectionTemporal.xiiShader â€” radiance-space clamped accumulation.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      // Pass 41: RTReflectionDenoise.xiiShader â€” two-stage diffuse/specular split denoise.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
void xiiPopulateRTShadowPass(xiiRTShadowPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<RTShadowData>(
    ...
    [hDepth, hNR, hVelocity, uiW, uiH, &passData, makeR8](RTShadowData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH;
      data.uiRPP = passData.m_uiRaysPerPixel; data.uiHistory = passData.m_uiTemporalHistoryLen; data.fLightRadius = passData.m_fLightRadius;
    },
    [](const RTShadowData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("RT Shadows");
      // Pass 36: RTShadow.xiiShader (TraceRays) â€” passData.m_uiRaysPerPixel per pixel, jittered soft shadow.
      cmd.TraceRays(data.uiW, data.uiH, 1u);
      // Pass 37: RTShadowTemporalAccumulate.xiiShader â€” reproject + history clamping.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      // Pass 38: RTShadowSpatialDenoise.xiiShader â€” normal+depth gated bilateral filter.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from ScreenSpaceReflectionsPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ScreenSpaceReflectionsPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateScreenSpaceReflectionsPass(xiiScreenSpaceReflectionsPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<SSRData>(
    ...
    [](const SSRData& data, xiiRGPassContext& context)
    {
      ...
      // Skip pixels with roughness > fRoughThresh â€” they fall back to IBL.
      cmd.Dispatch((data.uiW + 7u) / 8u, (data.uiH + 7u) / 8u, 1u);
      cmd.PopDebugGroup();
    }
  );
...
// ----- Moved from ShadowCascadeSetupPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ShadowCascadeSetupPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateShadowCascadeSetupPass(xiiShadowCascadeSetupPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  // Full implementation requires sun direction from scene â€” defaulting here.
...
// ----- Moved from ShadowCasterCullPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ShadowCasterCullPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from SkinningAndMorphPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/SkinningAndMorphPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from TemporalResolvePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/TemporalResolvePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateTemporalResolvePass(xiiTemporalResolvePass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<TAAData>(
    ...
    [hHDR, hVelocity, hDepth, uiW, uiH, resolvedDesc, uiCurrent, uiHistSrc, &passData](TAAData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.fAlpha = passData.m_fBlendAlpha; data.fSharpness = passData.m_fSharpness;
...
// ----- Moved from ToneMappingPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/ToneMappingPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateToneMappingPass(xiiToneMappingPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<TonemapData>(
    ...
    [hHDR, hBloom, hExposure, ldrDesc, uiW, uiH, &passData](TonemapData& data, xiiRGBuilder& builder)
    {
      ...
      data.uiW = uiW; data.uiH = uiH; data.uiOperator = static_cast<xiiUInt8>(passData.m_Operator); data.fBias = passData.m_fExposureBias;
    },
...
// ----- Moved from TransparentPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/TransparentPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateTransparentPass(xiiTransparentPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  auto [pData, hPass] = graph.AddPass<TransparentData>(
    ...
    [hDepth, hHDRScene, hDrawArgs, uiW, uiH, &passData](TransparentData& data, xiiRGBuilder& builder)
    {
      ...
      if (hHDRScene.IsValid()) data.hHDRScene = builder.WriteTexture(hHDRScene, xiiGALResourceStateFlags::RenderTarget);
      if (hDrawArgs.IsValid()) data.hDrawArgs = builder.ReadBuffer(hDrawArgs,   xiiGALResourceStateFlags::IndirectArgument);
      data.uiW = uiW; data.uiH = uiH; data.bOIT = passData.m_bUseWeightedBlendedOIT;
      builder.SetPassAllowMerge(false);
    },
...
// ----- Moved from UICompositePass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/UICompositePass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
// ----- Moved from VolumetricFroxelSetupPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/VolumetricFroxelSetupPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
...
void xiiPopulateVolumetricFroxelSetupPass(xiiVolumetricFroxelSetupPass& passData, xiiView& view, xiiRenderGraph& graph, xiiRenderGraphBlackboard& blackboard)
{
  ...
  scatterDesc.m_uiWidth = passData.m_uiFroxelCountX; scatterDesc.m_uiHeight = passData.m_uiFroxelCountY;
  scatterDesc.m_uiDepth = passData.m_uiFroxelCountZ; scatterDesc.m_uiMipLevels = 1u;
  ...
  auto [pData, hPass] = graph.AddPass<FroxelData>(
    ...
    [metaDesc, scatterDesc, &passData](FroxelData& data, xiiRGBuilder& builder)
    {
      ...
      data.cX = passData.m_uiFroxelCountX; data.cY = passData.m_uiFroxelCountY; data.cZ = passData.m_uiFroxelCountZ;
    },
    [](const FroxelData& data, xiiRGPassContext& context)
    {
      xiiGALCommandList& cmd = context.GetCommandList();
      cmd.PushDebugGroup("Volumetric Froxel Setup");
      // FroxelSetup.xiiShader: [numthreads(8,8,1)] â€” computes per-froxel bounds+phase terms.
...
// ----- Moved from VolumetricIntegrationPass.cpp -----
#include <GraphicsCore/GraphicsCorePCH.h>
#include <GraphicsCore/Pipeline/Passes/VolumetricIntegrationPass.h>
#include <GraphicsCore/Pipeline/PipelineBlackboardKeys.h>
#include <GraphicsCore/Pipeline/RenderGraph.h>
