...
#include <GraphicsCore/AnimationSystem/AnimGraph/AnimNodes/BoneWeightsAnimNode.h>
...
void xiiBoneWeightsAnimNode::Step(xiiAnimController& ref_controller, xiiAnimGraphInstance& ref_graph, xiiTime tDiff, const xiiSkeletonResource* pSkeleton, xiiGameObject* pTarget) const
{
  ...
  if (pInstance->m_pSharedBoneWeights == nullptr && pInstance->m_pSharedInverseBoneWeights == nullptr)
  {
    ...
    pInstance->m_pSharedBoneWeights = ref_controller.CreateBoneWeights(name, *pSkeleton, [this, pOzzSkeleton](xiiAnimGraphSharedBoneWeights& ref_bw) {
      for (const auto& rootBone : m_RootBones)
      {
        int iRootBone = -1;
    ...
    if (m_InverseWeightsPin.IsConnected())
    {
      ...
      pInstance->m_pSharedInverseBoneWeights = ref_controller.CreateBoneWeights(name, *pSkeleton, [this, pInstance](xiiAnimGraphSharedBoneWeights& ref_bw) {
  ...
  if (m_WeightsPin.IsConnected())
  {
    xiiAnimGraphPinDataBoneWeights* pPinData = ref_controller.AddPinDataBoneWeights();
...
bool xiiBoneWeightsAnimNode::GetInstanceDataDesc(xiiInstanceDataDesc& out_desc) const
{
