...
// clang-format off
XII_BEGIN_STATIC_REFLECTED_TYPE(xiiAnimState, xiiNoBase, 1, xiiRTTIDefaultAllocator<xiiAnimState>)
{
  XII_BEGIN_PROPERTIES
  {
    XII_MEMBER_PROPERTY("Loop", m_bLoop),
    XII_MEMBER_PROPERTY("ApplyRootMotion", m_bApplyRootMotion),
    XII_MEMBER_PROPERTY("PlaybackSpeed", m_fPlaybackSpeed)->AddAttributes(new xiiDefaultValueAttribute(1.0f)),
    XII_MEMBER_PROPERTY("FadeIn", m_FadeIn),
    XII_MEMBER_PROPERTY("FadeOut", m_FadeOut),
    XII_MEMBER_PROPERTY("ImmediateFadeIn", m_bImmediateFadeIn),
    XII_MEMBER_PROPERTY("ImmediateFadeOut", m_bImmediateFadeOut),
  }
  XII_END_PROPERTIES;
}
...
void xiiAnimState::RampWeightUpOrDown(float& inout_fWeight, float fTargetWeight, xiiTime tDiff) const
{
  if (inout_fWeight < fTargetWeight)
  {
    ...
    else
    {
      ...
      inout_fWeight = xiiMath::Min(inout_fWeight, fTargetWeight);
    }
  }
  else if (inout_fWeight > fTargetWeight)
  {
    ...
    else
    {
      ...
      inout_fWeight = xiiMath::Max(fTargetWeight, inout_fWeight);
    }
  }
}

bool xiiAnimState::WillStateBeOff(bool bTriggerActive) const
{
...
void xiiAnimState::UpdateState(xiiTime diff)
{
  ...
  if (m_State == State::Running)
  {
    ...
    if (fSpeed >= 0)
    {
      ...
      tInRampDownArea   = xiiMath::Min(fNormalizedStep, m_fNormalizedPlaybackPosition - tEnd);
    }
    else
    {
      ...
      tInRampDownArea   = xiiMath::Min(fNormalizedStep, tRampDownNorm - m_fNormalizedPlaybackPosition);
    }
...
xiiResult xiiAnimState::Serialize(xiiStreamWriter& inout_stream) const
{
...
xiiResult xiiAnimState::Deserialize(xiiStreamReader& inout_stream)
{
