...
/// Mesh resource usage flags, used to specify intended usage patterns and GPU feature support for a mesh resource.
...
/// Selection mode for LODs of a mesh resource, used to determine how LODs are chosen at runtime.
...
/// Mesh section describing a contiguous range of primitives with the same material and other properties, used for rendering and culling.
...
/// Mesh LOD (Level of Detail) containing multiple sections, used to define different levels of detail for a mesh resource.
...
/// Mesh morph target containing vertex offsets and bounding volume, used for shape animation of a mesh resource.
...
/// Mesh bone data containing the global inverse rest pose matrix and bone index, used for skeletal animation of a mesh resource.
...
/// Authoring and streaming descriptor for xiiMeshResource.
///
...
{
public:
  /// Returns a reference to the mesh buffer resource descriptor contained within this mesh resource descriptor.
  ...
  /// Returns a handle to an existing mesh buffer resource that can be used for this mesh resource, or an invalid handle if no existing mesh buffer can be used.
  const xiiMeshBufferResourceHandle& GetExistingMeshBuffer() const;

  /// Returns a reference to the array of material paths contained within this mesh resource descriptor.
  xiiArrayPtr<const xiiString> GetMaterials() const;

  /// Returns a reference to the array of sections contained within this mesh resource descriptor.
  xiiArrayPtr<const xiiMeshSection> GetSubMeshes() const;

  /// Returns a reference to the array of LODs contained within this mesh resource descriptor.
  xiiArrayPtr<const xiiMeshLOD> GetLODs() const;

  /// Returns the bounding volume for this mesh resource, used for culling and LOD selection.
  const xiiBoundingBoxSphere& GetBounds() const;

public:
  /// Default constructor, initializes an empty mesh resource descriptor.
  xiiMeshResourceDescriptor();

  /// Clears all data from the descriptor, resetting it to an empty state.
  void Clear();

  /// Specifies that the mesh buffer for this mesh resource should be created from the given descriptor, instead of using an existing mesh buffer resource.
  void UseExistingMeshBuffer(const xiiMeshBufferResourceHandle& hBuffer);

  /// Adds a new material slot to the mesh resource descriptor with the given path, and returns the index of the new material slot.
  xiiUInt32 AddMaterialSlot(xiiStringView sPathToMaterial);

  /// Sets the material path for the material slot at the given index.
  ///
  ...
  /// Adds a new section to the mesh resource descriptor with the given properties, and returns a reference to the new section.
  ///
  ...
  /// Adds a new sub-mesh (section at LOD 0) to the mesh resource descriptor with the given properties.
  ///
  ...
  /// Adds a new LOD to the mesh resource descriptor with the given properties, and returns a reference to the new LOD.
  ///
  ...
  /// Collapses all sub-meshes (sections at LOD 0) into a single section, used to optimize meshes that have multiple small sections.
  void CollapseSubMeshes();

  /// Computes the bounding volume for the mesh resource based on the vertex data in the mesh buffer descriptor, used for culling and LOD selection.
  void ComputeBounds();

  /// Builds meshlets for the mesh resource based on the sections and LODs defined in the descriptor, used for rendering with mesh shaders.
  ///
  ...
  /// Sets the bounding volume for this mesh resource, used for culling and LOD selection.
...
/// Mesh resource class representing a renderable mesh in the engine, containing geometry, materials, LODs, and other properties for rendering and animation.
...
public:
  ...
  /// Returns the descriptor used to create this mesh resource, containing all the data and properties for this mesh.
  const xiiMeshResourceDescriptor& GetDescriptor() const;

  /// Returns a handle to the mesh buffer resource associated with this mesh, used to access the vertex/index/meshlet data for rendering.
  const xiiMeshBufferResourceHandle& GetMeshBuffer() const;

  /// Returns a reference to the material resource handle for the material slot at the given index, used to access the material for rendering.
  ///
  ...
  /// Returns a reference to the array of material resource handles for this mesh, used to access the materials for rendering.
  xiiArrayPtr<const xiiMaterialResourceHandle> GetMaterials() const;

  /// Returns a reference to the array of LODs for this mesh, used for LOD selection and rendering.
  xiiArrayPtr<const xiiMeshLOD> GetLODs() const;

  /// Returns a reference to the array of sections for this mesh, used for rendering and culling.
  xiiArrayPtr<const xiiMeshSection> GetSections() const;

  /// Returns the bounding volume for this mesh, used for culling and LOD selection.
  const xiiBoundingBoxSphere& GetBounds() const;

  /// Returns the number of LODs for this mesh.
  xiiUInt32 GetLODCount() const;

  /// Returns the number of sections for this mesh.
  xiiUInt32 GetMeshletCount() const;

  /// Returns the usage flags for this mesh, used to specify intended usage patterns and GPU feature support.
  xiiBitflags<xiiMeshResourceUsageFlags> GetUsageFlags() const;

  /// Returns the LOD selection mode for this mesh, used to determine how LODs are chosen at runtime.
...
private:
  ...
  /// Creates the mesh buffer resource for this mesh based on the mesh buffer descriptor contained in the provided mesh resource descriptor, used to initialize the GPU buffers for this mesh.
  void CreateMeshBufferFromDescriptor(xiiMeshResourceDescriptor& inout_descriptor);

  /// Loads the material resources for this mesh based on the material paths contained in the provided mesh resource descriptor, used to initialize the materials for this mesh.
