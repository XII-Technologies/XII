...
Occluder::~Occluder()
{
  XII_DELETE_RAW_BUFFER(xiiFoundation::GetAlignedAllocator(), m_vertexData);
...
void Occluder::bake(const __m128* vertices, xiiUInt32 numVertices, __m128 refMin, __m128 refMax)
{
  assert(numVertices % 16 == 0);

  // Simple k-means clustering by normal direction to improve backface culling efficiency
  xiiHybridArray<__m128, 32, xiiAlignedAllocatorWrapper> quadNormals;
  for (xiiUInt32 i = 0; i < numVertices; i += 4)
  {
  ...
  xiiHybridArray<__m128, 32, xiiAlignedAllocatorWrapper> centroids;
  xiiHybridArray<uint32_t, 32>                           centroidAssignment;
  ...
  xiiHybridArray<__m128, 64, xiiAlignedAllocatorWrapper> orderedVertices;
  for (uint32_t k = 0; k < centroids.GetCount(); ++k)
  {
    for (xiiUInt32 j = 0; j < numVertices / 4; ++j)
    {
  ...
  occluder->m_vertexData  = XII_NEW_RAW_BUFFER(xiiFoundation::GetAlignedAllocator(), __m256i, orderedVertices.GetCount() * 4);

  for (xiiUInt32 i = 0; i < orderedVertices.GetCount(); i += 32)
  {
    __m128i v[8];
  ...
  for (xiiUInt32 i = 0; i < orderedVertices.GetCount(); ++i)
  {
...
XII_STATICLINK_FILE(GraphicsCore, GraphicsCore_Rasterizer_Thirdparty_Occluder);
