...
Rasterizer::Rasterizer(uint32_t width, uint32_t height) :
  m_width(width), m_height(height), m_blocksX(width / 8), m_blocksY(height / 8)
{
...
{
  // Frustum culling is not necessary, because XII only calls this functions for objects that are definitely inside the frustum
  //
  // Frustum cull
...
XII_STATICLINK_FILE(GraphicsCore, GraphicsCore_Rasterizer_Thirdparty_Rasterizer);
