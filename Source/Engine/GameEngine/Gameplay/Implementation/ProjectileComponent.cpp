...
// clang-format off
XII_BEGIN_STATIC_REFLECTED_ENUM(xiiProjectileReaction, 1)
...
XII_BEGIN_COMPONENT_TYPE(xiiProjectileComponent, 4, xiiComponentMode::Dynamic)
{
  XII_BEGIN_PROPERTIES
  {
...
void xiiProjectileComponent::Update()
{
  ...
  if (pPhysicsInterface)
  {
    ...
    queryParams.m_ShapeTypes.Remove(xiiPhysicsShapeType::Trigger);
    //queryParams.m_ShapeTypes.Remove(xiiPhysicsShapeType::Character); // TODO: expose this ??
    ...
    if (pPhysicsInterface->Raycast(castResult, pEntity->GetGlobalPosition(), vCurDirection, fDistance, queryParams))
    {
      ...
      else
      {
        ...
        else if (interaction.m_Reaction == xiiProjectileReaction::Reflect)
        {
...
void xiiProjectileComponent::SerializeComponent(xiiWorldWriter& stream) const
{
...
void xiiProjectileComponent::DeserializeComponent(xiiWorldReader& stream)
{
...
void xiiProjectileComponent::SetTimeoutPrefab(const char* szPrefab)
{
...
const char* xiiProjectileComponent::GetTimeoutPrefab() const
{
  if (!m_hTimeoutPrefab.IsValid())
    return "";
...
XII_STATICLINK_FILE(GameEngine, GameEngine_Gameplay_Implementation_ProjectileComponent);
