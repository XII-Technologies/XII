...
xiiGameApplication::xiiGameApplication(const char* szAppName, const char* szProjectPath /*= nullptr*/) :
  xiiGameApplicationBase(szAppName), m_sAppProjectPath(szProjectPath)
{
  m_pUpdateTask = XII_DEFAULT_NEW(xiiDelegateTask<void>, "", xiiMakeDelegate(&xiiGameApplication::UpdateWorldsAndExtractViews, this));
...
{
  xiiStringBuilder sb;
  sb.Format("FRAME {}", xiiRenderWorld::GetFrameCounter());
