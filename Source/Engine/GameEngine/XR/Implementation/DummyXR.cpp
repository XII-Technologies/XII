...
xiiUniquePtr<xiiActor> xiiDummyXR::CreateActor(xiiView* pView, xiiGALMSAASampleCount::Enum msaaCount, xiiUniquePtr<xiiWindowBase> pCompanionWindow, xiiUniquePtr<xiiWindowOutputTargetGAL> pCompanionWindowOutput)
{
  ...
  // Create dummy swap chain
  {
    ...
    textureDesc.SetAsRenderTarget(m_Info.m_vEyeRenderTargetSize.width, m_Info.m_vEyeRenderTargetSize.height, xiiGALResourceFormat::RGBAUByteNormalizedsRGB, msaaCount);
    textureDesc.m_uiArraySize              = 2;
    textureDesc.m_bAllowShaderResourceView = true;

    m_hColorRT = pDevice->CreateTexture(textureDesc);

    textureDesc.m_Format = xiiGALResourceFormat::D24S8;
