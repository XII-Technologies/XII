...
void xiiSensorSphereComponent::GetObjectsInSensorVolume(xiiDynamicArray<xiiGameObject*>& out_Objects) const
{
  ...
  xiiBoundingSphere sphere = xiiBoundingSphere(pOwner->GetGlobalPosition(), m_fRadius * scale);
...
{
  xiiBoundingSphere sphere = xiiBoundingSphere(xiiVec3::ZeroVector(), m_fRadius);
...
void xiiSensorCylinderComponent::GetObjectsInSensorVolume(xiiDynamicArray<xiiGameObject*>& out_Objects) const
{
  ...
  xiiBoundingSphere sphere       = xiiBoundingSphere(pOwner->GetGlobalPosition(), sphereRadius);
...
void xiiSensorConeComponent::GetObjectsInSensorVolume(xiiDynamicArray<xiiGameObject*>& out_Objects) const
{
  ...
  const float       scale  = pOwner->GetGlobalTransformSimd().GetMaxScale();
  xiiBoundingSphere sphere = xiiBoundingSphere(pOwner->GetGlobalPosition(), m_fFarDistance * scale);
...
void xiiSensorWorldModule::UpdateSensors(const xiiWorldModule::UpdateContext& context)
{
  ...
  m_Scheduler.Update(deltaTime, [this](const xiiComponentHandle& hComponent, xiiTime deltaTime) {
    ...
    const xiiGameObject* pSensorOwner = pSensorComponent->GetOwner();
    ...
    if (pSensorComponent->m_bTestVisibility)
    {
      ...
      for (auto pObject : m_ObjectsInSensorVolume)
      {
        ...
        xiiPhysicsQueryParameters params(pSensorComponent->m_uiCollisionLayer);
    ...
    if (m_DetectedObjects != pSensorComponent->m_LastDetectedObjects)
    {
      ...
      pSensorOwner->PostEventMessage(msg, pSensorComponent, xiiTime::Zero(), xiiObjectMsgQueueType::PostAsync);
    } });
}
