...
void xiiPropertyAnimComponent::SerializeComponent(xiiWorldWriter& inout_stream) const
{
...
void xiiPropertyAnimComponent::DeserializeComponent(xiiWorldReader& inout_stream)
{
...
void xiiPropertyAnimComponent::CreateGameObjectBinding(const xiiFloatPropertyAnimEntry* pAnim, const xiiRTTI* pOwnerRtti, void* pObject, const xiiGameObjectHandle& hGameObject)
{
  ...
  const xiiAbstractProperty* pAbstract = pOwnerRtti->FindPropertyByName(pAnim->m_sPropertyPath);
  ...
  auto pMember = static_cast<const xiiAbstractMemberProperty*>(pAbstract);
...
void xiiPropertyAnimComponent::ApplySingleFloatAnimation(const FloatBinding& binding, xiiTime lookupTime)
{
  ...
  {
    auto pTyped = static_cast<const xiiTypedMemberProperty<bool>*>(binding.m_pMemberProperty);

    pTyped->SetValue(binding.m_pObject, fFinalValue > 0.99); // this is close to what xiiVariant does (not identical, that does an int cast != 0), but faster to evaluate
    return;
  }
...
void xiiPropertyAnimComponent::ApplyFloatAnimation(const FloatBinding& binding, xiiTime lookupTime)
{
  ...
  {
    auto          pTyped = static_cast<const xiiTypedMemberProperty<xiiVec2>*>(binding.m_pMemberProperty);
  ...
  {
    auto          pTyped = static_cast<const xiiTypedMemberProperty<xiiVec3>*>(binding.m_pMemberProperty);
  ...
  {
    auto          pTyped = static_cast<const xiiTypedMemberProperty<xiiVec4>*>(binding.m_pMemberProperty);
