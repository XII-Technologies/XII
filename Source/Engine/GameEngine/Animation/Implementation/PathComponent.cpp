...
// clang-format off
XII_IMPLEMENT_MESSAGE_TYPE(xiiMsgPathChanged);
XII_BEGIN_DYNAMIC_REFLECTED_TYPE(xiiMsgPathChanged, 1, xiiRTTIDefaultAllocator<xiiMsgPathChanged>)
...
XII_BEGIN_COMPONENT_TYPE(xiiPathComponent, 1, xiiComponentMode::Static)
{
  ...
  {
    XII_MESSAGE_HANDLER(xiiMsgPathChanged, OnMsgPathChanged),
  }
...
void xiiPathComponent::OnMsgPathChanged(xiiMsgPathChanged& ref_msg)
{
  m_bControlPointsChanged            = true;
  m_bLinearizedRepresentationChanged = true;
}
