...
void xiiImguiRenderer::RenderBatch(const xiiRenderViewContext& renderContext, const xiiRenderPipelinePass* pPass, const xiiRenderDataBatch& batch) const
{
  ...
  xiiGALRenderCommandEncoder* pCommandEncoder = pRenderContext->GetRenderCommandEncoder();
  ...
  for (auto it = batch.GetIterator<xiiImguiRenderData>(); it.IsValid(); ++it)
  {
    ...
    pRenderContext->BindMeshBuffer(m_hVertexBuffer, m_hIndexBuffer, &m_VertexDeclarationInfo, xiiGALPrimitiveTopology::Triangles, pRenderData->m_Indices.GetCount() / 3);
...
void xiiImguiRenderer::SetupRenderer()
{
  ...
  // Create the vertex buffer
  {
    ...
    desc.m_uiStructSize                = sizeof(xiiImguiVertex);
