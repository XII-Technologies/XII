...
public:
  ...
  virtual xiiResult ReadImageHeader(xiiStreamReader& stream, xiiImageHeader& header, const char* szFileExtension) const = 0;

  /// \brief Reads the data from the given stream and creates the image from it. Errors are written to the given xiiLogInterface.
  virtual xiiResult ReadImage(xiiStreamReader& stream, xiiImage& image, const char* szFileExtension) const = 0;

  /// \brief Writes the data to the given stream in this format. Errors are written to the given xiiLogInterface.
  virtual xiiResult WriteImage(xiiStreamWriter& stream, const xiiImageView& image, const char* szFileExtension) const = 0;
  ...
  static xiiImageFileFormat* GetReaderFormat(const char* extension);
  ...
  static xiiImageFileFormat* GetWriterFormat(const char* extension);

  static xiiResult ReadImageHeader(const char* szFileName, xiiImageHeader& header);
