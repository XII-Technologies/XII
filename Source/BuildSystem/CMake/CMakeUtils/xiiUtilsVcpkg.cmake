...
function(xii_vcpkg_init)
	...
	if(CMAKE_TOOLCHAIN_FILE)
		...
		get_filename_component(XII_VCPKG_ROOT "${CMAKE_TOOLCHAIN_FILE}" DIRECTORY)
		get_filename_component(XII_VCPKG_ROOT "${XII_VCPKG_ROOT}" DIRECTORY)
		get_filename_component(XII_VCPKG_ROOT "${XII_VCPKG_ROOT}" DIRECTORY)

	else()
		if(DEFINED ENV{VCPKG_ROOT})
			set(XII_VCPKG_ROOT "$ENV{VCPKG_ROOT}")
			message(STATUS "EnvVar VCPKG_ROOT is specified, using that.")
		else()
			set(XII_VCPKG_ROOT "${CMAKE_CURRENT_SOURCE_DIR}/vcpkg")
			message(STATUS "EnvVar VCPKG_ROOT is not specified, using '${CMAKE_CURRENT_SOURCE_DIR}/vcpkg'.")
		endif()

		if(NOT EXISTS "${XII_VCPKG_ROOT}/vcpkg.exe" OR NOT EXISTS "${XII_VCPKG_ROOT}/scripts/buildsystems/vcpkg.cmake")
		...
		set(CMAKE_TOOLCHAIN_FILE ${XII_VCPKG_ROOT}/scripts/buildsystems/vcpkg.cmake CACHE STRING "" FORCE)
		message(STATUS "Forcing CMAKE_TOOLCHAIN_FILE to point to '${CMAKE_TOOLCHAIN_FILE}'")
	endif()

	message(STATUS "XII_VCPKG_ROOT is '${XII_VCPKG_ROOT}'")
	set_property(GLOBAL PROPERTY "XII_VCPKG_ROOT" ${XII_VCPKG_ROOT})
endfunction()

# #####################################
# ## xii_vcpkg_install(<packages>)
# #####################################
function(xii_vcpkg_install PACKAGES)
	xii_cmake_init()

	get_property(XII_VCPKG_ROOT GLOBAL PROPERTY "XII_VCPKG_ROOT")
	...
	foreach(PACKAGE ${PACKAGES})
		...
		execute_process(COMMAND "${XII_VCPKG_ROOT}/vcpkg.exe" install "${PACKAGE}:${VCPKG_TARGET_TRIPLET}" WORKING_DIRECTORY "${XII_VCPKG_ROOT}")
	endforeach()
endfunction()
