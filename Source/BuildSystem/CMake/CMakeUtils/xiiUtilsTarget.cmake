# #####################################
# ## xii_create_target(<LIBRARY | APPLICATION> <target-name> [NO_PCH] [NO_UNITY] [NO_QT] [ALL_SYMBOLS_VISIBLE] [EXCLUDE_FOLDER_FOR_UNITY <relative-folder>...])
# #####################################

macro(xii_create_target TYPE TARGET_NAME)
	xii_apply_build_filter(${TARGET_NAME})

	set(ARG_OPTIONS NO_PCH NO_UNITY NO_QT NO_XII_PREFIX ENABLE_RTTI NO_WARNINGS_AS_ERRORS NO_CONTROLFLOWGUARD ALL_SYMBOLS_VISIBLE NO_DEBUG)
	...
	if(ARG_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "xii_create_target: Invalid arguments '${ARG_UNPARSED_ARGUMENTS}'")
	endif()
	...
	if(DEFINED ARG_MANUAL_SOURCE_FILES)
		...
		xii_glob_source_files(${CMAKE_CURRENT_SOURCE_DIR} ALL_SOURCE_FILES)
	endif()

	# SHARED_LIBRARY means always shared
	# LIBRARY means SHARED_LIBRARY when XII_COMPILE_ENGINE_AS_DLL is on, otherwise STATIC_LIBRARY
	if((${TYPE} STREQUAL "LIBRARY") OR(${TYPE} STREQUAL "STATIC_LIBRARY") OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		if((${XII_COMPILE_ENGINE_AS_DLL} AND(${TYPE} STREQUAL "LIBRARY")) OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		...
		if(NOT ARG_NO_XII_PREFIX)
			xii_add_output_xii_prefix(${TARGET_NAME})
		endif()

		xii_set_library_properties(${TARGET_NAME})
	...
	elseif(${TYPE} STREQUAL "APPLICATION")
		...
		xii_set_application_properties(${TARGET_NAME})

	else()
		message(FATAL_ERROR "xii_create_target: Missing argument to specify target type. Pass in 'APP' or 'LIB'.")
	endif()
	...
	xii_set_common_target_definitions(${TARGET_NAME})

	xii_set_build_flags(${TARGET_NAME} ${ARGN})

	# On linux we want all symbols to be hidden by default. We manually "export" them.
	if(XII_COMPILE_ENGINE_AS_DLL AND XII_CMAKE_PLATFORM_LINUX AND NOT ARG_ALL_SYMBOLS_VISIBLE)
		target_compile_options(${TARGET_NAME} PRIVATE "-fvisibility=hidden")
	endif()

	xii_set_project_ide_folder(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR})

	# Pass the windows sdk include paths to the resource compiler when not generating a visual studio solution.
	if(XII_CMAKE_PLATFORM_WINDOWS AND NOT XII_CMAKE_GENERATOR_MSVC)
	...
	if(NOT ${ARG_NO_QT})
		xii_qt_wrap_target_files(${TARGET_NAME} "${ALL_SOURCE_FILES}")
	endif()

	xii_ci_add_to_targets_list(${TARGET_NAME} C++)

	if(NOT ${ARG_NO_UNITY})
		xii_generate_folder_unity_files_for_target(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR} "${ARG_EXCLUDE_FOLDER_FOR_UNITY}")
	endif()
