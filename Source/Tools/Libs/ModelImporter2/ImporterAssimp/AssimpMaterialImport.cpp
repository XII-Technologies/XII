...
namespace xiiModelImporter2
{
  ...
  static void TryReadAssimpProperty(xiiMap<PropertySemantic, xiiVariant>& inout_properties, PropertySemantic targetSemantic, const aiMaterial& assimpMaterial, const char* szKey, xiiUInt32 uiType, xiiUInt32 uiIdx, bool bInvert = false)
  {
    assimpType value;
  ...
  void TryReadAssimpTextures(xiiMap<TextureSemantic, xiiString>& out_textures, aiTextureType type, TextureSemantic targetSemantic, const aiMaterial& assimpMaterial)
  {
  ...
  xiiResult ImporterAssimp::ImportMaterials()
  {
    if (!m_pScene->HasMaterials())
      return XII_SUCCESS;

    xiiSet<xiiString> knownMaterialNames;
    knownMaterialNames.Insert("");

    for (xiiUInt32 matIdx = 0; matIdx < m_pScene->mNumMaterials; ++matIdx)
    {
