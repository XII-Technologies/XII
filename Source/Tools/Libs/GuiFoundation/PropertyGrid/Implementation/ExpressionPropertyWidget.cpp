...
xiiQtPropertyEditorExpressionWidget::xiiQtPropertyEditorExpressionWidget() :
...
void xiiQtPropertyEditorExpressionWidget::OnInit()
{
}

void xiiQtPropertyEditorExpressionWidget::InternalSetValue(const xiiVariant& value)
{
  ...
  else
  {
    ...
    QString newText = xiiMakeQString(value.ConvertTo<xiiString>());
...
void xiiQtPropertyEditorExpressionWidget::on_TextChanged()
{
  BroadcastValueChanged(xiiVariant(qtToXIIString(m_pWidget->toPlainText())));
}
...
{
  auto& knownTypes       = xiiExpressionParser::GetKnownTypes();
  auto& builtinFunctions = xiiExpressionParser::GetBuiltinFunctions();
    ...
    switch (state)
    {
      ...
      case Identifier:
        if (ch.isSpace() || !(ch.isDigit() || ch.isLetter() || ch == '_'))
        {
          ...
          xiiTempHashedString tokenHashed(token.toUtf8().data());
