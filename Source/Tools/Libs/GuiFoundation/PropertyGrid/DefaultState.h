...
/// \brief Registry for all xiiDefaultStateProvider factory functions.
class XII_GUIFOUNDATION_DLL xiiDefaultState
{
public:
  /// \brief The functor interface for the xiiDefaultStateProvider factory function
  ///
  ...
  typedef xiiSharedPtr<xiiDefaultStateProvider> (*CreateStateProviderFunc)(xiiObjectAccessorBase* pAccessor, const xiiDocumentObject* pObject, const xiiAbstractProperty* pProp);

  /// \brief Registers a xiiDefaultStateProvider factory method. It is safe to register / unregister factories at any time.
  static void RegisterDefaultStateProvider(CreateStateProviderFunc func);
  /// \brief Unregisters a xiiDefaultStateProvider factory method.
