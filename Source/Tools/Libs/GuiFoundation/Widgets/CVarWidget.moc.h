/// Copyright (c) Theophilus Eriata. All Rights Reserved.

#pragma once

#include <GuiFoundation/GuiFoundationDLL.h>

#include <Core/Console/Console.h>
#include <Foundation/Basics.h>
#include <Foundation/Containers/Deque.h>
#include <Foundation/Containers/Map.h>
#include <Foundation/Strings/String.h>
#include <Foundation/Types/Variant.h>
#include <GuiFoundation/ui_CVarWidget.h>
#include <QItemDelegate>
#include <QPointer>
#include <QWidget>

class QStandardItemModel;
class QSortFilterProxyModel;
class xiiQtCVarModel;
class xiiQtCVarWidget;

class xiiQtCVarItemDelegate : public QItemDelegate
{
  Q_OBJECT

public:
  explicit xiiQtCVarItemDelegate(QObject* pParent = nullptr) :
    QItemDelegate(pParent)
  {
  }

  virtual QWidget* createEditor(QWidget* pParent, const QStyleOptionViewItem& option, const QModelIndex& index) const override;
  virtual void     setEditorData(QWidget* pEditor, const QModelIndex& index) const override;
  virtual void     setModelData(QWidget* pEditor, QAbstractItemModel* pModel, const QModelIndex& index) const override;

  xiiQtCVarModel* m_pModel = nullptr;

private:
  mutable QModelIndex m_Index;

private Q_SLOTS:
  void onComboChanged(int);
};

class xiiQtCVarModel : public QAbstractItemModel
{
  Q_OBJECT
public:
  xiiQtCVarModel(xiiQtCVarWidget* pOwner);
  ~xiiQtCVarModel();

  void BeginResetModel();
  void EndResetModel();

public: // QAbstractItemModel interface
  virtual QVariant      headerData(int iSection, Qt::Orientation orientation, int iRole = Qt::DisplayRole) const override;
  virtual QVariant      data(const QModelIndex& index, int iRole) const override;
  virtual bool          setData(const QModelIndex& index, const QVariant& value, int iRole = Qt::EditRole) override;
  virtual Qt::ItemFlags flags(const QModelIndex& index) const override;
  virtual QModelIndex   index(int iRow, int iColumn, const QModelIndex& parent = QModelIndex()) const override;
  virtual QModelIndex   parent(const QModelIndex& index) const override;
  virtual int           rowCount(const QModelIndex& parent = QModelIndex()) const override;
  virtual int           columnCount(const QModelIndex& parent = QModelIndex()) const override;

public:
  struct Entry
  {
    xiiString               m_sFullName;
    QString                 m_sDisplayString;
    Entry*                  m_pParentEntry = nullptr;
    xiiDynamicArray<Entry*> m_ChildEntries;

    QString    m_sPlugin;      // in which plugin a CVar is defined
    QString    m_sDescription; // CVar description text
    xiiVariant m_Value;
  };

  Entry* CreateEntry(xiiStringView sName);

  xiiQtCVarWidget*        m_pOwner = nullptr;
  xiiDynamicArray<Entry*> m_RootEntries;
  xiiDeque<Entry>         m_AllEntries;
};

/// Data used by xiiQtCVarWidget to represent CVar states
struct XII_GUIFOUNDATION_DLL xiiCVarWidgetData
{
  mutable bool m_bNewEntry = true;

  xiiString m_sPlugin;      // in which plugin a CVar is defined
  xiiString m_sDescription; // CVar description text
  xiiUInt8  m_uiType = 0;   // xiiCVarType

  // 'union' over the different possible CVar types
  bool      m_bValue = false;
  float     m_fValue = 0.0f;
  double    m_dValue = 0.0;
  xiiInt32  m_iValue = 0;
  xiiString m_sValue;
};

/// Displays CVar values in a table and allows to modify them.
class XII_GUIFOUNDATION_DLL xiiQtCVarWidget : public QWidget, public Ui_CVarWidget
{
  Q_OBJECT

public:
  xiiQtCVarWidget(QWidget* pParent);
  ~xiiQtCVarWidget();

  /// Clears the table
  void Clear();

  /// Recreates the full UI. This is necessary when elements were added or removed.
  void RebuildCVarUI(const xiiMap<xiiString, xiiCVarWidgetData>& cvars);

  /// Updates the existing UI. This is sufficient if values changed only.
  void UpdateCVarUI(const xiiMap<xiiString, xiiCVarWidgetData>& cvars);

  void AddConsoleStrings(const xiiStringBuilder& sEncoded);

  xiiConsole& GetConsole() { return m_Console; }

Q_SIGNALS:
  void onBoolChanged(xiiStringView sCVar, bool bNewValue);
  void onFloatChanged(xiiStringView sCVar, float fNewValue);
  void onDoubleChanged(xiiStringView sCVar, double fNewValue);
  void onIntChanged(xiiStringView sCVar, int iNewValue);
  void onStringChanged(xiiStringView sCVar, xiiStringView sNewValue);

private Q_SLOTS:
  void SearchTextChanged(const QString& text);
  void ConsoleEnterPressed();
  void ConsoleSpecialKeyPressed(Qt::Key key);

private:
  QPointer<xiiQtCVarModel>        m_pItemModel;
  QPointer<QSortFilterProxyModel> m_pFilterModel;
  QPointer<xiiQtCVarItemDelegate> m_pItemDelegate;

  void OnConsoleEvent(const xiiConsoleEvent& e);

  xiiConsole m_Console;
};
