...
xiiQtDoubleSpinBox::xiiQtDoubleSpinBox(QWidget* pParent, bool bIntMode) :
...
void xiiQtDoubleSpinBox::setDefaultValue(const xiiVariant& val)
{
  double fValue = 0;
  if (xiiToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setDefaultValue(fValue);
}

void xiiQtDoubleSpinBox::setMinimum(const xiiVariant& val)
{
  double fValue = 0;
  if (xiiToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMinimum(fValue);
...
void xiiQtDoubleSpinBox::setMaximum(const xiiVariant& val)
{
  double fValue = 0;
  if (xiiToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMaximum(fValue);
...
void xiiQtDoubleSpinBox::setValueInvalid()
{
  m_bInvalid        = true;
...
double xiiQtDoubleSpinBox::value() const
{
  if (m_bInvalid)
    return 0.0;

  XII_ASSERT_DEBUG(!xiiMath::IsNaN(QDoubleSpinBox::value()), "Spin box valid value should never be NaN!");
  return m_bIntMode ? xiiMath::Round(QDoubleSpinBox::value()) : QDoubleSpinBox::value();
}

void xiiQtDoubleSpinBox::focusInEvent(QFocusEvent* event)
{
...
void xiiQtDoubleSpinBox::focusOutEvent(QFocusEvent* event)
{
  QDoubleSpinBox::focusOutEvent(event);
}

void xiiQtDoubleSpinBox::mousePressEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void xiiQtDoubleSpinBox::mouseReleaseEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void xiiQtDoubleSpinBox::mouseMoveEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
    if (m_bDragging)
    {
      ...
      {
        m_LastDragPos     = event->globalPosition().toPoint();
        const QRect dsize = xiiWidgetUtils::GetClosestScreen(event->globalPosition().toPoint()).availableGeometry();
...
void xiiQtDoubleSpinBox::keyPressEvent(QKeyEvent* event)
{
...
bool xiiQtDoubleSpinBox::event(QEvent* event)
{
...
void xiiQtDoubleSpinBox::onCustomContextMenuRequested()
{
  if (!isReadOnly())
  {
