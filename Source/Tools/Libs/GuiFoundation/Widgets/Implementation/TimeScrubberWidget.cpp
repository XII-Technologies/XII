...
xiiQtTimeScrubberWidget::xiiQtTimeScrubberWidget(QWidget* pParent) :
  QWidget(pParent)
{
  setMinimumWidth(100);
  setMinimumHeight(24);
...
xiiQtTimeScrubberWidget::~xiiQtTimeScrubberWidget() = default;

void xiiQtTimeScrubberWidget::SetDuration(xiiUInt64 uiNumTicks)
{
...
void xiiQtTimeScrubberWidget::SetDuration(xiiTime time)
{
  SetDuration(static_cast<xiiUInt64>(time.GetSeconds() * 4800));
}

void xiiQtTimeScrubberWidget::SetScrubberPosition(xiiUInt64 uiTick)
{
  if (m_uiScrubberTickPos == uiTick)
    return;

  m_uiScrubberTickPos     = uiTick;
  m_fNormScrubberPosition = xiiMath::Clamp((double)uiTick / (double)m_uiDurationTicks, 0.0, 1.0);

  update();
}

void xiiQtTimeScrubberWidget::SetScrubberPosition(xiiTime time)
{
  SetScrubberPosition(static_cast<xiiUInt64>(time.GetSeconds() * 4800));
}

void xiiQtTimeScrubberWidget::paintEvent(QPaintEvent* event)
{
  ...
  xiiWidgetUtils::AdjustGridDensity(fFineGridDensity, fRoughGridDensity, rect().width(), fMaxDuration, 20);
...
void xiiQtTimeScrubberWidget::mousePressEvent(QMouseEvent* event)
{
...
void xiiQtTimeScrubberWidget::mouseReleaseEvent(QMouseEvent* event)
{
...
void xiiQtTimeScrubberWidget::mouseMoveEvent(QMouseEvent* event)
{
...
void xiiQtTimeScrubberWidget::SetScrubberPosFromPixelCoord(xiiInt32 posX)
{
  double fNormPos = (double)posX / (double)rect().width();
  fNormPos        = xiiMath::Clamp(fNormPos, 0.0, 1.0);

  const xiiUInt64 uiTickPos = (xiiUInt64)(fNormPos * m_uiDurationTicks);
...
xiiQtTimeScrubberToolbar::xiiQtTimeScrubberToolbar(QWidget* pParent) :
  QToolBar("Time Scrubber", pParent)
{
  m_pScrubber = new xiiQtTimeScrubberWidget(this);
  ...
  // Pass event through
  connect(m_pScrubber, &xiiQtTimeScrubberWidget::ScrubberPosChangedEvent, this,
          [this](xiiUInt64 uiNewScrubberTickPos) { Q_EMIT ScrubberPosChangedEvent(uiNewScrubberTickPos); });
...
void xiiQtTimeScrubberToolbar::SetDuration(xiiUInt64 uiNumTicks)
{
  m_pScrubber->SetDuration(uiNumTicks);

  xiiQtScopedBlockSignals _1(m_pDuration);

  const double oldVal = m_pDuration->text().toDouble();
  const double newVal = (double)uiNumTicks / 4800.0;

  if (xiiMath::IsEqual(oldVal, newVal, 0.01))
    return;

  m_pDuration->setText(QString("%1").arg(newVal, 0, 'f', 2));
}

void xiiQtTimeScrubberToolbar::SetScrubberPosition(xiiUInt64 uiTick)
{
  m_pScrubber->SetScrubberPosition(uiTick);
}

void xiiQtTimeScrubberToolbar::SetButtonState(bool bPlaying, bool bRepeatEnabled)
{
  if (bPlaying)
