  ...
  QWidget(pParent)
{
  ...
  m_ControlPointBrush.setColor(xiiToQtColor(xiiColorScheme::LightUI(xiiColorScheme::Gray)));
  m_ControlPointBrush.setStyle(Qt::BrushStyle::SolidPattern);

  m_SelectedControlPointBrush.setColor(xiiToQtColor(xiiColorScheme::LightUI(xiiColorScheme::Yellow)));
  m_SelectedControlPointBrush.setStyle(Qt::BrushStyle::SolidPattern);

  m_TangentLinePen.setCosmetic(true);
  m_TangentLinePen.setColor(xiiToQtColor(xiiColorScheme::LightUI(xiiColorScheme::Orange)));
  m_TangentLinePen.setStyle(Qt::PenStyle::DashLine);

  m_TangentHandleBrush.setColor(xiiToQtColor(xiiColorScheme::LightUI(xiiColorScheme::Orange)));
  m_TangentHandleBrush.setStyle(Qt::BrushStyle::SolidPattern);
}

void xiiQtCurveEditWidget::SetCurves(xiiCurveGroupData* pCurveEditData)
{
  ...
  for (xiiUInt32 i = 0; i < pCurveEditData->m_Curves.GetCount(); ++i)
  {
  ...
  m_fMinValue       = xiiMath::HighValue<float>();
  m_fMaxValue       = -xiiMath::HighValue<float>();

  for (xiiUInt32 i = 0; i < m_CurvesSorted.GetCount(); ++i)
  {
    xiiCurve1D& curve = m_CurvesSorted[i];
    ...
    if (!m_bLowerExtentFixed)
      m_fMinExtentValue = xiiMath::Min(m_fMinExtentValue, m_CurveExtents[i].x);
    if (!m_bUpperExtentFixed)
      m_fMaxExtentValue = xiiMath::Max(m_fMaxExtentValue, m_CurveExtents[i].y);

    m_fMinValue = xiiMath::Min(m_fMinValue, fMin);
    m_fMaxValue = xiiMath::Max(m_fMaxValue, fMax);
  }
...
void xiiQtCurveEditWidget::SetScrubberPosition(double fPosition)
{
...
void xiiQtCurveEditWidget::FrameCurve()
{
  double fLowPoint = m_fMinValue;
  double fUprPoint = m_fMaxValue;

  if (m_fLowerRange > -xiiMath::HighValue<double>() * 0.5)
    fLowPoint = xiiMath::Min(fLowPoint, m_fLowerRange);
  else
    fLowPoint = xiiMath::Min(fLowPoint, 0.0);

  if (m_fUpperRange < xiiMath::HighValue<double>() * 0.5)
    fUprPoint = xiiMath::Max(fUprPoint, m_fUpperRange);
  else
    fUprPoint = xiiMath::Max(fUprPoint, 1.0);

  double fLeftPoint  = xiiMath::Min(0.0, m_fMinExtentValue);
  double fRightPoint = xiiMath::Max(1.0, m_fMaxExtentValue);

  double fWidth   = xiiMath::Max(1.0, fRightPoint - fLeftPoint);
...
void xiiQtCurveEditWidget::FrameSelection()
{
...
void xiiQtCurveEditWidget::Frame(double fOffsetX, double fOffsetY, double fWidth, double fHeight)
{
  m_bFrameBeforePaint = false;

  fWidth  = xiiMath::Max(fWidth, 0.1);
  fHeight = xiiMath::Max(fHeight, 0.1);
...
QPoint xiiQtCurveEditWidget::MapFromScene(const QPointF& pos) const
{
...
QPointF xiiQtCurveEditWidget::MapToScene(const QPoint& pos) const
{
  double x = pos.x();
  double y = pos.y();
  x /= m_SceneToPixelScale.x();
...
void xiiQtCurveEditWidget::ClearSelection()
{
...
void xiiQtCurveEditWidget::SelectAll()
{
  m_SelectedCPs.Clear();

  for (xiiUInt32 curveIdx = 0; curveIdx < m_Curves.GetCount(); ++curveIdx)
  {
    for (xiiUInt32 cpIdx = 0; cpIdx < m_Curves[curveIdx].GetNumControlPoints(); ++cpIdx)
    {
...
bool xiiQtCurveEditWidget::IsSelected(const xiiSelectedCurveCP& cp) const
{
...
void xiiQtCurveEditWidget::SetSelection(const xiiSelectedCurveCP& cp)
{
  m_SelectedCPs.Clear();
...
void xiiQtCurveEditWidget::ToggleSelected(const xiiSelectedCurveCP& cp)
{
...
bool xiiQtCurveEditWidget::GetSelectedTangent(xiiInt32& out_iCurve, xiiInt32& out_iPoint, bool& out_bLeftTangent) const
{
...
QRectF xiiQtCurveEditWidget::ComputeViewportSceneRect() const
{
...
void xiiQtCurveEditWidget::paintEvent(QPaintEvent* e)
{
  ...
  xiiWidgetUtils::AdjustGridDensity(fFineGridDensity, fRoughGridDensity, rect().width(), viewportSceneRect.width(), 20);
...
void xiiQtCurveEditWidget::ClampZoomPan()
{
  m_SceneTranslation.setX(xiiMath::Clamp(m_SceneTranslation.x(), -2.0, 50000.0));
  m_SceneTranslation.setY(xiiMath::Clamp(m_SceneTranslation.y(), -200000.0, 200000.0));
  m_SceneToPixelScale.setX(xiiMath::Clamp(m_SceneToPixelScale.x(), 0.0005, 10000.0));
  m_SceneToPixelScale.setY(xiiMath::Clamp(m_SceneToPixelScale.y(), -10000.0, -0.0005));
}

void xiiQtCurveEditWidget::mousePressEvent(QMouseEvent* e)
{
  ...
  if (e->buttons() == Qt::LeftButton) // nothing else pressed
  {
    ...
    if (clickedOn == ClickTarget::Nothing || clickedOn == ClickTarget::SelectedPoint)
    {
      ...
      XII_ASSERT_DEBUG(!m_bBegunChanges, "Invalid State");
...
void xiiQtCurveEditWidget::mouseReleaseEvent(QMouseEvent* e)
{
  ...
  if (m_State != EditState::MultiSelect && m_pRubberband)
  {
    ...
    {
      xiiDynamicArray<xiiSelectedCurveCP> change;
...
void xiiQtCurveEditWidget::mouseMoveEvent(QMouseEvent* e)
{
  ...
  if (m_State == EditState::DraggingPoints)
  {
    ...
    {
      if (xiiMath::Abs(m_TotalPointDrag.x()) > xiiMath::Abs(m_TotalPointDrag.y()))
      {
...
void xiiQtCurveEditWidget::mouseDoubleClickEvent(QMouseEvent* e)
{
...
void xiiQtCurveEditWidget::wheelEvent(QWheelEvent* e)
{
...
void xiiQtCurveEditWidget::keyPressEvent(QKeyEvent* e)
{
...
void xiiQtCurveEditWidget::PaintCurveSegments(QPainter* painter, float fOffsetX, xiiUInt8 alpha) const
{
  ...
  for (xiiUInt32 curveIdx = 0; curveIdx < m_CurvesSorted.GetCount(); ++curveIdx)
  {
    const xiiCurve1D& curve  = m_CurvesSorted[curveIdx];
    const xiiUInt32   numCps = curve.GetNumControlPoints();

    if (numCps == 0)
      continue;

    const xiiColorGammaUB curveColor = m_pCurveEditData->m_Curves[curveIdx]->m_CurveColor;
    ...
    if (bRenderRealCurve)
    {
      ...
      {
        const xiiCurve1D::ControlPoint& cp = curve.GetControlPoint(0);
      ...
      for (xiiUInt32 cpIdx = 1; cpIdx < numCps; ++cpIdx)
      {
        const xiiCurve1D::ControlPoint& cpPrev = curve.GetControlPoint(cpIdx - 1);
        const xiiCurve1D::ControlPoint& cpThis = curve.GetControlPoint(cpIdx);
      ...
      {
        const xiiCurve1D::ControlPoint& cp = curve.GetControlPoint(numCps - 1);
    ...
    if (bRenderLinearCurve)
    {
      ...
      for (xiiUInt32 i = 1; i < linear.GetCount(); ++i)
      {
...
void xiiQtCurveEditWidget::PaintOutsideAreaOverlay(QPainter* painter) const
{
...
void xiiQtCurveEditWidget::PaintControlPoints(QPainter* painter) const
{
  painter->save();
  ...
  for (xiiUInt32 curveIdx = 0; curveIdx < m_Curves.GetCount(); ++curveIdx)
  {
    const xiiCurve1D& curve = m_Curves[curveIdx];

    const xiiUInt32 numCps = curve.GetNumControlPoints();
    for (xiiUInt32 cpIdx = 0; cpIdx < numCps; ++cpIdx)
    {
      const xiiCurve1D::ControlPoint& cp = curve.GetControlPoint(cpIdx);
...
void xiiQtCurveEditWidget::PaintSelectedControlPoints(QPainter* painter) const
{
  painter->save();
  ...
  {
    const xiiCurve1D&               curve = m_Curves[cpSel.m_uiCurve];
    const xiiCurve1D::ControlPoint& cp    = curve.GetControlPoint(cpSel.m_uiPoint);
...
void xiiQtCurveEditWidget::PaintSelectedTangentLines(QPainter* painter) const
{
  painter->save();
  painter->setBrush(Qt::NoBrush);
  painter->setPen(m_TangentLinePen);

  xiiHybridArray<QLine, 50> lines;
  ...
  for (const auto& cpSel : m_SelectedCPs)
  {
    ...
    xiiVec2d leftHandlePos  = cp.m_Position + xiiVec2d(cp.m_LeftTangent.x, cp.m_LeftTangent.y);
    xiiVec2d rightHandlePos = cp.m_Position + xiiVec2d(cp.m_RightTangent.x, cp.m_RightTangent.y);
    ...
    const xiiCurveTangentMode::Enum tmLeft  = m_pCurveEditData->m_Curves[cpSel.m_uiCurve]->m_ControlPoints[cpSel.m_uiPoint].m_LeftTangentMode;
    const xiiCurveTangentMode::Enum tmRight = m_pCurveEditData->m_Curves[cpSel.m_uiCurve]->m_ControlPoints[cpSel.m_uiPoint].m_RightTangentMode;
...
void xiiQtCurveEditWidget::PaintSelectedTangentHandles(QPainter* painter) const
{
  painter->save();
  ...
  for (const auto& cpSel : m_SelectedCPs)
  {
    ...
    if (bDrawLeft && tmLeft != xiiCurveTangentMode::Linear && tmLeft != xiiCurveTangentMode::Auto)
    {
      ...
      else
      {
        const xiiVec2d leftHandlePos = cp.m_Position + xiiVec2d(cp.m_LeftTangent.x, cp.m_LeftTangent.y);
        const QPointF  ptPosLeft     = MapFromScene(QPointF(leftHandlePos.x, leftHandlePos.y));
        // const xiiVec2 dir = MapDirFromScene(cp.m_LeftTangent).GetNormalized() * 50.0f;
    ...
    if (bDrawRight && tmRight != xiiCurveTangentMode::Linear && tmRight != xiiCurveTangentMode::Auto)
    {
      ...
      else
      {
        const xiiVec2d rightHandlePos = cp.m_Position + xiiVec2d(cp.m_RightTangent.x, cp.m_RightTangent.y);
        const QPointF  ptPosRight     = MapFromScene(QPointF(rightHandlePos.x, rightHandlePos.y));
        // const xiiVec2 dir = MapDirFromScene(cp.m_RightTangent).GetNormalized() * 50.0f;
...
void xiiQtCurveEditWidget::PaintMultiSelectionSquare(QPainter* painter) const
{
...
void xiiQtCurveEditWidget::PaintScrubber(QPainter& p) const
{
  if (!m_bShowScrubber)
    return;
  ...
  const xiiInt32 xPos = MapFromScene(xiiVec2d(m_fScrubberPosition, 0)).x();
...
void xiiQtCurveEditWidget::RenderVerticalGrid(QPainter* painter, const QRectF& viewportSceneRect, double fRoughGridDensity)
{
  double lowX, highX;
  xiiWidgetUtils::ComputeGridExtentsX(viewportSceneRect, fRoughGridDensity, lowX, highX);
  lowX = xiiMath::Max(lowX, 0.0);
  ...
  // render grid lines
  {
    ...
    xiiHybridArray<QLine, 100> lines;
...
void xiiQtCurveEditWidget::RenderSideLinesAndText(QPainter* painter, const QRectF& viewportSceneRect)
{
  double fFineGridDensity  = 0.01;
  double fRoughGridDensity = 0.01;
  xiiWidgetUtils::AdjustGridDensity(fFineGridDensity, fRoughGridDensity, rect().height(), xiiMath::Abs(viewportSceneRect.height()), 20);

  painter->save();

  const xiiInt32 iFineLineLength  = 10;
  const xiiInt32 iRoughLineLength = 20;
  ...
  {
    double lowY, highY;
    xiiWidgetUtils::ComputeGridExtentsY(viewportSceneRect, fFineGridDensity, lowY, highY);

    if (lowY > highY)
      xiiMath::Swap(lowY, highY);
  ...
  {
    double lowY, highY;
    xiiWidgetUtils::ComputeGridExtentsY(viewportSceneRect, fRoughGridDensity, lowY, highY);
...
void xiiQtCurveEditWidget::RenderValueRanges(QPainter* painter)
{
...
bool xiiQtCurveEditWidget::PickCpAt(const QPoint& pos, float fMaxPixelDistance, xiiSelectedCurveCP& out_Result) const
{
  const xiiVec2 at((float)pos.x(), (float)pos.y());
  float         fMaxDistSqr = xiiMath::Square(fMaxPixelDistance);

  out_Result.m_uiCurve = 0xFFFF;

  for (xiiUInt32 uiCurve = 0; uiCurve < m_Curves.GetCount(); ++uiCurve)
  {
    const xiiCurve1D& curve = m_Curves[uiCurve];

    for (xiiUInt32 uiCP = 0; uiCP < curve.GetNumControlPoints(); ++uiCP)
    {
      const auto& cp = curve.GetControlPoint(uiCP);

      const QPoint  diff = MapFromScene(cp.m_Position) - pos;
      const xiiVec2 fDiff(diff.x(), diff.y());
...
static inline xiiVec2d ToVec(const QPoint& pt)
{
  return xiiVec2d(pt.x(), pt.y());
}

xiiQtCurveEditWidget::ClickTarget xiiQtCurveEditWidget::DetectClickTarget(const QPoint& pos)
{
  const xiiVec2d vScreenPos(pos.x(), pos.y());
  float          fMinDistSQR = xiiMath::Square(15);
  xiiInt32       iBestCurve  = -1;
  xiiInt32       iBestCP     = -1;
  xiiInt32       iBestComp   = -1;
  ...
  for (xiiUInt32 i = 0; i < m_SelectedCPs.GetCount(); ++i)
  {
    ...
    const xiiVec2d point(cp.GetTickAsTime().GetSeconds(), cp.m_fValue);

    const xiiVec2d ptPos = ToVec(MapFromScene(point));
    xiiVec2d       ptLeft;
    xiiVec2d       ptRight;
...
void xiiQtCurveEditWidget::ExecMultiSelection(xiiDynamicArray<xiiSelectedCurveCP>& out_Selection)
{
  out_Selection.Clear();
...
bool xiiQtCurveEditWidget::CombineSelectionAdd(xiiDynamicArray<xiiSelectedCurveCP>& inout_Selection, const xiiDynamicArray<xiiSelectedCurveCP>& change)
{
  bool bChange = false;

  for (xiiUInt32 i = 0; i < change.GetCount(); ++i)
  {
...
bool xiiQtCurveEditWidget::CombineSelectionRemove(xiiDynamicArray<xiiSelectedCurveCP>& inout_Selection, const xiiDynamicArray<xiiSelectedCurveCP>& change)
{
  bool bChange = false;
...
bool xiiQtCurveEditWidget::CombineSelectionToggle(xiiDynamicArray<xiiSelectedCurveCP>& inout_Selection, const xiiDynamicArray<xiiSelectedCurveCP>& change)
{
  bool bChange = false;
...
void xiiQtCurveEditWidget::ComputeSelectionRect()
{
  m_SelectionBRect = QRectF();
  ...
  for (const auto& cpSel : m_SelectedCPs)
  {
    ...
    bbox.ExpandToInclude(xiiVec3(cp.m_Position.x, cp.m_Position.y, cp.m_Position.x));
  }
...
xiiQtCurveEditWidget::SelectArea xiiQtCurveEditWidget::WhereIsPoint(QPoint pos) const
{
  if (m_SelectionBRect.isEmpty())
...
xiiInt32 xiiQtCurveEditWidget::PickCurveAt(QPoint pos) const
{
  const QPointF scenePos = MapToScene(pos);
  const float   x        = scenePos.x();

  xiiInt32 iCurveIdx    = -1;
  xiiInt32 iMinDistance = 15;
  ...
  for (xiiUInt32 i = 0; i < m_CurvesSorted.GetCount(); ++i)
  {
    ...
    const xiiInt32 dist = xiiMath::Abs(pixelPos.y() - pos.y());
