...
xiiQtEventTrackWidget::xiiQtEventTrackWidget(QWidget* pParent) :
  QWidget(pParent)
{
...
void xiiQtEventTrackWidget::SetData(const xiiEventTrackData* pData, double fMinCurveLength)
{
  m_pEditData       = pData;
...
void xiiQtEventTrackWidget::RecreateSortedData()
{
  ...
  for (xiiUInt32 idx = 0; idx < m_pEditData->m_ControlPoints.GetCount(); ++idx)
  {
...
void xiiQtEventTrackWidget::FrameCurve()
{
  ...
  else if (m_SelectedPoints.GetCount() > 1)
  {
    ...
    for (const auto& cpSel : m_SelectedPoints)
    {
      ...
      minX             = xiiMath::Min(minX, pos);
      maxX             = xiiMath::Max(maxX, pos);
    }
...
QPoint xiiQtEventTrackWidget::MapFromScene(const QPointF& pos) const
{
...
QPointF xiiQtEventTrackWidget::MapToScene(const QPoint& pos) const
{
  double x = pos.x();
  double y = pos.y();
  x /= m_SceneToPixelScale.x();
...
void xiiQtEventTrackWidget::ClearSelection()
{
...
void xiiQtEventTrackWidget::GetSelection(xiiHybridArray<xiiUInt32, 32>& out_selection) const
{
  out_selection.Clear();
...
bool xiiQtEventTrackWidget::IsSelected(SelectedPoint cp) const
{
  return m_Categories[cp.m_uiCategory].m_SortedPoints[cp.m_uiSortedIdx].m_bSelected;
}

void xiiQtEventTrackWidget::SetSelection(SelectedPoint cp)
{
  ClearSelection();
...
void xiiQtEventTrackWidget::SetSelection(const xiiHybridArray<SelectedPoint, 32>& selection)
{
  for (const auto& cpSel : m_SelectedPoints)
  {
    m_Categories[cpSel.m_uiCategory].m_SortedPoints[cpSel.m_uiSortedIdx].m_bSelected = false;
...
void xiiQtEventTrackWidget::ToggleSelected(SelectedPoint cp)
{
...
void xiiQtEventTrackWidget::SetSelected(SelectedPoint cp, bool set)
{
...
QRectF xiiQtEventTrackWidget::ComputeViewportSceneRect() const
{
...
static xiiColorGammaUB g_EventColors[10 * 3] = {
  xiiColorGammaUB(255, 102, 0),
  xiiColorGammaUB(76, 255, 0),
  xiiColorGammaUB(0, 255, 255),
  xiiColorGammaUB(239, 35, 0),
  xiiColorGammaUB(127, 255, 0),
  xiiColorGammaUB(0, 0, 255),
  xiiColorGammaUB(205, 92, 92),
  xiiColorGammaUB(120, 158, 39),
  xiiColorGammaUB(81, 120, 188),
  xiiColorGammaUB(255, 105, 180),
  xiiColorGammaUB(0, 250, 154),
  xiiColorGammaUB(0, 191, 255),
  xiiColorGammaUB(220, 20, 60),
  xiiColorGammaUB(0, 255, 127),
  xiiColorGammaUB(30, 144, 255),
  xiiColorGammaUB(240, 128, 128),
  xiiColorGammaUB(60, 179, 113),
  xiiColorGammaUB(135, 206, 250),
  xiiColorGammaUB(178, 34, 34),
  xiiColorGammaUB(46, 139, 87),
  xiiColorGammaUB(65, 105, 225),
  xiiColorGammaUB(211, 122, 122),
  xiiColorGammaUB(144, 238, 144),
  xiiColorGammaUB(135, 206, 235),
  xiiColorGammaUB(219, 112, 147),
  xiiColorGammaUB(0, 128, 0),
  xiiColorGammaUB(70, 130, 180),
  xiiColorGammaUB(255, 182, 193),
  xiiColorGammaUB(102, 205, 170),
  xiiColorGammaUB(100, 149, 237),
};

void xiiQtEventTrackWidget::paintEvent(QPaintEvent* e)
{
...
void xiiQtEventTrackWidget::ClampZoomPan()
{
  m_fSceneTranslationX = xiiMath::Clamp(m_fSceneTranslationX, -2.0, 50000.0);
...
void xiiQtEventTrackWidget::mousePressEvent(QMouseEvent* e)
{
  ...
  if (e->buttons() == Qt::LeftButton) // nothing else pressed
  {
    ...
    if (clickedOn == ClickTarget::Nothing || clickedOn == ClickTarget::SelectedPoint)
    {
      if (e->modifiers() == Qt::NoModifier)
      {
        ...
        {
          case xiiQtEventTrackWidget::SelectArea::None:
            break;
          case xiiQtEventTrackWidget::SelectArea::Center:
            m_State          = EditState::DraggingPoints;
            m_TotalPointDrag = QPointF();
            break;
          case xiiQtEventTrackWidget::SelectArea::Left:
            m_State               = EditState::ScaleLeftRight;
            m_ScaleReferencePoint = m_SelectionBRect.topRight();
            break;
          case xiiQtEventTrackWidget::SelectArea::Right:
            m_State               = EditState::ScaleLeftRight;
...
void xiiQtEventTrackWidget::mouseReleaseEvent(QMouseEvent* e)
{
  ...
  if (m_State != EditState::MultiSelect && m_pRubberband)
  {
    ...
    {
      xiiHybridArray<SelectedPoint, 32> newSelection = m_SelectedPoints;
      xiiHybridArray<SelectedPoint, 32> change;
...
void xiiQtEventTrackWidget::mouseMoveEvent(QMouseEvent* e)
{
...
void xiiQtEventTrackWidget::mouseDoubleClickEvent(QMouseEvent* e)
{
...
void xiiQtEventTrackWidget::wheelEvent(QWheelEvent* e)
{
...
void xiiQtEventTrackWidget::keyPressEvent(QKeyEvent* e)
{
...
void xiiQtEventTrackWidget::PaintOutsideAreaOverlay(QPainter* painter) const
{
  const int iRightEdge = MapFromScene(QPointF(xiiMath::Max(0.0, m_fMaxCurveExtent), 0)).x();
...
void xiiQtEventTrackWidget::PaintControlPoints(QPainter* painter) const
{
  ...
  xiiHybridArray<QLineF, 100> lines;
  xiiHybridArray<QRectF, 100> rects;
  xiiHybridArray<QLineF, 100> linesSelected;
  xiiHybridArray<QRectF, 100> rectsSelected;
  ...
  for (xiiUInt32 catIdx = 0; catIdx < m_Categories.GetCount(); ++catIdx)
  {
    ...
    if (!rects.IsEmpty())
    {
      const xiiColorGammaUB& col = g_EventColors[catIdx % XII_ARRAY_SIZE(g_EventColors)];
...
void xiiQtEventTrackWidget::PaintMultiSelectionSquare(QPainter* painter) const
{
...
void xiiQtEventTrackWidget::PaintScrubber(QPainter& p) const
{
  if (!m_bShowScrubber)
    return;
  ...
  const xiiInt32 xPos = MapFromScene(QPointF(m_fScrubberPosition, 0)).x();
...
void xiiQtEventTrackWidget::RenderVerticalGrid(QPainter* painter, const QRectF& viewportSceneRect, double fRoughGridDensity)
{
  double lowX, highX;
...
bool xiiQtEventTrackWidget::PickCpAt(const QPoint& pos, float fMaxPixelDistance, SelectedPoint& out_Result) const
{
  ...
  for (xiiUInt32 catIdx = 0; catIdx < m_Categories.GetCount(); ++catIdx)
  {
    ...
    if (xiiMath::Abs(diffY) < 20)
    {
      const auto& allPoints = cat.m_SortedPoints;

      for (xiiUInt32 ptIdx = 0; ptIdx < allPoints.GetCount(); ++ptIdx)
      {
        const auto& point = allPoints[ptIdx];

        double        ptPosX = MapFromScene(QPointF(point.m_fPosX, 0)).x();
        const xiiVec2 fDiff(ptPosX - pos.x(), diffY);
...
xiiQtEventTrackWidget::ClickTarget xiiQtEventTrackWidget::DetectClickTarget(const QPoint& pos)
{
  SelectedPoint ptSel;
...
void xiiQtEventTrackWidget::ExecMultiSelection(xiiHybridArray<SelectedPoint, 32>& out_Selection)
{
  out_Selection.Clear();
...
void xiiQtEventTrackWidget::ComputeSelectionRect()
{
  m_SelectionBRect = QRectF();
  ...
  xiiBoundingBox bbox;
  bbox = xiiBoundingBox::MakeInvalid();
  ...
  //  bbox.ExpandToInclude(xiiVec3(pos, cpSel.m_uiCategory - 1, 0));
  //  bbox.ExpandToInclude(xiiVec3(pos, cpSel.m_uiCategory + 1, 0));
  //}
...
xiiQtEventTrackWidget::SelectArea xiiQtEventTrackWidget::WhereIsPoint(QPoint pos) const
{
  if (m_SelectionBRect.isEmpty())
