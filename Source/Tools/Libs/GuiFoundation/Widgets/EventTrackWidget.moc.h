...
class XII_GUIFOUNDATION_DLL xiiQtEventTrackWidget : public QWidget
{
  Q_OBJECT
...
public:
  struct SelectedPoint
  {
    ...
    xiiUInt32 m_uiCategory;
    xiiUInt32 m_uiSortedIdx;
  };

  xiiQtEventTrackWidget(QWidget* parent);
...
private:
  ...
  struct PointCategory
  {
    ...
    xiiHybridArray<Point, 32> m_SortedPoints;
  };

  bool IsSelected(SelectedPoint cp) const;
  void SetSelection(SelectedPoint cp);
  void SetSelection(const xiiHybridArray<SelectedPoint, 32>& selection);
  ...
  void        ExecMultiSelection(xiiHybridArray<SelectedPoint, 32>& out_Selection);
  bool        CombineSelection(xiiHybridArray<SelectedPoint, 32>& inout_Selection, const xiiHybridArray<SelectedPoint, 32>& change, bool add);
  ...
  const xiiEventTrackData* m_pEditData = nullptr;
  ...
  xiiHashTable<xiiHashedString, xiiUInt32> m_NameToCategory;
  xiiHybridArray<PointCategory, 8>         m_Categories;
  xiiHybridArray<SelectedPoint, 32>        m_SelectedPoints;
};
