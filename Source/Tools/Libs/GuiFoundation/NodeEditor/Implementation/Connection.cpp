...
void xiiQtConnection::UpdateGeometry()
{
  ...
  else if (pScene->GetConnectionStyle() == xiiQtNodeScene::ConnectionStyle::SubwayLines)
  {
...
void xiiQtConnection::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
  ...
  if (decorationFlags.IsSet(xiiQtNodeScene::ConnectionDecorationFlags::DrawDebugging))
  {
    const float offset   = fmod(xiiTime::Now().GetSeconds(), 1.0f);
...
void xiiQtConnection::DrawSubwayPath(QPainterPath& path, const QPointF& startPoint, const QPointF& endPoint)
{
