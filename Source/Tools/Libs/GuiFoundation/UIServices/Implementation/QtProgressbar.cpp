...
xiiQtProgressbar::xiiQtProgressbar() = default;

xiiQtProgressbar::~xiiQtProgressbar()
{
  SetProgressbar(nullptr);
  EnsureDestroyed();
}

void xiiQtProgressbar::SetProgressbar(xiiProgress* pProgress)
{
  if (m_pProgress)
  {
    m_pProgress->m_Events.RemoveEventHandler(xiiMakeDelegate(&xiiQtProgressbar::ProgressbarEventHandler, this));
    m_pProgress = nullptr;
  }
  ...
  if (pProgress)
  {
    ...
    m_pProgress->m_Events.AddEventHandler(xiiMakeDelegate(&xiiQtProgressbar::ProgressbarEventHandler, this));
  }
}

void xiiQtProgressbar::ProgressbarEventHandler(const xiiProgressEvent& e)
{
  switch (e.m_Type)
  {
    case xiiProgressEvent::Type::ProgressStarted:
    {
    ...
    case xiiProgressEvent::Type::ProgressEnded:
    {
      EnsureDestroyed();
      --m_iNestedProcessEvents;
    }
    ...
    case xiiProgressEvent::Type::ProgressChanged:
    {
      ++m_iNestedProcessEvents;
    ...
    case xiiProgressEvent::Type::CancelClicked:
      break;
  }
}

void xiiQtProgressbar::EnsureCreated()
{
  if (m_pDialog)
    return;
...
void xiiQtProgressbar::EnsureDestroyed()
{
  if (m_pDialog)
  {
    delete m_pDialog;
