...
static xiiQtImageCache* g_pImageCacheSingleton = nullptr;

XII_IMPLEMENT_SINGLETON(xiiQtImageCache);

// clang-format off
XII_BEGIN_SUBSYSTEM_DECLARATION(GuiFoundation, QtImageCache)

ON_CORESYSTEMS_STARTUP
{
  g_pImageCacheSingleton = XII_DEFAULT_NEW(xiiQtImageCache);
}

ON_CORESYSTEMS_SHUTDOWN
{
  XII_DEFAULT_DELETE(g_pImageCacheSingleton);
}

XII_END_SUBSYSTEM_DECLARATION;
// clang-format on

xiiQtImageCache::xiiQtImageCache() :
...
void xiiQtImageCache::InvalidateCache(xiiStringView sAbsolutePath)
{
  ...
  xiiUInt32 id = e.Value().m_uiImageID;
...
const QPixmap* xiiQtImageCache::QueryPixmap(xiiStringView sAbsolutePath,QModelIndex index,QVariant    userData1,QVariant    userData2,xiiUInt32*  out_pImageID)
{
  ...
  if (itEntry.IsValid())
  {
    ...
    itEntry.Value().m_LastAccess = xiiTime::Now();
...
void xiiQtImageCache::StopRequestProcessing(bool bPurgeExistingCache)
{
...
void xiiQtImageCache::EnableRequestProcessing()
{
  XII_LOCK(m_Mutex);
...
void xiiQtImageCache::EmitLoadedSignal(QString sPath, QModelIndex index, QVariant UserData1, QVariant UserData2)
{
  Q_EMIT ImageLoaded(sPath, index, UserData1, UserData2);
}

void xiiQtImageCache::LoadingTask(QString sPath, QModelIndex index, QVariant UserData1, QVariant UserData2)
{
  QImage     Image;
  const bool bImageAvailable = Image.load(sPath);

  xiiQtImageCache* pCache = xiiQtImageCache::GetSingleton();

  XII_LOCK(pCache->m_Mutex);
  ...
  pCache->m_iCurrentMemoryUsage -= xiiMath::SafeMultiply64(entry.m_Pixmap.width(), entry.m_Pixmap.height(), 4);
  ...
  entry.m_LastAccess = xiiTime::Now();
...
void xiiQtImageCache::CleanupCache()
{
  XII_LOCK(m_Mutex);
  ...
  for (xiiInt32 i = 5; i > 2; --i)
  {
    ...
    for (auto it = m_ImageCache.GetIterator(); it.IsValid();)
    {
      if (tNow - it.Value().m_LastAccess > tPurgeThreshold)
      {
        ...
        m_iCurrentMemoryUsage -= xiiMath::SafeMultiply64(it.Value().m_Pixmap.width(), it.Value().m_Pixmap.height(), 4);
