...
void xiiQtColorAreaWidget::paintEvent(QPaintEvent* event)
{
...
void xiiQtColorAreaWidget::UpdateImage()
{
  ...
  for (int y = 0; y < height; ++y)
  {
    ...
    {
      xiiColor c = xiiColor::MakeHSV(m_fHue, (double)x / (width - 1), (double)y / (height - 1));
...
void xiiQtColorAreaWidget::mouseMoveEvent(QMouseEvent* event)
{
  if (event->buttons().testFlag(Qt::LeftButton))
  {
    ...
    const int sat   = xiiMath::Clamp(coord.x(), 0, width - 1);
    const int val   = xiiMath::Clamp((height - 1) - coord.y(), 0, height - 1);
...
void xiiQtColorAreaWidget::mousePressEvent(QMouseEvent* event)
{
...
void xiiQtColorRangeWidget::paintEvent(QPaintEvent* event)
{
  QPainter painter(this);
  painter.setRenderHint(QPainter::RenderHint::Antialiasing);
...
void xiiQtColorRangeWidget::UpdateImage()
{
  ...
  for (int x = 0; x < width; ++x)
  {
    xiiColor c = xiiColor::MakeHSV(((double)x / (width - 1.0)) * 360.0, 1, 1);
...
void xiiQtColorRangeWidget::mouseMoveEvent(QMouseEvent* event)
{
  if (event->buttons().testFlag(Qt::LeftButton))
  {
    const int width  = rect().width();
...
void xiiQtColorRangeWidget::mousePressEvent(QMouseEvent* event)
{
  mouseMoveEvent(event);
}
...
void xiiQtColorCompareWidget::SetNewColor(const xiiColor& color)
{
  if (m_NewColor == color)
    return;
...
void xiiQtColorCompareWidget::SetInitialColor(const xiiColor& color)
{
  m_InitialColor = color;
  m_NewColor     = color;
}

void xiiQtColorCompareWidget::paintEvent(QPaintEvent*)
{
  const QRect area = rect();
  ...
  QColor qInCol = xiiToQtColor(inLDR);

  xiiColor newLDR = m_NewColor;
  ...
  QColor qNewCol = xiiToQtColor(newLDR);
