...
public:
  ...
  /// \brief Overload for xiiStringBuilder to fix ambiguity between implicit conversions.
  ...
  /// \brief Returns a relative path including the data directory the path belongs to, e.g. "Testing Chambers/Objects/Barrel.xiiPrefab".
  xiiStringView GetDataDirParentRelativePath() const;
  /// \brief Returns a path relative to the data directory the path belongs to, e.g. "Objects/Barrel.xiiPrefab".
  xiiStringView GetDataDirRelativePath() const;
  /// \brief Returns absolute path to the data directory this path belongs to, e.g.  "C:/xiiEngine/Data/Samples/Testing Chambers".
  ...
  /// \brief If a xiiDataDirPath is de-serialized, it might not be correct anymore and its data directory reference must be updated. It could potentially no longer be part of any data directory at all and become invalid so after calling this function, IsValid will match the return value of this function. On failure, the invalid data directory paths should then be destroyed.
  /// \param dataDirRoots A list of normalized absolute paths to the roots of the data directories. These must not end in a "/" character.
