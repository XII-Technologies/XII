...
public:
  ...
  /// \param sAbsolutePath Path to the document. Must be in the model.
  /// \param callback Called once the file was opened and hashed. The xiiFileStatus contains the up to date info for the file, including hash. This function should return the document Id as it will automatically call LinkDocument.
  ...
  xiiResult ReadDocument(xiiStringView sAbsolutePath, const xiiDelegate<xiiUuid(const xiiFileStatus&, xiiStreamReader&)>& callback);
...
public:
  xiiEvent<const xiiFolderChangedEvent&, xiiMutex> m_FolderChangedEvents;
  xiiEvent<const xiiFileChangedEvent&, xiiMutex>   m_FileChangedEvents;
