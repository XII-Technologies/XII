...
xiiQtStatVisWidget* xiiQtStatVisWidget::s_pWidget   = nullptr;
xiiInt32            xiiQtStatVisWidget::s_iCurColor = 0;

namespace StatVisWidgetDetail
{
  static QColor s_Colors[xiiQtStatVisWidget::s_uiMaxColors] = {
...
xiiQtStatVisWidget::xiiQtStatVisWidget(QWidget* parent, xiiInt32 iWindowNumber) :
  ads::CDockWidget(QString("StatVisWidget") + QString::number(iWindowNumber), parent), m_ShowWindowAction(parent)
{
  ...
  XII_VERIFY(nullptr != QWidget::connect(&m_ShowWindowAction, SIGNAL(triggered()), this, SLOT(on_ToggleVisible())), "");
}


xiiQtStatVisWidget::~xiiQtStatVisWidget() {}

void xiiQtStatVisWidget::on_ComboTimeframe_currentIndexChanged(int index)
{
...
void xiiQtStatVisWidget::on_SpinMin_valueChanged(double val)
{
  xiiStringBuilder sStatHistory;
  sStatHistory.Format("StatHistory{0}", m_iWindowNumber);
...
void xiiQtStatVisWidget::on_SpinMax_valueChanged(double val)
{
  xiiStringBuilder sStatHistory;
  sStatHistory.Format("StatHistory{0}", m_iWindowNumber);
...
void xiiQtStatVisWidget::on_ToggleVisible()
{
  toggleView(isClosed());
...
void xiiQtStatVisWidget::on_ListStats_currentItemChanged(QListWidgetItem* current, QListWidgetItem* previous)
{
  ButtonRemove->setEnabled(current != nullptr);
}

void xiiQtStatVisWidget::on_ButtonRemove_clicked()
{
...
void xiiQtStatVisWidget::Save()
{
  ...
  const xiiUInt32 uiNumFavorites = m_Stats.GetCount();
...
void xiiQtStatVisWidget::Load()
{
  ...
  for (xiiUInt32 i = 0; i < uiNumFavorites; ++i)
  {
    ...
    AddStat(xiis, bChecked, false);
  }

  f.close();
}

void xiiQtStatVisWidget::AddStat(const xiiString& sStatPath, bool bEnabled, bool bRaiseWindow)
{
  if (bRaiseWindow)
  {
    toggleView(true);
  ...
  {
    Stat.m_uiColor = s_iCurColor % xiiQtStatVisWidget::s_uiMaxColors;
    ++s_iCurColor;
...
void xiiQtStatVisWidget::UpdateStats()
{
  if (isClosed())
    return;
  ...
  for (xiiMap<xiiString, StatsData>::Iterator it = m_Stats.GetIterator(); it.IsValid(); ++it)
  {
    if (it.Value().m_pListItem->checkState() != Qt::Checked)
      continue;

    const xiiDeque<xiiQtMainWidget::StatSample>& Samples = xiiQtMainWidget::s_pWidget->m_Stats[it.Key()].m_History;

    if (Samples.IsEmpty())
      continue;

    const xiiUInt32 uiColorPath = it.Value().m_uiColor;

    xiiUInt32 uiFirstSample = 0;

    const xiiTime MaxGlobalTime = xiiQtMainWidget::s_pWidget->m_MaxGlobalTime;
