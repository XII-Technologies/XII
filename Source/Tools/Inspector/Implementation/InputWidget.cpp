...
void xiiQtInputWidget::UpdateSlotTable(bool bRecreate)
{
  ...
  if (bRecreate)
  {
    ...
    for (xiiMap<xiiString, SlotData>::Iterator it = m_InputSlots.GetIterator(); it.IsValid(); ++it)
    {
      ...
      pIcon->setPixmap(xiiQtUiServices::GetCachedPixmapResource(":/Icons/Icons/InputSlots.png"));
...

  {
    ...
    for (xiiMap<xiiString, SlotData>::Iterator it = m_InputSlots.GetIterator(); it.IsValid(); ++it)
    {
      ...
      // Value
      {
        ...
        {
          sTemp.Format(" {0} ", xiiArgF(it.Value().m_fValue, 4));
...
void xiiQtInputWidget::UpdateActionTable(bool bRecreate)
{
  ...
  if (bRecreate)
  {
    ...
    for (xiiMap<xiiString, ActionData>::Iterator it = m_InputActions.GetIterator(); it.IsValid(); ++it)
    {
      ...
      QLabel* pIcon = new QLabel();
      pIcon->setPixmap(xiiQtUiServices::GetCachedPixmapResource(":/Icons/Icons/InputActions.png"));
      pIcon->setAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
      ...
      for (xiiInt32 slot = 0; slot < xiiInputActionConfig::MaxInputSlotAlternatives; ++slot)
      {
        if (it.Value().m_sTrigger[slot].IsEmpty())
          ...
          sTemp.Format("  [Scale: {0}] {1}  ", xiiArgF(it.Value().m_fTriggerScaling[slot], 2), it.Value().m_sTrigger[slot]);
      ...
      // Value
      {
        ...
        else
        {
          if (it.Value().m_bUseTimeScaling)
            sTemp.Format(" {0} (Time-Scaled) ", xiiArgF(it.Value().m_fValue, 4));
          else
            sTemp.Format(" {0} (Absolute) ", xiiArgF(it.Value().m_fValue, 4));
