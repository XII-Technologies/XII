...
xiiQtDataWidget::xiiQtDataWidget(ads::CDockManager* pDockManager, QWidget* pParent) :
...
void xiiQtDataWidget::ResetStats()
{
  m_Transfers.Clear();
...
void xiiQtDataWidget::ProcessTelemetry(void* pUnuseed)
{
  if (!s_pWidget)
    return;
  ...
  while (xiiTelemetry::RetrieveMessage('TRAN', msg) == XII_SUCCESS)
  {
    ...
    if (msg.GetMessageID() == 'DSBL')
    {
      ...
      if (it.IsValid())
      {
        xiiInt32 iIndex = s_pWidget->ComboTransfers->findText(sName.GetData());
    ...
    {
      xiiString sBelongsTo, sName, sMimeType, sExtension;
      ...
      if (Transfer.IsValid())
      {
        ...
        xiiMemoryStreamWriter Writer(&tdo.m_Storage);
...
void xiiQtDataWidget::on_ButtonRefresh_clicked()
{
  if (ComboTransfers->currentIndex() < 0)
    return;

  const xiiStringBuilder sName = ComboTransfers->currentText().toUtf8().data();
...
void xiiQtDataWidget::on_ComboTransfers_currentIndexChanged(int index)
{
  ComboItems->clear();
  ...
  xiiStringBuilder sName = ComboTransfers->currentText().toUtf8().data();
  ...
  {
    xiiQtScopedUpdatesDisabled _1(ComboItems);
...
xiiQtDataWidget::TransferDataObject* xiiQtDataWidget::GetCurrentItem()
{
  ...
  xiiString sItem = ComboItems->currentText().toUtf8().data();
...
xiiQtDataWidget::TransferData* xiiQtDataWidget::GetCurrentTransfer()
{
  xiiString sTransfer = ComboTransfers->currentText().toUtf8().data();
...
void xiiQtDataWidget::on_ComboItems_currentIndexChanged(int index)
{
  if (index < 0)
    return;
  ...
  const xiiString sMime  = pItem->m_sMimeType;
  ...
  {
    xiiUInt32 uiWidth, uiHeight;
    Reader >> uiWidth;
    Reader >> uiHeight;

    xiiDynamicArray<xiiUInt8> Image;
  ...
  {
    const xiiUInt32 uiMaxBytes = xiiMath::Min<xiiUInt32>(1024 * 16, Reader.GetByteCount32());

    xiiHybridArray<xiiUInt8, 1024> Temp;
...
bool xiiQtDataWidget::SaveToFile(TransferDataObject& item, xiiStringView sFile)
{
...
void xiiQtDataWidget::on_ButtonSave_clicked()
{
  auto pItem = GetCurrentItem();

  if (!pItem)
  {
    QMessageBox::information(this, QLatin1String("xiiInspector"), QLatin1String("No valid item selected."), QMessageBox::Ok, QMessageBox::Ok);
    return;
  }
...
void xiiQtDataWidget::on_ButtonOpen_clicked()
{
  auto pItem = GetCurrentItem();
  ...
  if (!QDesktopServices::openUrl(QUrl(pItem->m_sFileName.GetData())))
    QMessageBox::information(this, QLatin1String("xiiInspector"), QLatin1String("Could not open the file. There is probably no application registered to handle this file type."), QMessageBox::Ok, QMessageBox::Ok);
}
