...
void xiiQtResourceWidget::ResetStats()
{
  m_Resources.Clear();
...
void xiiQtResourceWidget::UpdateStats()
{
  if (!m_bUpdateTable)
    return;
...
{
public:
  ByteSizeItem(xiiUInt32 uiBytes, const char* szString) :
...
void xiiQtResourceWidget::UpdateTable()
{
  if (!m_bUpdateTable)
    return;
  ...
  if (m_bUpdateTypeBox)
  {
    xiiQtScopedUpdatesDisabled _1(ComboResourceTypes);
  ...
  xiiQtScopedUpdatesDisabled _2(Table);
  ...
  for (auto it = m_Resources.GetIterator(); it.IsValid(); ++it)
  {
    ...
    if (res.m_bUpdate)
    {
      ...
      if (!m_bShowDeleted && res.m_LoadingState.m_State == xiiResourceState::Invalid)
      {
        bShowItem = false;
      }
...
void xiiQtResourceWidget::UpdateAll()
{
  m_bUpdateTable = true;
...
void xiiQtResourceWidget::on_LineFilterByName_textChanged()
{
...
void xiiQtResourceWidget::on_ComboResourceTypes_currentIndexChanged(int state)
{
  if (state == 0)
    m_sTypeFilter.Clear();
  else
...
void xiiQtResourceWidget::on_CheckShowDeleted_toggled(bool checked)
{
...
void xiiQtResourceWidget::on_ButtonSave_clicked()
{
...
void xiiQtResourceWidget::ProcessTelemetry(void* pUnuseed)
{
  if (!s_pWidget)
    return;
  ...
  while (xiiTelemetry::RetrieveMessage('RESM', Msg) == XII_SUCCESS)
  {
    s_pWidget->m_bUpdateTable = true;

    xiiUInt64 uiResourceNameHash = 0;
    ...
    if (Msg.GetMessageID() == ' SET')
    {
      ...
      xiiUInt8 uiPriority = 0;
      Msg.GetReader() >> uiPriority;
      rd.m_Priority = (xiiResourcePriority)uiPriority;

      xiiUInt8 uiFlags = 0;
      Msg.GetReader() >> uiFlags;
      rd.m_Flags.Clear();
      rd.m_Flags.Add((xiiResourceFlags::Enum)uiFlags);

      xiiUInt8 uiLoadingState = 0;
      Msg.GetReader() >> uiLoadingState;

      rd.m_LoadingState.m_State = (xiiResourceState)uiLoadingState;
    ...
    {
      rd.m_Flags.Remove(xiiResourceFlags::IsQueuedForLoading);
      rd.m_LoadingState.m_State = xiiResourceState::Invalid;
    }
  }
}
