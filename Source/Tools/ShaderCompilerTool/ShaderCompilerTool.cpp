...
xiiResult xiiShaderCompilerApplication::CompileShader(const char* szShaderFile)
{
  XII_LOG_BLOCK("Compiling Shader", szShaderFile);
  ...
  for (xiiUInt32 perm = 0; perm < uiMaxPerms; ++perm)
  {
    ...
    if (sc.CompileShaderPermutationForPlatforms(szShaderFile, PermVars, xiiLog::GetThreadLocalLogSystem(), m_sPlatforms).Failed())
      return XII_FAILURE;
  }

  xiiLog::Success("Compiled Shader '{0}'", szShaderFile);
  return XII_SUCCESS;
}

xiiResult xiiShaderCompilerApplication::ExtractPermutationVarValues(const char* szShaderFile)
{
  ...
  {
    xiiLog::Error("Could not open file '{0}'", szShaderFile);
    return XII_FAILURE;
  }
