...
float EvaluateAmbientCube(in AmbientCubeFloat ac, float3 normal)
{
  ...
  return normalSquared.x * ac.m_Values[isNegative.x + 0] + normalSquared.y * ac.m_Values[isNegative.y + 2] + normalSquared.z * ac.m_Values[isNegative.z + 4];
}
...
float4 EvaluateAmbientCube(Texture2D ambientCubeTexture, int slotIndex, float3 normal)
{
  float3 normalSquared = normal * normal;
  int3   isNegative    = normal < 0.0;
  return normalSquared.x * ambientCubeTexture.Load(int3(isNegative.x + 0, slotIndex, 0)) + normalSquared.y * ambientCubeTexture.Load(int3(isNegative.y + 2, slotIndex, 0)) + normalSquared.z * ambientCubeTexture.Load(int3(isNegative.z + 4, slotIndex, 0));
}
...
float EvaluateCompressedSkyVisibility(uint compressedSkyVisibility, float3 normal)
{
  ...
  return normalSquared.x * float((compressedSkyVisibility >> bitShift.x) & 0x1F) + normalSquared.y * float((compressedSkyVisibility >> bitShift.y) & 0x1F) + normalSquared.z * float((compressedSkyVisibility >> bitShift.z) & 0x3F);
}
