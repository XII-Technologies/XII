...
xiiPerClusterData GetClusterData(float3 screenPosition)
{
...
float CalculateShadowTerm(xiiMaterialData matData, float3 lightVector, float distanceToLight, uint type,
...
float3 ComputeReflection(inout xiiMaterialData matData, float3 viewVector, xiiPerClusterData clusterData)
{
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    const xiiPerReflectionProbeData probeData = perPerReflectionProbeDataBuffer[probeIndex];
...
AccumulatedLight CalculateLighting(xiiMaterialData matData, xiiPerClusterData clusterData, float3 screenPosition, bool applySSAO)
{
  ...
  for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    xiiPerLightData lightData = perLightDataBuffer[lightIndex];
...
void ApplyDecals(inout xiiMaterialData matData, xiiPerClusterData clusterData, uint gameObjectId)
{
  uint firstItemIndex = clusterData.offset;
  ...
  for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    xiiPerDecalData decalData = perDecalDataBuffer[decalIndex];
...
void ApplyRefraction(inout xiiMaterialData matData, inout AccumulatedLight light)
{
