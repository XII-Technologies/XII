...
declare function xiiTestFailure(file: string, line: number, func: string, msg: string): void;
...
function TestFailed(msg: string) {
    ...
    xiiTestFailure(file, line, func, msg);
}
...
export function FLOAT(f1: number, f2: number, epsilon: number = 0.001) {
    ...
    if (!xii.Utils.IsNumberEqual(f1, f2, epsilon)) {
        TestFailed(f1 + " does not equal " + f2);
    }
}

export function VEC2(v1: xii.Vec2, v2: xii.Vec2, epsilon: number = 0.001) {
...
export function VEC3(v1: xii.Vec3, v2: xii.Vec3, epsilon: number = 0.001) {
...
export function QUAT(q1: xii.Quat, q2: xii.Quat, epsilon: number = 0.001) {
...
export function ARRAY(N: number, v1: number[], v2: number[], epsilon: number = 0.001): void {
    ...
    for (let i = 0; i < N; ++i) {
        ...
        if (!xii.Utils.IsNumberEqual(v1[i], v2[i], epsilon)) {
...
export function COLOR(c1: xii.Color, c2: xii.Color, epsilon: number = 0.001) {
